...
describe('web release version', () => {
   ...
   *
   * 1.0.4 is what the delivered APK/AAB and the Xcode project carry; a web
   * product that reported an *older* number than the binary built from the
   * same tree would be the drift `version:check` exists to catch. The gate
   * enforces the ordering across files; this pins it from the web's side so a
   * tree in which every copy had drifted together still fails somewhere.
   */
