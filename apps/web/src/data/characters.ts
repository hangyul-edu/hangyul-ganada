...
import { CHARACTER_COPY } from './characterCopy';
...
function letter(spec: LetterSpec): HangulCharacter {
  ...
  return {
    ...
    translations: withCopy(spec.character, spec.translations),
  };
}

/**
 * The English and Korean written beside the letter, plus the other six.
 *
 * Merged here rather than in `characterCopy.ts` so that a character declared
 * without copy in some language fails loudly: `data.test.ts` asserts every
 * character resolves in every shipping locale, and it reads what this returns.
 */
function withCopy(
  character: string,
  authored: LocalizedMap<CharacterTranslation>,
): LocalizedMap<CharacterTranslation> {
  return { ...CHARACTER_COPY[character], ...authored } as LocalizedMap<CharacterTranslation>;
}
...
function syllable(
  ...
  return {
    ...
    translations: withCopy(character, translations),
  };
}
