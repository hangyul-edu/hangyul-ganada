...
**Current figures and historical figures.** A number in §2.2, §8, §9, §10.8, §11.1, §14, §16 and §20AD is a *current* figure: it was read from `docs/content-inventory.json` (`npm run content:inventory`), `docs/exercise-inventory.json` (`npm run exercise:inventory`) or `docs/report-metrics.json` for this edition, and `docs:consistency:check` fails if it drifts. A number inside a pass narrative (§20 through §20AC) is the figure *at that pass* and is left as written — 3,333 in the fourteenth pass, 3,370 in the seventeenth, 3,867 in the eighteenth, 3,864 from the nineteenth — so that the passes can be read as history. Where a narrative said *now*, the current value is in §2.2, not in the narrative.
...
is 3,864 taught words against a 10,000 target. §8 gives the honest distance in
...
| | |
| --- | --- |
| Commit | see `result/build-info.json` → `commit`. The artefacts in `result/` and `app_result/` were built from a clean checkout of it, and `release:current` fails if they were not |
| Working tree | **clean at build time** — `build-info.json` → `source_state` records the fingerprint and the dirty flag of the exact tree the artefacts came from. It read `"dirty": true` in the previous edition, and two files in `result/` named two different commits; see §20Q.6 and I-152 |
| Node | v24.19.0 |
...
| --- | --- | --- |
| Words shipping | 3,864 | `vocabulary.json` `words.length`; the same figure is `taught_entries` in `docs/content-inventory.json` |
| Categories | 18 | `vocabulary.json` `categories` |
| Study sets | 779 (five words each) | per category, ⌈words ÷ 5⌉ summed |
...
| Pronunciation notes | 900 | words carrying a `say` field in `vocabulary.json` |
| Of those, shown as a note on a card | 703 | `pronunciation.note_for`; liaison is taught once in the lesson instead |
| Sound-change patterns taught | 7 | `vocabulary.json` `sound_patterns` — ㄴ 첨가 added in the nineteenth pass |
| Patterns a word card may note | 6 | `vocabulary.json` `noted_patterns` |
| Dictionary headwords | 29,711 | `public/dictionary/manifest.json` |
| Dictionary senses | 38,746 | same |
| Dictionary examples | 3,579 | same |
...
| Numbers translated keys | 295 × 32 | `numbers:qa` |
| Numbers audio clips | 45 words + 51 examples, all recorded | `numbers:qa`, `public/audio/manifest.json` |
| Vocabulary packs complete | 32 | `locale:content:qa`; `docs/content-inventory.json` `locales.complete` |
| Vocabulary packs at 600 words | 0 | `locale:content:qa` — the twelve were finished in the fifteenth pass |
| Unwritten vocabulary rows | 0 | every one of the 32 content packs holds all 3,864; `translation:audit:check` reads every row |
| Long *More about it* definitions | 99 | third element in `vocabulary.en.json` |
| Level-test items, English | 4,156 | `public/level-test/manifest.json` |
| Level-test contextual items | 576 | the bank, `kind === "context"`; 39 of them hand-written and recorded in both voices |
| Level-test reach, every non-English language | 2,212 items each | `manifest.json` `reach` |
| Level-test distinct words asked about | 1,791 | `leveltest:bank:check`; `docs/content-inventory.json` |
| Level-test items at levels 1–10, from the curated corpus | 100% | `leveltest:bank:check`; see I-162 |
| Audio clips | 15,828 | distinct files in `public/audio/manifest.json` |
| Audio voice slots | 15,948 | the same manifest, two voices per entry |
| Vocabulary levels populated | 30 of 30 | distinct `level` in the corpus |
| Words at levels 28–30 | 436 | the corpus, by level; the thinnest zone is now 24-26 with 191 words (§20AB.3) |
...
**The pronunciation-note row keeps its two definitions.** 902 words carry a
recorded spoken form because some rule changes them; 703 of those get a *note
...
they build, then 3,864 everyday words, each with a hand-written example
sentence, a recording in two voices and a meaning in the learner's own language.
There is no account, no server and no network request during a lesson. It is
used by children, and from this edition every string a learner can be shown
is governed by a written child-safe content policy (§20Y,
`docs/CHILD_SAFE_CONTENT_POLICY.md`).

## What this cycle did

The twentieth pass (§20AD) was the final production-delivery audit, web only:
this report read page by page against the tree, the product reviewed as a
release candidate, the 32 interface languages read where a learner answers
questions, a cleanup with proof, and the full verification run again on the
final tree. Two reliability defects came out of reading the code rather than
any gate. Every screen but Home is a lazy chunk, and a chunk that fails to
load — the connection gone before the worker had precached it, or a tab kept
open across a deploy — rejected inside `Suspense` with no error boundary
above it, so React unmounted the whole app: a blank cream page with nothing
to tap, reproduced against the built bundle. There is a boundary now, around
the routed screen and nothing else, with a Reload and a Home, and the failed
state is one of the renders `screens:audit` measures at every width, in the
dark and at 200 % text (I-249). The second is worse and had been in the
product since the failure path was written: a launch that could not *read*
the stored profile fell back to a fresh one, correctly, but left every
repository wired to the store — and the first answered question saved the
fresh settings over the learner's level, streak, plan and placement result
while Settings reported *saving*. The repositories are detached on that path
now; the session runs in memory, the stored rows are left for the next launch
to read, and My Learning says so in its own sentence (I-250). The reading of
the interface strings found what the mechanical gates cannot: Czech called a
Hangul letter *litera*, the printer's word, on 37 strings; Greek addressed
the learner as *Πατήστε* in the matching and build exercises and as *Γράψε*
on the next screen; Hungarian had one formal sentence, Romanian one
ungrammatical count, Russian and Ukrainian a masculine verb on a button.
All are fixed, recorded row by row, and the editorial gate now reads the
register in the verb and carries a per-locale glossary, each negative-tested
(I-251). The cleanup removed an unused dependency, 36 exports no file
imported, a stylesheet made of comments, 22 report screenshots no document
or script produced, five Windows download stubs and an empty file named `=`,
with the import graph and the whole-repository searches that prove each one
(I-252). The iOS build is the one thing this edition does not claim: there is
no Xcode on this machine, and §20AD.6 says exactly what was and was not run.

The nineteenth pass and those before it are kept below as they were written.

## What the nineteenth pass did

The nineteenth pass (§20AC) was web-only and had three briefs: a learning
streak that counts from Day 1 and counts only learning; a reading of every
Hangul lesson, letter exercise, vocabulary entry, gap-fill, Level Test item,
Numbers question and pronunciation note as Korean; and a full audit of this
report against the tree. The streak had been counting from Day 1 already, but
it counted twenty seconds on a session screen as a learning day, credited a
Numbers answer only when its lesson completed, showed a never-active learner
"0 days" beside a flame, and — through a React updater whose flag was read
before it ran — never credited an item reaching *learned* to the day. All
four are fixed at the source under one rule, `isQualifyingDay` (§20B). The
Korean reading found what green gates cannot: seven ㅀ-stem verbs whose
pronunciation note named the wrong sound rule, two words transcribed against
표준 발음법 §29, a conjugation panel printing 먹으세요 under *Please do*,
sixty-four gap-fills with a second defensible answer, a letter-sound question
that could draw "i" beside "i" about one seed in twenty-five, reading
questions offering a near-synonym as a wrong answer, and a matching grid that
in thirty-one languages could show one meaning on two rows. Each is fixed,
held by a test, and broken on purpose in `scripts/content-gates-negative.sh`
(N12–N21) to show the test fires; N21 is a review session that, opened before
the profile had loaded, told a learner with twelve words due that nothing was
due. `docs/exercise-inventory.json` now lists
every learner-reachable question with its evidence, and
`content/korean-gold-set.json` holds the taught rules independently of the
code. Three corpus entries were retired as non-standard headwords and the
corpus is 3,864 words.

## What the sixteenth pass did

This pass began with four customer screenshots and a version number, and every
one of them was true of the tree the previous edition certified. The launch
screen on Android 12+ opened on an orange brand mark the approved splash does
not carry; *Show answer* in Today's Words printed 정답은 …예요 under four
still-live options and counted the tap after it as a pass; four gap-fill
chips wrapped three and one; the word-ordering tray and the matching grid
named their goal and not their first action; and the recall sentence on the
activity screen said 첫 번에 떠올렸어요 about a number that counts unaided
answers, not first tries. The tree carried 1.0.6 on Android and 1.0.3 on iOS;
the release is 1.0.4, at build 25 on both.

Each was fixed at its source and held by a gate that did not exist before —
§20Z.1 to §20Z.7 — and two requested quotations were added in 32 languages
with an honest attribution and a locale-by-locale audit generated from the
runtime strings and 768 rendering measurements (§20Z.3, I-222 for the human
part). Reading this document against the tree also found two of its own
...
## What the fifteenth pass did

This pass began with a report from production, and the report was true: a
learner sitting the Vocabulary Level Test could be asked what 섹스하다 means.
The word was a level-12 question in the bank the previous edition certified,
the Korean answer to *to have sex* in its twin, and a distractor in two
level-10 items; the dictionary answered a search for 섹 with it; and
`content:safety:check` was green. The gate compared 66 whole headwords against
taught words, produce options and gap-fills, declared the dictionary — the
pool the upper levels are drawn from — out of scope, and never resolved the
option ids of meaning items. Three lists in three files had no word in common
with the row that shipped.

The fix is a policy, not a word. One versioned child-safe content policy in
seven categories, with surface forms in 32 languages, two evaluators held to
355 shared fixtures, and seven gates that read it — import, dictionary
publication, assessment generation, the finished item in every language, the
runtime on every bank load, CI over 14 content families and a million fields,
and the packaged APK and AAB. Scanned under it, the shipped commit held 4,333
findings across nine families. 23 taught words were retired with tombstones,
694 dictionary headwords and 525 senses refused at publication, one
injury-centred example rewritten with its 31 translations and two recordings,
and the bank, the gap-fills, the audio manifest and the relations regenerated.
The fixed tree scans at 0. A learner keeps every progress row; a stored day
loses only the retired words it had not yet taught, and a stored sitting that
named a refused item is not resumed. §20Y.

The fourteenth pass is kept below as it was written, and §20Y.6 says which of
its safety claims survived being re-read.

## What the fourteenth pass did

This pass began from a tester who had recently learnt to read Hangul, and from
the fourth question they were shown:
...
화분 — a **level-28** word. The item was **level 7**, because 죽다 is a level-7
word and **a contextual item took its difficulty from the word removed from it
and from nothing else**: 629 of 629 in the shipped bank. 61 of them contained a
word ranked above their own level.

The second half was not in the bank. The selector aims at the midpoint of its
bracket, whose upper bound is 30 until something is missed, and the posterior's
prior is centred at 15 — so for the first several questions the sitting believed
something near 15 about everybody, and `MAX_STEP_UP` was the only thing holding
it down. It climbed two levels a question **whatever the learner answered**. A
learner who declined a level-4 word was shown level 6.

And the kind plan made **question two a sentence for every learner who ever sat
the test**, before anything was known about whether they could read one.

§20W is the whole account. What changed: a sentence's level is now the higher of
its answer's level and what the sentence itself demands — its hardest word, its
grammar, its length — which moved 68 items upward and emptied the foundation band
down to three single-clause sentences. Difficulty is now **earned** rather than
walked toward: six bands, and a band opens on three correct answers inside it
across two distinct question kinds, so recognising three words no longer opens
the band that holds sentences. The first five questions are words and the first
sentence is question six. A beginner who answers nothing correctly never leaves
levels 1–3.

It cost 3.3 points of placement accuracy, stated at the gate that measures it.

---

The previous cycle began from a photograph of a single question too, and that
question was this:

```
  일곱 시에 ____.        도와요 · 연습해요 · 일어나요 · 잘해요
```

Keyed *일어나요*. *일곱 시에 연습해요* — *I practise at seven* — is as ordinary
a sentence as *일곱 시에 일어나요*, so a learner who reads Korean correctly is
told they are wrong. **Every gate over that bank was green, and every gate was
right to be**: the rule that exists for a sentence which constrains nothing tests
whether the sentence carries an argument particle, and `시에` is an argument
particle. A particle is not a constraint. §20V is the whole account.

Pulling that thread found four more items of the same shape, three more classes
of the same kind — a subject slot that admits any person, a predicate that
excludes no noun, and sixteen sentences the daily queue keyed two different ways
— and two checkers that had not been measuring what they reported: the question
ledger validated **336 of 368** Numbers questions against a field that means
*render this through i18n*, not *this is the answer*; and twenty-four locale
packs went on translating example sentences that had been replaced, past five
gates that all read those files and none of which compares a translation to the
Korean it translates.

None of it changed what a learner can do. All of it changed what this repository
could honestly say it had checked. The fixes are in the generator and the
verifier rather than in the artefact: four rules about the *stem* rather than
about the pair, a cross-surface invariant, a reviewed list of twenty-four pairs
that no rule decides, three new release-blocking gates, and 5,191 questions
enumerated, read and written down in `docs/AMBIGUITY_LEDGER.md`.

---

The rest of this summary is the previous cycle's, which began from a customer
sentence rather than from a gate: **the level test is too hard.** Every gate over it was green, its calibration figures were
good, and the sentence was right — for two reasons that had nothing to do with
each other. §10 is the whole account; this is what it comes to.

**The first question every learner had ever seen was at level 14 of 30.** Not a
bug in the estimator, which placed people accurately: with nothing asked yet the
posterior is the prior, the prior is uninformative and therefore centred, so the
most informative item is in the middle of the scale. That is the correct answer
to the wrong question, and the selector was asking it from the first item and
then moving up to six levels between questions. Measured over 6,000 simulated
sittings: **42.8% of a level-2 learner's opening five questions were more than
six levels above them**, and a learner who answered everything wrongly was asked
level 1 **twenty-five times running**.
...
average over sittings cannot see a bad sequence, and the sequence is the only
part a learner experiences.

**The bank asked a beginner what 누가 means and marked "nougat" correct.**
Dictionary headwords are levelled by the frequency rank of their *spelling*, and
Korean writes many different words the same way. 누가 ranks 107th because it
means *who*; 내 ranks 3rd because it means *my* and was keyed to "smell"; 위해
ranks 131st because it means *for the sake of* and was keyed to "harm". Sixty-nine
such items sat at levels 1 to 5, and a learner who knew the word was marked wrong
for knowing it.

No gate could have caught it. `leveltest:ambiguity` reads contextual items and
these were `meaning` and `produce`; `leveltest:locale` asks whether a string came
from the right language pack, and "nougat" is impeccable English.

Levels 1–10 are now entirely curated corpus words — the band the taught corpus
exists to cover, where an unvetted headword adds no coverage and all of the risk.
Above it, where the corpus thins to twenty words a level and a rare word *is* the
right thing to ask, dictionary anchors stay and are filtered: 40 whose rank
...
other language's askable bank at that pass from 1,014 items to 1,962 (the current reach is in §2.2) because a taught word has
...
in.** Distractors were rejected on their *English* gloss. 확실히 and 정확히 are
*definitely* and *exactly* in English and one word — *aniq* — in Uzbek, so an
Uzbek learner met a question with two identical correct answers and no way to pass
it. Collision is now tested in every language a meaning exists in.

**And *I don't know* was worth exactly nothing.** The button is on every question,
and this report, the source comments and the patent disclosure all said the model
weighs it as cleaner evidence than a wrong guess. Under a model where both can
only happen when the learner does not know, the two likelihoods differ by a
constant factor that cancels under normalisation — ten declared blanks and ten
wrong answers gave **identical estimates to twelve decimal places**. A slip term
separates them, and against a population that mis-taps 5% of what it knows it is
worth a quarter of a level.

**A sitting now survives the app being closed.** It was component state; eleven
questions in, a phone call cost eleven questions, and the screen carried a leave
guard whose whole job was to warn about that. What is written to the device is the
ids already presented, the response to each and a seed — nothing about the future
of the test — and every later item is recomputed from that seed, so a resumed
sitting shows the same question and keeps adapting. Schema 14, and the first new
persisted field since 13.

**Two things this pass had to fix before it could verify itself.** The previous
release bumped the Xcode project's version in Xcode and did not adopt the lock
file with it, so `verify:release` could not pass on a clean checkout of `main`
(I-166). And the patent package's own README carried two file indexes, the second
of which sent counsel to two documents the first says were deleted (I-167). Both
are the failure the tooling around them exists to prevent, and both now have a
gate.

**Twelve languages that taught six hundred words now teach all of them.** The
twelve partial content packs — kk, ky, mn, nl, pl, ro, sv, ta, te, tr, uk, uz —
carried meanings for 609 of the corpus's 3,333 words, and that is not a cosmetic
gap. `locale:practice:qa`, written this cycle, drives the real planner and
question builder for every shipping language at levels 1, 5, 10, 15, 20, 25 and
30 over fourteen days. At level 21 it measured this:
...
A Turkish learner past the core band met **no meaning question and no matching
grid**, because neither can be built without a meaning in the learner's
language, and the intro card fell back to the English gloss. Somebody had paid
for a Korean course in Turkish and was being taught in English. The gate counted
217 findings, all of them in those twelve languages; it counts 0 now and it is
in `verify:release`.

65,376 strings were written to close it — 2,724 meanings and 2,724 example
translations per language, with the *More about it* note on the 36 words that
carry one. The four debts that fall due the moment a language crosses from
partial to complete were paid with them: **192 example-translation collisions**
where two Korean sentences arrived at one target sentence in a language whose
English pack keeps them apart, all 192 separated on the distinction the Korean
makes and none moved to the accepted-merges ledger; **43 polarity findings**,
forty of them content and one a real defect — 어쩌다 이렇게 됐어요? had become a
Turkish statement; **three that were the gate's fault**, where Kyrgyz harmonises
its negative suffix across four vowels and the marker list enumerated two, so
«Коркпоңуз» read as having lost a negation it plainly carries; and **twelve
gloss collisions that were mistranslations** rather than honest merges — 아저씨
as *amca* (uncle) in Turkish, 탐정 as *тыңшы* (spy) in Kazakh.

`locale:content:check` reads **32 complete · 0 partial** and I-19 closes.

**Then the corpus freeze lifted and sixty words went in at the top of the
scale.** `docs/CONTENT_COMPLETION_STATE.md` had said *no new word until 32/32
complete*, for the good reason that a corpus growing under twelve half-written
packs makes the gap permanent. With the gap closed, sixty entries were authored
against the place the course runs out first: forty-eight measured into levels
28–30 and eleven more into 27, taking the top band from **477 to 524 words** and
the corpus to **3,393**. Level is computed rather than declared, so where a word
lands is a measurement. I-79 is no longer wholly open — it is PARTIAL: about
eleven weeks of new words at the top instead of about nine, on the same problem.

**Korean text is now compared as it is read, not as it happens to be encoded.**
Nothing in the product normalised Unicode. A macOS or iOS text field, a clipboard
round trip or any NFD pass hands back 학교를 as six conjoining jamo, and every
rule in the morphology package is written against precomposed syllables —
`endsWith('를')` is a one-character test. So dictionary search over 30,334
headwords, corpus search over the taught words, morphological analysis and answer
equivalence all returned *nothing* or *different* for two strings a reader cannot
tell apart. `normalise` composes first now, and the composition is in `decompose`
itself so it cannot be forgotten at a call site.

**A typed Korean answer can be graded, and the reason comes back as data.**
`validate(typed, expected, options)` accepts an answer differing only in spacing
or in the alternant of a particle, rejects a counting form, a politeness level or
an inflection, and separates a wrong *particle* from a wrong *word* where the
stems agree — 학교에 for 학교를 is one keystroke and a taught rule; 마리 for 개 is
a different lesson. The correction is structured fields, never a sentence,
because the interface has to say it in thirty-two languages. The library is
tested; **no screen in the product yet lets a learner type Korean**, and the
disclosure says which of the two it is.

**Two screenshots, and both defects were the same mistake one level down.** A
`ㅌ` whose third stroke number was cut off flat, and a Numbers course list whose
lesson titles started a thumb's width in from the page. In both cases a gate was
watching and passing, because in both cases the gate was measuring what the code
computes rather than what a reader sees: the badge's *fill* rather than the ring
it is drawn with, and a row's *boxes* rather than where its letters start. An
empty box is perfectly aligned. §20Q.

**Twenty-one badges over twenty characters were being clipped, not one.** An SVG
ring straddles the circumference, so a disc with `r = 5.6` and
`stroke-width: 0.9` paints to 6.05, and the placement clamped the centre to 5.6.
`ㅌ` #3 was the one photographed; `ㄷ` #2 lost ring on two sides inside a
`border-radius` whose size in viewBox units nothing in the layout could even
name. The bound is written against the painted radius now, the paper is a
`<rect>` inside the SVG so nothing clips, and `strokes:markers` measures all 269
badges for clipping, ink, collisions, leaders, order and legibility. No per-glyph
override was added and the gate asserts there are none.

**The Numbers list gave 32 px of every row to an icon most rows do not draw.**
Three left edges on one screen — 20 px, 32 px and 49 px — and a three-word lesson
title wrapping onto three lines beside a quarter-row of nothing. One rail now,
the tick inside the **Completed** pill, and a title block that keeps the width
because the badge wraps below it rather than squeezing it. `numbers:layout`
measures the ink at 45 cases across seven sizes, three text scales, both
appearances and all 32 languages; restoring the icon column fails it 2,565 times.

**A learner who cannot hear could not finish a single Numbers lesson.** Not a
harder route — none. A listening question's whole stimulus is a clip, all
nineteen lessons list that kind, a mastery check asks every item, and passing
one is what completes a lesson. `sound_free` has existed since §36 and the review
scheduler has always honoured it; this course never asked, and never asked the
player either. It asks both now, and every lesson still asks every item without
the heard-only kind.

**The Numbers course was read end to end and seven gates were written for what
the reading found.** 224 translated strings across thirty-two languages that no
screen can draw. 금요일 glossed as *the fifth day of the week*, which is the
sixth on a Korean calendar — the correct answer looking wrong. One prompt still
writing 을(를) longhand in a product that built a josa formatter to stop doing
that. And a lesson status that read the visit rather than the evidence, so an
interrupted learner could come back to a row saying *you have not started this*.

**The delivery's own paperwork disagreed about which commit built it.**
`RELEASE_VALIDATION.md` named one commit, `build-info.json` another, and recorded
that the tree was dirty when the build ran. `docs:consistency` had been saying so
since the artefacts were built. They are rebuilt from a clean checkout here.
...
**A question that answered itself, and the machine that wrote it.** 사 → 4,
answered correctly, with *사는 4예요* underneath. The feedback line was being
generated from the item and a fixed ending, so on a correct answer it could only
repeat what the learner had just tapped. Feedback is authored and typed per
outcome now, and most correct answers get the verdict and nothing else. The gate
written for it renders all 43,744 feedback strings the engine can produce and
found two more defects on its first run — a misconception shown to learners as a
raw key, 1,024 times, and the counting-word explanation under questions about
numerals. §20N.1.

**The document was a scroll container, and one hidden checkbox had made it one.**
On My Learning the page kept moving past its content and carried the bottom tab
bar with it, leaving blank ground underneath; the same offset survived a route
change into Privacy, which is why that page was reported as *sometimes* broken
near the bottom. An absolutely positioned checkbox whose label was not positioned
escaped its scroll pane and stretched the document to 1,961 px against an 844 px
window. The shell owns the viewport now, and 139 rendered cases across two new
suites hold it there. §20N.2.

**Four pixels.** 초기화 sat right of its own description, and so did 백업, 앱 and
화면 — invisible to every box measurement in this repository, because the padding
moved the glyph and not the element. The new gate measures ink. §20N.3.
...
every gate run. 69 of the 78 land at levels 28–30, which now hold 436 words
...
The 10,000-word target is not met and is not close: 3,867 of 10,000 after the eighteenth pass (§20AB) added 497 words in seven reviewed batches, 6,133 short. No locale
has been reviewed by a native speaker, and this pass added **69,156 more
strings** — 65,376 finishing the twelve packs and 3,780 for the new entries — to
that unread surface rather than shrinking it. That is the honest shape of this
cycle: coverage went from a real gap to none, and *review* went further out of
reach in the same movement, because there is now more unread text than there was
this morning. The onward hand-off still has no destination, and none was
invented.

## The verdict

**RELEASE CANDIDATE — EVERYTHING THIS REPOSITORY CAN SETTLE IS SETTLED, AND THIS
PASS FOUND, FOR THE SECOND EDITION RUNNING, THAT THAT SENTENCE WAS NOT TRUE WHEN
IT WAS LAST WRITTEN.**

**This edition's correction comes first, because it reached a child.** The
placement test could ask what 섹스하다 means. The bank that carried it was
certified by the previous edition, the gate that certified it was green, and
§20Y.6 lists every safety claim this document had made and marks the ones
that were wrong — CONTRADICTED, in this document's own vocabulary. What
replaced the gate is a policy every builder, the runtime, CI and the artefact
scan read from one file; what it found in the shipped commit is counted in
§20Y.2; what it costs is in §20Y.5; what it cannot do — find a term nobody
listed, or read thirty languages the engineer does not speak — is in §20Y.7
and tracked as I-213. The sentence at the top of this verdict is now made
with that correction in it.

Two things were wrong with the previous edition's version of this line as
well, and both are worth stating before the good news.

**`verify:release` could not pass on a clean checkout of the commit this
document was written from.** The release before it bumped the Xcode project's
version in Xcode and did not adopt the lock file that guards that file, so
`ios:project:check` — the fifth step of `verify:quick` — reported four findings
on `main`. The gate was right and its instruction was the fix (I-166). But the
previous edition claimed every gate ran green, and it could not have.

**And a customer sentence beat every gate in the repository.** *The level test is
too hard.* All of §10 came out of it: a first question at level 14 for every
learner, sixty-nine beginner items keyed to the wrong sense of a homograph — 누가
answered by "nougat" — fifteen questions with two identical options in some
language, and an *I don't know* button the estimator could not distinguish from
a wrong guess. Every gate over the Level Test was green throughout, and none of
...
(I-03), and one is a deliberate refusal to invent data (I-13). I-126, which an earlier edition of this paragraph listed as needing a frequency corpus that is not open data, is RESOLVED: the eighteenth pass adopted the National Institute of the Korean Language's learner list (KOGL type 1) as independent level evidence — §20AB.3. **The second reading of the eighteenth pass (§20AB.8–§20AB.12) found that the first reading's *done* was not so on four counts, and each is now closed with its evidence**: the tree could not be committed (1,092 zero-byte git objects from the crash — quarantined, index rebuilt, `fsck` clean, working tree byte-identical, then committed and pushed once); the chevron on the 소리가 만날 때 card sat 78 px inside the card while the lesson rows kept theirs at 16 px (one grid for every card, a rendered-position test, before/after screenshots — I-233); ten evasion shapes passed the child-safe policy (four rules in both evaluators, 38 fixtures, two negative cases — I-234); and 76 of the 108 shared English glosses were still shared (0 now — I-231). What still cannot be settled here is unchanged and is listed below: no native speaker has read any of the 32 locales, the 64 quotation rows or the 884 Korean interface strings.

**Three things stand between this document and shipping, and none can be done
from this machine:**

1. **A speaker of each language reads the pack — and now the policy.** 32
   languages, and Korean is one of them. §22, I-17 and I-213 say what has and
   has not happened. This pass makes the
   need sharper rather than smaller: the Level Test's item difficulty is
   curriculum-based and has never been calibrated against a learner, so what §10
   proves is that the machinery is sound, not that the thirty levels correspond
   to anything a person would recognise (§10.7).
...
curriculum units, fifteen lessons, forty letters, 33 syllable blocks, 3,864
words. Everything a learner needs is in the binary: the curriculum, the fonts,
the stroke data and 15,828 pronunciation clips in two voices.
...
`screens:audit` renders 17 routes and 7 transient states across eight profiles —
320, 360, 390, 412, 430, 390 in dark, 390 at 200% text and 320 at 200% text —
which is 171 renders. The seventh state is new in the twentieth pass: a screen
whose chunk the network refused, which used to be a blank page and is now the
failed-screen panel (I-249). All 171 are clean: nothing clipped, nothing overlapping, nothing below
contrast, no touch target under size. **The 200% profile measured normal text
until the seventeenth pass** — the gate set the root font size and every type
size is a pixel token; it scales the tokens now, asserts the text is really
≥ 1.9×, and fails anything pushed past the viewport (§20AA.1). The 143 above
are the re-measured figure; the 320-at-200 % profile was added in the
eighteenth pass, where the category grid and the tab labels broke at that
width and no other.
...
3,864 taught words in 18 categories and 779 study sets of five — 3,864 and not
the 3,867 of the eighteenth pass, because the nineteenth pass retired three
non-standard headwords (고장나다 and 신경쓰다 taught joined, 고마워 an inflected
form) with tombstones. Every entry has
one taught sense, a hand-written Korean example, and a meaning and an example
translation in every one of the 32 languages — `locale:content:check` reads
32 complete · 0 partial (§11.1); the ten/twenty-two split this paragraph used
to describe ended in the fifteenth pass and the sentence had outlived it.
...
| `copy/<12 partial>.json`, core band only | 12 meanings + 12 example translations |
...
3,864 of 10,000. The deficit is **6,136 entries, about 386,000 authored
strings** — an entry now costs 63 written strings rather than 21, because every
one of the thirty-two packs is complete and a new word owes a meaning and an
example translation to all of them. That is the honest distance and it is not closable by generation
...
`vocabulary:qa:target` is the one gate whose job is to state the distance
rather than to be satisfied: it prints *INFORMATIONAL (not a release blocker):
3,864 headwords — 6,136 short of the 10,000 target* and exits 0 (§19.2 records
when and why it stopped failing). It has not been disabled, weakened or
excluded from `verify:release`; this paragraph had said "fails on this tree"
since before that change and is corrected in the twentieth pass.

Two further facts belong with the number, because "we just need to write more"
is not the whole picture:

* Every word added is now written in **all thirty-two** languages, not one. The
  partial-locale backlog that made this bullet a warning is gone — §11.1 — and
  what replaced it is a higher unit cost: 63 authored strings per entry against
  21 when twelve packs were allowed to lag. The sixty entries added this cycle
  cost 3,780 written strings and 240 recordings.
* The precache does not fit at the target. It does fit today, at 1,277 kB
  against a 1,600 kB budget; the
  measured line for it is *corpus, whole at 10,000*, 925.5 kB against a 900 kB
  budget that is forecast rather than enforced. See §16 and I-92.

## 8.4 The dictionary layer — **VERIFIED, and it is not the corpus**

29,711 searchable headwords, 38,746 senses and 3,579 examples, fetched from
`public/dictionary` at runtime in chunks, read from
`public/dictionary/manifest.json` this cycle rather than carried forward. The
30,282 the previous editions carried here had been stale since the fifteenth
pass: the manifest went to 30,334 with the Numbers course and to 29,711 when
the child-safe policy retired the prohibited entries (`15ca8402`), and this
paragraph was not re-read either time — §2.2 was right and §8.4 was not. It is
...
| Gate | Result |
| --- | --- |
| `examples:qa` | 3,864 examples, all PASS; 0 REVIEW, 0 REWRITE |
| `vocabulary:qa` | passes; `--target` prints the distance it exists to state: *3,864 headwords — 6,136 short of the 10,000 target* |
...
| `dailyvocab:qa` | clean |
| `content:coverage` | every applicable row at 100%, and all 58 unobserved words carry a written reason (three reasons written in the twentieth pass) |
...
This section is the largest change in this pass, and it started from a customer
sentence rather than from a gate: **the level test is too hard.** Every gate over
it was green, its calibration figures were good, and the sentence was right.

## 10.1 What it is — **VERIFIED**

An adaptive placement test over 30 levels and a **4,156-item** bank, 576 of them
...
the device after every answer, so closing the app does not throw it away (§10.6).

## 10.2 Why it felt too hard, part one: the sequence — **the headline defect, fixed**

Not the estimator. The estimator was right about every learner it placed.

The selector asked it the wrong question. With nothing asked yet the posterior
*is* the prior; the prior is deliberately uninformative and therefore centred at
15; so the item carrying the most information is at level 14. That was question
one, for everybody, including a learner who had finished the alphabet an hour
earlier. Selection was equally unconstrained thereafter — the next item was
whichever carried the most information, from anywhere on the scale.

Measured on the shipped build, over 200 simulated sittings at each of 30 levels:

| | before | after | how it is held |
| --- | --- | --- | --- |
| Opening level, every learner | **14** | **2** | the warm-up ladder; `leveltest:qa:check` fails on any opening above 6 |
...
six levels between questions.** An average over sittings cannot see a bad
sequence, and the sequence is the only part a learner experiences. §10.7 is what
was added to the gate so that it can.

The fix is four rules on top of the information criterion, each documented at its
constant against the behaviour it replaces:

* **a warm-up.** The first three items come off a ladder — 2, 4, 6 for a new
  learner — rather than off the estimator.
* **a bounded step.** Whatever the estimator wants, the next level is within
  three of the last, so the walk is readable as a walk.
* **a repeat limit.** No level three times running. Without it the argmax parks
  at the ends of the scale, which is where the twenty-five came from.
* **a confirmation group.** The last items before the earliest stop are drawn at
  the settled estimate and a level either side, so the reported level rests on
  items chosen for it.

Inside those bounds the choice is still the most informative available item. The
adaptation is intact; it is only prevented from being abrupt.

**A returning learner's stored level moves the ladder and not the estimate.** It
is the only evidence available about who is sitting down, and a retake exists
because it may be wrong, so folding it into the prior — the obvious use — would
drag every retake toward the old number. It reaches the warm-up and nothing
else. A simulated learner retaking with the *inverse* of their true level (a
...
This is the worse half, and it is the clearer failure.

Every anchor is levelled by the frequency rank of its **spelling**. For a taught
word that is safe, because the curriculum supplies the level instead. For a
dictionary headword the rank is the only evidence there is — and it is evidence
about the string, not about the sense the question asks. Korean writes a great
many different words the same way, so the two came apart, and they came apart
worst exactly where a beginner meets them:

| shipped at | word | why the rank is high | what the question keyed as correct |
| --- | --- | --- | --- |
| level 1 | 누가 | rank 107 — it means *who* | "nougat" |
| level 1 | 내 | rank 3 — it means *my* | "smell" |
| level 1 | 위해 | rank 131 — it means *for the sake of* | "harm" |
| level 1 | 거야 | rank 19 — a sentence ending | "last night" |
| level 1 | 일다 | rank 1 — which belongs to 일 | "to rise" |
| level 2 | 나랑 | rank 361 — it means *with me* | "country" |

**Sixty-nine such items sat at levels 1 to 5.** A learner who correctly knew that
누가 means *who* was marked wrong for it, on an early question of the test that
decides what they will be taught.

No gate could have caught it, and neither gate was wrong to miss it.
`leveltest:ambiguity` reads the *contextual* items and these are `meaning` and
`produce`. `leveltest:locale` asks whether each string resolved from the right
language pack, and "nougat" is impeccable English.

**The fix is a floor, and the argument is the floor.** Levels 1–10 are the first
1,835 words of Korean — precisely the band the curated 3,393-word teaching corpus
exists to cover, and precisely where a beginner is placed. Below level 11 the
curriculum has a vetted word, with a vetted sense and a vetted level, for every
slot the test needs: 45 to 80 distinct words at each of those levels, against the
twenty a sitting can ask about. An unvetted dictionary headword adds no coverage
there and carries all of the risk.

Above it the position reverses. The corpus thins to twenty words at level 26, so
dictionary anchors are what makes the top of the scale exist at all — and a rare
word *is* the right thing to ask somebody at level 26. That asymmetry is the whole
content of the constant.

Filters do the rest of the work above the floor, because a floor cannot help
where the anchors are needed:

* **40 headwords whose rank belongs to an inflected form of another word.** The
  conjugator settles this exactly rather than by pattern: `analyse` guesses a
  dictionary form from the shape of the surface and conjugates the guess back,
  keeping only guesses that round-trip. 부탁해요 glossed "please" is 부탁하다;
  팔고 glossed "eight Duḥkhas" is 팔다; 신나 glossed "thinner" is 신나다.
* **one-syllable dictionary headwords.** 수, 자, 줄, 가, 신, 데 and fifty more.
  Each is a real entry with a real gloss and each takes its rank from a different
  word written the same way. Nothing recovers which sense earned it.
* **truncated and grammar-page glosses.** "Past tense of in the plain style.
  Formed from the…" shipped as the correct answer to a level-2 question.

## 10.4 Fifteen questions had two right answers, in languages nothing read them in — **P1, fixed**

Distractors were rejected when their **English** gloss matched the answer's or
shared a content word with it. That is the right rule read in one language, and
thirty-one learners are reading a different one.

| item | language | what the learner actually saw |
| --- | --- | --- |
...
| 가깝다 | Filipino | **mabait** · **mabait** · malapit · inaantok |

The first row is the shape that matters: 확실히 and 정확히 are *definitely* and
*exactly* in English and one word in Uzbek, so an Uzbek learner met a question
with two identical correct answers and no way to pass it.

The builder now folds each candidate meaning — case, Unicode form, punctuation, a
leading article or infinitive marker — and compares it against every other option
in **every language the two words both have a meaning in**. A distractor that
collides anywhere is not used anywhere. That is stricter than it has to be, and
it is the right trade: the bank is one artefact that all thirty-two languages
share.

`answerability:check` had covered this exact class for the runtime-generated
banks — 806,252 questions across 32 languages — and could not see the Level Test,
because the Level Test's bank is a **file** rather than something generated per
learner. That asymmetry is what left it open, and closing it is what `leveltest:bank`
is for.

## 10.5 *I don't know* was worth nothing — **P2, fixed**

The screen offers it on every question. This report, the source comments and the
patent disclosure all said the model weighs it as cleaner evidence than a wrong
guess. It could not.

Under a model in which both a wrong answer and a declared blank can only happen
when the learner does not know:
...
They differ by a constant factor, and a constant factor cancels when the
posterior is normalised. Ten declared blanks and ten wrong answers at the same
level produced **identical estimates to twelve decimal places**, and a sitting
answered entirely *I don't know* asked exactly the same thirty questions as one
answered entirely wrongly.
...
It was component state. Eleven questions into an eight-minute assessment, a phone
call cost eleven questions, and the screen carried a leave guard whose whole job
was to warn the learner about that.

What is stored is the ordered ids already presented, the response to each, a
per-sitting seed, the deadline and the interface language. **Nothing about the
future of the test.** That is the design decision rather than a detail: a resumed
adaptive test has to keep two promises at once — the questions already presented
do not change, and the answers already given still count. A pre-computed list of
remaining items keeps the first and freezes the adaptation; re-choosing freely
keeps the adaptation and breaks the first. Storing only what has happened, and
making every future choice a pure function of the seed and the question's index,
keeps both.

The deadline is stored with it, so force-quitting cannot buy more time. The
locale is, so a sitting is never resumed into a test re-rendered from a different
pack. A finished sitting is cleared in the *same write* that stores its result,
so there is no instant at which the device holds both. Schema 14; the leave guard
has gone with the thing it warned about.

`level-test.spec.ts` drives it through the built product across a full document
reload: four answers in, the fifth question on screen, reload, and the same fifth
question comes back with the same four options — and sixteen more finish the
sitting, not twenty.

## 10.7 What the gate now measures, and what it still cannot — **VERIFIED**

`leveltest:qa:check` measured accuracy and length. It now also measures the
*shape* of a sitting — opening level, largest step, longest run at one level, and
the share of opening questions far above the learner — and fails on each, plus a
retake simulation carrying an inverted stored level, plus six named response
patterns (all correct, all wrong, all *I don't know*, alternating, struggles then
improves, succeeds then hits a limit) each required to see at least three distinct
difficulties.

`leveltest:bank:check` is new and reads the shipped bank rather than the intention
that built it: no dictionary headword below the floor, none whose rank belongs to
an inflected form, no one-syllable dictionary headword, no truncated gloss, no
duplicated option, no item without exactly one correct option, no prompt
containing its own answer, no two options meaning the same thing in any of 32
languages, and a per-level floor on both items and distinct words.

**What neither can do, and this is the important paragraph.** The item difficulty
in this bank is **curriculum-based**: a word's frequency rank in Korean, or the
level the curriculum gives it. It is *not* psychometrically calibrated, because
there is no learner response data to calibrate against — the app opens no network
connection at runtime and collects nothing. Every accuracy figure above is against
a *simulated* learner answering the way the model says someone of that ability
would, which is a check on the machinery and not on the assumption underneath it.
A simulation reports excellent accuracy for a badly calibrated bank as long as the
bank is wrong in the same way the simulation is. Nothing in this report should be
read as evidence that the thirty levels correspond to anything a real learner
would recognise.

And neither gate reads Korean. Every rule in both is structural, and a structural
rule is a proxy for a judgement. The judgement is a person's, and no locale —
Korean included — has been read by a qualified native speaker. See §11.3 and I-17.
...
what the adaptive walk spends: every non-English language now has **2,212**
...
Removing 176 English-only anchors from the beginner band therefore made the other
thirty-one languages *denser*, not thinner. That was not the reason for the change
and it is the clearest evidence that the change was right.

## 10.9 Item quality above the floor — **VERIFIED, with the same caveat**

`leveltest:ambiguity` checks twenty-seven rules, nine photographed regressions and
eight frame fixtures over all 3,995 items and passes. It passed before this cycle
too, on a bank in which a quarter of the contextual items had more than one right
answer, and what closed that was not another rule of the same kind — the builder now conjugates every
option into the answer's own form, checks the particle the frame carries, and
refuses the frame outright when more than one option survives. It writes its
surviving questions to `data/generated/cloze.json`, so Today's Vocabulary and
Review ask the questions the Level Test validated rather than building their own.
One place decides.

Three items found by reading, whose shared cause is the cause of the larger class:
...
All three were a bare frame whose only verb fits anything. The fix went into the
content rather than the builder, because each sentence was weak as a *teaching
example* for the same reason.

**No new gate for that class, deliberately** — *and that was reversed in the
following pass; see §20X.4.* The reasoning here was that the tempting
generalisation, a noun blank whose sentence ends in a general verb, fires on 54
of the 164 noun items, and that the constraint usually comes from the sentence's
other argument: `____에서 채소를 사요` is pinned by 채소 no matter that 사다 is
general. That is still true, and it is why the rule as finally written requires
the blank to be the sentence's **only** argument — which narrows it to the cases
this paragraph was worried about deleting, and leaves `____에서 채소를 사요`
alone. What the paragraph got wrong was treating "a rule that deletes 54 items"
as the only available shape of rule.
...
**Word content**: **32 complete at 3,864 words. There is no partial language.**
`locale:content:check` reads *32 complete · 0 partial · 0 with no vocabulary
...
at 2,724 meanings, 2,724 example translations and 36 long *More about it*
paragraphs each: 65,376 strings, written rather than derived from a dictionary.
The language-picker row that used to say *600 of 3,333* for those twelve now
says what it says for every other language.
...
**They are model-written and no native speaker has read them.** I-19 was the
coverage issue and it closes, because coverage is now complete and a gate proves
it. Review is a different claim and it is I-17, which does not close: not one of
the thirty-two languages has been read by a native speaker, including Korean.
Coverage is not review, and this document may not use the second word for the
first thing.
...
runs now: 12,800 four-option questions drawn from all 3,393 taught words, asked
...
`locale:editorial` reports **0 errors and 13 warnings** for a person to read,
re-run this cycle — all thirteen the *split-translation* class, one English
button worded two ways in a language; the Korean ones were read and are correct — a unit title and
...
## 13.1a Schema 14, and the gate that walks every version to it — **new**

Schema 14 adds one field, `level_test_sitting`, for the resumable Level Test
(§10.6). It is the first new persisted field since 13, and the migration writes
`null` and touches nothing else.

That is easy to say and it is not what a paid, offline, account-less app has to
be able to prove. A learner's history exists in exactly one place, so the
question is not whether *this* migration is correct — it is whether **a customer
who bought version 1.0.0 can install this one without losing anything**. That is
a question about the whole ladder rather than about any rung, and the
per-migration tests could not answer it: each starts from a hand-made row at its
own version, so a migration that misbehaves because the one before it left an
unexpected shape has nowhere to show.

`storage/upgradeCompatibility.test.ts` is new and answers it directly. It plants
the **same complete, used learner** — three letters at three stages, two words
with review memory and due dates, a streak, a notebook, a day of activity, saved
words, a level-test result and a half-finished daily plan — at **every one of the
twelve schema versions the repository still supports**, in the shape that version
actually wrote, and walks each of them to 14. Then it reads back every category a
learner would notice missing:

| Asserted at every version | |
| --- | --- |
| Learned letters, mastered words, and no progress row lost | 5 rows in, 5 rows out |
| Completed lessons and the attempt history behind them | sessions and attempts intact |
| Theme, interface language, voice, daily target, guide settings | all preserved |
| The streak, which one lost day destroys | `active_days` byte-identical |
| Learning time and the daily completed count | `activity` roll-ups intact |
| The review queue — due dates, stability, lapses, confusion pairs | `memory` intact |
| Saved vocabulary | `saved_items` intact |
| The half-finished day, with the credited word still credited | `daily_plan.completed` intact |
| The level and when it was measured | `level_test` intact |
| The learner's own word order | `content_seed` intact |
| No counter rises, and none falls | attempts and passes compared before and after |
| No duplicated review item, no content id disconnected from its progress | keys compared as a set |
| Running the migrations a second time changes nothing | every store compared, `last_opened_at` aside |
| An interrupted run resumes to the same profile as a clean one | one migration applied, the app "killed" before the version record, then retried |

Seventy-four assertions, and it is in `npm test`, which is in `verify:quick`.

Two details are worth stating because they are where this kind of test usually
lies to itself. The **interrupted-run** case compares two independent drivers,
and one field is *supposed* to differ between them — `content_seed` is the
learner's own shuffle of the corpus, generated at v11 so that two people at the
same level are not taught the same ten words on the same day. It is asserted
present rather than equal, and asserted *unchanged* where the fixture already had
one. And the **idempotence** case compares the stores' contents through the
driver's own reader rather than a dump method written for the test, deep-copied
before the clock field is stripped, because a snapshot that edits what it read is
a test comparing a profile it changed itself.
...
**15,828 distinct files over 15,948 voice slots, 81.9 MB**, two Korean neural
...
Every one of the 3,864 taught words has a headword clip and an example clip in
both voices — 3,864 of each in the manifest, which is the check that the 17
...
**§36, and the course that had no route through it.** A learner who cannot hear
could not finish a single Numbers lesson: a listening question's whole stimulus
is a clip, all nineteen lessons list that kind, a mastery check asks every item,
and passing one is what completes a lesson. `settings.sound_free` has existed
since §36 and `domain/review.ts` has always honoured it; the Numbers course
never asked it, and never asked the player either, so a build with no audio in
it produced the same dead end. It asks both now and `numbers:qa` §11 measures
that every lesson still asks every item without a heard-only question. See
§20Q.3 and I-146.

**The per-question escape, which is the one that is actually reachable.**
`MyPage` removed the switch that turns `sound_free` on, so a learner arriving
today cannot set it. The letter side's answer to that is a **Can't use audio?**
under the prompt of any question whose whole stimulus is a clip, which swaps it
for an equivalent visual question — same item, same options, same answer, same
scoring. The Numbers course now offers the same control, on the same string, on
349 of the 352 listening questions the engine can build; the three it cannot are
`만 원`, where nothing on the screen would tell 만 원 from 만 without the clip,
and the button is not drawn rather than a worse question being invented.

What is still owed is a reading by somebody who actually needs it. Everything
above is a route that exists and is gated; whether it is the route a deaf or
hard-of-hearing learner would choose is a question this repository cannot
answer about itself.
...
Every enforced budget is met at 3,864 words, and the row that used to be the
finding is now the fix (re-measured on the twentieth pass's production
build, after the error boundary and its copy were added — the block had been
carried forward from the seventeenth pass's tree):

```
first load                       322.8 kB /  460.0 kB   70%
corpus, first paint               56.3 kB /   64.0 kB   88%
corpus, whole                    357.6 kB /  900.0 kB   40%
largest route chunk               20.5 kB /   24.0 kB   85%
everything precached            1277.3 kB / 1600.0 kB   80%
```

The 77 kB the precache gained this pass is the 24 new *More about it* notes
in 32 languages, which land in band 1 by design; the largest route chunk is
the Greek locale pack, not the safety evaluator, which stays at 16.7 kB and
is why the English safety lists are not carried on the device (§20AA.3).
...
macOS and Xcode are unavailable in this environment — `xcodebuild`, `xcrun`
and `pod` are absent from the path, and there is no macOS to put them on. The
Xcode project is delivered in `result/ios-project`;
`result/BUILD_OR_SIGNING_BLOCKERS.md` records what a Mac would need to do.
...
| Suite | Cases |
| --- | --- |
| Web unit (`vitest`) | **1,570** (89 files) |
| Handwriting core (`vitest`) | **96** (5 files) |
| Korean morphology (`vitest`) | **242** (4 files) |
| Content safety (`vitest`) | **1,298** (4 files) |
| End-to-end (`playwright`) | **640** (320 × 2 projects) |

The twentieth pass grew the web suite by four — the failed-screen boundary
(3) and the write guard on a failed hydration (1, with two of the existing
hydration cases rewritten to the new rule) — and touched no other suite; the
end-to-end count is unchanged and the full run on the final tree is recorded
in §20AD.6. The paragraphs below describe the sixteenth pass's growth and are
left as written.

The sixteenth pass grew the web suite by 103 — the answer-reveal state machine (6), the
...
## 19.2 `verify:release` — one step is informational, one fails on a dirty tree by design — **VERIFIED**

This has to be said plainly, because a reader who runs one command should know
what each result means.

`verify:release` is 52 steps, of which the first is `verify:quick`'s own 69.
...
The other steps were run against this tree and all pass:
`verify:quick` (51 checks — this pass added `policy:runtime:check` and moved
`content:safety:check`, now the whole-inventory scan, into the quick chain so
that a content change cannot reach a commit without it), the store listing,
the curriculum export, the fonts, the three jamo and face measurements, the
status group, the modals, the 143 rendered screens, the app icons, the
relations, the four content builds, the four dictionary gates, the dictionary
performance budget, the content and example QA, the composite
Korean-education gate, Word Detail, the audio and pronunciation gates, the
coverage report, the issue tables, the documentation figures, the stroke
measurements, the end-to-end suite, **the packaged-artefact content scan**
(`content:safety:bundle:check`, new) and the release currency check. The
sentence this paragraph used to carry — *the learner-safety gate* among the
passing steps — is the one §20Y.6 marks CONTRADICTED: it passed, and the bank
it passed had 섹스하다 in it.
...
## 19.4a This pass, and the two gates that were checking themselves

Every rule added this pass was restored to its defect and watched to fail:

| gate | what was restored | what it said |
| --- | --- | --- |
| `levelTest.test.ts` | `SLIP = 0` — the model in which *I don't know* is worth nothing | the separation case fails: 6.676281133587935 is not less than 6.676281133587908 |
| `levelTest.test.ts` | `WARMUP_ITEMS = 0` — the old level-14 opening | 124 of 175 fail |
| `levelTest.test.ts` | `MAX_STEP = 30` — the old unbounded selection | 33 fail |
| `levelTest.test.ts` | `REPEAT_LIMIT = 99` — the old parking at one level | 21 fail |
| `leveltest:qa:check` | the same three | *the difficulty stepped 11 levels between two questions*; *one level was asked 13 times running*; exit 1 |
| `leveltest:bank:check` | `DICTIONARY_LEVEL_FLOOR = 1` — the floor that let 누가 in | *the builder and this gate disagree about the floor*; exit 1 |
| `leveltest:bank:check` | the reviewed grammatical-form list emptied | six findings naming each term the builder would ship again; exit 1 |
| `upgradeCompatibility.test.ts` | the migration ladder, one rung applied and the app "killed" | the interrupted profile is compared to a clean one and must match |

**Three of those did not fail on the first attempt, and the reason is the finding.**

`levelTest.test.ts` asserted `Math.abs(step) <= MAX_STEP`, with `MAX_STEP`
imported from the module under test. `leveltest:qa:check` did the same. So
setting `MAX_STEP` to 30 raised the assertion with it: **the old unbounded
selection produced 174 passing tests and a green gate.** The same was true of
`REPEAT_LIMIT`. The checks were measuring a constant's agreement with itself.

`leveltest:bank:check` had the identical fault dressed as a virtue: it parsed
`DICTIONARY_LEVEL_FLOOR` out of the builder, with a comment claiming that copying
the number into the gate would let the gate keep passing after somebody lowered
it. The comment had it exactly backwards — reading the constant is what lets the
guard evaporate silently.

And then a third time, after both of those were fixed. The same gate read the
reviewed `grammaticalForm` exclusion list out of the content file, with a comment
arguing that a reviewed list — unlike a threshold — *cannot* be weakened by being
read, because adding a term makes the gate stricter. That is true in one
direction and the other one is the one that matters: **emptying** the list empties
the builder's filter and the gate together, and the negative test that emptied it
passed.

All four now write the values out as literals and separately assert that the code
or the content still agrees with them, so moving a bound takes two deliberate
edits and a moved bound is named rather than merely tolerated. That is what §19.4's first
line means: a gate that has never failed is a gate nobody has tested, and a gate
that *cannot* fail is worse, because it reports success.

This is the third instance of the same class in this document — §7.2's compound
vowels, §20.1's ambiguity rules, and now this. Each time the gate answered the
...
`domain/activity.ts`, and the one definition it applies is `isQualifyingDay`.
A streak day is **a local calendar day with at least one qualifying learning
activity**: an answered question — a letter, word, review or Numbers question
the learner committed an answer to, right or wrong, including a checked
handwriting attempt — or an item reaching *learned* or a Numbers lesson
completing. The first such day is Day 1, never Day 0; the next local day is
Day 2; a missed day ends the run and the next learning day starts again at 1;
several activities on one day are one day. Home's chip, the Learning Activity
screen, My Learning's *days studied* and the weekly summary all read this one
function with the same two stores.

**What does not count.** Opening the app, visiting a screen, reading a
lesson's introduction cards, playing a recording, saving a word, changing a
setting, or sitting the Vocabulary Level Test. Measured foreground time
(`recordStudyTime`) is recorded and shown as study time and is not a streak
day on its own. This is the rule the nineteenth pass (§20AC.1) put in place
of the earlier union of "any recorded study activity", under which twenty
seconds on a session screen with nothing answered was a learning day.

**How the number is kept.** `settings.active_days` is the durable list and is
written only by qualifying events; the activity rows are read as corroboration
(a row whose `attempts` or completion counters are non-zero is a learning day
even if the settings write behind it was lost), and rows that carry only study
time are ignored. Nothing is migrated: the streak is derived from the evidence
on every read, so an existing learner's history is neither rewritten nor
re-counted. A learner who has never answered anything has `status: never` and
Home shows *Start today* instead of a flame beside a zero; a lapsed learner
reads 0 days, which is true. The chip is not rendered until the profile has
loaded, so a returning learner never sees the empty default for the instant
IndexedDB takes to answer.

**Edge cases pinned** (20 fixtures in `domain/streak.test.ts`, 11 in
`store/streak.test.tsx`, 3 browser cases in `e2e/streak.spec.ts`): the first
answered question, right or wrong, is Day 1; more answers the same day are
still Day 1; the following day is Day 2; a missed day and a new activity is a
new Day 1 with `longest` kept; reload, unmount-and-reopen and a cut network
leave the number unchanged; a duplicate write of the same day does not
double-count; a study-time-only day is a gap on every screen; a DST change and
a repeated or skipped local day neither split nor lose a day (`dateKey` is the
local calendar); a future-dated row cannot inflate the run and a back-dated
row is history; an existing profile's recorded days are kept as they were; the
level test counts for nothing; a Numbers answer counts before its lesson
completes and a completed lesson is credited once. Negative cases N12 and N13
in `docs/content-gates-negative.json` put the zero-based count and the
viewing-counts rule back and require the fixtures to fail.
...
| | |
| --- | --- |
| Unit and integration tests | **3,206 across 102 files** — handwriting-core 96, korean-morphology 242, content-safety 1,298, web 1,570 (74 of them the Numbers journeys, negative tests, migration fixtures, exercise-engine tests of §20K and the question-type cases of §20P) |
...
I-126 stayed partial after that pass, and the v1.0.5 web pass closed it at the source (§20AA). Ten of the twelve concreteness anchors still did work then,
...
# 20N. The third screenshot pass

Three reports and a photograph. The photograph was a Numbers question answering
itself; two of the three reports turned out to be the same defect seen from
different screens.

## 20N.1 사는 4예요 — **P2, fixed**

The screen: 사 → 4, answered correctly, 맞았어요, and underneath it *사는
4예요*. The question's entire content was that 사 is 4.

There was no special case for 사. The previous pass replaced a fixed sentence
per exercise kind — every listen question ending with the same words — with one
built from the item: `{{subject}} {{value}}예요`, the word plus its particle plus
an ending. That fixed the complaint it was aimed at and created this one,
because on a **correct** answer a sentence generated from the item can only
repeat what the learner has just tapped.

Nothing is composed at runtime now:

| outcome | what is shown |
| --- | --- |
| correct | the item's authored note, or **nothing**. Seven items have one |
| incorrect | the note, the lesson's teaching line, or **nothing** for a plain numeral — *정답은 오* is already printed above the body |
| a distractor with a misconception | the sentence written for that mistake, where one exists |

The seven notes are the facts a question does not contain: *10,000원은 만
원이라고 읽어요*, *전화번호의 0은 보통 공이라고 읽어요*, *시간은 세 시 십 분처럼
말해요*, *가격은 오천 원처럼 말해요*, the two irregular months, 십육 → 심뉵, and
the counting forms. 일 is 1, and there is nothing else to say about it.

The empty wrapper went with the sentence. `FeedbackState` already declined to
draw its body when given nothing — but a JSX fragment is truthy whatever is
inside it, so a correct answer with no note drew an empty padded box under the
verdict, which is the same defect in the shape of a gap.

### The gate, and the two defects it found on its first run

`copy:generated` builds every exercise the engine can build, resolves the same
keys the page resolves with the same interpolation, and reads the result: 95
items, 19 lessons, practice, a retry and mastery, every outcome, every
distractor path, 32 languages — **43,744 rendered strings**.

The string ledger from the last pass could not have caught any of this. Not one
of its 834 entries said *사는 4예요*; the sentence had no entry anywhere.

| found | occurrences |
| --- | --- |
| `wrong_system_context` shown to learners as a **raw key** — declared, attached to two options, never written in any bundle | 1,024 |
| the counting-word line under questions about numerals, because every sibling distractor was labelled `wrong_counter` whatever the item was | 32 |

It fails on a tautology after a correct answer, an empty render, a raw key or
placeholder residue, English leaking into another language, a category label, a
claim about learners, a repeated verdict, and the counting-word line on a
non-counter. **Negative-tested**: restoring the generated template produces
7,927 findings, the first being *ko … korean_to_digits 삼 [correct]: says only
what the question said — "삼은 3예요."*

## 20N.2 The document was a scroll container — **P1, fixed**

Two of the three reports, and one cause.

`.toggleInput` is `position: absolute` with no offsets — the ordinary way to
keep a real checkbox focusable while a styled track draws the control. Its label
was not positioned, so its containing block walked up to `#root`. **An
absolutely positioned box is clipped by an ancestor only when that ancestor is
in its containing-block chain**, so the scroll pane's `overflow: hidden` did not
clip it. Two of them sit near the bottom of My Learning, at document y≈1908 and
y≈1960, and they stretched the document to 1,961 px against an 844 px window.

| | before | after |
| --- | --- | --- |
| document scrollHeight / clientHeight | 1961 / 844 | 844 / 844 |
| tab bar y, before and after scrolling | 780 → moved with the page | **780 → 780** |
| blank ground below the bar | present | 0 px |
| pane scrollTop after a full scroll | 0 (the document moved instead) | 1919 of 2699 |

A scrollable document carries the whole app when it moves — the tab bar with it,
blank ground underneath. That is the My Learning screenshot.

**It is also the Privacy report.** A document's scroll offset survives a route
change. Carry a non-zero one from My Learning into a shorter page and Privacy
renders shifted, its bottom clipped, its bar adrift. The intermittency was never
mysterious: it depended on whether a gesture had reached the document, which
happened when one landed outside the pane or after the pane hit its end. An
earlier pass recorded Privacy as *not reproduced* after a single direct visit,
which is the wrong shape of answer for an intermittent fault — a direct visit is
the one entry path that carries nothing in.

Fixed in two layers. `html, body { overflow: hidden }`: the shell owns the
viewport, everything a learner scrolls is inside it and says so with
`data-scroll-region`, and the class of defect becomes structurally impossible
rather than this instance of it fixed. `.toggleLabel { position: relative }`
stops the leak at source.

**Negative-tested both ways.** Reverting the shell guard fails `tab-bar.spec.ts`
with `Expected: <= 845, Received: 1961`. Reverting only the containing-block fix
leaves both suites green — which is the evidence that the guard, not the patch,
is what makes it impossible.

### The two suites

`e2e/tab-bar.spec.ts` — 31 cases. The bar's viewport rectangle before and after
a real wheel gesture, to the pixel, at seven sizes including landscape, three
text scales and both appearances; the document at `scrollTop` 0 with
`scrollHeight <= clientHeight`; the pane stopping at its own bottom; nothing
blank below the bar; every tab still navigating after a scroll; a fresh tab
starting at its own top; a dialog leaving the bar and the scroll where they were.

`e2e/privacy-stability.spec.ts` — **108 entries**: six state-carrying paths — a
deep link, a walk from a scrolled My Learning, one after a dialog was opened and
closed, Back then Forward, a reload on the page, and three locales — at three
sizes, six repetitions each, measured on arrival *and* after scrolling to the
end. No settings group, no voice content, exactly one scroll region, the
document unmoved, the pane never past its own range, the bar unmoved, nothing
blank beneath it, the last line of prose above it.

Its first run reported a defect it had caused itself: the reload path measured
before the launch overlay had cleared, so the wheel landed on the splash and
nothing moved. The harness waits for the launch now. A gate that has never been
wrong has not been used.

## 20N.3 Four pixels — **P3, fixed**

초기화 sat four pixels to the right of 맨 처음 글자부터 다시 시작해요, and 백업,
앱 and 화면 did the same. Every box measurement this repository had said the
column was straight, and it was: `.groupTitle` carried
`padding-inline-start: var(--hg-space-1)`, which moves the first glyph without
moving the element.

| | box | ink |
| --- | --- | --- |
| `초기화` (h2) before | 20.0 | **24.0** |
| its description (p) | 20.0 | 20.0 |
| `초기화` after | 20.0 | 20.0 |

The padding was an optical nudge for the uppercase tracking, and an optical
nudge applied to one of two things sharing a rule is a misalignment with a
reason attached.

`align:sections` measures a `Range` over each element's first text node rather
than the element — the only way to see the difference between *the box is
aligned* and *the words line up*. My Learning, Privacy and Legal, five widths,
100/150/200% text, Korean, German and Arabic, where the axis is mirrored rather
than skipped: 60 groups, 240 lines. **Negative-tested**: restoring the padding
produces 180 findings.

## 20N.4 The PDF made four parsers complain

`pdftotext` reported *name token is longer than what the specification says it
can be*, twice. Chromium turns each heading `id` into a PDF named destination
and escapes every non-ASCII byte as `#xx`; a Korean syllable is three bytes and
therefore nine characters in the name, so a heading like *20M.6 1개 연습 · 1개는
바로 떠올랐어요 · 1개는 곧 다시 나와요* produced a token several hundred bytes
long against the specification's 127.

The document rendered correctly either way — a warning is a warning — but a
report that makes a parser complain is one somebody has to decide to ignore, and
these go to a store submission. `build-pdf.mjs` now keeps the readable slug when
its *encoded* length fits and falls back to a short deterministic hash when it
does not, so an anchor does not change between builds and the contents list
still shows the heading's real words. `pdfinfo`, `pdftotext`, `pdftoppm` and
`pdftocairo` are all silent.

# 20O. The fourth screenshot pass

Six reports and two photographs of malformed letters. The two photographs were
one defect class each; four of the six reports were one defect each; and the
two that were not — a Back button that meant something different on every
screen, and a correct answer that did not count — were both cases of a rule
existing in more than one place.

The pattern this pass has in common with the three before it: **every one of
these was green**. Not one was caught by a failing gate, and in three cases the
gate was looking at the right screen and asking the wrong question.

## 20O.1 ㅌ had a notch, and it was a family — **P1, fixed**

The photograph: the tracing glyph for ㅌ with a square bite taken out of its
upper-left corner.

Nothing about it was a rendering accident. The upright was authored to start at
`[24, 20]` while the top bar started at `[24, 18]` — two units below it. Both
are stroked centrelines with butt caps, so in the two units before the upright
begins, nothing covers its outer half, and the letter's corner is missing a
rectangle 4.5 units wide and 2 deep.

`strokeVectors` does close corners. An end that meets another stroke's end is
classified `corner` and extended by half a pen, and only the *later* stroke is
extended so that a finished stroke never grows towards one not yet written.
That rule is right and it cannot help here, because the extension runs along
the stroke's own tangent: it lengthens the upright upwards and can never move
the bar sideways. A flush corner needs the two centrelines to **start at the
same point**, which is how ㅁ and ㅂ were authored and why they have never had
this defect.

It was a family. `digeut` and ㄹ carried the same two units, so the corner was
open on ㄷ, ㄸ, ㅌ, ㄹ and on 다, 라, 말 and 글 — eight of the seventy-three
taught items. Measured at 512 px over the 100-unit box, as the fraction of a
one-pen square at the junction that holds ink:
...
ㄹ was the worst in the curriculum and had a second offset on top of the first:
its waist began 3.3 units left of the stem it meets. Moving the *stem* right by
half a pen was tried and was wrong, and the reason is worth keeping — 
`shapeToFace` scales the authored letter and **not** the pen, so a half-pen
offset typed in authored units is not half a pen once the letter has been
fitted to the face's proportions. Measured, 4.5 became 6.09 and the top bar
came out 1.6 units proud of the stem; `letters:face` read it immediately as a
left stem that no longer stands against the letter's edge. The waist moved to
the stem's own coordinate instead. A number that means "half a pen" cannot be
written down in that file; a coincident point can.

## 20O.2 꽃 was bridged through the middle — **P1, fixed**

The photograph: 꽃, with the two ㄱ of its ㄲ visibly unlike each other and a
stray vertical between them.

The two ㄱ are in fact identical — measured, both 0.219 of the block wide and
0.197 deep. What made them look unlike was a third thing standing between them:
the ㅗ's stem, rising into the gap, touching the left ㄱ's leg. 85 px of shared
ink at 512², a bridge across the middle of a tense consonant.

The measured part boxes overlap. The face puts 꽃's onset at y 0…0.343 and its
vowel at 0.227…0.486, and that is a correct measurement rather than a bad one:
in Pretendard the ㅗ's stem really does rise between the two ㄱ. The face can
afford it because its stem is thin and that gap is wide. This model writes
every stroke with one uniform pen, and at that weight the same arrangement puts
the stem *through* the leg.

Separating the **boxes** was the first attempt and it was wrong on blocks that
are perfectly fine. 고 is a ㄱ over a ㅗ whose leg comes down beside the vowel's
stem; their boxes overlap by nearly half the block and their ink never touches.
That interlock is what makes the block read as one character, and pushing the
vowel clear of the consonant took 고's ㅗ down to a bar with no stem at all —
which `glyphshape:qa` reported within seconds as the guide and the demonstration
teaching different shapes.

So the correction is driven by ink. Compose, measure segment against segment,
shorten the lower box from the top a quarter-pen at a time, redraw. Of the
thirty-three measured blocks it moves exactly one. Only the top edge moves, so
the vowel's bar stays where the face put it and only the stem shortens — which
is the part of ㅗ that was in the wrong place.

The result is a 12% component drift from the face for 꽃's vowel, and it is
**declared** — in `CANONICAL_COMPONENT_DEVIATIONS`, against that block and that
component, with the reason. Not as an exemption for tense consonants; per
component, per block, arguable.

## 20O.3 The gate that would have caught both

Three gates already look at glyphs. `strokes:qa` reads the data,
`strokes:visual` watches the ink behave as the animation runs, and
`glyphshape:qa` scores the finished letter against the reference face as
intersection over union. All three were green on both photographs.

The reason is one sentence: **a similarity percentage cannot see a defect
smaller than its own tolerance.** ㅌ's notch is 4.5 × 2 units of a 100-unit box,
about a thousandth of the glyph's ink, against a floor of 93% explained. 꽃
scored 97% while its tense consonant read as one shape with a spur in it.

`glyph:structure` asks questions with yes/no answers and no tolerance to hide
inside:

| | |
| --- | --- |
| **Junction continuity** | the one-pen square at every meeting of two stroke ends is solid ink |
| **Predicted islands** | the ink separates into exactly the pieces the geometry says it should |
| **Tense-pair separation** | the halves of ㄲ ㄸ ㅃ ㅆ ㅉ are two pieces, standalone and inside every block |
| **Part collision** | no two letters of a block share ink |
| **Clipping** | no ink reaches the edge of its box |
| **One geometry** | every taught character is actually wired to the canonical source |

Two of those needed a correction before they measured anything. The junction
probe first looked only at ends the model *classifies* as `corner` — which is
the later stroke's end, always covered by its own extension — and ㅌ passed it.
A junction is a place, not a property of one stroke, so both ends that arrive
at it are probed now. And the island *prediction* first asked whether two
centrelines came within a pen of each other, which is right for strokes that
cross and wrong for two collinear ones: the two ㅂ of ㅃ have centrelines six
units apart, well inside a nine-unit pen, and six units of clear paper between
them. It is a rectangle-overlap test now, exact for a butt-capped stroke.

The inventory is discovered from `characters.ts` rather than listed. Nothing in
that file names 73, and the only place a letter is named is a declared
exception with a reason — of which there are none.

  73 items · 86 junctions · weakest 100.0% · islands drawn = predicted 73/73
  6 doubled letters · 33 blocks checked for part collision · 0 exceptions

**What it deliberately cannot see.** Every question it asks is of the canonical
geometry and its own raster, so a defect that is *consistent* is invisible to
it. Deleting the base of ㄴ leaves a letter whose junctions, islands and parts
all agree perfectly with a geometry that is simply wrong, and the gate passes
it. Measured: it does. `glyphshape:qa` reports that mutation as ㄴ, 나, 다, 안,
산 and 한 — a missing segment in the six places it appears — which is why the
licensed typeface stays in the pipeline as an independent oracle. The two gates
answer different questions and the file's header says so.

Negative-tested five ways, each restored afterwards:

| mutation | what fired |
| --- | --- |
| the damaged ㅌ corner | junction 91%, and the named ㅌ fixture |
| the 꽃 composition restored | 85 px shared, and the named 꽃 fixture |
| ㄴ loses its base | `glyphshape:qa`: ㄴ 나 다 안 산 한 |
| the ㄲ halves merged | 1,564 px shared standalone, 475 px inside 꽃 |
| a character taken off the canonical geometry | two sources for one letter |

## 20O.4 Looking at all seventy-three

`docs/glyph-audit/` holds three contact sheets — 35 standalone letters, 5
doubled, 33 blocks — each captioned with the character and its drawn/predicted
island count. Every one was looked at. The corrected ㄷ, ㅌ and ㄹ have flush
upper-left corners; 꽃 draws two clean ㄱ over a ㅗ over a ㅊ; 다, 라, 말 and 글
carry the ㄹ and ㄷ fixes into their blocks.

That looking is the point of the sheets and not a formality. Both photographs
this section is about were of glyphs that scored 97% and 100%.

## 20O.5 Handwriting, re-measured against the corrected source

Fixtures regenerated from the corrected canonical geometry — 73 letters, 437
face-entries changed — and the robustness corpus re-run.

| | before | after |
| --- | --- | --- |
| FRR, all faces | 0.937% | 0.972% |
| FAR, all faces | 0.000% | 0.000% |

One new rejection, in gaegu: ㄸ, *small and drifted*. ㄸ's ㄴ moved up two units
to close its corner, which makes the letter marginally taller, and an already
marginal synthetic variation fell under the pass mark. It is one of 2,880
adversarial attempts and it is reported rather than tuned away; no threshold
was moved. The 96 handwriting cases pass unchanged.

The badge layout moved with the geometry and is the one place a fence nearly
had to. 꽃's ㅊ tick is the most enclosed anchor in the curriculum, and once the
ㅗ's stem shifted, badge 3 took the pocket badge 5 had been using and 5 landed
at 4.35 radii from its anchor — past the 4.25 the marker suite holds. The fence
did not move. Badges are packed **most-constrained-first** now, which is the
standard answer to a packing problem, and the tether is short again. (The first
version of that computed the crowding inside the sort comparator, which runs
O(n log n) times; it made one glyph slower to lay out than the whole suite used
to be. Measured once per stroke now.)

## 20O.6 The Level 30 learner and 그래서 — **P1, fixed**

The report said the level rules were proven. A learner measured at Level 30
opened Today's Vocabulary and was taught 그래서, a Level 2 conjunction, first.

The selector was not at fault. Walked across all thirty levels against the full
corpus, `pickNewWords` chose correctly every time; a Level 30 learner's new
words came from 28–30. The word arrived through the other pool.

```
recordIntroduced   writes a progress row when a word is shown
recordReview       writes a memory row when one is answered
weakestRecall      returned 0 for a word with no memory row
```

0 is the strongest claim that function can make. It is below `WEAK_RECALL`, so
the word is classified `weak`, and `weak` is scheduled ahead of everything else
in the day. Every word a learner had ever been *shown and walked away from*
therefore outranked every word their level had chosen. Open the app untested,
be shown three beginner words, answer none, sit the Level Test, come out at 30:
the next morning opens on 여자, 아버지 and 걷다.

It returns `null` for no evidence now, and the plan reads that as what it is —
an introduction that went nowhere, not a memory being lost. Such a word returns
to the *new* pool, where the learner's level governs whether it is ever offered
again, and `recentlyIntroduced` keeps it out for a fortnight regardless. For a
Level 30 learner, 그래서 never comes back.

Two further things, from the same brief:

**The day opens in the learner's band.** Consolidation outranks new material on
purpose, and consolidation reaches any level on purpose, and together they open
a Level 30 learner's day on 것 — a genuinely due word, correctly brought back,
and the wrong thing to put on the first screen of the day. Only the first
position changes: if the head word is outside the teaching zone and the plan
holds one inside it, that word leads. Nothing is dropped.

**A review says it is a review.** The plan always knew — `PlannedWord.source` —
and the screen never said. A caption above the question now reads 새 단어 or
복습, in all 32 locales, so a due word can come round without wearing the shape
of today's new material.

### Why the existing gate could not see it

`dailyvocab:qa` simulates three learners — levels 8, 14 and 20 — with **empty
progress**, and asserts an **aggregate**: that most new words land within one
level of the learner. The defect lived in exactly those two gaps. The learner in
the report had a state only the app creates, and the wrong words were three of
ten, which a distribution check is comfortably satisfied by.

`dailyplan:level` runs five profiles × thirty levels × thirty days, asks about
the **first** word as well as the distribution, and writes
`docs/vocabulary-plan-audit.json` — learner level, teaching zone, word, its
level, why it is there, mastery state, locale eligibility, plan date, level
epoch. There was no way to ask the product why 그래서 was in the plan. There is
now.

  4,500 plans · 45,000 words · 0 days opened outside the band · 0 short days
  new words: 61% exactly at level, 100% within one

`store/levelAppropriate.test.tsx` drives `LearnerProvider` rather than the
selector, because the state that caused this is one only the app creates. A
domain test that constructed both rows would never have found it, and one that
constructed neither would have passed.

Negative-tested: restoring recall-0-for-no-evidence makes the gate name 그래서
by name and fails the store test; removing the open-in-band rule produces 25
findings from level 4 upward.

## 20O.7 A correct answer the plan had nowhere to put — **P1, fixed**

*"Sometimes you finish a word and the counter does not move."*

`completeWord` returned the plan unchanged for a word the plan did not list.
That reads as a safety rule and behaves as a leak: the session had already moved
on, so nothing retried and nothing reported.

The two plans are allowed to disagree, and do. `WordSessionPage` freezes its
queue once — deliberately, so that a plan changing underneath cannot shorten the
sitting or run the bar backwards. The plan is a *derivation*, and one of its
inputs is `levelFromProgress`, which moves as words are learned. A learner who
finishes the word that takes them past two thirds of their level flips the
planning level **mid-sitting**; the plan is rebuilt, its unresolved new words
are replaced, and every remaining question in the frozen queue is about a word
the stored plan has never heard of. They answer correctly, and nothing counts.

Crediting is total now. A word the learner answered correctly in a sitting this
app posed is credited to today, and if the plan no longer lists it the plan
**adopts** it. The goal does not move. Adoption cannot inflate anything, because
a sitting only ever asks about words some plan for today chose. Still idempotent
on `completed`, which is what makes a double tap, a re-render and a retry of an
already-finished word count once.

Two tests were pinning the old behaviour and are corrected rather than deleted,
because both had written the leak down as an invariant — `vocabularyDay.test.ts`
asserting that an unlisted word is *ignored*, and the property suite expecting
that such a word *credits nothing, because it is not a target*. The property
still asserted is the one that matters, exactly-once, over 2,000 random sittings
and 1,000 with mid-day retakes.

And an introduction no longer looks ignored. A new word owes intro then meaning,
and the day's counter measures *words finished*, so pressing 알겠어요 correctly
moves nothing — the honest reading of which is that the app dropped the answer.
The caption now carries the word's own position, `새 단어 · 1 / 2`, counted from
the frozen queue so it cannot disagree with what is on screen.

## 20O.8 One Back button — **P2, fixed**

Fifteen screens passed their own `onBack`, and the phone's button had a
sixteenth rule of its own: pop whatever this session pushed, and only at depth
zero consider Home. The two disagreed by construction — the level test alone
said `navigate(-1)` on four headers and `navigate('/me')` on a fifth — and the
depth rule made the same screen behave differently depending on how the learner
had arrived at it.

`ui/routePolicy.ts` is the single answer: every route the router serves, with
its tab, its parent, whether it is a sitting, and whether that sitting may guard
against being abandoned. `resolveBack` is pure and is walked route by route;
`SystemBack` runs the answer and owns the two dialogs it can ask for.
`AppHeader` no longer takes `onBack` at all, so a screen cannot hold a private
opinion about where back is.

| where the learner is | what Back does |
| --- | --- |
| a modal is open | the overlay answers and the route is never consulted |
| Home | the exit confirmation, immediately, whatever the stack holds |
| a bottom-tab root | Home, in one press, from any history |
| a nested screen | the screen they came from, or its declared parent |
| a sitting | the context that owns it, never Home |
| a sitting with work in it | asked first — the level test, a mastery check |

The Korean is 앱을 종료할까요? / 취소 / 종료.

Three defects surfaced while wiring it, each fixed at its cause:

- **The tab bar built a history chain.** Words → Letters → Review needed four
  presses to reach the exit. It pushes once leaving Home and replaces after
  that, which also keeps the browser's own Back inside the app — replacing
  *every* tap was the first version, and it left a learner one press from
  leaving the site from the first screen they touched.
- **The router handler could take a press meant for a modal.** React runs a
  child's effects before its parent's, so a screen deep-linked with a dialog
  already open registered that dialog *before* the router handler and lost the
  press to it. Back handlers declare an `overlay`/`route` tier now.
- **History depth was counted in a React effect.** React Router navigates inside
  a transition it may hold while a lazy route chunk loads, and in that window
  the browser is on the new screen while any React-derived counter still says
  the app has pushed nothing. Moving it to a layout effect narrowed the window
  and did not close it — the problem is that the *commit* is deferred, not when
  in the commit the callback runs. It reads `history.state.idx` now, the index
  React Router writes synchronously at push time. The end-to-end spec fell into
  that window on every full-file run and passed in isolation, which is what a
  race looks like from the outside.

`route:policy:check` reads `App.tsx` and fails if the table drifts from the
router: a missing rule, a stale one, a pattern ordered where it can never match,
a parent that is not a route, or a parent chain that never reaches Home.
Negative-tested three ways. 23 routes, 23 rules; 17 end-to-end cases.

## 20O.9 The Home row nobody needed, and three sentences

**The weekly row.** `이번 주 · 7일 중 1일 · 0분` sat under Home's cards,
restating what the streak pill above it already says and linking where the
streak pill already links. The row, its stylesheet, its click target and the
`weeklyReport` query that existed only to fill it are removed — not hidden.

**The backup sentence.** It ended `어디에도 올라가지 않아요` — an implementation
detail, phrased as a privacy promise, on a screen a customer reaches when they
are changing phones. What they need there is what the button does. Rewritten in
32 locales; the upload claim stays on the Privacy screen, where it is the
subject and can be supported.

**The soft verdict.** `조금 달라요` — *it's a little different* — softened a
verdict the screen had already delivered in red. A learner told their answer was
*a little* different has to work out whether it counted. It is `틀렸어요` now,
decisive in all 32 languages.

`정답은 8` went with it. That was the **third** time the screen said the same
thing: the tapped option already carries a red cross, the right one a blue tick,
and both marks already carry their own screen-reader text on the option itself.
So the sentence was for nobody — and on a bare numeral question, where there is
no rule to explain either, it was the only thing in the box. The body is the
authored note or nothing, passed as `null` rather than as a fragment that
renders to nothing, so no empty padded box is left behind.

## 20O.10 The explanation that was a paragraph — **P2, fixed**

The photograph: the lesson on numbers past ten, opening with

> 부분을 순서대로 읽어요. 십일은 십-일, 11. 이십은 이-십, 20. 삼십오는 삼-십-오,
> 35예요.

Three worked examples, three arabic numerals and three hyphenated
pseudo-spellings in one block of prose, in a course for people who cannot yet
read the alphabet fluently. Every problem with it is the same problem: the
*structure* of a number is a spatial fact, and it was being delivered as a
sentence to hold in the head and rebuild.

십-일 was the worst of it. It is not how 십일 is written, spoken or spelled — a
diagram drawn in punctuation, which a beginner has no way to tell from the word.

`NumberBreakdown` draws it. The numeral, a way to hear it, one chip per
morpheme coloured by what it does, then the word they make:
...
Hear, see the parts, put them together — and the practice phase that already
follows is try one. The sentence beside it is a caption: 부분을 왼쪽에서
오른쪽으로, 순서대로 읽어요. `ExplainStep` carries which numbers a step draws,
so this is a decision in the lesson data rather than a special case in a page.

**`numberParts` segments the authored Korean and never generates a reading**,
which is the whole of why it can be trusted: it can only return pieces that were
already in the word. The obvious alternative — take the value and build the
Korean — is the way to ship 일만 for 10,000 and 육월 for June. 십육 gets no
diagram, because it is said 심뉵 and chips reading 십 + 육 would hide the whole
lesson; neither does a single-part numeral, a counter or a phrase. Ten unit
cases pin what it refuses to do, and one of them reads the lessons themselves,
so a number added to a `show` list is covered the day it is added.

The colour carries nothing on its own — two fills, not seven, and the order,
the spacing and the words say the same thing without them.

## 20O.11 A gate that could not read its own arithmetic

The previous edition of this report stated **1,289** unit cases and a breakdown
of **976 + 216 + 96**, which is 1,288. A reader can check that in three seconds
and the document could not.

Every figure in it was derived except the web count *in those two sentences*.
`docs:consistency` matches the number in the suite table — `| Web unit
(vitest) | **977** |` — and the two prose shapes carrying the same number
matched no pattern, so the gate reported it as "not stated anywhere" and passed.

A gate whose patterns are narrower than the document's prose certifies the half
it can read. Both shapes are patterns now, so the total and its parts come from
one derivation and cannot disagree.

## 20O.12 I-126, re-measured and split

The named cause of I-126 — a word called abstract for having an opposite — is
fixed in `_concreteness`, and was fixed in the previous pass. What was left was
the eighteen words moved by hand, and the brief for this pass asked for the
model to do that work instead of the list.

So the model was asked what it would give each overridden word if the override
were removed. Every one is still load-bearing:
...
| 모르다 | 7 | 1 |
| 싸다 | 14 | 5 |
| 가깝다 · 높다 · 닫다 · 멀다 · 빠르다 | 8 | 2–4 |

The concreteness term is fixed and the words are still wrong, which means the
concreteness term was not the whole cause. The component breakdown says where
the rest is: 길다 scores frequency **0.840**, 짧다 0.852, 가깝다 0.813 — a
frequency *cost*, where 1.0 is a word the corpora never saw. Those are ranks
around 2,000 to 2,700 for first-semester adjectives, and the reason is I-73's:
the matcher counts surface forms, and an adjective lives its life inflected —
길어요, 긴, 짧은, 가까워요 — so the citation form ranks as if it were rare. The
other half is 모르다, ranked correctly at 46 and *still* levelled 7, because
linguistic 0.320 and semantic 0.361 outweigh it under `WEIGHTS`.

Neither is the antonym rule. The measurement that came out of asking is now
**I-133**: 85 words an editor marked maximally useful sit above level 6,
including 월요일, 화요일, 수요일, 목요일 and 일요일 at 9, 한국 at 10, 덥다 at 12
and 비싸다 at 15. A beginner course that does not teach the days of the week
until level 9 is not teaching them when they are needed.

The fix is written down — make editorial usefulness a **ceiling** rather than
one of four weighted terms — and is deliberately **not landed here**. It
re-levels the corpus, which regenerates the level-test bank, the anchors and the
editorial QA, and that is not a change to make and verify in the hours before a
...
# 20Q. The sixth screenshot pass — what a badge is, and what a row reserves
...
thumb's width in from the page. Both had been shipping. Neither was visible to
any of the gates, and in both cases the reason is the same shape: **the gate was
measuring the thing the code computes rather than the thing the reader sees.**

## 20Q.1 A badge is not its fill — **P1, fixed (I-144)**

![Stroke-order badges, bounded by the fill and then by the ring](report-assets/stroke-badges-bound.png)

A numbered badge is a `<circle r="5.6">` with `stroke-width: 0.9`. An SVG ring
straddles the circumference, so half of it is *outside* the radius and the disc
paints out to 6.05. Every bound in `strokeMarkers.ts` was written against 5.6.

```
label.x clamped to [radius, 100 - radius]      the centre of the fill
painted extent                                  radius + MARKER_RING / 2
the difference                                  0.45 units, cut off by the viewport
```

Twenty-one badges over twenty characters sat at exactly that clamp. `ㅌ` #3 at
(5.6, 5.73) is the one that was photographed. `ㄷ` #2 at (5.6, 5.6) lost ring on
two sides at once, because `.paper` also carried `border-radius: 20px` — and an
element whose overflow is hidden clips at its curve. That radius could not be
reconciled with anything in the layout even in principle: 20 px is 13.3 viewBox
units at the 150 px the gallery draws and 10 at the 200 px the introduction card
draws, so there was no single number the placement could have been given.

Three checks were watching and all three passed:

| | what it asked | why it passed |
| --- | --- | --- |
| `strokes:qa` | do two badges overlap? | they did not |
| `strokeMarkers.test.ts` | `label.x >= radius`? | it was, exactly |
| `strokes:visual` | is a badge standing on the ink? | it was not |

**The fix is in the placement, not on `ㅌ`.** `MARKER_RING` is a constant the
layout reads and the stylesheet writes — `strokeMarkers.test.ts` reads the CSS
back and asserts they agree — and `paintedRadius` is what every bound is now
written against. `ontoPaper` is the whole bound in one exported function: the
straight sides, then the four corner arcs, pulling a candidate back along the
radius of the arc it is outside. The layout, the unit test and the gate all call
it, so there is no second opinion about where the paper is.

The paper itself moved *inside* the SVG. It is a `<rect rx="12">` in the same
coordinate system as the ink and the badges, `.paper` declares `overflow:
visible`, and the element clips nothing at all. The hairline uses
`vector-effect: non-scaling-stroke` so a 1-unit border is one device pixel at
every size rather than 1.5 px on one card and 2 px on another.

One more thing came out of measuring the ring: badge-to-badge clearance was also
written against the fill (`radius * 2 + 0.8`), and `글`'s third and fifth rings
were within a tenth of a unit of touching. They clear by 0.8 of painted radius
now.

No per-glyph override was added, and the gate asserts there are none: a
hand-placed `ㅌ` would have fixed `ㅌ` and left the other twenty.

## 20Q.2 Thirty-two pixels reserved for something most rows do not draw — **P1, fixed (I-145)**

![The Numbers course list, before and after](report-assets/numbers-list-rail.png)

Every lesson row began with a 20 px `<span>` holding a tick when the lesson was
finished and nothing at all otherwise — which is most of the course, most of the
time — followed by the row's own 12 px gap.

```
module heading   padding 4 px      →  text at 20 px from the edge of the phone
summary card     padding 16 px     →  text at 32 px
lesson title     16 + 1 + 20 + 12  →  text at 49 px
```

Three left edges on one screen, and the one the eye follows was the furthest in.
The cost a learner feels is in the picture: on a 390 px phone *Things take 하나,
둘 · prices take 일, 이* wrapped onto **three** lines with a quarter of the row
empty beside it. The status pill would not shrink either, so on a narrow phone
it took its width out of the title rather than moving out of the way.

There is no leading column now. The tick lives inside the **Completed** pill —
the same fact said once instead of twice — and one `--numbers-rail` custom
property puts the module number, the module goal, the summary card and every
lesson title on one rule, with the chevrons and the module lesson-counts on one
rule at the other end. The card's inline padding is deliberately one pixel short
of the rail: it draws a border and the heading above it does not, and what has
to line up is the *ink*.

The row is one flex line and the title block is `flex: 1 1 9rem`. That is a
flex-basis rather than a `min-width`, which is the difference between "the title
wants this much beside a badge" and "the row overflows at 200% text": when 9rem
cannot fit beside the pill, the *pill* wraps to a second line and the title keeps
the full width; on its own line the title can shrink below 9rem, because
`min-width: 0`. No breakpoint is involved, because what decides it is not how
wide the phone is — it is how wide this row's title and badge are, in this
language, at this text scale. A media query would have to guess both, and would
be wrong for thirty of the thirty-two languages.

`.lessonLocked`, `.lessonLockedRow` and its hover rule had been dead since the
prerequisite gate was removed in v1.0.2, and the page's bottom padding reserved
the phone's gesture bar a second time under a navigation bar that is a *sibling*
of the scroll area rather than an overlay on it. Both are gone.

The picture also shows **I-150**: a completion tick beside *Review due*, the one
status on that screen that is asking the learner to act. It was drawn from
`isDone`, which includes `review_due`; the rule at the top of `NumbersPage.tsx`
has always said *completed and mastered and nothing else*, and the code now does
what the comment above it says.

## 20Q.3 A learner who cannot hear could not finish a Numbers lesson — **P1, fixed (I-146)**

Not a harder route through the course. None.

A `listen_choose` question's whole stimulus is a clip: the prompt carries an
audio id and deliberately no text, because printing the word would be printing
the answer. That is right for a listening question. What is not right is that
**all nineteen lessons list that kind**, a mastery check asks every item, and
passing a mastery check is what completes a lesson.

`settings.sound_free` has existed since §36 and `domain/review.ts` has always
honoured it. This course never asked it — and never asked the player either, so
a build with no clips in it or a manifest that failed to load produced the same
dead end from the other direction.

The run is built without the heard-only kind now, and dropping it is enough
rather than a degradation: `numbers:qa` §11 measures that every lesson still asks
every item, in guided practice and in the mastery check, sound-free. Two lessons
— minutes and weekdays — fall to one question shape, and the gate reports that
rather than hiding it. A shorter lesson somebody can finish beats a richer one
they cannot.

The decision is taken once, when a run mounts, and held for the run: a manifest
that finishes loading half-way through a check must not change the questions
under the learner or the count printed on the way in. And it is
`ready && !available`, not `!available`, so *the manifest has not answered yet*
is never mistaken for *there is no audio*.

**And a second accommodation, because the first one is unreachable.** `MyPage`
removed the switch that turns `sound_free` on — the *word* questions it existed
to avoid no longer exist — so a learner arriving today cannot set it and only a
migrated profile carries it. Honouring a preference nobody can express is not an
accommodation.

The letter side had already solved this and the note in `ChoiceExercise` is the
argument: *a setting is remembered, and the cost of a remembered setting is that
nobody who has not already found it can turn it on*. Its answer is a small
**Can't use audio?** under the prompt, per question, which swaps the clip for an
equivalent visual one — same item, same options, same answer, same scoring, so
the learner is not skipping the question, they are being asked it another way.
The Numbers course does the same now, using the same string.

What the substitute is has to be worked out per question, and the first attempt
was wrong twice over — which is what the gate caught rather than a reviewer.

| the item | the substitute | why not the obvious one |
| --- | --- | --- |
| a numeral | *Say this number with 일, 이, 삼* over the digits | the digits alone have two answers: a numeral's distractors include the other system's word for the same value, and 하나 and 일 are both 1 |
| anything with a gloss | *Which of these means this?* over the gloss | — |
| `만 원` | **nothing, and the button is not drawn** | 만 원 and 만 are both 10,000 in the same set, and 만 원 has no gloss. Nothing on the screen tells them apart without the clip, and a worse question is not the answer |

349 of the 352 listening questions the engine can build carry one, and
`numbers:qa` §11 proves each substitute identifies **exactly one** of the
options it is shown beside — computed from the options rather than from the
item, and resolved through the id each option was built with, because 오천 원 is
both a price and a context phrase and 세 시 삼십 분 is both a clock time and a
pitfall.

## 20Q.4 The Numbers course, read end to end

Ninety-five items, nineteen lessons, every explanation step, example, gloss,
note, prompt and feedback line, and every question the engine can build — 972 of
them over three attempts, 284 distinct. `numbers:qa` was green before the
reading and green after it. What the reading found is what no rule was asking
about.

**224 strings in thirty-two languages that no screen can draw (I-147).**
`ItemCard` draws `example_gloss` inside `{item.example && …}`, so on an item with
no example it cannot appear. Seven keys were in that state — the six phrases of
the *which system?* lesson and one clock time. `example.ctxPeople` read *사람은
하나, 둘, 셋으로 세요*, written and translated thirty-two times, on no screen, and
it is the sentence `lesson.choosing.step1` already prints two screens earlier
with the same three examples in it. `example.quarterPastTwo` was *2시 15분*,
which is `gloss.quarterPast` word for word. A thirty-third string,
`rationale.adjacent`, was unreachable for a different reason:
`MISCONCEPTION_FEEDBACK.adjacent` is `null` on purpose — *정답은 오* is already on
the screen — so nothing could ask for it.

**금요일 was glossed 한 주의 다섯째 날 (I-148).** Friday is the fifth day of the
week counting from Monday and the sixth counting from Sunday, which is how a
Korean calendar prints it. A learner who counted found the *correct answer*
wrong. 월요일 carried the same fault one place along. Both are relational now —
한 주가 시작되는 날, 주말 바로 앞의 날 — and true under either convention. Korean
only: the other thirty-one bundles gloss the words as *Monday* and *Friday*,
which needs no ordinal.

**One prompt still wrote its particle out longhand (I-149).** *순서대로 눌러서
{{value}}을(를) 만들어 보세요* — the register `i18n/josa.ts` was built to remove,
and the one occurrence left in any bundle in any namespace. The formatter could
not have rescued it: the placeholder renders a digit, and 35 is 삼십오 (를) while
20 is 이십 (을), so `endsInConsonant` returns null and falls back to the
parenthesis. The sentence puts the number first now and needs no particle.

**A lesson's status read the visit rather than the evidence (I-151).**
`lessonStatus` asked `opened_at === null → available` before it asked about any
work. That is right in every ordinary sitting and wrong in the one case worth
guarding: the stores are written optimistically, so `lesson_opened` can be the
write that loses a race with a reload while the work that followed it was saved.
A learner who had read two explanation steps would come back to a row with no
badge on it, reading *you have not started this*.

Seven new sections in `numbers:qa`, each written against something the reading
found or something the reading proved that nothing was holding:

| | |
| --- | --- |
| §11 | built sound-free, every lesson still asks every item, in both phases |
| §12 | no question is *about* an item a later lesson introduces, and every option that comes from a later lesson is a declared misconception rather than a stray — 60 of them, all `system_swap` or `adjacent` |
| §13 | no question is asked twice inside one phase of one run |
| §14 | a listening question's clip says exactly the option the grader accepts |
| §15 | every `rationale.*` key is reachable from `exercises.ts`, and an `example_gloss` belongs to an item that has an example |
| §16 | no bundle writes 은(는), 이(가), 을(를), 과(와) or 으로(로) longhand |
| §17 | the completion state machine, walked per lesson, including the six transitions that must **not** complete one |

## 20Q.5 What proves it, and what it was proved against

| | |
| --- | --- |
| `strokes:markers` | 73 characters, 269 badges: clipping against the straight sides and the rounded corners, ink overlap, ring-to-ring collision, leaders that end off their own stroke or run under another badge, badge order against the animation, legibility at 152 px, and the stylesheet read back so a clip cannot grow again |
| `numbers:layout` | 45 cases, 3,465 elements: seven sizes including landscape, 100/150/200% text, light and dark, and all 32 languages at 320 px, with four lessons seeded to real evidence so the badges are on screen to be laid out. It measures *ink*, not boxes — an empty box is perfectly aligned, which is why every box measurement on the old screen was correct |
| `numbers:qa` | 6 modules · 19 lessons · 95 items · 972 questions built, 284 distinct, 17 sections, 0 problems |
| `scripts/regression-gates-negative.sh` | five defects restored one at a time, all five caught, then restored and green |
| `strokeMarkers.test.ts` | 30 cases over every taught character, three of them new |
| `e2e/numbers*.spec.ts` | 33 cases on the running page at the mobile project |

The negative run is the part worth naming, because a gate that has only ever
been seen passing has proven nothing:

```
G1  the empty icon column at the head of every lesson row   2,565 findings
G2  a badge bounded by its fill rather than by its ring         24 findings
G3  a blank whose option list holds two words that fit it       12 findings
G4  둘 개 — the plain numeral in front of a counter               6 findings
G5  a completed_at with no evidence behind it, kept              19 findings
```

G3 is worth a sentence. The obvious sabotage — give two items the same gloss key
— does **not** work, because the builder already drops an option whose text
equals the answer's, so the question quietly loses a button instead of gaining
an answer. The way a second right answer actually reaches a learner is a blank
that two words fit, and the guard against that is `slot_group`: 맥주 한 병 beside
맥주 한 잔. Removing it produces twelve questions with two answers, and
`numbers:qa` §8 catches them by holding its own short list of same-slot pairs
rather than by reading the declaration back.

## 20Q.6 The delivery's paperwork disagreed with itself — **P2, fixed (I-152)**

`result/` is what a buyer is handed, and two files inside it named two different
commits as the source of the same binary: `RELEASE_VALIDATION.md` said
`2f282d9b`, `build-info.json` recorded `33291f0c` with `"dirty": true` and three
changed files. HEAD had moved two commits further on again, so the delivered APK
was three commits behind the tree this report describes.

`docs:consistency` had been reporting it since the artefacts were built — the
rule exists and was doing its job; nothing had acted on it. The artefacts in this
edition are rebuilt from a clean checkout of the commit named in §2.2.

## 20Q.7 Not claimed

No native speaker has read anything this pass changed. Three Korean strings were
rewritten and the reasons are in `docs/copy-audit-ko.json`; that ledger says
plainly that the reading was done by the model that wrote the sentence, and §22
still says the same about all thirty-two languages.

`locale:editorial` reports **70** warnings on this tree, not the fifteen I-39
claimed — the nine straight-apostrophe findings in Filipino, French, Italian and
Turkish were mechanical and are fixed (I-153); the seventy that remain are all
split-translation judgements about whether two contexts should share a sentence,
which is exactly the question a native reader is for.

No `.ipa` was produced and no physical device ran the binary.

# 20R. The seventh screenshot pass — a name, a corner, and a ceiling

Two photographs of the running app, and neither of them was a rendering bug.

The first was ㄸ being written. Both of its lids began a visible distance to the
right of where the letter begins, and became flush only once the ㄴ under each of
them was drawn. The finished glyph was perfect, which is why every gate said so.

The second was the Numbers course. *이 숫자를 일, 이, 삼으로 말해 보세요.* *0을
읽고, 어디에 어떤 말을 쓰는지도 익혀요.* *1, 2, 3을 한국어로 말해요.* *마무리
확인 통과 — 10문제 중 8개* under a heading reading *아직 끝나지 않았어요*, with
*마무리 확인 통과하기* listed underneath as the thing still to do. Three
sentences about one fact, and a course full of demonstratives.

Both have the same shape as every defect this document has recorded: a check was
watching, and it was measuring the thing the code computes rather than the thing
a reader sees.

## 20R.1 A corner belongs to both strokes that arrive at it — **P1, fixed (I-154)**

`strokeVectors.classify` decides what each stroke end is doing. An end that
lands inside another stroke is a `join` and is cut square; an end that meets
another stroke's own end is a `corner` and is extended half a pen so the two
close flush. The extension was **asymmetric**: of two ends that met, only the
*later* stroke was extended.

The reasoning was written down and is worth quoting, because it is wrong twice
over: *extending the earlier one puts ink beyond where the pen has been while
the later stroke is still grey.*

**It is not ink beyond where the pen has been.** A pen has a width. Set down on
the corner point of a ㄷ and drawn to the right, its nib covers the corner —
that is what a round or square cap would draw, and the butt cap this model uses
is the simplification, not the truth.

**And it is not beyond the letter.** At a shared end the two strokes are
perpendicular, so one stroke's extension lies inside the other's ink band: the
tip is at most half a pen from the partner's centreline, measured perpendicular
to it, which is where the partner's ink already reaches.

What the rule cost was every frame before the last one. Where a corner's second
stroke is written first — ㄷ's lid before its ㄴ, ㅌ's top bar before its
upright, ㅁ's left post before the turn — the first stroke was drawn half a pen
short of the letter's own corner and stayed short for as long as it was the only
ink on the paper. Measured on this tree before the change:

```
41 stroke ends over 16 characters
   ㄷ 1   ㄹ 2   ㅁ 3   ㅂ 2   ㅌ 1   ㄸ 2   ㅃ 4
   다 1   라 2   마 3   바 2   말 5   밤 5   밥 4   글 2   부 2
```

ㄸ is where it shows worst, because both of its components are written lid-first
and a learner watching it sees the defect twice.

**Why nothing caught it.** Every glyph gate measures the *finished* letter.
`glyph:structure` probes the pen-square at each junction of the completed
character, where the second stroke has arrived and filled the hole — 100% ink,
every junction, before and after. `glyphshape:qa` scores the completed character
against the face. `letters:face` measures its parts. `strokes:visual` renders
1,345 animation frames and asks about pacing, detachment, clipping and badges —
none of which is *does this stroke reach the corner it is drawn from*.

The extension is symmetric now. The finished glyph does not move: junctions
100%, shape 99.6% mean explained against the face, `letters:face` unchanged, the
vector fixtures regenerated with no face-entry change.

### The gate

`npm run strokes:corners` — 73 taught characters, 510 stroke ends, 86 joints, 82
corner terminals — and its rule has no reference to stroke order at all:

> An end that arrives at another stroke's **end** is a corner, and must be drawn
> out to it. An end that arrives in another stroke's **body** is a join, and
> must not be.

That is a property of two points and a pen. It is true of the first frame and of
the last one, and there is no completed letter it can hide inside. It asks six
more things at the same time: that a `corner`'s extension is actually in the
drawn path and points out of the stroke; that a `join` or `free` end carries no
extension (a nub outside the letter); that no tip runs more than half a pen past
its own end; that the pen-square at each joint is filled by everything written
**up to the later of the two strokes that meet there**, which is the
frame-accurate version of what `glyph:structure` asks of the whole letter; and
that the five doubled letters are one component written twice — ㄲ ㄸ ㅃ ㅆ ㅉ, 12
corresponding stroke pairs, compared as an exact translation rather than as a
similarity score.

Negative-tested by restoring the asymmetric rule: **41 findings, exit 1.**

`strokes:visual`'s *adrift* check had to change with it, and the change is a
strengthening rather than an accommodation. It compared the finished frame
against the union of the strokes drawn separately, with an allowance of four
pixels per stroke — a number tuned against ㅃ's measured twenty-four. Symmetric
terminals put a flush edge on both sides of every corner, which is exactly where
the two sides of that comparison round differently: composited alphas of 101 and
136 against a threshold of 96. It reported four letters that had not changed
shape. The tuned constant is gone and the structural claim it was standing in
for is asserted instead — every drift pixel must lie against ink both renders
agree on, and none may be displaced.

## 20R.2 The two number sets had no name — **P1, rewritten (I-155)**

A pass two editions ago banned the linguistic labels from the Numbers course —
한자어 수, 고유어 수, 체계 — on the grounds that a beginner cannot act on them,
and made the course call each set by its own first three words instead: 일, 이,
삼 and 하나, 둘, 셋.

That fixed the register and created the defect in the screenshot. **A set with
no name has to be pointed at every time it comes up**, and pointing at it is
what produced a course full of demonstratives: *이 숫자*, *이 방식*, *어디에
어떤 말*. A learner cannot ask a question about a thing that has no name, and
cannot tell two of them apart when both are called *이 숫자*.

The sets have names again — 한자어식 and 고유어식 in Korean, each language's own
equivalent in the other thirty-one — and the course teaches them before it asks
anything about them.

| | Before | After |
| --- | --- | --- |
| Module 1 title | 1부터 10까지 말해요 | 한자어식 수와 고유어식 수 |
| Module 1 goal | …0을 읽고, 어디에 어떤 말을 쓰는지도 익혀요. | 한자어식 일, 이, 삼과 고유어식 하나, 둘, 셋을 열까지 말해요. 0을 영과 공으로 읽고, 각각 어디에 쓰는지 익혀요. |
| Lesson 1 title | 1, 2, 3을 한국어로 말해요 | 한자어식 수: 일에서 십까지 |
| Lesson 1 step 1 | 일, 이, 삼부터 들어 볼게요. … | 일, 이, 삼처럼 읽는 수를 한자어식 수 표현이라고 해요. 날짜와 달, 분, 가격, 전화번호에 써요. |
| Say-the-number prompt | 이 숫자를 일, 이, 삼으로 말해 보세요 | 한자어식으로 읽으면 무엇일까요? |
| Which-set prompt | 여기에는 어떤 숫자를 쓸까요? | 여기에는 한자어식과 고유어식 중 무엇을 쓸까요? |

The two lessons that introduce the sets now carry what a learner needs to use
them. 한자어식: dates and months, minutes, prices and phone numbers first; then
floors, addresses, measurements like length and weight, and any number past a
hundred. 고유어식: counting things and people, an age, the hour on a clock — and
then, in its own step, the five forms that change before a counting word:

```
하나 → 한 개   둘 → 두 개   셋 → 세 개   넷 → 네 개   스물 → 스무 개
```

Nine later lessons stopped saying *일, 이, 삼 쪽* — 쪽 is *this side, that side*,
the vague subject the copy rules already forbid.

`numbers:copy` enforces the opposite rule to the one it used to. It fails a
demonstrative standing in for a name (*이 숫자를*, *이 방식*, *이런 말*, *어디에
어떤 말*), and it fails the six strings whose whole job is to say which set if
they do not say it, in that language's own words — 192 labels checked. Two
things did not change, because neither was ever about the names: **no origin**,
because where a set came from is not the answer to any question a beginner has
and is the one contested claim in the course; and **no apparatus**, because *두
체계*, *두 벌* and *two sets of numbers* frame the thing as grammar to be held
before anything usable is said.

## 20R.3 The answer result draws the verdict and nothing else — **P2, done (I-156)**

Four passes were spent on the sentence under a right or wrong answer. The
generated *사는 4예요* was removed, then the restatement *정답은 8*, then the
counting-word line under questions about numerals, then `rationale.adjacent`.
Each pass removed the worst of them and left the rest.

There are none now. The `feedback` field is off the exercise, `MISCONCEPTION_FEEDBACK`
is gone, the `rationale` block is out of all 32 bundles, and *틀려도 괜찮아요.
필요한 곳에는 설명이 따라와요.* has lost its second sentence, because the
sentence it promised no longer arrives.

What a learner needs after answering is on the screen and is not prose: the
option they tapped is marked with a cross, the right one with a tick, and both
marks carry their own screen-reader text on the option itself. The teaching is
in the explanation steps, which are read before the exercise and are recorded as
evidence towards completing the lesson.

`copy:generated` was the gate that policed those sentences and is now the gate
that says there are none: it builds all 530 exercises across practice, a retry
and mastery, and fails if one carries a result body under any of five names. It
still renders the prompts that *are* composed — `prompt.counterForm` and
`prompt.orderParts` — in all 32 languages, 16,960 of them, and reads them for
emptiness, placeholder residue, English leakage, category labels and claims
about learners.

## 20R.4 The final check said its threshold three ways — **P1, fixed (I-157)**

Before the check: *{{count}}문제예요. {{pass}}% 이상 맞히고, 모든 항목을 한 번
이상 맞혀야 해요.* — a ratio, and a learner left to work out what 80% of ten is.

After it: *마무리 확인 통과 — 10문제 중 8개* — a fragment, in a different unit.

And under that, in the list of things still to do: *마무리 확인 통과하기*, a
third sentence about the fact the score above it had just stated, drawn beneath
a heading reading *아직 끝나지 않았어요*.

`passMark(total)` is one function, beside `MASTERY_PASS`, and both screens call
it. Ceiling, because the number printed has to be a number that passes: 80% of
six is 4.8 and five is what a learner needs.

```
before   10문제예요. 80% 이상 맞히고, 모든 항목을 한 번 이상 맞혀야 해요.
         마무리 확인 통과 — 10문제 중 8개
         · 마무리 확인 통과하기

after    10문제 중 8문제 이상 맞히면 통과해요.
         마무리 확인을 통과했어요. 10문제 중 8문제를 맞혔어요.
         [다시 풀기]  [숫자 수업으로 돌아가기]
```

The summary drops the list line the score already covers, and *every item
answered once* — a genuinely different condition — is its own row instead of
sharing one with *pass the check*. Seven journey tests cover the combinations:
8/10, 9/10, 10/10, one below the mark, the retry after a failure, the resume
after leaving, and every lesson in the course refusing to complete below its own
mark. Negative-tested twice: rounding the pass mark instead of taking its
ceiling, and moving `MASTERY_PASS` to 0.5.

## 20R.5 A lesson header was one of its own answers, in thirty-one languages

얼마예요? is the money lesson's title. Korean glosses the item *가격을 묻는 말* —
a description — so the two never collide.

Every other language translated the phrase. So the header read **How much is
it?**, **¿Cuánto cuesta?**, **いくらですか？** directly above a *what does this
mean?* question whose correct option was that same sentence. Three lessons were
in that state — money, hours, age — and a fourth, weekdays, was one question
away.

`answerability` proves one option answers the question and says nothing about
the rest of the screen. `numbers:qa` now compares the **header** against the
**option list**, in all 32 languages: 2,208 pairs. The four glosses are
descriptions in every language now, as Korean already had them. Negative-tested
by restoring the English one: reported by name.

## 20R.6 Two questions a learner could answer without reading them

The bank has 276 distinct questions and every automated rule was green on all of
them. Reading them found two:

```
What does 원 mean?      won · 5,000 · 10,000 · 35,000
What does 열여섯 mean?    16 · three people · five thousand won · two o'clock
```

In each, the answer is the only option of its **shape**. `meaningOf` returns a
numeral for an item with a value and no gloss and a key for one with a gloss, so
a list can mix the two; where exactly one option is of a kind it is identifiable
without being read, and a learner who answers by shape has not been asked
anything.

`readChoose` refuses to build one now, and `numbers:qa` asserts it from outside
the builder over every question of every kind, so a second builder cannot
reintroduce the shape. The three prices carry a gloss — *5,000 won*, *10,000
won*, *35,000 won* — which is what keeps the money lesson able to ask about 원
and 얼마예요? at all.

**`docs/numbers-question-ledger.md` is the reading**, and it is the answer to
"how do you know every item was inspected?". One row per distinct question:
content id, lesson, question type, prompt, correct answer, distractors, result
and correction. The decisions live in a JSON file keyed by a hash of the prompt,
the answer and the distractors, so a question whose wording has moved is a
question that has to be read again — the same design as `copy:ledger`, and for
the same reason.

```
276 questions   265 sound   9 corrected   2 noted
```

The two notes are the part that makes it a reading rather than a rubber stamp.
유월's listening question offers 육, 오 and 칠 — the answer is the only
two-syllable option. It is kept, because those three are exactly what a learner
reaching for June wrongly says and syllable count is a weak tell in a question
whose whole content is a sound; and it is written down, so the next pass starts
from a reading and not from silence.

## 20R.7 The prompt/type contract stopped being one

Three gates kept their own copy of *which string heads which question type*, and
one of the three had drifted: it resolved `prompt.key` on every type, and on
`chooseSystem` that key is the item's gloss for the sound-free substitute, not
the heading. It read a gloss and called it an instruction.

There is one exported table now, `PROMPT_KEY_FOR_TYPE`, beside the builder.
Reading it from a gate is a tautology on its own — change the table and both
sides move together — so `numbers:qa` parses the switch in `NumberSessionPage`,
which is what a learner actually reads, and fails when the page and the table
disagree. Negative-tested: the mismatch is reported by name and by both keys.

## 20R.8 The days of the week were level 9 — I-133, finally

Open since the pass that measured it, and carried forward by the pass after
that, with the reason written down: *not landed in this pass, deliberately. It
re-levels the corpus, which regenerates the level-test bank, the anchors and the
editorial QA, and that is not a change to make and verify in the hours before a
release build.*

An editor marks every word 1 to 5 for how soon a learner needs it. 363 words
carry a 1, the highest, and **107 of them were above level 5**: the five weekday
names and 요일 itself at 9, 한국 at 10, 덥다 at 12, 심심하다 and 시원하다 at 13,
비싸다 at 15. Sixty-one words marked 2 were above 13, and forty-eight marked 3
above 21.

The cause is not subtle. `usefulness` was one of four weighted costs, at 0.26 of
`utility`, which is itself half concreteness — so an editor's judgement that a
word is needed in the first week could be outweighed by an irregular stem and a
polysemous gloss, and repeatedly was. Twenty-five rows of `level-overrides.json`
had been asserting that one word at a time, which is why the list kept growing.

`USEFULNESS_CEILING` makes the mark a **bound**. A word marked 1 cannot ship
above level 5, a 2 above 13, a 3 above 21; 4 and 5 have no ceiling. The three
numbers are the tops of `starter`, `everyday` and `confident` in
`vocabularyLevel.ts` — the bands the product already names — rather than three
...
Levels 1 to 3 therefore stay complete in all 32 languages, and the core 600-word
band is ordered by `difficulty_score` rather than by level, so the twelve
...
gate also refuses a word above the band its mark allows, and a ceiling that
promoted one. Negative-tested by emptying the ceiling: **215 findings, exit 1.**

It does not retire the override list; it narrows it, and the residual has a name.
With the ceiling in place the model would give 길다 5 where the editorial anchor
says 3, 나쁘다 5 against 3, 놀다 5 against 2 — a gap of one to four levels where
it was four to nine — and 싸다 5, which is where its override already put it, so
`build_vocabulary` now reports that row as redundant. What is left is I-126's
frequency term: the matcher counts surface forms, an adjective lives its life
inflected, and the citation form ranks as if it were rare. That needs a corpus
of spoken Korean and stays open.

## 20R.9 Seventy warnings, one class, all closed

`locale:editorial` had been carrying 70 warnings for a person to read, and the
previous edition said so. Read, they were all the same thing: two keys holding
the same English sentence, translated two ways.

* `levelTest:home.cta` and `levelTest:placement.take` — the same button, in 23 languages
* `home:finished.cta` and `learning:mistakes.emptyCta` — *start today's words*, in 7
* `learning:recognition.question` and `learning:review.prompt.distinguish` — *which one makes this sound?*, in 4
* three Numbers pairs where a lesson title and an item's gloss were one sentence — which is §20R.5, fixed at the source

Where the two contexts genuinely are the same action, every language's two
translations are now one. `locale:editorial` reports **0 errors and 0 warnings**
on this tree, which it has not done before.

## 20R.10 Eight gates that were defined and never run

`verify:release` did not include `answerability` — the 806,252-question
one-valid-answer check — nor the 118 synthetic journeys, the per-level vocabulary
audit, back coverage, reachable actions, section alignment or legal isolation.
Each was a script somebody wrote, ran once and left. They are in the chain now,
along with `numbers:copy` and the new `numbers:ledger` in `verify:quick`.

## 20R.11 The negative runs

A gate that has only ever been seen passing has proven nothing.
...
the documents it is pointed at and this was not one of them, so nothing
objected. Corrected to
...
written across 32 languages — the Numbers course's terminology, its completion
copy, four glosses in every language and the unified pairs above — and every one
of them was written by the model that is also reporting on them. §22 says the
same about all thirty-two languages and this pass does not change it.

Seventy-eight Korean rows in `docs/copy-audit-ko.json` carry a rewrite and a
reason. That ledger records who read the sentence, and the answer is the same
model that wrote it.
...
# 20U. The eleventh pass — two right answers, two counters, and a test that argued upward

Three things were reported from screenshots, and all three turned out to be
about the same habit: a rule that is *watching* something is not the same as a
rule that *bites*.

## 20U.1 A question with two right answers — **P1, fixed** (I-178)
...
물 and 약 are both ordinary Korean here. You drink water after a meal and you
take medicine after a meal, and the item is invalid whatever the answer key
says.

Two guards should have caught it and neither could.

* The **category** guard compares browse categories, and those are topical
  because that is where a learner looks a word up: 약 sits under body-health
  beside 의사, 물 under food. Different categories, so the pair passed.
* The **noun-class** guard would have caught it. It reads
  `if (answerClasses && otherClasses && classesCompete(...))` — and 약 carried
  no class at all, so it never ran.

The second one is the finding worth keeping. A pairwise guard conditioned on
both operands being classified is switched off by a gap in either. Measured over
the shipped bank, **205 of 268 noun-slot items — 76% — have an answer with no
noun class**, so for three quarters of the items that guard exists to protect it
was doing nothing (I-182).

The fix is therefore not another pair comparison. It reads the **frame**: the
predicate governing the blank selects a semantic class — 드시다 and 먹다 and
마시다 take anything ingestible, exactly as 하다 takes anything that can be done
— and any option in that class is refused. The judgement is made from the
candidate and the sentence, so it holds when the answer is unclassified, which
is the case the pairwise guards cannot cover. It is deliberately narrow: the
blank must carry the object particle, which keeps the comitative frame
*____와 점심을 먹어요* — a slot that wants a person — out of the rule. Scanning
every consumption frame in the bank found that one and no other.

```
  before   다음 · 물 · 산 · 약     keyed 약, and 물 is also right
  after    다음 · 산 · 약 · 하늘    keyed 약, and nothing else fits
```

The builder and the ambiguity gate read the rule from one module, so the
artefact is checked by the rule that produced it. `cloze.json` comes off the same
builder, which is what fixes the identical question in Today's Vocabulary and
Review rather than only in the Level Test.

## 20U.2 Every objective question, counted once — **new**

No gate could answer "how many questions are there, and has every one been
looked at". `questions:ledger` now enumerates all of them from the three sources
that generate them and re-applies the invariants across all three at once:

| Source | Questions |
|:---|---:|
| Vocabulary Level Test | 4,121 |
| Today's Vocabulary and Review (shared gap-fills) | 676 |
| Numbers course | 368 |
| **Total** | **5,165** |

Its first draft folded whitespace away when normalising options and reported
eighteen of the Numbers course's own spacing questions — 두 개 against 두개,
세 번째 against 세번째 — as having duplicate options. Korean spacing is the thing
those questions teach. The normaliser keeps it, and the near-miss is recorded
because a checker that destroys the distinction it is checking is the same
mistake in a different coat.

## 20U.3 The test argued upward — **P1, fixed** (I-180)

Reported as too hard, and as jumping after a single correct answer. Both are
measurable, and measuring them says the same thing. Over **157,799 adaptive
questions** — 200 simulated sittings at each of 30 levels — **23.7% were asked
above the running estimate** and 32.3% below it.

Nothing in the estimator was wrong, which is why nothing had caught it.
Information theory says the most informative item sits at the estimate, so an
unconstrained selector sits there and drifts upward whenever the bracket
midpoint is above the posterior mean. There was no floor under the *experience*,
and no gate measured one.

Two bounds, and they are deliberately not symmetrical:

* One adaptive item in five is drawn a level below the estimate, chosen by
  ordinal position rather than randomly so a resumed sitting reproduces the same
  order.
* The step is capped at **two levels upward** where it stays at three downward.
  Going down after a miss is the test agreeing with the learner; going up is it
  disagreeing with where it just put them, on the strength of one answer that
  under a four-option floor carries no information one time in four.

```
  before   32.3% below · 44.0% at · 23.7% above    climb 2, 4, 6, 9, 12, 15, 18
  after    40.1% below · 41.4% at · 18.4% above    climb 2, 4, 6, 8, 10, 12, 14
```

Accuracy did not move: ±3 at **90.2%**, mean absolute error **1.64** levels,
opening level 2, 20–30 items, a learner who opens with six blanks still recovered
to within a level of the truth. All three shares are now measured inside
`leveltest:qa` and bounded there, so the thing nobody was watching is the thing
that fails the build.

**One cost, named.** A hard upward cap of two means a learner who answers
everything correctly reaches the ceiling later, spends fewer of their twenty
questions there, and was reported at 29 rather than 30. The wide step is
therefore *earned* rather than removed: eight consecutive correct answers restore
it and one miss withdraws it. The top-of-scale test now asserts the top band
instead of the exact number, with the trade written beside it rather than
quietly relaxed.

## 20U.4 Two counters, one screen — **P2, fixed** (I-179)
...
The header counts words finished today. This counted the screen's position
*within its word*. Both are `x / y`, a few pixels apart, and the smaller one was
read as the bigger one: it was reported as "the app says there are two new words
today", by a learner who then met a third.

Nothing was miscounted. `sessionProgress` counts distinct completed words against
`max(goal, plan.words.length)`, the progress bar reads the same value, and the
plan really did hold three new words. The screen simply said `1 / 2` about a word
and let it be read as a day. Two counters in one view have to measure the same
thing or one has to go, and the day's counter is the one a learner needs.

The counter is gone from the markup, from the accessibility tree and with its
computation; no localisation key became unused. The *새 단어 / 복습* label stays —
it is what tells a learner the app has noticed their level, and removing it would
put back a defect already fixed once.

## 20U.5 Two gates that were not gates — **P2, fixed** (I-181, I-183)

* **G4 had stopped applying.** The negative case that proves `numbers:qa` catches
  둘 개 quoted a whole `n(...)` call; a `domain:` field was added to that call
  later, the quoted text stopped matching, and the case had been asserting
  nothing. Re-anchored on the one line it needs, it catches again.
* **The upgrade fixture never read the `numbers` store.** It asserts letters,
  words, sessions, attempts, the streak, the review queue, saved vocabulary, the
  daily plan's credited words, the level-test result and the seed — but not
  Numbers progress, and the v13 migration deliberately *deletes* Numbers rows
  from the letter stores. Nothing held it to leaving the real model alone. A
  completed lesson is now planted for every version that has the store; removing
  it fails two cases.

## 20U.6 What this pass ran

| | |
|:---|:---|
| Objective questions enumerated and re-checked | 5,165 across three sources, 0 findings |
| Level-test simulations | 6,000 sittings, 30 true levels, six named response patterns |
| Exposure shares | 40.1% below · 41.4% at · 18.4% above, all inside their bands |
| Unit suites | web 1,385 · morphology 237 · handwriting 96 |
...
was re-audited in full — every module, lesson, explanation, note and question —
and no P0 or P1 was found in it: the copy is already rule-shaped and concrete,
which is what §20K rebuilt it to be. iOS is unchanged and unbuilt; see §18.6.

---

# 20V. The twelfth pass — a particle is not a constraint

A reader photographed one question and it cost this pass its whole shape:
...
Keyed 일어나요. *일곱 시에 연습해요* — *I practise at seven* — is as ordinary a
sentence as *일곱 시에 일어나요*. The item has two answers and the learner is told
one of them is wrong.

What makes it worth a section rather than a line in the ledger is that **every
gate passed it, and every gate was right to.**

## 20V.1 The gate that could not see it — **I-185, P1, fixed twice**

> **Corrected 10 September 2026.** The rule this section describes was written
> and it never ran: `isUnconstrainedPredicateFrame` was called with
> `anchor.example` — the sentence with the answer still in it — and its first
> line is `if (!sentence.includes('____')) return false`. This document recorded
> the fix on the strength of the item leaving the bank, and the item left for an
> unrelated reason. See §20X.3 and I-207. Everything below is what was intended
> and is now what happens.


`leveltest:ambiguity` reads the shipped bank and applies a rule for exactly this
class: *nothing in the sentence constrains the blank*. It is the rule that
removed `천천히 ____ 주세요` two cycles ago. Run on the bank containing the
photographed item it reported **no findings at all**.

The rule tests whether the sentence carries an argument particle. `시에` is an
argument particle. The test is satisfied and the sentence still says nothing:

> **A particle is not a constraint.** A time says *when*. Every predicate a
> learner knows can happen at seven o'clock.

Four more items of the same shape were in the same bank — `수업 시간에 ____`,
`아침에 일찍 ____`, `발표 전에 ____`, `다음 달에 ____` — and each is the same
sentence with a different clock on it.

The fix is not a fourteenth rule of the same kind. It is a different kind of
rule: the **stem is classified** before an item is built from it. Each non-blank
eojeol is one of four things —

| Kind | How it is decided | Example |
|:---|:---|:---|
| temporal | a head from a closed list of time nouns; or 시 · 분 · 초 · 일 · 월 · 년 · 주 · 달 · 번 with a counting numeral in front of it, absorbing one bare modifier | `아침에`, `일곱 시에`, `수업 시간에` |
| free adverb | a closed list of degree, manner and frequency adverbs | `일찍`, `자주`, `혼자` |
| subject pronoun | `저는`, `제가`, `우리는` and their kin | `저는` |
| contentful | everything else, including any eojeol with a clause ending | `침대에서`, `잠이`, `가면` |

— and a **predicate blank with no contentful eojeol is not asked**. That is
`일곱 시에 ____.` exactly. The counted-noun requirement is what keeps
`이 일에 ____.` — *I am used to this work* — out of the rule, because `일` is a
time noun only when something counts it and the `이` in front of it is the
demonstrative *this*, not the numeral *two*. Reading the whole bank, the rule
fires on five items and on nothing else.

The five sentences were not deleted. Each was rewritten in the editorial pack so
the word keeps a contextual item and a better teaching example:

| | was | now |
|:---|:---|:---|
| 일어나다 | 일곱 시에 일어나요. | 알람 소리에 침대에서 일어났어요. |
| 졸다 | 수업 시간에 졸았어요. | 잠이 부족해서 수업 시간에 졸았어요. |
| 깨다 | 아침에 일찍 깼어요. | 시끄러운 소리에 잠이 깼어요. |
| 긴장하다 | 발표 전에 긴장했어요. | 저는 사람들 앞에 서면 긴장해요. |
| 이사하다 | 다음 달에 이사해요. | 다음 달에 새 집으로 이사해요. |

Each carries its seven in-entry translations, its twenty-four locale-pack
translations and two new recordings.

**Proof it bites.** Negative case **G7** puts the photographed item straight back
into the built bank and the gate exits 1; the same script restores the bank and
re-runs it green. Eight frame fixtures pin the classification in both directions
— the three time-only stems must stay rejected and the corrected stem,
`이 일에 ____.` and `밤에 혼자 가면 ____.` must stay accepted — and flipping any
one of them fails the gate.

## 20V.2 Reading the other 628 — **I-186, I-187, three more classes**

The rule above was written from one item. Reading every contextual item in the
bank and every gap-fill in the daily queue found three more shapes with the same
structure: a constraint that was being looked for in the **pair** when it lives
in the **frame**.

**`____이 문을 열었어요`, keyed 은행, offering 학생 and 형.** A student opening a
door and an older brother opening a door are both ordinary Korean, so three of
the four options were correct. The noun-class guard exists for this and never
fired: it compares each distractor against the *answer's* class, and 은행 is not
a person. The new rule reads the frame — a subject blank in front of an action
carrying an object admits **any** human agent — and refuses every person-class
candidate whatever the answer is. That is the same shape as the consumable rule
(I-118) one argument along, and for the same reason: the constraint is one-sided,
so it survives an answer that carries no class.

**`____이 마음에 들어요`.** A road, a present, a bus stop. And `이 ____이 마음에
들어요`, which shows that a demonstrative in front of the blank does not
constrain it either. Both words were given constraining sentences rather than
losing their item.

**Sixteen sentences keyed two ways — I-188, P1.** `불을 ____ 주세요` asked for 켜
in one item and 꺼 in another. `소리를 ____ 주세요` for 줄여 and 낮춰. `둘에
셋을 ____` for 더해요 and 곱해요. Thirty-four items over sixteen stems, all in
`cloze.json`, all reachable from Today's Vocabulary on the same morning.

The Level Test bank has had a `shared-prompt` rule since the pass that found
더해요/곱해요 *in it*. The daily queue never had one, and both surfaces are built
by the same generator from the same authored sentences. Each item is answerable
alone — the rival is not among its own four options — and the pair is the
generator's own proof that the sentence does not pin its answer down.

The invariant now lives in `question-ledger.mjs`, the only program that reads all
three surfaces, because **neither half of the defect is visible from inside
either surface.** Eighteen more sentences were rewritten so that every stem is
distinct. The same key on the same stem is not the defect and is not reported:
one authored sentence legitimately appears on both screens.

## 20V.3 Twenty-four pairs that no rule decides

The rules above are about the sentence. What is left is about two words, and
nothing in this corpus records it. `유령 이야기` and `역사 이야기` are both
ordinary Korean compounds; `결과를 얻다` and `결과를 날리다` are both idiomatic
and opposite; `생각을 지지하다`, `생각을 분석하다` and `생각을 품다` are three
things one does with an idea.

Each of these passes every automatic guard the builder has — no shared gloss
word, no shared category, no recorded synonym or antonym, no shared argument in
the two examples' own sentences — and still makes two right answers.

So they are written down. `content/vocabulary/answer-conflicts.json` holds
**24 reviewed pairs**, each with the frame it was found in and why both fit; the
builder consults it when it draws and the gate consults it when it reads the bank
back. It is a lookup table and it is described as one. It is recorded here rather
than left implicit because the alternative — listing only the rules — would read
as a completeness claim the rules do not support.

## 20V.4 The whole question surface, counted and read

| Surface | Questions | What was done |
|:---|:---|:---|
| Vocabulary Level Test — contextual | 629 | every stem composed with all four of its options and read, twice: once on the bank as shipped, and again after the rules and the rewritten examples changed every distractor draw |
| Vocabulary Level Test — meaning and produce | 3,522 | read as a spread; one gloss per taught sense, and a distractor sharing the answer's meaning in any of 32 languages is refused at build time by `collideInAnyLocale` |
| Today's Vocabulary and Review | 672 | read in full |
| Numbers course | 368 | read in full — every prompt, every option, every explanation |
| **Total** | **5,191** | `docs/AMBIGUITY_LEDGER.md` |

Twenty-two findings, each with the sentence, the two readings, the verdict and
what holds it closed. The Numbers course produced none: every stem there is a
number, a counter or a system choice, and each is pinned by the lesson it belongs
to.

## 20V.5 The ledger was checking the wrong field — **I-189, P1, fixed**

The check that certifies all of the above found the answer with
`options.find(o => o.isKey)`.

In this codebase `isKey` means *this option's text is a translation key, render
it through i18n*. It does not mean *this is the correct option*. Both buttons of
every 한자어식/고유어식 question carry it, and 240 of the 368 Numbers exercises
carry it on no option at all — which the ledger recorded as `answer: null` and
then skipped every answer invariant for.

Measured against `exercise.answer`, the index the running screen actually grades
on: **336 of 368 Numbers questions were recorded with an answer the application
does not accept, or with none.** The ledger then checked that its own wrong
answer appeared exactly once among the options, found that it did, and printed
*every question has one answer*.

The content was right. Nothing a learner meets was wrong. What was wrong is that
the sentence certifying it had not been measuring it, and the previous edition of
this report cited that sentence.

## 20V.6 The twenty-four packs that kept the old sentence — **I-190, P1, fixed**

Rewriting twenty-five example sentences moved the seven translations that live
inside the editorial entry, because they are in the same row. The other
twenty-four languages live in `content/vocabulary/copy/<locale>.json`, keyed by
word id, and they did not move. For a while this tree said *I get up at seven* in
Hungarian under a Korean sentence about an alarm and a bed.

Five gates read those files. `vocabulary:translation`, `translation:semantics`,
`locale:content`, `locale:editorial` and `copy:generated` all passed. None of
them compares a translated example to the Korean example — they check key parity,
gloss agreement and placeholder shape, which is **one level away from the thing
that was wrong**.

576 rows were retranslated. `copy:fresh` now records the Korean sentence each
pack's translations were written against and fails when the corpus moves and the
packs do not; it is in `verify:quick`. Its own header states the limit: it closes
the case where the change is silent, not the case where somebody reruns the
recorder without translating anything.

## 20V.7 The placement test, re-read against the brief

No defect was found in the estimator or the selector. The walk is what it should
be — the sitting opens at level 2, never steps more than three levels, never
climbs more than two on the strength of one answer, and asks 0.8% of its opening
five questions more than six levels above the learner. Two things were still
wrong and both were about what the file *says*.

**I-192.** `SUSTAINED_CORRECT` is 8 and the paragraph beside it said *three
consecutive correct answers*. `EASY_EVERY` is 5 and the comment in `nextLevel`
said *every third adaptive question*. The code was right in both places: the
simulated walk of a learner who answers everything correctly is 2, 4, 6, 8, 10,
12, 14, 16, 19 — eight two-level steps before the wide one is earned. A reader of
the source was being told something false about the product, and nothing failed,
because nothing asserted the boundary. A new case now asserts it in both
directions and reads the constant rather than a literal.

**Two simulations the brief asks for that the suite ran and did not print.**
`leveltest:qa` now reports them:

```
  interrupted and resumed, at 1, 3, 7, 12 and 18 answers:
    true level  2   20 items   reported  2   identical after every interruption
    true level  8   30 items   reported  8   identical after every interruption
    true level 15   30 items   reported 15   identical after every interruption
    true level 22   30 items   reported 22   identical after every interruption
    true level 30   24 items   reported 29   identical after every interruption

  noisy but stable at 15    30 items   reported 16
    2, 4, 6, 8, 10, 12, 14, 16, 15, 17, 15, 16, 18, 16, 14, 15, 16, 16, 15, 16, …
```

The second row is the ordinary learner an adaptive test has to be stable under:
at level 15, answering to their ability, and wrong about one question in six in
both directions. It settles at 15–17 and stays there.

## 20V.7a A resume that would have scored eleven of twelve — **I-195, P2, fixed**

Reading the resume path while checking that this cycle's content change could not
damage anybody's stored state found a defect that this cycle's change does not
trigger and the next one might.

`LevelTestPage` replays a stored sitting by looking each presented id up in the
bank and **skipping one it cannot find**. The bank is regenerated whenever the
corpus moves. So an answered question whose item has left the bank is dropped
from the evidence the estimate is built from — the learner answered twelve and is
scored on eleven, with nothing on screen saying so — and if the missing one is
the question that was on screen when the app closed, there is no current item to
draw at all.

The three existing disqualifications are all about the *stored row*: a changed
language, an expired clock, a row whose own two records of itself disagree. None
of them is about the bank this build ships.

`sittingIsServable()` adds the fourth. A sitting whose items this build cannot
supply is not offered for resume, and the learner starts a fresh one — which
costs nothing, because an unfinished sitting has never written a level, and it is
the same bar `isReadableSitting` sets one layer out: **discard rather than
repair**.

**This release does not trigger it.** No item id was removed: the bank went from
4,121 to 4,151 by addition, no word id or `senseId` moved, and no word changed
level, so saved vocabulary, mastery, review schedules, placement results and
recommendation state are untouched and schema 14 needed no migration.

## 20V.8 A blank string was not a failure — **I-191, P2, fixed**

`i18n:check` blocked on four things: a key missing from the source locale, an
incomplete plural set, a dropped placeholder, and a key the source does not have.
Setting `common:actions.confirm` to `""` in German and running `--check` exited
0. i18next renders an empty string rather than falling back, so the screen shows
a blank button and nothing anywhere says so.

Two rules close it — *present but empty*, and *lost every word* for a value that
is nothing but its placeholders where English said something. Both sweep the
locale's own keys, because the source loop short-circuits on a plural base and a
blank `activity:units.minute_one` would have gone through the first shape of the
fix as well.

The second rule is written about anything *visible* rather than about letters. A
first draft asked for letters and reported nineteen perfectly good translations:
`of {{levels, number}}` is `/ {{levels, number}}` in Korean, Japanese, Hungarian
and five more, and `1st` is `1.` in German, Czech and Hungarian. Both are the
right thing to write and neither contains a letter.

## 20V.9 Thirty-two languages, joined in one table

`docs/LOCALE_LEDGER.md` is new and is generated. One row per language, joining
the four layers a learner actually reads: 877 interface strings (fewer where the
language has fewer plural categories than English — Japanese, Korean, Chinese and
Indonesian have one), 3,393 word meanings and example translations, 73 letter
descriptions, and what the two stores are told. Every layer is complete in every
language.

It lists all **382 strings identical to their English source** rather than
counting them, because a match is a candidate and not a finding. Every one was
read. They fall into five classes and none of them is source-language leakage:
pure interpolation and numerals; a Korean word no language translates (`won`,
`Lv.`); the same word in that language (`Problem` in German, Polish and Swedish;
`verb` and `adverb` in Romanian and Swedish; `letters` in Dutch); a loanword the
language actually uses (`OK`, `App`, `Home`, `Privacy`, `Backup`); and `A–Z` on
the alphabetical sort control.

**What has not changed.** None of the thirty-two packs has been read by a native
speaker, Korean included. Structural completeness is what this table measures and
it is not the same claim. §11.3 still stands.

## 20V.10 The Numbers course, read a third time

Every module, every lesson, every explanation and all 368 questions, in English
and Korean. Nothing was found. The explanations are one concept each, with a
worked example and the confusable pair named out loud —
*십육* is said *심뉵*, June is *유월* and never *육월*, the hour is native and
the minutes are Sino-Korean. The questions are pinned by their lesson: a
listening item asks which set was heard, so 열여섯 and 십육 are a legitimate pair
of options rather than two right answers.

The only Numbers finding this pass is the one in §20V.5, and it is a finding
about the checker.

## 20V.11 The patent package

`patent/` is not committed — the remote is public and the disclosure is marked
confidential — so what follows describes work that exists on this disk and in the
regenerated PDFs.

Two things were wrong with it and one thing was missing.

**Wrong, and material.** §12's evidence table said the placement test places
95.2% of sittings within ±3 levels with a mean absolute error of 1.37. §6.8 of
the same document said 90.2% and 1.64. The second pair is right: §6.8 measures a
simulated learner who mis-taps 5% of the words they know, and §12's numbers were
left over from the population that never fumbles. **A disclosure that contradicts
itself about its own headline measurement is worse than one that reports the
lower number.** Eleven further figures were stale — question counts, option
counts, bank size, the gate count in the architecture diagram (38, now 94), the
assertion count of the level-test suite. All re-measured on this tree, in both
languages. That is I-193.

**Wrong, and a claim.** The disclosure's reading guide said the Questions group
answers *does this generated question have exactly one defensible answer, in
thirty-two languages*. §20V.1 is the proof that it does not. The framing is
corrected in both languages, and §1 now carries a paragraph saying explicitly
what the question mechanisms decide and what they leave to a person — because the
alternative reading is an overclaim that this repository has just been caught
making.

**Missing.** The frame classification of §20V.1 and §20V.2 is a mechanism, it is
implemented, it is verified by two negative cases and eight fixtures, and it was
not in the disclosure. It is now: §3.3 carries the limitation it answers, §6.3
carries the mechanism and the four rules, Figure 7 was redrawn to separate the
pairwise rules from the frame rules, and claim element **B** gained the second
paragraph. The reviewed-pair list is described and explicitly **not claimed**.

## 20V.12 What this pass ran

| | |
| --- | --- |
| `verify:release` | every step, in full |
| `leveltest:ambiguity:check` | 4,151 items, 629 contextual, 27 rules, 8 frame fixtures — 0 findings |
| `questions:ledger:check` | 5,191 questions across three surfaces — 0 problems |
| `ambiguity:ledger:check` | 13 families, 22 reviewed findings, 24 reviewed pairs |
| `locale:ledger:check` | 32 languages, 0 missing, 0 blank, 382 English-identical strings each read |
| `copy:fresh:check` | 3,393 example sentences × 24 packs |
| `i18n:check` | 32 packs, 0 blank, 0 wordless |
| `leveltest:qa:check` | 6,000 sittings, MAE 1.64, 90.2% within ±3, resume identical at 25 interruption points |
...
| Version | 1.0.5, versionCode 21, signed with the existing production certificate — superseded by build 23, see §20X |
...
* **No native speaker has read any pack**, and this pass did not change that.
* **The frame rules do not decide whether a Korean sentence has one defensible
  answer.** Each decides a named, mechanical property. What is left over is a
  human reading, and §20V.3 is where its residue is written down.
* **A regenerated bank draws different distractors.** The 629 items read this
  pass are the ones that ship; the next build will compose different four-option
  sets from the same stems. The rules and the reviewed pairs are what carry
  forward; the reading does not.
* **No physical device has run this binary** and no human learner has used the
  product.

---

# 20W. The thirteenth pass — difficulty was a fact about the answer

A tester who had recently learnt to read Hangul sat the level test. This was
question four:
...
Two clauses, a negation, a causal connective, an inference the learner has to
make about plants, and 화분 — a **level-28** word. The item was level 7.

A second tester, who answered the three opening questions correctly, met a
level-8 sentence at question four. A third, who declined a level-4 word, was
shown level 6 next.

Three reports, two causes, and neither of them is in the estimator.

## 20W.1 Difficulty was the word taken out of the sentence — **I-196, P1**

Every contextual item took its level from `anchor.level`: the frequency rank of
the word removed from it. Measured on the shipped bank, **629 of 629**.

So this was a level-1 question:

```
  지갑에 ____이 있어요.   → 돈
```

because 돈 is a level-1 word. `지갑` is a level-9 word. A learner meeting their
first question was being asked to read a frame built out of vocabulary they will
not be taught for eight more levels. **61 items — 10% of the contextual bank —
contained a word ranked above the item's own level**, the worst by 21.

The answer's rank is a fact about the answer. It says nothing about the sentence
the learner has to read to reach it, and a gap-fill is a reading task before it
is a vocabulary task.

`scripts/content/sentence_demand.py` reads the frame and returns what it asks
for. Three floors, and the item takes the higher of them and the answer's own
level, so it can only rise:

* **the hardest ordinary word in the frame**, from the same table the test levels
  its answers with;
* **twelve grammar constructions**, each with the level from which a learner can
  be expected to have met it — past tense at 3, negation at 5, a connective at 8,
  a relative clause at 10, 합쇼체 at 16, formal connectives at 22;
* **the eojeol count**, because a four-word sentence is not a two-word sentence
  whatever it is made of.

Grammar is read from the whole sentence and vocabulary from the frame. The
...
negation question whose negation *is* the answer.

**68 of 625 items moved, every one upward.** The ten largest:

| was | now | Sentence |
|---:|---:|:---|
| 7 | 28 | 물을 안 줘서 화분의 꽃이 ____. |
| 9 | 25 | 방에 들어온 모기를 ____. |
| 3 | 17 | 알람 소리에 침대에서 ____. |
| 8 | 22 | 방학 ____ 여행을 했어요. |
| 16 | 30 | 밥과 나물을 ____. |
| 14 | 26 | ____이 과녁에 맞았어요. |
| 18 | 30 | 유리컵에 ____이 남았어요. |
| 1 | 12 | ____으로 글씨를 써요. |
| 2 | 13 | ____이 피아노를 배워요. |
| 12 | 23 | 작은 ____가 큰 차이를 만들어요. |

The foundation band now holds **three** sentences, and these are all of them:

```
  아침에 ____가 떠요.        저는 커피를 ____.        ____가 문을 닫았어요.
```

Three eojeol or fewer, one clause, present or past polite, every supporting word
inside the first few hundred by frequency.

**Three traps, all hit while writing the model, and each is a fact about Korean
that a pattern cannot see.** `저는` and `넘어진` are the same shape — a noun with
a topic particle and a verb with an adnominal ending — so telling them apart
means asking whether the stem is *already a word*: 저 is, 넘어지 is not. Without
that test a level-1 question became level 10. `모기를` is a mosquito, not a
nominalised verb, and the same test resolves it. And the adnominal ㄴ fuses into
the stem's last syllable, so finding it means decomposing the syllable — while
the object particle 을 has a final ㄹ of its own, which turned **every object in
the corpus** into a relative clause until the particle was stripped first.

## 20W.2 Difficulty climbed whatever the learner said — **I-197, P1**

The other half, and it is not in the bank.

The selector targets the midpoint of the bracket while the bracket is wide. The
bracket's upper bound is **30 until something is missed**, so after three correct
answers its midpoint is 18. The posterior is no help either: its prior is centred
at 15 and deliberately weak, so for the first several questions it believes
something close to 15 about everybody.

`MAX_STEP_UP` = 2 was the only thing holding the walk down. Which meant the
sitting climbed two levels a question **whatever the learner answered** — the
learner who declined a level-4 word was shown level 6 because the walk was on its
way to 15 and a decline only slowed it.

> A bound on step size limits how fast a sitting climbs. It says nothing about
> whether the learner has given any reason to climb.
...
| 1 | 1–3 | foundation |
| 2 | 4–7 | elementary |
| 3 | 8–12 | lower intermediate |
| 4 | 13–18 | intermediate |
| 5 | 19–24 | upper intermediate |
| 6 | 25–30 | advanced |

`reach()` walks the response history and returns the highest band the learner has
proved. A band opens on **three correct answers inside it across two distinct
question kinds**. The kind requirement is what fixes the reported case: three
correct `meaning` answers is recognition of three words, the band above holds
sentences, and reading a sentence is a different skill.

`reachCeiling` is applied to four places, and the fourth was a leak of its own
(I-200): the selector's target, the bounded step, the warm-up ladder, and the
screen's pool fallback — which searches neighbouring levels when a kind is thin,
and band 1 holds three contextual items, so it fired on the first sentence of
nearly every beginner sitting and served a band-2 one.

**A miss costs one unit of evidence, not all of it.** Both were built and
measured. Wiping the evidence looks stricter and is worse: a real learner at
level 20 answers about one question in six wrongly, so every partial climb was
erased before it completed and the sitting never reached them — **13 points of ±3
accuracy** over 6,000 sittings. A miss also opens a two-question confirmation
window capped a level below what was missed, and never below the top of band 1,
because a learner who misses a level-1 question has nowhere easier to go and a
one-level-wide ceiling pinned an all-wrong sitting to level 1 for **nineteen
consecutive questions**.

## 20W.3 The second question was always a sentence — **I-198, P1**

`planKinds` cycled `meaning, context, produce, context`, so index 1 was
contextual for every learner who ever sat the test. The first five questions are
now word questions and **the first sentence is question six** — in all sixteen
simulated profiles, and in the running app.

The composition over the whole sitting is unchanged: twelve contextual, nine of
each other kind. It is reordered, not reduced.

## 20W.4 What a beginner now sees

Driven through the real app at 390×844, answering *I don't know* to everything:

| | before | after |
|---:|:---|:---|
| Q1 | 다시 — what does this mean? | 다시 — what does this mean? |
| Q2 | **밥 먹기 ____에 손을 씻어요.** | 차 — which word means "a car"? |
| Q3 | *(produce)* | 나무 — what does this mean? |
...
`docs/LEVEL_TEST_SIMULATION_RESULTS.md` is generated and carries the level
sequence, the kind sequence, the foundation share, the largest climb and drop and
the final band for every profile the brief asks for — and the first twelve
questions in full for every profile a beginner could be.

| Profile | Level | Foundation | First sentence | First band-5 item | Climb | Over ceiling |
|:---|:---|---:|:---|:---|---:|---:|
| Complete Hangul beginner | 1 (1–2) | 100% | Q6 | never | +2 | 0 |
| Recognises a few basic nouns | 1 (1–3) | 100% | Q6 | never | +2 | 0 |
| Knows basic nouns and verbs | 3 (1–7) | 57% | Q6 | never | +2 | 0 |
| Elementary | 8 (5–12) | 10% | Q6 | never | +2 | 0 |
| Intermediate | 16 (12–20) | 10% | Q6 | never | +3 | 0 |
| Advanced | 26 (22–30) | 10% | Q6 | Q12 | +3 | 0 |
| Near-perfect | 29 (26–30) | 13% | Q6 | Q12 | +3 | 0 |
| Every answer wrong | 1 (1–2) | 100% | Q6 | never | +2 | 0 |
| Every answer *I don't know* | 1 (1–2) | 100% | Q6 | never | +2 | 0 |
| Alternating | 2 (1–5) | 100% | Q6 | never | +1 | 0 |
| Early mistakes, then success | 21 (15–27) | 27% | Q6 | Q17 | +3 | 0 |
| Early lucky guesses, then failure | 1 (1–3) | 80% | Q6 | never | +2 | 0 |
| Strong vocabulary, weak sentences | 6 (2–10) | 40% | Q6 | never | +2 | 0 |

**Over ceiling** — questions presented above the band earned at that moment — is
zero in every profile, and `leveltest:policy` fails the build if it is not.

Advanced content is not removed, it is earned: a learner who answers everything
correctly reaches band 5 at question 12 and level 30 by question 21. All 31
사자성어 sit at level 30.

## 20W.6 The gate that would have caught it — **I-199, P2**

`leveltest:qa` measured *of a level-2 learner's first five questions, the share
more than six levels above them* and reported **1.0%** against a bound of 15%.

A level-7 question put to a level-2 learner is five levels above them. **The
threshold could not see the defect it was written for.** The level-8 sentence at
question four was inside it.

The fix is a different kind of measure, not a tighter threshold.
`leveltest:policy` replays six beginner profiles across eight seeds through the
real `nextLevel`, the real pool fallback and the real `pickIndex`, and asserts a
*property*: nothing above the earned band is ever presented, and no sentence
appears in the opening five. It also re-derives every contextual level from the
item's own recorded demand, so a builder that stopped computing it would be
caught rather than trusted.

Negative cases **G9** and **G10** put the reported item back at level 7 and drop
a two-clause sentence into the foundation band; both make the gate exit non-zero,
and the harness restores the tree and re-runs it green.

## 20W.7 Two gates were asserting the defect — **I-201, P3**

`leveltest:bank` asserted that an item's level equals its anchor's — which is
exactly the rule §20W.1 removes. It now allows a contextual item **above** its
anchor and still refuses one below, because an item easier than the word it asks
about is a question nobody can answer at the level it claims.

`leveltest:qa` classified each question against the posterior, which the evidence
gate does not let the sitting act on. That reported 77% of a sitting as *below
the estimate* — a number about the prior rather than about the learner. It now
compares against `min(estimate, reachCeiling)`: a question at the ceiling is the
hardest one available and is *at* the estimate in every sense a learner would
recognise.

## 20W.8 What it cost, stated plainly
...
| Questions above the learner's earned band | unbounded | **0** |
| Contextual items containing a word above their own level | 61 | **0** |

A sitting now spends its first ten to twelve questions climbing through bands
that a strong learner would previously have skipped in four, and with a
thirty-question ceiling that leaves less evidence at the top. The gate's accuracy
floor moved from 90% to 85%, with the reasoning written at the constant rather
than in a commit message.

**A test that measures an advanced learner half a level better, by asking a
beginner questions they cannot read, is not the better test.**

## 20W.9 A gate that was asserting nothing — I-202, P2

The release e2e run failed twice on one case and passed it in isolation, and the
easy reading — *a flake, the machine is busy* — was the wrong one.

```
  settings no longer asks a learner to manage files, or explains the architecture
    alone, quiet machine     2.4s   pass
    in the suite, loaded    11.8s   fail
    in the suite, loaded    12.2s   fail
```

The case is a run of `toHaveCount(0)` assertions immediately after
`page.goto('/me')`. **A count of zero is satisfied by a document that has not
rendered**: the first poll succeeds and Playwright's matcher returns without
retrying. On a quiet machine it beat hydration; while another project on this VM
was compiling, the first poll landed after the screen had drawn.

What it found once it ran late was real. `f731bd43`, three days ago, put *Save a
copy* and *Restore a copy* back on that screen deliberately — the product has no
account and no server, so a file the learner holds is the only honest answer to
*what happens when this phone dies* — and the case, written in the initial
commit, still forbade `/Back ?up/i`. **The product was right and the test was a
week stale, and the race is why nobody was told.**

Two more cases in the same file had the same shape. All three now assert
something that must be present before asserting what must not.

The timing was the evidence: same code, 2.4 seconds green and 11.8 seconds red.
A test that gets slower and starts failing is not always a flake — sometimes it
is a test that finally ran late enough to look.

## 20W.10 The flake that was a defect — I-203, P1

One case had been failing in the release run and passing on its own for four
sessions, and had been written off as a flake each time:

```
  an interrupted lesson resumes at the letter that is unfinished
    alone                    9.5s   pass
    in the suite            18.6s   fail
    in the suite            23.0s   fail
    in the suite            25.7s   fail
```

**Passing in isolation was the symptom, not the exoneration.** Driven at
`--repeat-each=8 --workers=4` it failed **6 of 8**, which made it a defect with
a reproduction rather than a flake with an excuse.

Two hypotheses were killed with measurements before the third was looked for. A
500 ms pause before the navigation changed nothing — so it was not the progress
write racing the page teardown. Dumping the object store through the failure
returned `character:ㅏ=learned` both before *and* after the navigation — so the
data was correct and it was the **reading** that was early:

```tsx
  const [index, setIndex] = useState(() => {          // ← at first render
    if (restart) return 0;
    const at = characters.findIndex(
      (c) => progressFor('character', c.character)?.stage !== 'learned',
    );
    return at < 0 ? 0 : at;
  });
```

`ready` is false until the stored profile has been read, and **the route mounts
before that**: `CorpusGate` lets the app through as soon as the corpus is
available, and `LaunchSplash` covers the screen rather than holding the router.
So on a machine slow enough for the profile to lose that race, the initialiser
asked about a profile that was not there, found nothing learned, and answered 0.
**A returning learner was put back on a letter they had already finished** —
which is the whole of §48 undone, on exactly the slow phone §48 was written for.

The cursor is now placed once, by an effect, on the first render where the
profile can be read; a `placed` ref keeps the invariant the original comment was
protecting, that finishing a letter never moves the cursor twice.
`recordIntroduced` is gated on the same flag: `applyIntroduced` only promotes an
`unseen` row, and a row it cannot see is a row it *invents*, so against an
un-hydrated profile it wrote a blank `introduced` row for whichever letter the
un-placed cursor happened to point at. **8 of 8 under the parallelism that had
produced 6 of 8 failures.**

Sweeping for the same shape found `useState(() => …store…)` in three more
places. Two seed from the clock and are clean. The third seeds the Numbers
question order from `attempts.total` and has the same early read — recorded as
**I-204** and left **open**, because its consequence is a shuffle seeded from
the wrong number rather than anything lost or miscounted, and believing a change
there means re-running five Numbers suites. It is written down rather than
quietly fixed or quietly ignored.

Two defects in one run, failing for opposite reasons: §20W.9 ran too early to
see anything and passed; this one ran late enough to see the truth and failed.

## 20W.11 Not claimed

* **The difficulty scale is still not calibrated against a learner.** The floors
  in the demand model are reasoned, not measured; no learner response data exists
  here and none can be collected. A simulation reports excellent behaviour for a
  badly calibrated bank for exactly as long as the bank is wrong in the same way
  the simulation is.
* **The model inherits the word table's judgements** rather than correcting them.
  `방학` is level 22 by frequency in this corpus, so a frame containing it is a
  level-22 frame, whatever a syllabus would say about when 방학 is taught.
* **No native speaker has read any of it**, Korean included.
* **The bank is regenerated with a different distractor draw whenever the corpus
  moves.** The rules and the 25 reviewed pairs carry forward; the reading of this
  build does not.

---

# 20X. The fourteenth pass — the other half of a four-option question

A reader photographed the Vocabulary Level Test at level 15:
...
Every gate over that bank was green and every gate was right to be. The item has
exactly one defensible answer, and that is the only thing any of them asked.

## 20X.1 A question fails in two directions and only one had a gate — **I-205, P1, fixed**

| | |
|:---|:---|
| a distractor that is **also right** | the item has two answers — twenty-seven rules, nine regressions, eight fixtures |
| a distractor that is **obviously wrong** | the item has no wrong answers worth reading — **nothing** |

A purpose, a bibimbap and an environment do not come in through a window. A
learner who has never met 빛 answers correctly by elimination; a learner who
knows 빛 learns nothing from being right.

**It was not a bad draw.** The builder's rule said a distractor may not come
from the answer's own browse category — written to stop the *first* failure —
so the further apart two words were, the more certainly they satisfied it.
Measured on the shipped bank:

```
context items with at least one same-category distractor:   0
context items with every distractor from another category:  625
```

All of them. The rule was selecting for absurdity at the scale of the whole
bank, and no gate could see it because every gate was a spelling of the first
question.

**The fix is the inversion, for nouns only.** A noun distractor must now
*share* the answer's category or one of its tags. `____에서 채소를 사요` offers
시장 against 도서관, 은행 and 약국 — four places, one of which sells vegetables.
Predicates keep the old rule: 내밀다 and 뻗다 are both actions of the hand and
both make `손을 ____` true, so there a same-area verb genuinely is a second right
answer. The two gates now own opposite halves and each names the other, because
two gates asserting the same thing about one artefact is how §19.4a's pair of
self-checking gates happened.

**The new gate.** `leveltest:distractors`, in `verify:quick`: nine checks read
off the shipped bank, eight negative fixtures that must fail, one corrected form
that must pass. **Negative-tested** by restoring the pre-fix rule and rebuilding:
**747 findings, exit 1**.
...
## 20X.2 The categories the new rule depends on were wrong — **I-206, P1, fixed**

A plausibility rule reading wrong metadata is worse than no rule, so the first
thing the inversion did was expose the classifier.

`scripts/content/categories.py` writes its rules as alternations
`\b(head|face|…|pain|hurt|…)` — a word boundary in front and **nothing behind**,
so every alternative was also a prefix rule:
...
**292 of 3,393 words took their category from a match that landed inside a
longer word.** It is a customer-facing defect on its own — the category is the
drawer the browse screen files a word in, so a learner opening Body & Health met
a painter, a birthday and a pillow.

The fix is a *no word character follows* lookahead on every rule. Closing the
accidental matches cost 196 real ones — `shoe` stopped matching *shoes*,
`inform` stopped matching *information* — so the matcher is now given the lemma:
every alphabetic token in the gloss is reduced by an inflection and derivation
table and the reductions are appended to the text the rules search. Two entries
in that table are worth naming because each was a live miscategorisation on the
first run:

* **`-ter` is not a suffix here.** It would give *laughter* → laugh, which is
  wanted, and *painter* → pain, which is the fault being removed.
* **`-y` and `-ly` need a four-letter stem.** Otherwise *many* → man (많다 to
  People), *busy* → bus (바쁘다 to Places & Travel), *early* → ear (일찍 to
  Body & Health).

Then all eighteen finished category listings were read word by word and the
residue written down: **`OVERRIDES` grew from 552 to 1,085 entries**, 134 words
changed category, and the drawers that were dumping grounds shrank to something
a learner can browse — Home 328 → 186, Body & Health 240 → 198, Time & Numbers
307 → 228.

## 20X.3 Two frame guards had never executed — **I-207, P1, fixed**
...
**The first of the two is the fix §20V.1 records for I-185** — the photographed
`일곱 시에 ____`, keyed 일어나요 and offering 연습해요. The rule was written, this
document recorded it as fixed, and it has never once fired for any item ever
built.

Nothing could see it. `leveltest:ambiguity` re-derives the frames from the
...
the offending options out for an unrelated reason. Inverting that rule removed
the accidental cover and four `consumable-option` findings came straight through,
which is how this was found.

**§20V.1's status is corrected from RESOLVED to RESOLVED (second attempt).** The
lesson is §7.2's, one level down: a green gate answers the question it was
written to ask, and passing does not prove that the code it was written about
ran.

## 20X.4 Five frame classes with no rule — **I-207, P1, fixed** (with §20X.3)

Found by reading all 625 contextual items. Each accepts the answer's whole kind,
so once the distractors are plausible every option is right:

| Class | Example |
|:---|:---|
| a bare noun modifying the noun behind it | `____ 가방을 샀어요.` — 녹색 and 흰색 too |
| a time noun in a 에 slot | `____에 바다에 가요.` — 월요일 too |
| a place in a 에 slot under a verb of going | `일요일에 ____에 가요.` — anywhere |
| a subject under 있다 / 생기다 / 나오다 | `오늘 저녁에 ____이 있어요.` — anything |
| a general verb whose only argument is the blank | `____을 새로 샀어요.` — everything |

The bare-modifier class fails in *both* directions at once, which is why it has
no distractor set: where the options are the same kind of word it has several
right answers, and where they are not it is decided by which option can stand in
front of a noun at all — 살인 공항 is not a phrase, so 국제 wins without any
Korean being known.

The general-verb rule needed one further correction before it worked. Its "is
there another argument?" test read 으로/로 as well as 을/를/에/에서, and 새로 —
an adverb — parses as 새 + 로, so `____을 새로 샀어요` reported an argument it
does not have and the rule stayed silent. 서로, 따로 and 함부로 have the same
shape.

## 20X.5 Forty-eight items read and refused

Where no rule decides, a person read the rendered item and wrote the verdict
down in the entry itself — `ctx: 0` with a `ctxWhy` carrying the rendered item
and the reason, which is the flag the builder already consulted for six words
recorded by earlier passes. A first draft of this pass added a second file for
the same decision and it was the fault this repository keeps finding: two
records of one thing, free to disagree. Forty-eight new entries, fifty-four in
total.

| Reason | Items | Example |
|:---|---:|:---|
| `twoAnswers` | 23 | `____에서 채소를 사요.` — 편의점에서 채소를 사요 is true of Korean convenience stores |
| `absurdOptions` | 24 | `____가 물에서 놀아요.` — a wing and fog do not play |
| `collocation` | 1 | the photographed item |

An entry there is the **last** resort. Every one was checked first against "is
there a rule here?", and the four classes in §20X.4 became rules instead of
forty more entries.

## 20X.6 The Korean itself

`창문으로 아침 빛이 들어와요` is grammatical and it is not the phrase. Korean says
아침 햇살 or 아침 햇빛; 빛 takes no 아침 modifier. The card now reads
**창문으로 빛이 들어와요** — the modifier dropped rather than the noun swapped,
because the card teaches 빛.

The contextual item for 빛 is refused rather than rebuilt, and the reason is
written into the blocklist: everything that comes in through a window — 바람,
소리, 공기, 냄새 — is a second right answer, and everything that does not is
scenery. There is no defensible gap-fill for that word at that level and the
honest outcome is not to build one.

All 3,393 taught example sentences were then scanned for the same shape of
fault — a bare noun directly modifying another noun, 496 pairs — and every pair
read. Two more:

| Word | Was | Now | Why |
|:---|:---|:---|:---|
| 핑크 | 핑크 색이 예뻐요. | 핑크색이 예뻐요. | 핑크색 is one word. A spacing error in a model sentence, in an app that teaches spacing. |
| 금발 | 금발 머리가 눈에 띄어요. | 금발이 눈에 띄어요. | 금발 already means blond *hair*. |

Both translations already read "pink" and "blond hair", so none of the
thirty-one per-sentence translations changed; the Korean and two recordings did.
The other 493 pairs are ordinary Korean — 두 사람, 다음 주, 수업 시간, 회의 자료.

## 20X.7 What the bank costs, stated plainly

| | before | after |
|:---|---:|---:|
| Bank items, English | 4,147 | 3,995 |
| Contextual items | 625 | 473 |
| Non-English reach | 2,122 | 1,972 |
| Distinct words asked about | 2,273 | 2,166 |

**174 items were removed and 22 were written by hand.** 126 went to the five new
frame rules, 48 to the editorial reading.

The removals fell almost entirely on the **beginner band**, and that is the one
consequence of this pass that a learner would have felt. A contextual item needs
a sentence with an argument in it and a predicate that rules something out, and
a level-2 card sentence is three words long — so after the rules ran, only 15 of
the taught words below level 6 kept a gap-fill, down from 50, and Today's
Vocabulary and Review stopped offering the `context` exercise to new learners
almost entirely. Two unit tests failed on exactly that and were right to.

A generator cannot write those sentences, so twenty-two are written by hand in
`content/vocabulary/context-items.json` — `친구와 ____을 잡았어요` against 눈, 입
and 몸; `____으로 그림을 봐요` against 손, 입 and 몸 — and every one is validated
against the *same* rules the generated items pass, including particle agreement
across all four options so the particle never gives the answer away. A curated
item that fails is a build failure, not an exception. The first draft of the
file had eleven such failures and the build printed all of them.

They carry **no sentence audio**, deliberately: the clip the renderer plays under
a gap-fill is the *card's* example recording, and a curated item is a different
sentence, so playing it would be a recording of something other than what is on
the screen. Recording them needs the speech plan to learn about a second source
of Korean sentences, which this pass did not do.

| | before | after |
|:---|---:|---:|
| Gap-fills at levels 1–5 | 50 | 36 |
| Gap-fills at levels 6–10 | 56 | 19 |

Levels 1, 2 and 4 still carry no contextual item. Those levels are carried by
`meaning` and `produce` items, which is what carried them before.

A smaller bank of items that test Korean is worth more than a larger one whose
items can be answered by elimination. That is a judgement, and it is this
document's, so it is stated as one.

## 20X.8 The audit, and what is generated rather than written

`npm run content:audit` writes `docs/content-quality-audit.json` and
`docs/CONTENT_QUALITY_AUDIT.md` from the tree in one pass — **8,254 rows**, one
per reachable Korean item, each with its grammar, naturalness, collocation,
unique-answer, distractor, difficulty and translation status. `--check` is in
`verify:quick`, so a content change that is not re-audited fails the build
rather than quietly ageing.

Four documents were added: `CONTENT_QUALITY_STANDARD.md`,
`CONTENT_QUALITY_AUDIT.md`, `CONTENT_REMEDIATION_LEDGER.md` and
`CONTENT_GENERATION_AND_REVIEW_PIPELINE.md`.

**And a compatibility suite, because none of this may cost a learner their
progress.** `contentCompatibility.test.ts` asserts that every shipped word
carries an id from the permanent ledger, that every id the ledger has issued
still resolves, that a word stays reachable when its browse category moves, that
no gap-fill has lost its own answer from its options, and that a curated
sentence never carries the card's recording. Measured on this tree: **no word id
was removed, none was added, no `senseId` moved and no word changed level** —
the 134 category moves are display metadata and the 174 removed questions are
Level Test items, which are never scheduled and never counted as progress. A
resumed sitting that referenced a removed item is refused rather than
mis-scored; that is I-195's guard, and it is the one this pass could have
tripped.

## 20X.9 This document was contradicting itself again

I-168 was closed on 8 September with *every artefact figure in the report is now
taken from `result/build-info.json`*. Reading all 308 pages against the tree
found the same class back, and one of the same sentences:

| Said | Is | Where |
|:---|:---|:---|
| the APK grew from 81.9 MB to 82.7 MB | 91,079,074 bytes, 86.9 MiB | §18.5 prose, *the sentence I-168 names* |
| built from `bc3bec14` | `ed44ed06` in `build-info.json` | §18.5 table |
| versionCode 21 | 22 in the delivered artefact | the cover, and §20V.12 |
| the audio set is at 13,738 slots | 13,996 | §18.5 |
| 2,061 items per non-English language | 2,122 | §2.2, §3, §10.8 |
| 619 words get a note on a card | 635 | §2.2, twice |
| 786 words carry a recorded spoken form | 804 — *the row above it in the same table* | §2.2 |
| 240 levels held to an editorial band | 237 | §2.2 |
| a 4,147-item bank | 4,147 was right; 625 contextual was right | §10.1 |

`docs:consistency:check` was green throughout and was right to be: it tracks 75
named metrics and none of these nine is one of them, and **it does not read the
front matter at all**, which is where versionCode 21 was printed on the cover.
Nine figures were corrected and the eight that are re-derivable were added to the
consistency gate.

## 20X.10 Not claimed

* **No native speaker has read any of this.** The naturalness and collocation
  verdicts in the audit are a model's, and every row says so. They are written
  down at the level of the individual line so that a native reader can disagree
  with a sentence rather than with a claim.
* **A plausible distractor is closer to being a second right answer than an
  absurd one.** The frame rules, the relation graph, the reviewed conflict
  pairs, the shared-argument rule and the new shared-predicate rule remove the
  classes a program can see; the residue is 48 hand-read refusals. There will be
  more, and they will be found by reading rather than by a gate.
* **The 134 category moves were reviewed by reading the gloss, not the corpus.**
  A word whose taught sense differs from its most common sense can still be in
  the wrong drawer.
* **Levels 1, 2 and 4 have no contextual item.** Stated in §20X.7 rather than
  buried.

# 20Y. The fifteenth pass — a child could be asked what 섹스하다 means

This pass began with a report from production, and the report was true. A
learner sitting the Vocabulary Level Test could be shown **섹스하다** — as a
level-12 question asking its meaning, as a level-12 question offering it as
the Korean for *to have sex*, and as a wrong answer to 이기다 and to 돌아가다 at
level 10. The same headword answered a dictionary search for 섹. Every one of
those surfaces was in the bank this document certified in §20X, in the web
build, in both native asset copies, and in the signed APK and AAB under
`result/`.

The gate written for exactly this — `content:safety:qa`, §19.4's *put 여자
back among the options of a 타다 frame* — was green on that tree, and it was
green for the reason gates in this document keep being green: it measured one
thing, correctly, and the defect was somewhere else.

## 20Y.1 What was wrong — three lists, three files, no word in common

| Where | What it compared | Why 섹스하다 walked past |
| --- | --- | --- |
| `learner-safety.json` → `excluded`, 66 terms | the whole headword, by equality | it held 섹스; the headword was 섹스하다 |
| `build_level_test.py` → `_UNSUITABLE`, 72 substrings | the headword, by substring | 섹스 was never on it — the file's own comment says the two lists were meant to divide the work |
| `build_level_test.py` → `unsuitable()`, 26 English words | the Wiktionary gloss, by substring | it looked for *sexual*; the gloss was *to have sex* |

Behind the lists, three structural faults, each of which the previous
editions had written down as a design decision:

* **The dictionary was out of scope.** `content-safety-qa.mjs` opened with
  *"It does not touch the dictionary. A dictionary is allowed to contain adult
  vocabulary and a learner who searches for a word has asked for it."* But
  §10.1 says where the upper levels of the Level Test come from: the
  dictionary. 30,334 Wiktionary headwords fed the assessment with no policy
  on them, and 섹스하다 is a quality-gated dictionary anchor at level 12.
* **Meaning items were never read.** The gate checked `produce` items'
  `options` — Korean strings — and never resolved the `optionIds` of `meaning`
  items, which are what a learner sees translated. A dictionary anchor used
  as a distractor in a meaning item was invisible to it.
* **Nothing read the finished item.** The anchor builder filtered anchors, the
  bank builder filtered distractors, and no step read the item the screen
  would show, in the language the screen would show it in.

`packages/content-safety/src/legacy.test.ts` reproduces the three old checks
verbatim and asserts that 섹스하다 passes all three; the same file asserts the
new policy refuses it on the headword alone, on the gloss alone, and as 섹스를,
섹스했어요, 섹 스 and 섹스하는.

## 20Y.2 What the whole inventory held — **MEASURED, on the shipped commit**

The fix was not a word. It was a policy, and the first thing the policy was
used for was to find out how much else the old gate had certified. A worktree
of the shipped commit (`456d4d67`) was scanned with the final policy
(`SAFETY_ROOT=… npm run content:safety:qa`):

| Category | Findings | Where |
| --- | --- | --- |
| Violence | 2,236 | dictionary rows (weapons, murder, war), eight taught words with their translations in 32 languages, and the bank items built from them |
| Sexual | 823 | dictionary rows; 섹스하다 across 16 bank surfaces; 벌거벗다 (*the child runs about bare*) |
| Political | 592 | dictionary rows — parties, ideologies, disputed territories; 이념, 파업 |
| Mortality | 194 | 죽음, 사망하다, 숨지다, 살신성인 and their translations; death vocabulary as bank options |
| Profanity | 149 | dictionary slurs and vulgarities; 바보, 멍청하다 |
| Gambling | 146 | dictionary rows |
| Drugs | 127 | dictionary rows; 취하다 (*I got drunk*), 술집 |
| Self-harm | 66 | dictionary rows — 자살, 자해, 자결 |

**4,333 findings, in 9 of 13 content families**: 16 taught headwords, 612
distinct Level Test items, 854 dictionary headwords, 23 gap-fills, 17 audio
transcripts. None in letters, UI copy, the Numbers course or the quotations.
The full family-by-family inventory, with source of truth, generated form,
runtime consumer, item and field counts, is `docs/CHILD_SAFE_CONTENT_AUDIT.md`
§3, and the machine record is `docs/child-safe-content-audit.json`.

## 20Y.3 What was done — **IMPLEMENTED**

**One policy.** `packages/content-safety/policy/child-safe-content-policy.json`,
version 1.0.0: seven prohibited categories (sexual, political, profanity,
drugs, gambling, violence, self-harm) plus a CONTEXT_BLOCK mortality category;
surface forms in all 32 locales; romanisations; per-surface match modes;
per-language exceptions; per-concept allow lists; nine context rules; English
gloss indicators. Two evaluators — TypeScript and a Python port — held to the
same 355 fixtures: 199 negative (every category, in every evasion the brief
names — conjugated, particle-attached, spaced, punctuated, zero-width,
misspelled, leet, romanised, translated, masked, and unsafe only in a choice,
hint, explanation or transcript) and 158 positive (성공하다, 성격, 약, 약국,
약을 먹어요, 죽 the porridge, 죽순, 총 as a total, 도와주세요, 병원에 가요,
아파요, countries, cities, civic words, and the ones calibration found —
씹다, 보지 마세요, 자지 마세요, 고양이 새끼, 이 년 동안, *basement*, *direction*,
*studied*, *deadline*, *killer whale*, *Protestant*, *computing*). The
pipeline — NFKC, invisible-character removal, case folding, de-obfuscation, a
compact form that joins single syllables and keeps word spaces, lexeme
matching, context rules, gloss classification, allow lists — is written down
in `docs/CHILD_SAFE_CONTENT_POLICY.md` §4.

**Seven gates, one policy.** Import (`build_vocabulary.py` stops with the row
named), dictionary publication (`build_dictionary.py` drops the row or the
sense — 694 headwords and 525 senses, counted in the manifest), assessment
generation (`build_level_test.py` on headword, gloss and example, with
retired words barred from the dictionary pool; `build_level_test.mjs` on every
distractor and every composed sentence, then on the finished item in all 32
languages), runtime (`apps/web/src/domain/contentSafety.ts` re-reads every
bank item on load through a Korean-and-romanised runtime subset of the policy, drops retired
words from a stored day's remaining obligations, and lets `sittingIsServable`
refuse a sitting whose items the guarded bank lacks), CI
(`content:safety:check` in `verify:quick` — the fixture self-test, then 14
families, 32 locales, 1,001,715 fields) and release packaging
(`content:safety:bundle:check` in `verify:release` — every JSON asset and every
Korean string literal in `dist/`, both native asset copies, and the APK and
AAB unpacked; it also refuses the fixtures and the full policy by file name,
so INTERNAL_ONLY text cannot ship).

**The content.** 23 taught words retired with tombstones — 살인, 강도, 무기,
총알, 미사일, 사격, 전쟁, 전투, 죽이다, 죽음, 사망하다, 숨지다, 살신성인, 취하다,
술집, 벌거벗다, 키스, 바보, 멍청하다, 협박하다, 이념, 파업, 좀비 — each with its
level, category, previous example and reason in
`docs/CHILD_SAFE_CONTENT_REMEDIATION_LEDGER.md`. One example rewritten (베다:
칼에 손을 베었어요 → 풀을 베어요) with its 31 translations and two recordings;
one Korean gloss rewritten (치다). Corpus 3,393 → 3,370; dictionary 30,334 →
29,711 headwords; bank 3,995 → 3,990 items with 0 refused at publication;
gap-fills 513 → 510; audio 6,998 → 6,952 entries. Every regenerated artefact
keeps its ids. The fixed tree scans at **0 findings**.

**What a learner keeps.** Every progress, memory, mistakes and saved-word row.
The plan repair drops a retired word only from what is still owed today and
returns the same object when nothing changed; a day already finished stays
finished. `contentCompatibility.test.ts` allows an id to stop resolving only
with a tombstone. The mistakes notebook keeps a retired word's row and stops
listing it. `contentSafety.test.ts` runs the four shipped rows through the
runtime gate in English, Korean and Thai — Thai and English because those
learners never see a Korean meaning, and the gate reads the dictionary
anchor's id instead.

## 20Y.4 Broken on purpose

| Gate | What was broken | What it said |
| --- | --- | --- |
| the old gate, reproduced | nothing — run as it was against 섹스하다 / "to have sex" | passes all three checks; `legacy.test.ts` asserts exactly that |
| the runtime gate | the four shipped rows fed to `guardLevelTestItems` | all four refused, in `en`, `ko` and `th`; the three safe rows kept |
| the import gate | left the 23 retired rows at `k: 1` and rebuilt | `build_vocabulary.py` stopped naming each row, its field and its term |
| the publication gate | the bank builder before the pool was clean | 99 finished items refused on the first run, 50 on the second, 0 once every hole above it was closed — the count is printed by every build |
| the composition rule | 일으키다's frame with 괴롭히다 inserted | `넘어진 아이를 괴롭혔어요` refused by the sentence, not the word |
| the fixtures against a weakened policy | the whole-word exception `놈이 요리` left in the exception list | `이 놈이 요리를 해요` passed; the fixture failed; the exception was removed |
| the Python port against the TypeScript evaluator | phrase escaping in the port | 7 of 355 fixtures disagreed, all phrases; fixed, 355/355 |

## 20Y.5 What it cost, stated plainly

The dictionary lost 694 of 30,334 headwords — 2.3% — and 525 senses out of
39,628. The course lost 23 of 3,393 words; no level band fell below 43
assessment anchors or the 30 taught words a day needs. Marketing version
1.0.5 → 1.0.6 and versionCode 23 → 24, because a customer can tell the
course is different and because 23 is spent. The runtime policy and its evaluator add one 24 kB-budgeted route chunk to
the Level Test's fetch path and nothing to the first paint.

Calibration found some forty words that a prohibited term shares its letters
with across the thirty locales the engineer does not speak — Swedish *sex* is
six, Filipino *patayin* is *turn off*, Arabic *القاعدة* is *the rule*,
Mongolian *бөмбөг* is a ball, Vietnamese *xử tử* sits inside *đối xử tử tế*,
*to treat kindly* — and each is now a fixture or an exception. That is
evidence the lists were written by a non-native speaker; it is not evidence
they are complete, and §20Y.7 says so.

## 20Y.6 What this document had said, re-read

| Claim, as this document made it | Section | Status now |
| --- | --- | --- |
| "nothing a learner can reach at random is unsafe, as a word or as a sentence" — the gate's own summary line, quoted as the learner-safety gate | §19.1, §20.1 | **CONTRADICTED** — the gate read one list on three surfaces; the dictionary and every meaning item were outside it |
| `content:safety:qa` broken on purpose: 여자 restored to a 타다 frame, exit 1 | §19.4, §20H.2 | **PARTIALLY VERIFIED** — the frame rules did and do work; they were the only part that did |
| "the learner-safety gate and the composite Korean-education gate" among the 43 passing steps | §19.2 | **VERIFIED as a run, CONTRADICTED as a certificate** — it passed a bank with 섹스하다 in it |
| §8.4 "The dictionary layer — VERIFIED, and it is not the corpus" | §8.4 | **VERIFIED as scale, CONTRADICTED as scope** — it *is* the corpus of the upper Level Test, and it carried 854 headwords the policy refuses |
| "Item quality above the floor — VERIFIED" | §10.9 | **PARTIALLY VERIFIED** — one answer each, yes; a child could read them, no |
| Persistence: a content change never orphans a saved row | §13.2 | **VERIFIED, extended** — retirement now has a tombstone form and a test |
| "EVERYTHING THIS REPOSITORY CAN SETTLE IS SETTLED" | §23 | **CONTRADICTED** for that edition; restated below with what this pass settled and what it cannot |

## 20Y.7 Not claimed

* **The thirty non-Korean, non-English surface lists are complete.** They are
  not, and nobody who speaks those languages has read them. They are marked
  EXTERNAL_REVIEW in the policy and tracked as I-213.
* **The 24 new translations of 풀을 베어요 read naturally.** Same reviewer, same
  issue.
* **A term absent from every list is found.** The policy is deterministic and
  lexical by design — the brief asked for that over a classifier — so what it
  finds is what it names. The frame rules and context rules catch the
  compositions of innocent words that are listed; a composition nobody has
  listed is not caught.
* **iOS.** The shared source and the iOS asset copy are synchronised and
  scanned; no IPA was built, for the reason §18.6 gives.

---

# 20Z. The sixteenth pass — four screenshots, one version, and the measurements that were not

This pass began with four customer screenshots and a number. The screenshots:
an orange logo filling the Android launch screen before the splash the customer
knew; a Today's Words question whose *Show answer* printed **정답은 어떤 일이나
행동을 이루다예요** under four still-live choices; four gap-fill chips laid out
as three and one; and a word-ordering screen — 여성인 사람, three dashed boxes,
five tiles — that the customer did not know how to begin, beside a matching
grid whose only help was the fragment 한국어 단어를 골라요. The number: the
release is to be **1.0.4**, and the tree said 1.0.6 on Android and 1.0.3 on
iOS.

Every one of those was true of the tree this document had certified, and the
reason is the one this document keeps giving: each gate measured one thing,
correctly, and the defect was somewhere else. Two of the gates turned out to be
measuring nothing at all (§20Z.8).

## 20Z.1 One version, everywhere — **I-214, P1, fixed**

| Site | Was | Now |
| --- | --- | --- |
| `app.identity.json` `version` / `buildNumber` | 1.0.6 / 24 | **1.0.4 / 25** |
| `project.pbxproj` `MARKETING_VERSION` / `CURRENT_PROJECT_VERSION` (both configurations) | 1.0.3 / 5 | **1.0.4 / 25** |
| `config/product.ts`, the mobile package, three legal documents, `store/release-notes.md`, this file's front matter | 1.0.6 (release notes had said 1.0.0 for three bumps before that) | **1.0.4** |
| The line a learner reads on the Legal and Privacy screens | `Version 1.0.6` through 32 per-locale strings | **`v1.0.4`** from `displayVersion()` in one place |

The build number went *up* while the marketing version went *down*, and that is
deliberate: 24 is the versionCode the previous delivery used, a store refuses a
reused code whatever the version name says, and Play orders builds by
versionCode alone. Nothing was ever uploaded under 1.0.5 or 1.0.6 — `registered`
is false for both stores and every `build-info.json` on record says so — so
there is no store holding a higher marketing version and no blocker to report.

The iOS project file was edited from Linux for exactly two settings, in both
configurations, and `ios:project:check` records the adopted file so any other
change to it still fails. `config/product.test.ts` pins the literal `1.0.4` and
the form `v1.0.4`; `version:check` pins the same literal and now reads the
release notes and this file's front matter, the two files a customer or a
reviewer reads first and the two it had never read. Setting `product.ts` back
to 1.0.6 fails the gate (exit 1).

## 20Z.2 The launch screen — **I-215, P1, fixed**

Android 12 and newer draw a system splash before any app code runs: a
background colour and an icon. Left unset the icon is the launcher icon, which
this product fixed in an earlier pass by handing the system "the splash's own
mark" — `brand/logo-symbol.png`, the flat orange with ㄱㄴㄷ on it. The approved
splash artwork is a wordmark on a soft peach wash and carries no mark. So a
cold start showed an orange nobody else in the app draws, then the *Han gyul*
artwork, then the app; the customer photographed it and called the orange the
old logo, which is what it is.

The system frame is now the first frame of the artwork: `mipmap/splash_icon`
is an adaptive icon whose foreground is a feathered cut of the approved
artwork's own centre circle, generated by `build_app_icons.py` from the same
file as the launch bitmaps, drawn on the artwork's ground. Adaptive rather
than a plain bitmap because of how Android 12 sizes the two — a bitmap came
out as an 84 dp disc on the emulator's 412 dp screen, an adaptive icon is
drawn at 240 dp and masked to its 66/108 circle, a 160 dp disc, much nearer
the artwork's own. The activity's first frame draws the whole artwork with the
same circle in the same place, and the wordmark and jamo appear on a frame the
learner is already looking at.

`scripts/check-native-splash.py` proves it, and proves it against the package:

| Check | Source resources | Delivered APK (`--apk`) |
| --- | --- | --- |
| The launch theme hands the system the artwork ground, the generated icon, no animation, and the artwork on pre-12 | pass | — |
| The icon is adaptive (transparent background, the cut as its foreground), and every density of the layer is wordless and mark-free, its centre the artwork's centre wash, its edge transparent | pass | pass, 5 packaged files |
| No Android source references the brand mark; no second splash resource is packaged | pass | pass |
| The ground colour agrees across `colors.xml`, `capacitor.config.ts` and the iOS storyboard | pass (#FFF1E1) | — |
| iOS launches on the wordless `Splash` image set, aspect-filled, every file listed | pass | — |
...
the device locale, and the in-app splash a frame later follows the *app*
locale. A learner whose phone is in English and whose app is in Korean sees
one wordmark and then the other. That is a real gap, it was already the design
...
Carlyle. The library's policy since the fourth pass has been *every quotation
names a person* — and the first of the two is the very line that policy had
withdrawn, because it circulates under three names and belongs to none of
them.

It is back because it was asked for, and the way it is back is the whole of
the change to the policy: a row may ship as `authorship: "unknown"` with a
localized *author unknown* byline — 작자 미상, *Author unknown*, *Auteur
inconnu* — which is the fact stated rather than a name invented. What is still
forbidden is a name on that row. The Carlyle line ships under his name as
requested and is marked `sourceStatus: "attributed"`, because the English
sentence it renders is quoted under his name in every collection and has not
been located in his works; the gate allows a hedge in `source` only on a row
so marked, and I-222 carries the question to a person.

| | Quote 1 | Quote 2 |
| --- | --- | --- |
| Korean, verbatim | 꿈을 크게 가져라. 깨져도 그 조각이 크다. | 길을 걷다가 돌이 나타나면 약자는 그것을 걸림돌이라고 말하고, 강자는 그것을 디딤돌이라고 말한다. |
| Byline | 작자 미상, localized in 32 languages | 토머스 칼라일 / Thomas Carlyle, in the conventional local form where one exists |
| Translations | 32, one per shipped locale | 32 |
| Reading result | PASS 19 · CORRECTED AND PASS 4 · NATIVE-SPEAKER REVIEW REQUIRED 9 · BLOCKED 0 | PASS 15 · CORRECTED AND PASS 5 · NATIVE-SPEAKER REVIEW REQUIRED 12 · BLOCKED 0 |
| Corrections made in the reading | zh-CN, kk, ky, mn | es, pt-BR, hu, id, it |

A Korean original now **leads**: in a non-Korean interface the card shows the
Korean first and the translation beneath it in the quieter style, and the
byline is never blank; in Korean the line appears once. Other originals keep
the old order. The supported-locale list is the runtime's own
(`AVAILABLE_LOCALES`, 32 directories under `src/locales`), and the audit
document — `docs/QUOTE_TRANSLATION_AUDIT.md` — is **generated** from three
inputs so it cannot drift: the runtime strings through `renderQuote`, the
reviewer's judgment per row (back-translation, grammar, punctuation, what a
native speaker still has to confirm), and what `scripts/qa-quote-render.mjs`
measured on the rendered home screen — every locale at 320/360/375/390/412/430
px, at normal and doubled text, 768 measurements, no clipping, overflow or
overlap, Arabic right-to-left with the Korean pinned left-to-right.
`QuoteOfTheSession.test.tsx` renders both lines through the real component in
all 32 languages and fails if a table row and the runtime disagree — removing
the Tamil row or altering one English word fails it (both tried).

**Not claimed:** that a native speaker has read any of the 64 translations.
Twenty-one rows say so in their last column; the reviewer is the model that
wrote them.

## 20Z.4 A reveal is an outcome — **I-216, P1, fixed**

```
UNANSWERED ─▶ HINT_VIEWED ─▶ ANSWER_REVEALED ─▶ (next item; this one is owed again)
```

*Show answer* used to be the last rung of the hint ladder: it added the
sentence 정답은 X예요 above four buttons that stayed live, and the tap that
followed was written down as a pass at hint level 3 — a completion, a mastery
step, a point on the day's counter. Now the reveal settles the question. The
correct option takes the same blue box a right choice takes, with the check
mark; the others dim; every option is disabled; exactly one result is reported,
`correct: false, revealed: true`; the card says the answer was seen and that
the item comes back; a **Next** button moves on. The attempt row records
`revealed`, the memory model reads it as not recalled, the daily session puts
the word at the back of the retry pass rather than asking it next, and the
review sitting rescues it two exercises on. The build screen reveals the same
way, into its slots.

| Held by | What it proves |
| --- | --- |
| `answerReveal.test.tsx` (6) | hint then right; hint then reveal (blue box, no 정답은, one outcome, Next); reveal without hint; double tap, tap after reveal, Next twice report nothing more; a wrong pick is still a wrong pick; the build screen |
| `vocabularyDay.test.ts` (4) | a revealed word is owed again as a different question, deferred behind every other owed word, asked next only when it is the only one left, credited once on the later right answer |
| `memory.test.ts` (1) | a reveal never raises stability or streak, on a first meeting or an established memory, and a later genuine recall grows it again |
| `e2e/answer-reveal.spec.ts`, `e2e/hints.spec.ts` | in the real session: the counter does not move on a reveal, the session moves on, no sentence says the answer |

## 20Z.5 Four across or two by two — **I-217, P1, fixed**

The gap-fill's chips were a wrapping flex row, and wrapping is not a layout:
at 360 px the fourth chip fell through on its own. The chips are a grid now,
and `useChoiceColumns` decides four across or two by two from a measurement of
the chips themselves — rendered once, hidden, on a non-wrapping line, compared
with the container before paint and re-measured on resize. Until the first
measurement the shape is two by two, so nothing can paint as three and one
even for a frame. A two-by-two cell may wrap its own text; the grid mirrors in
a right-to-left interface with the same row-major order; phrase options stay a
single column, which cannot form an orphan row.

`e2e/choice-layout.spec.ts` puts a real gap-fill on screen — a review sitting
in `context` mode over seeded words, which is deterministic where the daily
plan is not — at 320, 360, 375, 390, 412 and 430 px, at 1× and 2× text, in
Korean, English and Arabic, and reads the rows the chips actually form: **[4]
or [2, 2]**, never [3, 1], nothing overflowing, every chip at least 44 px tall.
Measured: 320 px is two by two in all three languages; 430 px is four across.

## 20Z.6 The tray and the grid say the first action — **I-218, I-219, P1, fixed**

The word-ordering screen's instruction named the goal (단어를 맞춰 보세요) and
its slots were three identical dashed outlines. It now says the action — 아래
음절을 순서대로 눌러 단어를 완성하세요 — marks the next slot with an accent
outline and its number, shows a tapped tile leaving the tray (dashed, faded,
*placed*) and landing in that slot, offers **Undo**, and grades only on
**Check**, which is disabled until every slot is filled. That last point is a
defect fixed as well as a design: the tray used to grade itself the instant the
last tile landed, so the last tap was the one tap that could not be taken back.
A polite live region counts slots; every slot and tile has an accessible name.

The matching grid's caption, 한국어 단어를 골라요, was a fragment under an
unlabelled grid, and a wrong pair flashed and cleared — the answer by
elimination. The instruction is now the action in order (왼쪽에서 한국어 단어를
먼저 누른 다음, 오른쪽에서 알맞은 뜻을 선택하세요); the columns are headed **한국어
단어** and **뜻**; only the Korean column is enabled until a word is held, so a
first tap can only be the right kind of tap; the meanings then become the
marked target and the caption names the held word; a made pair carries the
same numbered badge on both tiles and either side takes it apart; **Check** is
disabled until every word is paired and grades all pairs together, in the same
blue and red the choice question uses. Both screens' strings exist in all 32
languages; `locale:editorial:check` caught two of the new ones addressing the
reader in the wrong register (Indonesian, Mongolian) before they shipped.

`buildInteraction.test.tsx` (9) and `MatchExercise.test.tsx` (13) walk the
first-time learner's sequence in taps and keystrokes — including the mistakes
a first-time learner makes: a meaning tapped first, two words tapped in a row,
a pair made and unmade from either side.

## 20Z.7 The copy — **I-220, P2, fixed**

이번 주에 32개를 첫 번에 떠올렸어요. 첫 번에 is not Korean anyone says, and the
metric behind it is not a first-try count: `weeklyInsights` counts attempts in
the last seven days that passed with no hint, and an attempt row carries no
first-try flag. Neither of the two candidate rewordings would have been
accurate, so the sentence now says what is measured, in all 32 languages —
이번 주에는 힌트 없이 32개를 맞혔어요 / *You got 32 right without a hint this
week* — and the heading over it is 얼마나 잘 기억하고 있는지. The metric was
left alone; the fix was to describe it.

Every Korean interface string — 881 — was read in sequence against its screen
and thirteen were rewritten; every reading is recorded with a hash in
`docs/copy-audit-ko.json`, and a string whose text changes without a new
reading fails `copy:ledger:check`. `docs/UX_COPY_AUDIT.md` says exactly what
was read and how, and what was not: the 26,794 pre-existing strings in the
other thirty languages were gate-checked (forbidden claims, placeholders,
interpolation, register, terminology, leakage, coverage), not individually
re-read. `docs/UX_COPY_STYLE_GUIDE.md` is the rule set. The generated
`docs/UX_COPY_REMEDIATION_LEDGER.md` carries 752 rows — key, path, old text,
new text, reason, the metric or behaviour the string represents, locales,
verification — and `copy:remediation:check` fails if a row's new text is not
what the bundle carries.

## 20Z.8 Two things this document had said that were not so — **I-223, I-226, I-224**

**Nine large-text measurements measured normal text.** `numbers:layout` "at
100/150/200% text", the tab bar "text ×2", the home header, the
reachable-actions audit, the section alignment, the legal pages and the
Numbers prompts all emulated a phone's font scale by setting the root font
size. Every type size in this product is a pixel token (`--hg-text-body: 15px`)
and four stylesheets use rem at all, so the body text stayed at 15 px in every
"×2" case and each measurement was of the normal size twice over. I-176 — the
tab bar below 44 px at 200% — was measured the same way. `lib/text-scale.mjs`
and `e2e/helpers/textScale.ts` double the tokens, which is what a WebView does
to every CSS pixel of text, and the nine sites use them. Re-run for real:
`numbers:layout` 45/45, `align:sections` 12/12, `legal:isolation` 16/16,
`scroll:audit` 210/210, 79 spec cases; the tab-bar targets are 64×64–67 px at
320 px in Tamil, German, Korean and Arabic, so I-176 is closed on
re-measurement. What the override does not scale is text set as a bare
`clamp()` — the Korean glyphs on the tiles — which is pinned by design and is
now stated as such rather than counted as measured.

**The launch bitmaps were not wordless.** I-26's record said all ten Android
launch bitmaps were generated wordless and tested so inside the APK. Since the
eighth pass the Android bitmaps have been the *localized* artwork — twenty-two
files, *Han gyul* by default and 한귤 under `drawable-ko` — and only the iOS
image and (now) the system-splash icon are wordless. The record is corrected
(I-226) and the gate tests each for what it is.

**The safety scan had read 27 of the quotation library's 1,052 strings.** The
quotations family pulled literals out of `quotes.ts` with a regular expression
that a backtick in a doc comment turned into one enormous match, and tagged
every non-Korean line as English. It now imports the library and scans every
original, translation and byline in its own language: 22 items, 1,452 fields,
0 findings (I-224). The retired-word tombstones are also enforced at corpus
ingestion, so a band that reaches the app from a stale cache, an old backup or
a mirror can never supply a retired word as an answer, a distractor or a
search hit; `retiredIngest.test.ts` feeds a stale band through the real loader
and fails with the guard removed.

## 20Z.9 Existing learners

Nothing in this pass touches a stored row's shape: the attempt record gains an
optional `revealed` flag that old rows simply lack, and `applyReview` prefers
`hint_level` and falls back as before. Progress, memory, mistakes, plans,
placement, settings, locale and the Numbers records are read and written by
the same code as the fifteenth pass; `upgradeCompatibility.test.ts` and the
schema walk (§13.1a) run unchanged. Quotation rotation keeps nothing on disk,
so adding two rows resets nothing. A retired word in a stored plan is still
repaired the same way; a retired word in a stale band is now refused earlier.
The version correction touches no migration.

## 20Z.10 What this pass ran

Every gate in `verify:release`, and the end-to-end suite on both projects;
figures in §2.2 and §19.1. New in this pass: `splash:check` and
`splash:bundle:check`, `quotes:audit:check` and `quotes:render:check`,
`copy:remediation:check`; `answerReveal`, `buildInteraction`,
...
audit-consistency test fails each); `check-native-splash.py --apk` against the
1.0.6 delivery (10 findings).

## 20Z.11 Not claimed

* **A native speaker has read anything.** Twenty-one quotation rows, the 752
  ledger rows in thirty languages and the thirteen Korean rewrites were read by
  the model that wrote them (I-17, I-213, I-222).
* **The Carlyle attribution is established.** It is conventional and marked so.
* **A physical device ran the binary.** The cold start was recorded on the API
  36 emulator; §18 still says *PHYSICAL DEVICE NOT VERIFIED*.
* **iOS was built.** The project carries 1.0.4 / 25 and is synchronised; no
  archive and no IPA exist, for the reason §18.6 gives.
* **The other thirty languages' interface strings were re-read.** They were
  gate-checked; §3.7 of the copy audit says so.

---


# 20AA. The seventeenth pass — the web on its own, at v1.0.5

**WEB TESTED / ANDROID NOT BUILT / IOS NOT BUILT.** This pass audited and
corrected the web product only. Nothing under `apps/mobile/android`,
`apps/mobile/ios`, no Gradle, Xcode or CocoaPods file and nothing in `result/`
was modified, synchronised, packaged or built; `git status` after every phase
was the proof, and the delivered native artefacts remain **1.0.4, versionCode
25 / build 25**. The web product is **v1.0.5**, and §2.1 of this document now
carries the two numbers separately. The four companion documents are
`docs/WEB_PRODUCT_CRITICAL_AUDIT_v1.0.5.md`,
`docs/WEB_UX_CONTENT_REMEDIATION_LEDGER_v1.0.5.md`,
`docs/WEB_CONTENT_REVIEW_v1.0.5.md` and `docs/WEB_QA_EVIDENCE_v1.0.5.md`;
`docs/WEB_RELEASE_NOTES_v1.0.5.md` is what a learner would be told.

## 20AA.1 What the sixteenth edition said that was not so

The rule in §1 was applied to the previous edition. Of the claims a learner
would feel, four did not survive re-derivation, and none was a defect a
learner had met — each was a gate or a figure saying more than it had
measured.

| Claim | Result |
| --- | --- |
| §5.2 / §15: `screens:audit` at 200 % text finds nothing; §20Z.8 "nine large-text measurements … re-run for real" | **CONTRADICTED, twice over.** Four sites were not on the sixteenth pass's list — `screens:audit` itself (root `font-size`), `modals:qa`, `accessibility.spec` and `confirm-dialog.spec` (`text-size-adjust`). And at eight of the nine sites it did correct, the token sheet was appended from an init script before the document had a `<head>`: the append threw on null, Playwright swallowed the throw, and every figure §20Z.8 quotes as "re-run for real" — `numbers:layout` 45/45, `align:sections` 12/12, `legal:isolation` 16/16, `scroll:audit` 210/210, 79 spec cases, the 64 px tab targets — was measured at normal text. The tokens are pixels. One shared injector now appends at `DOMContentLoaded`, every scaled site reads back the applied scale and refuses to measure an unscaled page, and the corrected gates assert the text really is ≥ 1.9×. On the corrected CSS the renders pass; on the old CSS the corrected gates fail at 320 px — the words category grid, the progress label, the tab labels and the conjugation rows overflowed, and are fixed; at doubled text the tab label *Learning* broke inside the word until its cap became width-aware |
| §16: first load 273.3 kB "at 3,393 words" | **stale.** 311.8 kB at the baseline commit, **314.1 kB** now (the 24 new *More about it* notes × 32 languages land in band 1); 3,370 words. Every budget met; none raised |
| §20K / I-204: the Numbers seed race | **not reproducible, and now pinned.** The phase has been gated on hydration since 8b489ebe; `numberSessionSeed.test.tsx` seeds stored attempts through a slow driver and proves the first question comes from the stored count. Resolved |
| §8.1: ten complete and twenty-two partial packs | **stale** — §11.1 of the same edition already said 32 complete · 0 partial |

## 20AA.2 What a learner met — **fixed, with tests**

- **Try again graded over the rejected ink (P1).** The second attempt at a
  letter was evaluated on the first attempt's strokes plus the new ones, so a
  correct retry read *Incorrect*. `journey.spec` and `feedback.spec` had
  clicked *Clear* first and never saw it. A fresh canvas mounts on every
  *Try again*; the specs now assert the empty canvas instead of clearing it.
- **The alphabet ended in silence (P1).** With no partner URL — the web
  build's state — the earned card rendered nothing. It now names the next
  step inside the product, *Today's Words*, in 32 languages;
  `NextStepCard.test.tsx` refuses a `null` and refuses an invented outward link.
- **Matching, after Check:** "3 of 4 pairs right." instead of a bare
  *Incorrect*, in 32 languages; the no-credit-unless-all-right rule is
  untouched and still tested.
- **A gap-fill blank is announced** to a screen reader (the label had sat on a
  `<span>`); **the 404 page has a title**; **the Level Test clock** no longer
  reads 8:01; **Numbers verdicts** use the same two words as every other
  exercise; **the Language screen** no longer claims some words are shown in
  English when none are.
- **What doubled text found once it was really doubled:** the words
  category grid, the progress label, the tab labels and the conjugation rows
  overflowed at 320 px; *My Learning* broke inside the word at 390 px; the
  Numbers course list panned 47 px sideways in German; a full-width *Got it —
  let's start* widened the unit intro to 355 px and cut the lesson counter
  off; the reset dialog's answers were asserted to share a row the stylesheet
  says they must not. All fixed and measured (ledger W-010, W-023, W-025,
  W-026); the twelve languages whose single tab word is wider than a fifth of
  a 320 px screen at normal size are a translation-length question for a
  native speaker, recorded rather than squeezed.

## 20AA.3 Content

- **Levels, re-derived at the source (I-126 → RESOLVED).** The frequency
  reader had credited 있어요 to 잇다 (rank 1) and 잘게 to 잘다, and had read an
  inflected adjective as rare (길다 893rd). It now has a form model by class
  and shares an ambiguous string in proportion to each owner's unambiguous
  evidence; `frequency:check` pins 96 forms (the old reader scores 52/84).
  419 of 3,370 words changed level, 161 anchors held, three overrides retired
  because the model now agrees with them. I-79 re-measured: 520 words at
  levels 28–30, 52 days for a level-30 learner at ten a day.
- **Thirty English card glosses** were dictionary definitions (*coffee,
  especially the beverage*; *older brother of a male* for 형) and are glosses
  now; **24 *More about it* notes** written in 32 languages (75 → 99).
- **The Level Test's beginner end:** 17 curated gap-fills (levels 1–5: 36 →
  53), each recorded in both voices, each refused by at least one bank rule
  before it was accepted; a curated item now plays **its own** sentence rather
  than the card's example. Simulation validity re-run and holding; real
  validity stays external, and is labelled so.
- **Child safety, three evasions closed:** spelled-out jamo (ㅅㅔㄱㅅㅡ), a loose
  final after an open syllable (세ㄱ스), and a Latin word in a Korean field
  (우리 sex 하자); internationally borrowed profanity is refused in every
  language. 368 fixtures, both evaluators agreeing; the full scan reads
  1,007,447 fields with 0 findings, and planting the three evasions in the
  Korean UI copy produced 3 findings and exit 1. The on-device subset does not
  carry the English lists (they would put the chunk past its 24 kB budget) —
  the policy says so now, and the runtime test names the fixtures it does not
  carry rather than skipping them silently.

## 20AA.4 The version, honestly

`product.ts` says 1.0.5 and a learner reads `v1.0.5`. The native projects say
1.0.4 / 25 and were not touched. `version:check` carries both numbers,
requires the web never to fall behind the native release, requires
`docs/WEB_RELEASE_NOTES_v1.0.5.md` and this file's `web_version`, and prints the
buildNumber-25 reuse as *pending, for a person with the store consoles* rather
than failing on it while the web leads. Set to 1.0.3, 1.0.6, or with the front
matter line removed, it fails (exit 1 each).

## 20AA.5 What was run, and what was not

Every web gate individually — typecheck, lint, 1,298 + 96 + 237 + 1,525 unit
tests (the counts of the seventeenth pass are superseded by §19.1), the content and copy gates of `verify:quick`, the web-safe members of
`verify:release`, the render gates against the production build (166 `screens:audit` renders,
now with a 320 px profile at 200 %), the e2e suites at both projects (314
and 314, none failed) — with exit codes and counts in
`docs/WEB_QA_EVIDENCE_v1.0.5.md`. Not run: anything that syncs, builds,
packages or reads a native project; `name:check` fails on `patent/**` (an
untracked, read-only directory) and on nothing else; `docs:consistency:check`
fails on one figure — `result/RELEASE_VALIDATION.md` records the 811
content-safety tests of the 1.0.4 delivery and there are 833 now — because
that file is the delivery's own record and only a delivery rewrites it. Every gate added or
corrected in this pass was broken on purpose and shown to fail before it was
believed.

# 20AB. The eighteenth pass — content, web only, at v1.0.5

**WEB TESTED / ANDROID NOT BUILT / IOS NOT BUILT.** Nothing under `apps/mobile/android`, `apps/mobile/ios` or `capacitor.config.*` was modified, synced or built; the native deliveries stay at 1.0.4, versionCode 25, and the web product at v1.0.5. This pass changed shared educational content, the scripts that generate and validate it, the web runtime's content-safety gate, web tests and this report.

## 20AB.1 What the seventeenth edition said that was not so

The previous edition claimed every figure was settled while `docs:consistency:check` failed on twenty-four of them. Read against the tree it described, the report gave the corpus three sizes on the same day — 3,370 in §2.2, 3,393 in §1, §3, §8, §9, §11 and §14, 3,333 in the pass narratives — said *20 packs complete, 12 at 600* in one table row and *32 complete* two chapters later, counted the beginner gap-fills as both 36 and 53, gave the test suites as 1,512/811/2,689 in one place and 1,523/833 in another, called the render pass 166 screens and 143 screens, and listed I-126 in the verdict as *needing a corpus that is not open data* while the ledger marked it RESOLVED. I-79's summary said *five weeks* and its impact said *52 days*. Each of these is labelled in `docs/WEB_CONTENT_QUALITY_AUDIT_v1.0.5.md` with what was found when it was re-derived: CONTRADICTED where two current sentences disagreed, PARTIALLY VERIFIED where a figure was right at its pass and stale as a current claim, VERIFIED where it held.

The mechanism was the one this report has named before about other numbers: figures were typed, not read. The fix is a single generated inventory — `docs/content-inventory.json`, written by `npm run content:inventory` from the shipped packs, the level-test bank, the audio manifest, the relations file and the safety audit — that every current figure in §2.2 now cites, and a `--check` in `verify:quick` that fails when the file is stale. §1 now says which sections are current and which are history, and the history is left as written.

## 20AB.2 The corpus: 3,867 words, 6,133 short

Seven batches, 497 words, `content/vocabulary/entries/300.jsonl` through `306.jsonl`. The candidates came from the National Institute of the Korean Language's learner vocabulary list (grades A and B first), filtered against the pack, the tombstones and the one-to-four-syllable rule, so that what was added is what a first- and second-year learner is taught rather than what a subtitle corpus happens to say. Every entry carries the same sixty-three authored strings as the rest of the corpus — the headword, a Korean definition, an English gloss, meanings in the seven inline languages and the twenty-four copy packs, a Korean example and its translation in thirty-one languages — and four recordings. Every batch went through the ten-step validation the brief asks for: the preflight before the pack, `examples:qa`, the sense and translation gates, the child-safe policy, the level model, the level-test rebuild, the audio plan and the recordings, the relations, the inventory, and the diff.

What the gates refused on the way is the useful part of the record: thirteen example translations that collided with another word's sentence in one language (평소 and 보통 both said *I usually get up at seven* in Uzbek), two two-sense meanings in Japanese, a French sentence that named a person the Korean did not, two words whose polarity a heuristic read as flipped, one gloss (*hanja*) the jargon rule would not take, seven examples the `--check` form of `examples:qa` refused that the writing form had let through (two clause joins, a question where the Korean is a statement, a supporting word harder than the headword), one level-test item that answered itself (빗 inside 빗었어요), three meanings the child-safe policy refused in Russian, Ukrainian and Tamil (an *автомат* that is also a rifle, a *கவர்ச்சி* that is also glamour), and five words with an unstable 받침 before a vowel for which somebody had to decide whether the vowel begins an ending or a word (꽃잎 is [꼰닙]; the other four are endings). Each was rewritten or decided, not waived; one word, 및, was cut with a written reason because every example that uses a formal connective trips the register rule by construction. Two female clips the recorder had returned at 170 ms were caught by `audio:qa:full` and re-recorded.

The target is 10,000 and it is not close: **3,867 of 10,000, 6,133 short.** The unit is now measured rather than estimated — a batch of about seventy-five words is roughly an hour of authoring and a twelve-minute rebuild — so the remaining distance is about 86 such batches. `vocabulary:qa:target` still prints it. Nothing about the target was lowered, hidden or restated.

## 20AB.3 Levels, read from two sources

The level model had one frequency source (two OpenSubtitles corpora) and an editorial usefulness mark written per batch, and the two together had put 맛 at 23, 미리 at 24 and 딱히 at 30. It now also reads the National Institute's learner list — 5,965 words graded 초급/중급/고급 (KOGL type 1, attributed in Legal & Licences) — and takes the *lower* of the two frequency costs and caps the editorial mark at the list's grade. 1,031 words moved down and none up; every anchor in `level-anchors.json` held; `vocabulary:level:qa`, `vocabulary:level:audit`, the level-test gates and the daily-plan gates are green. The negative case removes the learner term and requires the level gate to fail, and it does (`docs/content-gates-negative.json`, N7).

I-79, re-measured from the plan's own zone rule: levels 28–30 hold **436** words (they held 520 before the second source, and the difference is words the subtitle corpus had never seen, not words that got easier), a level-30 learner at ten new words a day has 43 days of all-new material and 87 with the reserved share; and the thinnest zone is no longer the top — it is 24-26, 191 words, 19 days at ten a day. The next batches should be aimed there.

## 20AB.4 The child-safe policy at runtime — I-228

The 24 kB runtime subset carried the language-neutral rules and the English lexicon, so a level-test item whose meaning the full policy refuses in Thai passed the runtime gate. There is now one supplement per shipped language under `packages/content-safety/policy/runtime/`, loaded lazily as its own chunk for the learner's language, and `guardLevelTestItems` reads meanings in every locale through `runtimeEvaluatorFor(locale)`. The route-chunk budget was not raised. `runtime.test.ts` proves every fixture the full policy refuses is refused by the supplemented evaluator; negative case N6 strips the English supplement and the suite fails. The policy version the runtime expects and the version the publication carries are compared at build time.

## 20AB.5 Translations: audited in full, read in part, reviewed by nobody

`translation:audit:check` reads every shipped row in every locale — 123,744 rows, 243,621 fields — for presence, writing system, Korean or English copied through, placeholders, the terminal punctuation the Korean has, and length against the locale's own distribution. Result: 160 length outliers, none of which is wrong when read, and **0** blocking findings; per-locale and per-row results are machine-readable in `docs/content-translation-audit.json`. That proves form. For meaning, the model read a seeded 90-row sample, three per locale, against the Korean and the gloss: 87 PASS and 3 FLAGGED and corrected — a Tamil verb that meant *to shine* under 반응하다, a Mongolian adjective that does not mean sweet under 불고기, a European Portuguese adverb in the Brazilian pack. The labels are exact: every row is AUTOMATED CHECKED; ninety are MODEL-REVIEWED; none is NATIVE-SPEAKER REVIEWED, because no native speaker has read one (I-17, I-213). The two home-screen quotations get the same treatment in `docs/QUOTE_TRANSLATION_REVIEW_v1.0.5.md`: 64 rows, 43 model-reviewed, 21 model-reviewed with a named point for a native reader, and the Carlyle line shown under Carlyle's name with the runtime and the audit both recording that it has not been located in his published works.

## 20AB.6 What scales and what does not

`docs/content-scalability.json` (`npm run content:scalability`) measures delivery as served: the first screen needs the shared tables, band 1 and one language of band 1 — **60.3 kB gzipped in the heaviest language**, against a 120 kB line and the application's 460 kB first-load budget — and that figure does not grow with the corpus, because band 1 is cut by the generator, not by the corpus size. The whole corpus in all thirty-two languages is 2,560 kB gzipped, the audio 81.9 MB; at 10,000 words, linearly, 6,621 kB and 211.9 MB. The audio is fetched per clip and is never in the first load; the dictionary and the level-test bank are fetched on first use. The negative case N5 pulls the first-render line under band 1 and the check fails.

## 20AB.7 What was run, and what was not

Every content gate in `verify:quick` and every content member of `verify:release`, after every batch and again on the final tree; the seven new checks (`content:inventory:check`, `translation:audit:check`, `content:scalability:check`, `quotes:review:check`, the runtime-supplement tests, the learner-list level check, the negative harness `scripts/content-gates-negative.sh`); typecheck, lint, the unit suites, the production build and its budgets, the render gates at 320/360/390/412/430 and 200 % text, and the e2e projects — with exit codes and counts in `docs/WEB_CONTENT_QA_EVIDENCE_v1.0.5.md`. Not run, by the brief's own boundary: anything that syncs, builds, packages or reads a native project.

## 20AB.8 The repository, repaired

The VM was torn down at 15:01 on 14 September in the middle of `git commit`. What that left is precisely known: 1,092 loose objects of zero bytes under `.git/objects`, every one with that minute's timestamp; `git fsck --full` reported 1,070 *missing blob* errors, every one referenced only by the index the aborted commit had been writing, and no missing tree or commit reachable from any ref — `HEAD` and `refs/heads/main` both still named `d06c7faa`, the last good commit, and the remote agreed. The lost commit `399d17ff` existed only as two reflog lines. The repair kept every constraint the brief set: the working tree was fingerprinted (`sha256` of every file) before anything was touched; the zero-byte objects were *moved* to a quarantine directory outside the repository, not deleted; the index was rebuilt from `HEAD` with `git read-tree HEAD` (no `reset`, no `checkout`, nothing that writes the working tree); the two reflog lines naming the unreachable commit were removed after the reflogs were backed up; `git fsck --full --no-dangling` then exited 0 with no output; and the fingerprint was taken again and matched byte for byte. `git status --short` works, lists the pass, and shows no native path. Every backup — the patch, the untracked-file inventory, the tarball of the changed tree, the object quarantine — is under `/root/hangyul-backup/` outside the repository. I-232 is RESOLVED, and the commits and the push are in §20AB.12.

## 20AB.9 The study-selection arrow

The screenshot that opened the second reading showed the chevron on the 소리가 만날 때 card sitting in the middle of the card while the lesson rows kept theirs at the edge. Measured on the production build, the lesson rows drew the arrow 16 px from the card's trailing edge, the Numbers card 46 px in, and 소리가 만날 때 78 px in. The cause was structural, not a typo: the two foot cards were a flex row whose text column had no `flex: 1`, so the arrow sat wherever the title happened to end, and any locale with a shorter or longer title moved it. `LettersPage.module.css` now gives every navigation card the same two-column grid — `minmax(0, 1fr)` for the text, a fixed 24 px trailing slot for the arrow, `justify-self: end` — so the arrow is on one guideline whatever the title says; the arrow is `aria-hidden`, the link is named by its title, the tap target is the whole row. The direction is a decision: in Arabic the trailing edge is the left one and the arrow is there, mirrored. `apps/web/e2e/letters-chevron.spec.ts` measures the *rendered* arrows in Korean, English and Arabic at 320, 360, 390, 412 and 430 px and at 200 % text, and requires every card's inset to agree within a pixel, the slot to be at least 24 px square, the link at least 44 px tall, the text column clear of the slot, and no sideways scroll; with the old layout put back it fails by 30–62 px. Before and after, from the production build: `docs/qa-screenshots/letters-chevron-before-ko-390.png` and `letters-chevron-after-{ko-390,ar-390,en-320,de-430}.png`. The Level Test's first question, which the same reading found taking up to two seconds to appear after the start button, is prefetched while the intro is on screen.

## 20AB.10 The corpus, read again

Every one of the 497 new entries was re-read as a set, looking for what bulk generation leaves behind: template sentence shapes (the commonest shape is shared by seven entries, which is what hand-written Korean at this length looks like), sentences that begin with a pronoun (normal), lengths (two to five eojeol, mean 3.2), two-synonym glosses (26, each read and kept), and identical English example translations across the whole corpus (29 pairs; five hid a real distinction and were rewritten with all 31 translations — 지지, 지원, 가면, 신장, 안쪽, 비교). The same reading closed I-231: the 108 shared English glosses that remained after the thirty-two rewrites — pairs such as 늘/언제나 *always* and 매우/아주 *very*, which are distractor collisions in every meaning exercise — were differentiated one by one (108 → 0, `duplicates.shared_english_glosses`). Nothing was retired; 및 stays cut; no entry was rejected on the second reading. The 884 learner-facing Korean interface strings were read in full: one was rewritten (다시 해보기 → 다시 해 보기, the auxiliary-verb spacing the rest of the copy uses), and the copy ledger records it.

## 20AB.11 Thirty-two locales, one table; sixty-four quotations, fourteen columns; the policy against evasions

`docs/LOCALE_AUDIT_v1.0.5.md` (generated by `scripts/build-locale-audit-v105.mjs`) is one row per locale — fields reviewed, meanings corrected, examples corrected, interface strings corrected, exercise collisions corrected or rejected at build, safety findings corrected, layout findings from the rendered screens, what remains for a native reader, and the status — with every count diff-derived and its word list in `docs/locale-audit-v105-input.json`. Every locale is HUMAN REVIEW REQUIRED, because no native speaker has read any of them, and the table says so in every row. `docs/QUOTE_TRANSLATION_REVIEW_v1.0.5.md` now carries the fourteen-column table the brief asks for — quote, locale, Korean source, translation, displayed attribution, meaning, metaphor, naturalness, register, punctuation, model-review status, native-review status, required correction, final status — for all 64 rows: 43 RESOLVED, 21 HUMAN REVIEW REQUIRED, none NATIVE-SPEAKER REVIEWED; the Carlyle line's attribution is stated with the same limit as before, and no source was verified that could not be.

The child-safe policy was run against twenty-six further evasion shapes beyond the fixtures it had. Ten passed it: a two-character Korean term split once with its ending still attached (섹 스하다, 자 살하고, 마 약을 — the run rule joined only bare syllables and the flat form only three-character terms), an ideographic space, Cyrillic look-alike letters inside a Latin word (sеx, fuсk), a Latin or Cyrillic term with a Korean particle attached (sex를, casino에), a parenthesis used as a mask, a spelled-out term with a hyphen inside it, and Han or kana inside a Korean field (殺人). Each is now refused, in both the TypeScript evaluator and its Python port, by four rules with their false-positive cost measured: single syllables join *forward* to the word after them and never across punctuation (measured over the 84,038 Korean strings the app ships: no new finding); look-alike letters are folded only inside a token that mixes scripts, so a Russian word is never touched; a change of script is a token boundary; and a stray word in Cyrillic, Han or kana is read against the lists of the languages written in it, the way a stray Latin word already was. The fixtures went from 368 to 406 and the two evaluators agree on all of them; the runtime suite holds every fixture through the learner's supplement (1,298 cases); negative cases N9 and N10 put each rule back the way it was and require the fixtures to fail, and they do.

## 20AB.12 What was run, and the commits

Every gate in §20AB.7, again, on the tree after the second reading, with exit codes in `docs/WEB_CONTENT_QA_EVIDENCE_v1.0.5.md`; `qa:locales:check` over 256 rendered locale screens and `screens:audit:check` over 166 device renders; both e2e projects; the production build. The commits are listed in the QA evidence document with their hashes; the push was made once, without force, and the local and remote heads compared afterwards. Not run, by the brief's own boundary: anything that syncs, builds, packages or reads a native project; `result/` and `app_result/` untouched.

# 20AC. The nineteenth pass — the streak, the Korean read as Korean, and the report read against the tree

**WEB TESTED / ANDROID NOT BUILT / IOS NOT BUILT.** Nothing under `apps/mobile/android`, `apps/mobile/ios`, `capacitor.config.*`, `result/` or `app_result/` was modified, synced or built; the native deliveries stay at 1.0.4, versionCode 25, and the web product at v1.0.5. The pass began from `d511ff1e` on `main` with a clean tree and a clean `git fsck`. The named attachment `report(20260916-012042).pdf` was not on this machine; `docs/report.pdf` (sha256 `514fd1f3…`, 426 pages, created 2026-09-14T17:06Z) is the eighteenth edition and was read as the current source.

## 20AC.1 The streak

The brief said the learner-facing streak was zero-based. Reproduced against the live store, the first answered question already produced Day 1; what was wrong was what a day *was*. `recordStudyTime(20_000)` — twenty seconds on a session screen with nothing answered — made `streak_days` 1; ten answered Numbers questions made it 0 until the lesson's mastery check passed; a learner who had never answered anything read *0 days* beside a flame; and Home drew the chip before the profile had loaded. Probing the store also found a defect nobody had reported: an item reaching *learned* through a checked attempt was never credited to the day, because `updateProgress` noted the completion inside a React updater and read the note after it — and React, with `trackActivity`'s update already queued ahead, had not run the updater yet. *Letters finished* on the Activity screen was 0 for every letter learned by writing.

The rule is now written once, in `isQualifyingDay` (§20B), and every consumer reads it: a learning day has at least one answered question or completed item; opening, viewing, listening, saving and the level test are not learning days; study time is study time. Crediting happens inside the updater from the same `prev`, idempotent under a repeated run. Home shows *Start today* (32 locales) for a learner with no learning day and nothing until `ready`; the 64 streak strings were checked against each locale's CLDR plural categories and read. 20 domain fixtures, 11 store fixtures and 3 browser cases hold the cases the brief lists; N12 puts the count back to zero and N13 makes viewing count, and 15 and 3 fixtures fail respectively.

The new chip cost something the streak tests could not see and the header's rendered spec could: *Start today* is wider than *0 days*, and on the narrowest phone at 200 % text the brand header's `auto 1fr` grid answered by narrowing the logo's box to 29 px while its height stayed 26 — a smeared wordmark on the screen the app opens to. Measured in the production build, the same squeeze had already been happening to a learner with a one-day streak (98 px, distorted); the invitation only made it visible. The header is now a flex row in which the heading gives way first and by scaling — both dimensions, the asset's own ratio, a floor of 88 px — and the invitation folds onto two lines before it clamps; at the normal text size two lines of caption are 31 px and the 32 px chip already holds them, so the fold costs nothing there. Fresh profile at 320 px: 115 / 88 / 88 px at 100 / 150 / 200 % text, never distorted; 25 header cases green (I-247).

## 20AC.2 Every question, written down

`scripts/exercise-inventory.mts` writes `docs/exercise-inventory.json`: one row for every learner-reachable question — 73 characters × read, listen, distinguish and write; 3,864 words × read, produce, gap-fill and build; the matching grid per level band; the 4,156 Level Test items; the 368 Numbers questions — with prompt, answer, accepted alternatives, distractors, hint keys, explanation, example, transcript, part of speech, taught sense, level, origin, and the checks that ran, plus a progression map of where each Korean term and sound rule is introduced, practised and assessed. It validates every row in every locale it has text for: 548,852 checks. The letter rows validate the whole seeded draw (400 seeds per character) rather than three of them, and the matching rows compare every pair of same-level words in every pack. Three defect classes came out of it that no existing gate had seen, §20AC.4.

## 20AC.3 The corpus read as Korean

All 3,867 entries were read — headword, part of speech, level, English gloss, Korean definition, example and its English — together with the 73 characters' names, romanisations, stroke counts, hints and mnemonics, the fifteen lesson titles, all 902 pronunciation notes against 표준 발음법, every gap-fill with all four options substituted (four times, as the bank re-drew), the 368 Numbers questions from the ledger, and the conjugation panel's rows. The findings, by dimension:

| Dimension | Found | Fixed how |
| --- | --- | --- |
| **Phonology** | ㅀ-stems 싫다·잃다·끓다·옳다·뚫다·앓다·꿇다 labelled 된소리되기 (P1); 옮기다·굶주리다 labelled tensing with none; 담요 [다묘] and 식용유 [시굥유] against §29; 큰일·별일 under liaison; 솔직히 [솔지키], 일석이조, 일일이 without §26/§29 | `pronunciation.py`: aspiration excluded from the tensing recogniser, sonorant clusters only before endings; a seventh taught pattern *ㄴ 첨가* with copy in 32 languages; §26 forms listed; every §29 candidate a named decision (91 read); pattern-label fixtures and the gold set in `qa_pronunciation.py` |
| **Morphology** | 먹으세요·마시세요·자세요·있으세요·말하세요 under *Please do* (P1) | `displayConjugations` substitutes 드세요·주무세요·계세요·말씀하세요 on the card; the analyser still takes a typed 먹으세요 to 먹다 |
| **Orthography** | 고장나다 and 신경쓰다 taught joined; 한잔 하실래요 spaced against 한잔하다; 고마워 an inflected headword; the ㅡ hint in 합쇼체 beside 해요체 | three editorial tombstones (`retired-words.json`, category `editorial`); the 한잔 example re-recorded; the hint rewritten |
| **Semantics** | 그렇게 glossed *so, so much* under *don't do that*; 빼다 defined as *take out* while taught as *subtract*; 어서 defined as a greeting; 해마다 and 빗 defined with the answer inside the definition (a production prompt in Korean showed its own answer) | entries and the 24 copy packs corrected where the sense had drifted (ja そんなに → そのように, uz shuncha → shunday, …) |
| **Syntax / answer validity** | 64 gap-fills with a second defensible answer (§20AC.4) | 42 pairs banned, 36 words withdrawn from gap-fills |
| **Pragmatics** | 당신 at level 1 | override to 12 with the reason on the record |
| **Pedagogy / level** | 등 (the back) at level 1 on the frequency of 등 *etc.*; 곳곳 tagged adverb; one lesson title translated differently in 30 languages (kept: both describe the lesson) | override to 9; part of speech corrected |

Nothing about the target moved: the corpus is **3,864** taught words, three fewer than the eighteenth pass, and `vocabulary:qa:target` still prints the distance to 10,000.

## 20AC.4 Three defects the green suite could not see

**Two identical answer buttons.** The letter-sound question for ㅢ draws its wrong answers from a pool that joins the letter's confusion group with the syllables built from its components — so ㅣ and 이 (both *i*), ㅡ and 으 (both *eu*). `answerability` had built three seeds; over three thousand, 139 draws showed the same label twice, about one in twenty-five. The builder now draws wider and keeps the first three distinct romanisations; the sweep is a test.

**A synonym as a wrong answer.** Reading and production questions refused a distractor only when its gloss was *identical* to the answer's after folding. 잠깐 *a moment* was offered beside 잠시 *a moment, short time*; 사용하다 *to use* beside 이용하다 *to make use of*; 좋아지다 beside 나아지다; 최소한 beside 적어도 — 220 such pairs in English alone, read one by one, more in the other packs. `sameMeaning` now reads a gloss as its senses and refuses a distractor when any sense folds to the answer's or its content words are all among the answer's, in the learner's own language, when the question is built. Twenty fixture pairs hold it in both directions, five look-alike pairs hold that *here*/*there* and *last week*/*last month* stay available.

**One meaning on two rows.** 아빠 and 아버지 are both *baba* in Turkish, *ayah* in Indonesian and *apa* in Hungarian; 좋다 and 괜찮다 are *iyi olmak*; 세상 and 세계 are *العالم*. Both words of each pair sit at the same level, so both could land in one day's matching grid, and the grid graded by id. 896 such pairs across the packs and none in English, which is why the English reading of the grid never met one. The grid now keeps the taught word's row and drops a later member whose meaning folds alike, and the dropped word is credited through the question that did ask about it.

**And the gap-fills.** Every one was read with all four options substituted and classified. Sixty-four had a second answer Korean supports: 의사가 환자를 *보호했어요*, 결과를 *찾아봐* 주세요, 대사/*사위*가 한국에 왔어요, 생일에 *냄비*를 받았어요, 저녁에 *소설*을 봐요, 태양이 지구를 돌아요 (wrong astronomy, right Korean). Where the rival was one word, the pair is banned in `answer-conflicts.json` (42 pairs); where the frame admits any member of its class — a person arriving, a gift received, an animal drinking, anything in a drawer — the word is withdrawn from gap-fills with `ctx: 0` and stays taught (36 words). The builder had drawn every item's distractors from one random stream, so the first two rounds re-drew 592 and 558 items and each had to be read again; it is now seeded per item, and the third round changed 15. A-24 to A-87 in `docs/AMBIGUITY_LEDGER.md`; the bank is 608 gap-fills and 576 Level Test contextual items.

**And one the half-hour run found.** Two cases of the chip-layout spec, Korean and Arabic, failed only inside the full mobile run and passed alone. The review session resolves its plan once, on mount, and keeps it — so that answering cannot shorten the queue under the learner — and the router mounts as soon as the corpus core is in, which on a loaded machine is before the profile has been read from IndexedDB. The plan was resolved over the empty placeholder and frozen: *Nothing to review right now*, over twelve due words, until the learner left and came back. The word session and the Numbers order had the same class of defect earlier in v1.0.5; this was the third instance. The page now resolves a fresh plan only once the store has answered and draws nothing until then; `reviewSessionHydration.test.tsx` mounts it over a store that answers slowly and, with the old line planted (N21), two of its three fixtures fail (I-248).

## 20AC.5 The gold set

`content/korean-gold-set.json` holds the taught rules independently of the code: the 40 letters with names, romanisations and stroke counts; syllable decomposition; 받침 neutralisation; 27 sound-change rows each with its 표준 발음법 article; particles including 로 after ㄹ; every supported irregular class with its forms; the suppletive honorifics; spacing; weak frames; gloss collisions in three scripts; translation collisions; pragmatic rows; and safety evasions — each family with rows the validators must refuse. `apps/web/src/data/koreanGoldSet.test.ts` and `qa_pronunciation.check_gold_set` run it; `npm run gold:check` runs both and the morphology suite.

## 20AC.6 Thirty-two locales

Every question was built in every locale by the inventory: answer present once, no two options alike, one language per screen, prompt not containing its answer, audio present, policy passed. The gloss-overlap heuristic flags 30,736 answer/distractor pairs for a reader (`totals.gloss_overlap_hits_by_locale`); the 220 English pairs were read and the real synonyms fixed (§20AC.4); the other 31 languages are a reading list for a native speaker and are labelled so (I-246). Three new interface strings and two new lesson strings were written in all 32 languages by the model and are labelled MODEL-REVIEWED. No locale, Korean included, is NATIVE-SPEAKER REVIEWED; the `translation:audit` labels are unchanged.

## 20AC.7 Child safety

`content:safety:qa` re-run over the rebuilt tree: 443,000-odd items and 1.12 million fields in 32 locales, 0 findings; the new lesson copy, the new interface strings, the re-recorded example and the three tombstones are inside it. The runtime policy's retired-id list carries 26 ids. The gold set's evasion rows (a split term, an ending attached, leetspeak, spelled-out jamo) are refused by the same evaluator the app ships.

## 20AC.8 This report, read against the tree

The eighteenth edition was read in full as `docs/report.md` and its PDF, and every current claim in §2.2 re-derived. Findings: §20B described the streak as *any recorded study activity including session time* — true of the code and the reason for §20AC.1, and now CONTRADICTED by the product rule; §2.2's word count, pronunciation-note count, pattern counts, Level Test item counts, test counts and issue count were all true at the eighteenth pass and stale as current figures (PARTIALLY VERIFIED → regenerated by `docs:consistency`); §1 carried its current/historical paragraph twice (CONTRADICTED → one copy); the verdict's *3,393 taught words* limitation was two passes old (PARTIALLY VERIFIED); the claim that every gap-fill had been read with all four options substituted was VERIFIED for the passes that made it and no longer sufficient, since a re-drawn bank invalidates a reading (§20AC.4); the claim that `answerability` covers every generated question was PARTIALLY VERIFIED — it covered three seeds; the native-review claims (none) and the delivery claims (1.0.4 / 25, not rebuilt) were VERIFIED. The named attachment could not be compared byte for byte because it is not on this machine (EXTERNAL); the PDF in the tree is what was read.

## 20AC.9 What was run

Every gate of `verify:quick` except `name:check` (which fails on the gitignored `patent/` directory before this pass and after it) on the final tree, step by step with exit codes; the production build; `audio:pronunciation:check`, `answerability:check`, `exercise:inventory:check`, `gold:check`, the negative harness with its ten new cases (N12–N21); `docs:consistency`; the e2e suites named in `docs/WEB_CONTENT_QA_EVIDENCE_v1.0.5.md` §9, where the commands, counts and exit codes are. Not run, by the brief's boundary: anything that syncs, builds, packages or reads a native project.

# 20AD. The twentieth pass — the final delivery audit, and what a green suite had not walked to

**WEB TESTED / ANDROID NOT BUILT / IOS NOT BUILT.** Nothing under `apps/mobile/android`, `apps/mobile/ios`, `capacitor.config.*`, `result/` or `app_result/` was modified, synced or built — the 218 tracked Android files were fingerprinted before the pass and match byte for byte after it, and `git diff` from the starting commit across `apps/mobile` and `result/android-project` is empty. The native deliveries stay at 1.0.4, versionCode 25 / iOS build 25; the web product at v1.0.5. The pass began from `65c023af` on `main` with a clean tree. The named attachment `report(20260917-042600).pdf` was not on this machine; `docs/report.pdf` (446 pages, created 2026-09-16T06:08Z) is the nineteenth edition and was read as the current source — every page rendered and looked at, and its current sections re-derived against the tree (§20AD.5).

**The version, before anything was touched.** The brief said the app is at 1.0.5 and that no version or build field may change. The repository's state is two numbers, not one, and both were recorded before the first edit: the web product reads **1.0.5** (`apps/web/src/config/product.ts`, `docs/WEB_RELEASE_NOTES_v1.0.5.md`, this report's front matter); the native deliveries read **1.0.4**, build **25** — `app.identity.json`, `apps/mobile/package.json`, `MARKETING_VERSION` and `CURRENT_PROJECT_VERSION` in both configurations of `project.pbxproj`, `CFBundleShortVersionString` and `CFBundleVersion` resolving to them, `versionName` and `versionCode` in `build.gradle` reading the identity file. That is a conflict with the brief's expectation and it is reported as one rather than resolved: moving the native number to 1.0.5 would be exactly the edit the brief forbids, and `version:check` documents the arrangement (the web may lead the native deliveries, never lag them) as deliberate. Every one of those six files is byte-identical at the end of the pass — `sha256` before and after — except `product.ts`, whose only change is the removal of two exports nothing imported (§20AD.4); its `version: '1.0.5'` literal is untouched and `product.test.ts` pins it.

## 20AD.1 A screen that could not load unmounted the app — **P1, fixed (I-249)**

Every screen but Home is a `lazy()` chunk, which is right — a learner on the alphabet should not download the level test — and a chunk is a network request the first time it is wanted. `main.tsx` mounts the router inside `Suspense` and nothing else: no error boundary anywhere in the tree. React's rule for a rejected import inside `Suspense` is to unwind to the nearest boundary, and the nearest one was the root.

Reproduced against the built bundle before anything was changed: a static server over `dist/` that answers every request and 404s the Letters chunk once the Home screen is up. Tapping *Letters* left `#root` with **0 children** — a cream page with no header, no navigation and nothing to tap — and one page error, `Unable to preload CSS for /assets/LettersPage-….css`. In a browser the learner has the reload button; in the native shell they have nothing. The two ways to meet it are ordinary: a connection that drops after install and before the worker has finished precaching (the precache is 1.3 MB and is not awaited), and a tab kept open across a deploy that renamed every hashed file.

`ui/ScreenErrorBoundary.tsx` now wraps the routed screen — and only the routed screen — in the tab layout, the focus layout and the not-found route, so the header, the bottom navigation and Home stay reachable. Its panel is the not-found page's shape (the mascot, one heading, one sentence, the way out) with the two actions that recover the situation: **Reload**, because a rejected `lazy()` stores its error and never retries, and **Go to home**, which is not lazy. It is keyed to the path so that leaving the failed screen clears it. Copy in 32 locales (`errors:screen.title`, `.body`, `.reload`), register matched to each pack, MODEL-REVIEWED.

What proves it: `screenErrorBoundary.test.tsx` renders a route whose import rejects and holds three things — the panel appears instead of an empty tree, Reload calls the injected reload once, and navigating to Home clears the failure while returning fails again. The same server that produced the blank page now produces the panel with the tab bar under it, in light and dark. And `screens:audit` has a seventh state, *screen failed*, reached the way it happens — the context has no worker so the request reaches the network, and the network refuses the chunk — rendered on the five layout-deciding profiles including 320 px, dark and 200 % text: **171 renders, nothing clipped, nothing overlapping, nothing unreadable.** The audit's contexts now block the service worker for every profile, because a chunk the worker has precached never reaches the network and could not have been refused; the worker itself is measured by `routing:check`, not here.

## 20AD.2 A launch that could not read the profile wrote over it — **P1, fixed (I-250)**

`LearnerProvider.hydrate()` opens the store, wires eight repositories to it, runs the migrations and reads everything. Its failure path is deliberate and was tested: a launch that cannot read the profile gives the learner a working app on a fresh in-memory profile, and does *not* tell them their browser is broken, because the probe that decides that is a real write/read/erase round trip and it may well come back healthy. Both tests were green.

Neither test asked what happens next. The repositories had been wired before the read failed, every write goes through them, and every write carries whatever `state` holds — which was now the fresh profile. So the first answered question credited the day and saved *default settings* over the learner's stored row: level, streak days, daily plan, placement result, content seed. The first letter written saved a fresh progress row over a learned one. The storage verdict said **saving** throughout. A transient failure to read — one damaged record, one migration that throws on one odd row — became a permanent loss on the learner's next tap, and nothing in the product could have told them.

Reproduced in `storageWarning.test.tsx` against a durable driver holding a level-12 profile whose `getAll` rejects once: with the guard removed, **four writes** reached the driver after one answered question and one preference change. With it, none, and the stored row reads back byte for byte. The fix is the smallest one that is also right: on that path the repositories are detached (`wireRepositories(null)`), the session runs in memory, the stored rows are left for the next launch to read, and the verdict is `not-saving` with `reason: 'unreadable'` — which My Learning renders as its own sentence (*your saved progress could not be read this time, so nothing from this session is being kept; it is still on this device — close the app and open it again*, 32 locales) rather than the sentence about a browser that cannot keep data. A restore from a backup wires the repositories again, because a restore replaces the stored rows on purpose and then has to read them back.

## 20AD.3 Thirty-two languages, read where a learner answers

The eighteenth and nineteenth passes audited every translation row mechanically and read a 90-row sample of the content packs. Nobody had read the *interface* strings a learner meets while answering — the prompts, hints, feedback, matching and build instructions, mistake cards — in every language. This pass did, for all 32, and then checked the whole of each pack mechanically for interpolation, tags, brackets, whitespace, English left in a non-Latin script, and two keys whose translations collide where the English differs. The mechanical pass found nothing the existing gates had not (every collision is two labels that never share a screen, or a plural form a language does not distinguish); the reading found six things the gates cannot see:

| Locale | Found | Fixed |
| --- | --- | --- |
| cs | A Hangul letter was a *litera* on 37 strings — the tab, the home card, the daily goal, every lesson prompt — and a *písmeno* on two. *Litera* is the printer's word for a piece of type; a beginner's word for a letter of the alphabet is *písmeno* | every string says písmeno, with the declension and agreement rewritten (Dokončená písmena, u tohoto písmene, Písmena dnes) |
| el | Fourteen exercise instructions addressed the learner in the formal plural — Πατήστε, Είδατε, Βάλτε, Διαβάστε, μπορείτε — while 110 strings around them use the informal singular (Γράψε, Δες, Απάντησες) | rewritten in the informal |
| hu | One formal sentence in an informal pack: *Nem tudja? Nyugodtan koppintson…* | *Nem tudod? Nyugodtan koppints…* |
| ro | *Greșit de {{count}} dată* renders *de 1 dată*, which is ungrammatical | *Greșit o dată* |
| ru, uk | *С этим разобрался* / *З цим розібрався*: a masculine past tense on the mistake card's button, assuming a male learner | *С этим всё ясно* / *З цим усе зрозуміло* |

How the first three had passed `locale:editorial`: its register rule counted pronouns, and Hungarian and Greek drop the pronoun and carry the register in the verb. The rule now reads the commonest imperatives in both registers for those two languages (*koppintson/koppints*, *πατήστε/πάτησε* and their kin), and a per-locale glossary was added with Czech as its first entry (*litera* → *písmeno*, matched as a whole word in every case). Restoring any one of the three defects fails the gate, and the gate then names the string; the negative runs were done and reversed. Every changed string is a row in `docs/ux-copy-remediation.json` — old text, new text, reason, verification — and `copy:remediation:check` holds the new text to the bundle; `locale:editorial` reports **0 errors, 13 warnings**, all the split-translation class for a person.

Also read and left alone, with the reason: Tamil's bare-stem imperatives on buttons (*தொடங்கு*, *எழுது*) beside formal *-ங்கள்* prose, which is the convention Tamil interfaces use; Mongolian's *дарна уу* request form on exercise instructions beside *чи* elsewhere, which is the neutral request Mongolian interfaces use and would need a native reader to call a mix. Two new strings were written in all 32 languages for §20AD.1 and §20AD.2; they are MODEL-REVIEWED, not native-reviewed, and I-17 stands.

## 20AD.4 What was removed, and the proof for each

Nothing was deleted on a guess. Three kinds of evidence were built first: an import graph over `apps/web`, `packages` and `scripts` — static, dynamic and `require` imports, CSS `url()` and `@import`, and path strings inside scripts — walked from every entry point (`main.tsx`, every test and spec, every script, the Vite, Playwright and ESLint configs); a whole-repository search for every exported symbol by name, outside this report and the issue ledger; and the report's image references against `docs/report-assets` and against the list of names `capture-report-shots.mjs` writes.

| Removed | Proof |
| --- | --- |
| `@tanstack/react-query` (apps/web dependency) | imported by no file in the repository. `npm uninstall` also brought `package-lock.json` back in step with the workspaces — the lock recorded `apps/mobile` at 1.0.5 while its `package.json` says 1.0.4, and lacked `@hangyul-ganada/content-safety` — without changing any version field |
| 36 exports no file imports | `familyName`, `HOME_ROUTE`, `getUnit`, `LETTER_GROUPS`, `CURRICULUM_SYLLABLES`, `corpusProgress`, `resetLevelTestBank`, `NUMBER_UNITS`, `vectorGlyphPaths`, `STROKE_ORDER_CHARACTERS`, `strokesFor`, `difficultyLevels`, `wordsTagged`, `readableWords`, `lessonsInCategory`, `RETIRED_POLICY_VERSION`, `levelProgress`, `levelOfRank`, `CUMULATIVE_WORDS`, `readNativeInsets`, `MetaRepository`, seven icons no screen draws, `PRACTICE_GLYPH_OPACITY`, `countDifference`, `withFinal`, and in `shared-types` the `CharacterAttempt`, `AppConfig`, `CreateSessionRequest`, `RecordAttemptRequest` and `ApiError` types of an API this product never had — each referenced by exactly one file, its own declaration, and the locals that only they read (`difficultyLevelCount`, `META_KEY`, `SchemaMeta`) with them |
| `apps/web/src/pages/LetterSessionPage.module.css` | eleven lines of comment, no rule, imported by nothing; its own note points at `SessionPage.module.css` |
| 22 files under `docs/report-assets` | referenced by no document and written by no script (`r-*`, `native-*`, `device-home`, `step-trace`, `word-*`, …): evidence of passes whose text has since been rewritten |
| five `*.png:Zone.Identifier` stubs and the empty file `=` | Windows download-provenance streams tracked from the initial import (`.gitignore` already names them as not part of the product) and a shell-redirect accident |
| `result/docs/report.pdf` (on disk, ignored) | the sixteenth edition's delivery copy; the canonical report lives only under `docs/`. `result/checksums.sha256` still carries its row, because that file is the Android delivery's manifest and the next `result:build` rewrites it |

Kept, with the reason: the five brand PNGs and `word_study.jpg` at the repository root (the owner's design references), the wordmark PNGs under `public/brand` (46 kB, documented in `docs/DESIGN_AUDIT.md` as the delivered logo package), the six screenshots the script regenerates every run, and all 132 scripts — each is named by `package.json`, by another script or by a document. `typecheck`, `lint`, all four unit suites and the production build are green after the removals; the exports that remain in `data/*.ts` and `domain/*.ts` and are not imported by the app are imported by tests or scripts, which is what they are for.

## 20AD.5 This report, read against the tree

The nineteenth edition was rendered page by page (446 pages, contact sheets, ink per page) and its current sections re-derived. **Fourteen pages carried under 3 % ink**, each a chapter tail or a single table row carried onto a fresh page before the next chapter's forced break — the class I-225 records; `build-report.mjs` now sets `orphans: 3; widows: 3` on paragraphs and list items, and this edition's count is in §20AD.6. Stale claims in sections the document itself labels *current*: §3, §4.1, §8.1, §8.3, §9.1, §11.1 and §14 said **3,867** words where the corpus has been 3,864 since the nineteenth pass and §2.2 already said so (PARTIALLY VERIFIED → corrected); §8.1 said 783 study sets (779); §8.4 said 30,282 dictionary headwords, a figure stale since the fifteenth pass's retirements took the manifest to 29,711 (CONTRADICTED by §2.2 → corrected, with the history); §8.3 and §16 quoted a 1,454 kB precache against a 1,500 kB budget and a 3,779 kB projection, none of which `bundle:budget` prints (1,277 kB of 1,600 kB; the forecast line is *corpus, whole at 10,000*, 925.5 kB) — replaced with the measured block; §5.2 said seven profiles and 143 renders where the gate has run eight profiles since the eighteenth pass; §11.3 said 38 editorial warnings (13); §19.1 said 237 morphology cases (242); §3's verdict said the artefacts were at versionCode 24 (25); and `docs:consistency` had been green throughout, because every one of those sentences sits outside the patterns it reads. The sections that name the delivery (1.0.4 / 25, not rebuilt), the native-review status (none) and the iOS status (not built) were VERIFIED. `docs:consistency` is unchanged in what it reads; the lesson §20X.9 drew — a checker reads what it was pointed at — is recorded here a third time rather than fixed by adding thirty patterns, because the honest fix is the one this section is: read the document.

## 20AD.6 What was run, what it said, and what could not be run

Every command below ran on the final product tree (`8e5a42cd`), step by step, with its exit code recorded; the report and issue files were edited afterwards and their own gates re-run last. `name:check` is the one step of `verify:quick` left out, for the reason the nineteenth pass gave: it fails on the gitignored `patent/` directory before and after this pass and on nothing in the tree.

Read the table with two keys. **0** is the exit code the step returned on the final tree. **1 †** marks the three steps whose red is expected of a web-only pass and is explained in its row: the end-to-end suite's two load timeouts (re-run green), and the two steps that compare the tree with the Android delivery that was, by the brief, not rebuilt. Steps that were red on their first run and green after the fix say so in the row, with what was fixed. Also run, outside the chain: `security:audit` over the delivered APK (14,764 entries, no findings, one accepted `http://localhost` constant of React Router's), the negative runs of §20AD.1–§20AD.3, and the two runs of `screens:audit:check` recorded in §20AD.1.

| Step | Exit | Result |
| --- | --- | --- |
| `route:policy:check` | 0 | every route has a back policy, in the order the router resolves them. |
| `mobile:identity:check` | 0 | every file agrees; the launcher label and the store identity are what they claim. |
| `version:check` | 0 | every file agrees with its own product; the number in Settings is the web release, and the native deliveries a |
| `ios:project:check` | 0 | green — exit 0, nothing to report |
| `splash:check` | 0 | every launch frame comes from the approved artwork; no mark, no words, one ground. |
| `locales:native:check` | 0 | every native declaration matches the shipped languages. |
| `content:fresh:check` | 0 | frequency self-test, vocabulary, corpus, level-test and dictionary artefacts all current |
| `content:inventory:check` | 0 | stale on the first run (four new interface keys); regenerated, green on the second |
| `policy:runtime:check` | 0 | green — exit 0, nothing to report |
| `content:safety:check` | 0 | no learner-facing surface, in any locale, carries prohibited content. |
| `i18n:check` | 0 | green — exit 0, nothing to report |
| `locale:content:check` | 0 | no language can produce a mixed-language question. |
| `locale:ledger:check` | 0 | green — exit 0, nothing to report |
| `copy:audit:check` | 0 | 0 error(s), 0 warning(s) for a human to read. |
| `copy:ledger:check` | 0 | every Korean string a learner can see has been read at its current wording. |
| `copy:remediation:check` | 0 | green — exit 0, nothing to report |
| `copy:generated:check` | 0 | 112 items · 20 lessons · practice, a retry and mastery |
| `copy:fresh:check` | 0 | every pack was written against the Korean sentence the corpus still carries. |
| `locale:editorial:check` | 0 | 0 error(s), 13 warning(s) for a person to read. |
| `letters:copy:check` | 0 | green — exit 0, nothing to report |
| `strokes:qa:check` | 0 | 73 items, 269 strokes |
| `strokes:corners:check` | 0 | every stroke starts and finishes where the letter does, and every doubled |
| `strokes:visual:check` | 0 | 73 items, 269 strokes, 1345 frames rendered at 256px |
| `strokes:markers:check` | 0 | every badge is on the sheet, clear of the ink, and readable at 152 px. |
| `glyphshape:qa:check` | 0 | every undeclared item matches the face; the guide, the mask and the demonstration are one geometry. |
| `glyph:structure:check` | 0 | every taught glyph is structurally sound: solid junctions, the pieces the |
| `letters:face:check` | 0 | Every letter has the face's parts, in the face's places. |
| `hints:qa:check` | 0 | every hint shown is both safe and useful. |
| `conjugation:qa:check` | 0 | every recorded surface form is reachable from the generated stem. |
| `conjugation:display:qa:check` | 0 | green — exit 0, nothing to report |
| `leveltest:qa:check` | 0 | 30 items, opening on the warm-up ladder and never stepping more than 3 levels. |
| `leveltest:bank:check` | 0 | 4,156 items, 2,276 distinct words, 30 levels |
| `leveltest:ambiguity:check` | 0 | every contextual item has one conjugated answer and three that are not it. |
| `leveltest:distractors:check` | 0 | no findings |
| `content:audit:check` | 0 | green — exit 0, nothing to report |
| `leveltest:policy:check` | 0 | every contextual level follows from its own sentence, the foundation band is |
| `leveltest:simulations:check` | 0 | green — exit 0, nothing to report |
| `leveltest:content:check` | 0 | 576 items, 12 in the foundation band |
| `questions:ledger:check` | 0 | every question has one answer, distinct options, and no frame collision. |
| `ambiguity:ledger:check` | 0 | green — exit 0, nothing to report |
| `leveltest:locale:check` | 0 | No answer option, in any language, resolved from anywhere but that language. |
| `dailyvocab:qa:check` | 0 | green — exit 0, nothing to report |
| `dailyplan:fresh:check` | 0 | no learner was shown the same plan twice, and nothing was re-taught as new. |
| `dailyplan:level:check` | 0 | every level is taught from its own band, and no day opens on a word from |
| `numbers:qa:check` | 0 | 112 items · 9 exercise kinds · 0 problem(s) |
| `numbers:domain:check` | 0 | no option stands beside another it cannot be compared with. |
| `numbers:copy:check` | 0 | every string that is about one of the two sets says which; none points at it |
| `numbers:ledger:check` | 0 | every question in the bank has been read at its current wording. |
| `quotes:qa:check` | 0 | ok   nothing about the quotation is persisted |
| `quotes:audit:check` | 0 | green — exit 0, nothing to report |
| `quotes:review:check` | 0 | green — exit 0, nothing to report |
| `strokes:fixtures:check` | 0 | green — exit 0, nothing to report |
| `vocabulary:qa:check` | 0 | green — exit 0, nothing to report |
| `romanization:qa:check` | 0 | 2 screens read the romanisation; 32 labels checked |
| `vocabulary:relations:qa` | 0 | green — exit 0, nothing to report |
| `vocabulary:sense:qa:check` | 0 | Every word has one taught sense in every complete language, and the pinned ones held. |
| `vocabulary:translation:check` | 0 | no card teaches less than the word it is for. |
| `translation:semantics:check` | 0 | every written row keeps the script, the mood and the polarity the Korean has. |
| `translation:audit:check` | 0 | green — exit 0, nothing to report |
| `content:scalability:check` | 0 | green — exit 0, nothing to report |
| `tokens:check` | 0 | green — exit 0, nothing to report |
| `lint` | 0 | green — exit 0, nothing to report |
| `typecheck` | 0 | green — exit 0, nothing to report |
| `test` | 0 | 3,206 cases (1,298 + 96 + 242 + 1,570). Inside each chain one or two jsdom cases timed out at 5 s under a load average of 12 from other sessions on this machine; the suite passed in full when run again at 17:00 |
| `build` | 0 | green — exit 0, nothing to report |
| `bundle:budget:check` | 0 | every budget met. |
| `routing:check` | 0 | 0 error(s). |
| `share:check` | 0 | 0 error(s). |
| `store:check` | 0 | red on the first run — the listings said 3,370 words; corrected to 3,864, green on the second |
| `curriculum:check` | 0 | red on the first run — one Japanese gloss from the nineteenth pass; rebuilt, green on the second |
| `fonts:audit` | 0 | 0 error(s), 0 warning(s) |
| `jamo:measure:check` | 0 | green — exit 0, nothing to report |
| `jamo:centering:check` | 0 | every glyph lands within 3% of centre once the shipped correction is applied. |
| `face:size:check` | 0 | every face reads at the same size, and takes the same room, on the page. |
| `status:qa:check` | 0 | red on the first run — the gate looked for digits in the *Start today* chip; it now seeds a learning day first; 120 combinations green |
| `modals:qa:check` | 0 | every action sits inside its own modal, at every width, in its longest language. |
| `screens:audit:check` | 0 | 17 routes and 7 states across 8 profiles = 171 renders; nothing clipped, overlapping or unreadable |
| `leveltest:viewport:check` | 0 | 30 combinations green |
| `numbers:layout:check` | 0 | green — exit 0, nothing to report |
| `mobile:icons:check` | 0 | green — exit 0, nothing to report |
| `vocabulary:qa:target` | 0 | INFORMATIONAL: 3,864 headwords — 6,136 short of the 10,000 target |
| `vocabulary:relations:check` | 0 | green — exit 0, nothing to report |
| `content:vocabulary:check` | 0 | green — exit 0, nothing to report |
| `content:dictionary:check` | 0 | green — exit 0, nothing to report |
| `content:corpus:check` | 0 | green — exit 0, nothing to report |
| `content:leveltest:check` | 0 | bank current; 2,212 items reachable in every non-English language |
| `answerability:check` | 0 | every question has exactly one option that answers it. |
| `exercise:inventory:check` | 0 | 548,852 checks, 32 locales |
| `synthetic:users:qa:check` | 0 | All journeys PASS. |
| `vocabulary:level:audit:check` | 0 | every level has a fortnight of new words, an example and an English meaning for every entry, |
| `back:coverage:check` | 0 | green — exit 0, nothing to report |
| `scroll:audit:check` | 0 | every screen's last action is reachable, clear of the navigation and the safe area. |
| `align:sections:check` | 0 | every heading, title and description in a group starts on the same rule, within a pixel. |
| `legal:isolation:check` | 0 | 16 renders |
| `dictionary:qa:check` | 0 | green — exit 0, nothing to report |
| `dictionary:coverage:check` | 0 | every taught word the dictionary holds is reachable from search |
| `dictionary:morphology:check` | 0 | every inflected form in the list reaches its dictionary entry. |
| `perf:dictionary:check` | 0 | within budget, phone-adjusted ×4 |
| `content:qa:check` | 0 | green — exit 0, nothing to report |
| `korean:education:check` | 0 | all composed gates pass; prints THIS DOES NOT PROVE NATIVE NATURALNESS |
| `examples:qa:check` | 0 | every example passes. |
| `worddetail:qa:check` | 0 | no card shows an example of a sense it does not teach. |
| `audio:qa` | 0 | the sampled run inside the chain reported two clips as not decoding (ffmpeg "Conversion failed!" under memory pressure); both decode, and the full run decoded all 15,948 |
| `audio:pronunciation:check` | 0 | 0 error(s), 0 warning(s) |
| `content:coverage:check` | 0 | red on the first run — three unobserved words without a reason; reasons written and checked against the corpora, green on the second |
| `issues:check` | 0 | green — exit 0, nothing to report |
| `vocabulary:level:qa:check` | 0 | every level is valid, populated and harder than the one below it, |
| `vocabulary:recommendation:qa:check` | 0 | every learner is taught from their own level, and no two ends of the scale meet. |
| `locale:practice:check` | 0 | 0 error(s). |
| `patent:evidence:check` | 0 | every path, command and figure the patent package cites is present. |
| `docs:consistency:check` | 0 | red inside the chain (a test cell at 3,202 and a dirty tree); green on the final tree |
| `strokes:measure:check` | 0 | green — exit 0, nothing to report |
| `quotes:render:check` | 0 | every card wraps inside its box, Korean first, Arabic right-to-left |
| `test:e2e` | 1 † | 640 cases, 638 passed in the 56-minute full run; two mobile cases (the verdict-panel width in `feedback.spec`, the typeface trace in `journey.spec`) timed out under the same load and passed on re-run alone with their spec files, 40 of 40 |
| `native:bundle:check` | 1 † | expected: it compares the web build with the delivered APK, which is four passes older and was not rebuilt — 2,417 files differ |
| `splash:bundle:check` | 0 | every launch frame comes from the approved artwork; no mark, no words, one ground. |
| `content:safety:bundle:check` | 0 | no packaged asset carries prohibited content. |
| `release:current` | 1 † | expected: the web is ahead of the native delivery by design, and the tree was dirty at run time (the four files of the store/coverage commit) |
| `gold:check` | 0 | 242 morphology cases + 11 gold-set cases + the pronunciation gold set |
| `audio:qa:full` | 0 | 15,948 clips decoded, 0 errors, 1 pre-existing loudness warning (못, female) |

**The end-to-end suite**: **640 cases** (320 × 2 projects): 638 passed in the 56-minute full run and two mobile cases timed out under a load average of 12 from other sessions sharing this machine — the verdict-panel width in `feedback.spec.ts`, whose launch screen had not cleared in 10 s, and the typeface trace in `journey.spec.ts`, which takes 40 s on an idle desktop project and hit the 60 s budget. Both passed on re-run alone with their spec files (40 of 40 mobile cases, 5.6 minutes), and both passed in the desktop project of the full run. The unit suites showed the same class inside the chains — one or two jsdom cases at 5 s — and passed 3,206 of 3,206 when run again on their own.

**iOS.** The brief asked for an iOS-only build using the repository's workspace, scheme, configuration and signing. It could not be run: this is a Linux machine (WSL), `xcodebuild`, `xcrun` and `pod` do not exist on it, and there is no macOS to run them on. What was run in its place is the whole of what the repository can settle about the iOS project from here — `ios:project:check` holds `project.pbxproj` to its adopted lock (no drift), `version:check` holds `MARKETING_VERSION` 1.0.4 and `CURRENT_PROJECT_VERSION` 25 in both configurations, `mobile:identity:check` holds the bundle identifier and display name, and `locales:native:check` the declared locales. Not run, and not claimed: `cap sync ios`, an archive, an IPA, a simulator, a device. `docs/DEPLOYMENT.md` and `result/BUILD_OR_SIGNING_BLOCKERS.md` say what a Mac would do. This is the one thing the brief asked for that this edition does not deliver, and it is a machine limit, not a repository one.

**Android.** Not built, not synced, not modified: the 218 tracked files under `apps/mobile/android` and `result/android-project` were fingerprinted before the pass and match byte for byte after it; `git diff 65c023af..HEAD -- apps/mobile result/android-project` is empty. The two package scans that read the delivered APK (`native:bundle:check`, `splash:bundle:check`, `security:audit`) read it and wrote nothing.

**This document.** `docs/report.pdf` was built with `docs:report`, 271 pages, and every page rendered and looked at; `report:pdf:inspect` reports 271 pages, 0 blank, 0 replacement characters, 0 markdown artefacts, and 18 pages under 3 % ink — chapter tails and one table row carried over, the class I-225 records, down from 21 without the orphan rule. The nineteenth edition's 446-page file, for the record, had been built with the generic `report:pdf` builder (its `Title` read *Product Truth Report*); this one is built with `docs:report`, which is the delivered document. The commits and the push are in §20AD.7.

## 20AD.7 The commits

Product commits, each on `main`, in order — the tree was verified at the last of them (§20AD.6) and the document commit that follows carries this edition, the regenerated ledgers and the evidence documents the gates rewrote:

| Commit | What |
| --- | --- |
| `7d929b84` | fix(web): a screen whose chunk fails to load shows a way out instead of a blank page — I-249 |
| `82d75071` | fix(web): a launch that cannot read the stored profile no longer writes over it — I-250 |
| `bc766742` | fix(i18n): Czech says písmeno, Greek and Hungarian keep one register, and four single-string slips — I-251 |
| `96208944` | chore: remove dead code, an unused dependency and stale files, each proved unreferenced — I-252 |
| `8e5a42cd` | fix(content,qa): store copy says 3,864 words, three unobserved words get their reason, and two gates catch up with the tree |
| *(this document)* | docs(report): the twentieth pass — report, issues, ledgers, evidence documents and `docs/report.pdf` |
...
| --- | --- | --- | --- | --- | --- |
| **I-04** | Vocabulary | **P1** | 3,864 of a stated 10,000 words | Buyers compare corpus size | **OPEN** |
...
| **I-222** | Home | **P2** | 21 quotation translation rows and one attribution need a person | Nine Quote 1 rows and twelve Quote 2 rows are grammatically and semantically sound as far as the reviewer can establish and are marked NATIVE-SPEAKER REVIEW REQUIRED for register and idiom (vi, th, ar, bn, fil, hi, kk, ky, mn, ta, te, uz); the Carlyle attribution is conventional, not located in his works. | **OPEN** |
| **I-246** | Localization | **P2** | The exercise inventory's gloss-overlap heuristic flags 30,736 answer/distractor pairs across 32 locales for a human reader, and no locale is native-reviewed | Most hits are look-alike pairs by design ("here"/"there"); the pairs the runtime now refuses are the confirmed class. The remaining hits are a reading list, not a defect list. | **OPEN** |
...
| **I-230** | Localization | **P2** | A 32-locale automated translation audit and a 90-row model reading: three wrong rows found and fixed, and no row is native-reviewed | A Tamil learner was shown துலங்கு (to shine) for 반응하다 (to react); a Mongolian learner an adjective that does not mean sweet under 불고기; a Brazilian learner the European Portuguese *a miúdo* under 비일비재. | **PARTIAL** |
...
| **I-79** | Vocabulary data | **P2** | A learner at the top of the scale runs out of new words in about six weeks — and the thinnest zone is no longer the top | Levels 28–30 hold 436 words between them. A learner placed at 30 draws from that zone and, at the default ten new words a day, exhausts it in 43 days if every word is new (87 days with the plan's reserved-new share). The thinnest zone is now 24-26 with 191 words: 19 days at ten a day. | **PARTIAL** |
| **I-20** | Vocabulary | **P3** | The hand-written More about it block is on 99 words of 3,370 | Word Detail is no longer a short page followed by nothing, but the paragraph written for the words where one line genuinely is not enough is on 99 of them — 3% of the corpus. | **PARTIAL** |
...
| **I-131** | App shell | **P1** | The document could scroll, carrying the bottom tab bar off its anchor | On My Learning the page kept moving past its content, the tab bar rode up with it, and a band of blank ground appeared underneath. The same offset carried into Privacy, which is why that page was reported as intermittently corrupted near the bottom. | **RESOLVED** |
| **I-134** | Handwriting · glyph geometry | **P1** | ㅌ, ㄷ, ㄹ and ㄸ had a notch bitten out of their upper-left corner | The tracing guide, the grading mask and the demonstration for ㅌ all showed a square chip missing from the corner of the letter, on the screen where a learner is being shown what the letter looks like. Eight of the seventy-three taught items carried it: ㄷ, ㄸ, ㅌ, ㄹ and the blocks 다, 라, 말, 글. Photographed by QA. | **RESOLVED** |
| **I-135** | Handwriting · glyph geometry | **P1** | 꽃’s ㄲ was bridged through the middle by the vowel’s stem | The two ㄱ of 꽃’s tense consonant read as one shape with a spur between them, so the letter a learner traced was not the letter 꽃. Photographed by QA. | **RESOLVED** |
| **I-136** | Vocabulary levels | **P1** | A learner measured at Level 30 was taught 그래서 | A learner who sat the Level Test and came out at 30 opened Today’s Vocabulary and found a Level 2 conjunction as the first word of their day. The level system looked ignored on the one screen where it is supposed to show. | **RESOLVED** |
| **I-137** | Today's Vocabulary | **P1** | A correct answer could be credited to nothing, and the counter did not move | "Sometimes you finish a word and the counter does not move." The learner answers correctly, the screen advances, and the day’s number stays where it was — with no error and no retry. | **RESOLVED** |
| **I-143** | Release engineering | **P1** | Four release builds in a row packaged a four-hour-old web bundle | A release build that succeeds and delivers something older is the defect this product has shipped more times than any other. This is the same failure through a route `release:current` cannot see: the APK recorded the right commit, passed every gate, and contained the web app from whenever somebody last ran `cap sync`. | **RESOLVED** |
| **I-144** | Handwriting | **P1** | Twenty-one numbered stroke badges were drawn with a slice missing | On the screen whose whole job is to show a learner where the pen goes, ㅌ's third number was cut off flat down its left-hand side and ㄷ's second lost ring on two sides at once. Nineteen more badges over eighteen other characters were in the same state — every basic vowel with a stem, ㅊ, ㅋ, ㅌ, ㅎ, and five of the compound vowels. | **RESOLVED** |
| **I-145** | Numbers | **P1** | Every Numbers lesson row reserved 32 px for an icon most rows do not draw | A lesson title started 49 px from the edge of the phone while the module heading above it started at 20 and the summary card above that at 32 — three left edges on one screen, and the one the eye follows was the furthest in. On a 320 px phone the row had 256 px inside its padding, the empty column and the chevron took 48 of them, and a three-word title wrapped to three lines beside a quarter-row of nothing. The status pill would not shrink either, so on a narrow phone it took its width out of the title rather than moving. | **RESOLVED** |
| **I-146** | Accessibility | **P1** | A learner who cannot hear could not finish a single Numbers lesson | Not a harder route through the Numbers course — none. Passing a mastery check is what completes a lesson, a mastery check asks every item, and a listening question's whole stimulus is a clip: the prompt carries an audio id and deliberately no text, because printing the word would print the answer. | **RESOLVED** |
| **I-158** | Numbers | **P1** | Three questions whose options could not be compared with each other | A learner could answer three of the course's questions correctly without reading any Korean: the option lists gave the answer away by shape. | **RESOLVED** |
| **I-161** | Vocabulary Level Test | **P1** | The first question of every learner's first test was at level 14 of 30 | A beginner opened a placement test and was asked a word from the middle of the scale, then another six levels away from it. Learners reported the test as excessively difficult, and the measurement of *why* was worse than the report: 42.8% of the first five questions put to a simulated level-2 learner were more than six levels above them, and a learner who answered everything wrongly was asked level 1 twenty-five times running. | **RESOLVED** |
| **I-162** | Vocabulary Level Test | **P1** | A level-1 question asked what 누가 means and marked "nougat" correct | A beginner who correctly knew that 누가 means *who* was told they were wrong, on an early question of the test that decides what they are taught. Sixty-nine such items sat at levels 1–5. | **RESOLVED** |
| **I-163** | Vocabulary Level Test | **P1** | Fifteen questions offered the same answer twice, in languages nothing read them in | An Uzbek learner asked what 정확히 means was offered *aniq* twice — the correct answer and a distractor, identical on screen. A Turkish learner asked about 좋다 was offered *iyi olmak* twice. The question had no defensible answer and no way to pass it. | **RESOLVED** |
...
| **I-178** | Vocabulary Level Test | **P1** | 밥을 먹고 ____을 드세요 accepted two of its four options | A learner at level 4 met a question with two right answers and was marked wrong for choosing one of them. 물 and 약 are both ordinary completions — you drink water after a meal and you take medicine after a meal — and the item is invalid whatever the answer key says. It was found by a reader looking at the screen, not by any gate. | **RESOLVED** |
| **I-180** | Vocabulary Level Test | **P1** | Nearly a quarter of the questions were harder than the test's own estimate of the learner | The test was reported as too hard, and as jumping to a much higher level after a single correct answer. Both are measurable rather than impressions. | **RESOLVED** |
| **I-182** | Content QA | **P1** | A pairwise option guard was conditioned on both words being classified, so an unclassified answer disabled it | Indirect but wide: it is the reason I-178 shipped. The rule reads `if (answerClasses && otherClasses && classesCompete(...))`, so a noun with no entry in `noun-classes.json` silently turned the guard off for every question built on it. | **RESOLVED** |
| **I-185** | Educational content | **P1** | A gap-fill whose sentence said only *when* — 일곱 시에 ____ took both 일어나요 and 연습해요 | A learner reading Korean correctly is marked wrong. The item was reachable in the Vocabulary Level Test at level 3 and in Today's Vocabulary, so it was on the path of every learner who has just finished the alphabet. | **RESOLVED** |
| **I-186** | Educational content | **P1** | ____이 문을 열었어요 offered two person nouns beside 은행, so three of four options were correct | Same as I-185, at level 7. | **RESOLVED** |
| **I-188** | Educational content | **P1** | Sixteen sentences in the daily queue were keyed two different ways | `불을 ____ 주세요` asked for 켜 in one item and 꺼 in another; `소리를 ____ 주세요` for 줄여 and 낮춰; `둘에 셋을 ____` for 더해요 and 곱해요. Today's Vocabulary can serve both on one morning, and a learner who has met one is being taught the opposite by the other. | **RESOLVED** |
| **I-189** | Release engineering | **P1** | The question ledger validated 336 of 368 Numbers questions against the wrong answer | None to a learner — the content was right. The gate that certifies it was not measuring it, and the report cited that gate. | **RESOLVED** |
...
| **I-190** | Localization | **P1** | Twenty-four locale packs went on translating example sentences that had been replaced | A learner reading Hungarian would have seen *I get up at seven* under a Korean sentence about an alarm and a bed. | **RESOLVED** |
| **I-196** | Vocabulary Level Test | **P1** | A gap-fill's difficulty was the word taken out of it, and nothing else — 629 of 629 items | A learner who had recently finished the alphabet was shown `물을 안 줘서 화분의 꽃이 ____.` as **question four**: two clauses, a negation, a causal connective, an inference, and 화분 — a level-28 word. The item was level 7. Reported by a tester. | **RESOLVED** |
| **I-197** | Vocabulary Level Test | **P1** | Difficulty climbed two levels a question whatever the learner answered | A learner who answered *I don't know* to a level-4 word was shown level 6 next. A learner who answered the three opening questions correctly met a level-8 sentence at question four. | **RESOLVED** |
| **I-198** | Vocabulary Level Test | **P1** | The second question of every sitting was a sentence | A learner still becoming comfortable reading Hangul met `밥 먹기 ____에 손을 씻어요` — a nominaliser and four eojeol — as question two, before anything was known about whether they could read a sentence at all. | **RESOLVED** |
| **I-203** | Letters | **P1** | A returning learner was put back on a letter they had finished, whenever the profile lost a race with the router | Reopening a lesson resumes at the first unfinished letter — the whole point of §48. On a machine slow enough that the stored profile had not been read by the time the route rendered, it resumed at letter one instead, and the learner was asked to redo work they had already done. The progress itself was never lost: the record said `learned` throughout. What was wrong was the screen's reading of it. | **RESOLVED** |
| **I-205** | Level Test | **P1** | A distractor rule that forbade the answer's own subject area was selecting for absurdity | A reader photographed `창문으로 아침 ____이 들어와요` keyed 빛 and offering 목적, 비빔밥 and 환경. A purpose, a bibimbap and an environment do not come in through a window: the item is answerable by elimination without knowing 빛, and a learner who does know it learns nothing from being right. Measured across the shipped bank, all 625 contextual items had every distractor from a different category than the answer, so this was the whole bank rather than one row. | **RESOLVED** |
| **I-206** | Vocabulary | **P1** | 292 words took their browse category from a regex match inside a longer word | The category is the drawer the browse screen files a word in. A learner opening Body & Health met 화가 (a painter), 생일 (a birthday) and 베개 (a pillow); Animals & Nature held 창문 (window) and 샌드위치 (a sandwich). | **RESOLVED** |
| **I-207** | Level Test | **P1** | Two frame guards had never executed, one of them the fix recorded for I-185 | None directly — the items they were written to stop were being kept out by an unrelated rule. The impact is on this document: §20V.1 recorded a fix that had never run. | **RESOLVED** |
| **I-208** | Review | **P1** | The new frame rules took the beginner band's gap-fill exercise with them | Of the top thirty words by teaching priority, one still had a gap-fill. Today's Vocabulary and Review stopped offering the `context` exercise to new learners almost entirely — the people who need varied practice most. | **RESOLVED** |
| **I-211** | Content safety | **P1** | Sixteen taught words and 854 dictionary headwords carried violence, weapons, death, intoxication, insult or political content the policy excludes | Cards, daily plans, review questions and Level Test items could teach 살인, 무기, 총알, 미사일, 전쟁, 죽이다, 취하다 (I got drunk), 벌거벗다 (the child runs about bare), 바보, 협박하다 and others to a beginner; the dictionary returned 강간, 매춘, 마약, 자살, 도박 and party and ideology names to a search. | **RESOLVED** |
| **I-212** | Report | **P1** | The report certified content safety on a gate that read one list on one surface | None to a learner directly. §19.1, §19.4, §20.1 and §20H.2 presented `content:safety:qa` — 66 whole-headword terms over taught words, produce options and gap-fills, with the dictionary declared out of scope — as the learner-safety gate, and §23 declared everything the repository can settle settled. A reviewer reading those sections would have concluded the placement test was safe for children. | **RESOLVED** |
| **I-214** | Release | **P1** | The tree carried three marketing versions and none of them was the intended 1.0.4 | A customer quoting the version from the Legal screen (1.0.6), the store (nothing yet uploaded) and an iOS build (1.0.3) would have named three different products. The reported forms `v.1.0.6` and `v1.0.6` came from per-locale "Version {{version}}" strings and the drifted number behind them. | **RESOLVED** |
| **I-215** | Splash | **P1** | A cold start on Android 12+ showed an orange brand mark the app draws nowhere else, then the approved splash, then the app | A customer photographed it and called the orange the old logo. It was `mipmap/splash_icon`, cut from `brand/logo-symbol.png` for the system splash on the argument that it was "the splash's own mark"; the approved splash artwork is a wordmark on a soft wash and carries no mark, so the system frame was a second picture in front of the first. | **RESOLVED** |
| **I-216** | Today's Words | **P1** | Show answer printed a sentence restating an option and left the options live, so the tap that followed was recorded as a pass | 정답은 어떤 일이나 행동을 이루다예요 appeared under four still-tappable options; the learner tapped what they had just been told and the word was completed, credited to the day's counter and stepped up the mastery ladder at hint level 3. A learner who pressed through to the reveal on every question finished 10/10 having recalled nothing. | **RESOLVED** |
| **I-217** | Layout | **P1** | Four gap-fill chips rendered as three and one | On a 360 px phone the fourth candidate fell onto its own line under three; the eye reads three choices and something else. Photographed by a customer. | **RESOLVED** |
| **I-218** | Today's Words | **P1** | The word-ordering screen did not say what to do, and the last tile could not be taken back | A definition, three identical dashed boxes and five tiles; the instruction named the goal (단어를 맞춰 보세요), not the action. And the tray graded itself the instant the last tile landed, so a learner who saw their mistake as their finger lifted had already been marked wrong. | **RESOLVED** |
| **I-219** | Today's Words | **P1** | The matching grid did not say the first action, named neither column, and graded each pair on the tap | 단어와 뜻을 짝지어 보세요 named the goal; the caption 한국어 단어를 골라요 was a fragment disconnected from the grid; nothing said which side to tap first; a wrong pair flashed and cleared, giving the answer away by elimination and punishing the first guess. | **RESOLVED** |
| **I-223** | Release engineering | **P1** | Nine QA scripts and specs claimed 150% and 200% text measurements that measured 100% | The report said the Numbers list, the tab bar, the home header, the reachable-actions audit, the section alignment and the legal pages were measured at doubled text. They were not: the emulation set the root font size, every type size in the product is a pixel token and four stylesheets use rem at all, so the body text stayed at 15 px in every "×2" case. I-176 (the tab bar below 44 px at 200%) was measured the same way. | **RESOLVED** |
| **I-228** | Child safety | **P1** | The runtime safety gate read the learner's language in English only | The 24 kB runtime subset of the child-safe policy carried the language-neutral rules and the English lexicon; a level-test item whose meaning was refused by the full policy in, say, Thai passed the runtime gate because the runtime never read Thai. | **RESOLVED** |
| **I-232** | Release engineering | **P1** | The git object store held 1,092 zero-byte objects from a VM crash mid-commit; repaired without touching the working tree | None for a learner. For the delivery: until the repair, the eighteenth pass's content was on disk and verified but could not be committed or pushed. | **RESOLVED** |
| **I-234** | Child safety | **P1** | Ten evasion shapes passed the child-safe policy: split-once Korean terms, ideographic spaces, Cyrillic look-alikes, particle-attached foreign terms, parenthesis masks, Han in Korean fields | A string such as 섹 스하다, 마 약을, sеx (Cyrillic е), sex를 or 殺人 in a Korean field would have reached a learner if it had ever been authored or imported, because the normaliser joined only bare syllables, read only Latin as a foreign script, and treated a Hangul particle as part of the foreign word. | **RESOLVED** |
| **I-235** | Streak | **P1** | A streak day was any day with recorded activity, so twenty seconds on a session screen with nothing answered was a learning day; Numbers answers did not count until a lesson completed; a never-active learner read "0 days" beside a flame | A learner who opened Today's Words and closed it read "1 day" on Home; one who answered ten Numbers questions read nothing; a new learner saw an active zero-day streak. Home also rendered the chip before the profile had loaded, briefly showing 0 days and Lv. 1 to a learner on a long run. | **RESOLVED** |
| **I-236** | Progress | **P1** | An item reaching learned through a checked attempt was never credited to the day: the completion flag was noted inside a React updater and read after it, and React had not run the updater yet | The Activity screen's "Letters finished" and "Words finished" for a day, and "finished this week", stayed at 0 for every item learned through a writing or review attempt. | **RESOLVED** |
| **I-237** | Letters | **P1** | The letter-sound question for ㅢ could draw ㅣ and 이 — both "i" — or ㅡ and 으 — both "eu" — as two of its four buttons | About one seed in twenty-five, the learner was shown two identical answers with one of them marked wrong. | **RESOLVED** |
| **I-238** | Vocabulary | **P1** | Reading and production questions could offer a near-synonym gloss as a wrong answer: 잠깐 "a moment" beside 잠시 "a moment, short time", 사용하다 "to use" beside 이용하다 "to make use of", 좋아지다 beside 나아지다, 최소한 beside 적어도 — 220 such pairs in English, more in the other packs | A learner who knew the word was marked wrong for a defensible answer. | **RESOLVED** |
| **I-239** | Vocabulary | **P1** | A matching grid could show one meaning on two rows in 31 languages — 아빠 and 아버지 are both "baba" in Turkish, 좋다 and 괜찮다 both "iyi olmak", 세상 and 세계 both "العالم" — and mark one of the two wrong by which id was tapped | Every pair of same-level words sharing a gloss in a pack could land in one day's grid; 896 such pairs across the packs, none in English, which is why the English reading of the grid never met one. | **RESOLVED** |
| **I-240** | Korean content | **P1** | Seven ㅀ-stem verbs (싫다, 잃다, 끓다, 옳다, 뚫다, 앓다, 꿇다) carried a pronunciation note naming 된소리되기 for a change that is aspiration; 옮기다 and 굶주리다 named tensing where none occurs; 담요 and 식용유 were transcribed [다묘] and [시굥유] against 표준 발음법 §29; 큰일 and 별일 were filed under liaison; 솔직히 was [솔지키] | The word card under 싫다 → 실타 read "된소리로 바뀌어요": a false rule on the screen whose job is the rule. | **RESOLVED** |
| **I-241** | Conjugation | **P1** | The conjugation panel printed 먹으세요, 마시세요, 자세요, 있으세요 and 말하세요 under "Please do", where Korean uses the honorific verb: 드세요, 주무세요, 계세요, 말씀하세요 | The card taught the form a Korean teacher corrects in a learner's first month. | **RESOLVED** |
| **I-242** | Level Test | **P1** | Sixty-four gap-fills had a second defensible answer among their options — 의사가 환자를 보호했어요, 결과를 찾아봐 주세요, 대사/사위가 한국에 왔어요, 생일에 냄비를 받았어요, 저녁에 소설을 봐요 — found by reading every gap-fill with all four options substituted | A learner picking a sentence Korean supports was marked wrong in Today's Vocabulary, Review and the Level Test. | **RESOLVED** |
| **I-248** | Review | **P1** | The review session resolved its plan on mount and froze it, and the router mounts before the profile has been read — so a refresh or deep link into /review/session on a slow device read "Nothing to review right now" over words that were due | A learner who refreshed the review screen, resumed the app on it, or reached it before IndexedDB had answered was told nothing was due and sent back; the words were still there on the next visit. Invisible on a fast device, which is how it shipped; the word session (`wordSessionHydration.test.tsx`) and the Numbers order (I-204) had the same class of defect and were fixed earlier in v1.0.5; this is the third instance. | **RESOLVED** |
| **I-249** | Reliability | **P1** | A screen whose lazy chunk failed to load unmounted the whole app: a blank cream page with no header, no navigation and nothing to tap | Every screen but Home is a lazily imported chunk, and a chunk is a network request the first time. A learner who opened Letters, Words or Review before the worker had precached it and after the connection dropped — or who kept a tab open across a deploy that renamed every hashed file — got a rejected import, and with no error boundary React unmounted the root. The page went blank with no way back but the browser's own reload; in the native shell there is no such button. | **RESOLVED** |
| **I-250** | Persistence | **P1** | A launch that failed to read the stored profile kept every repository wired to the store and then wrote the fresh in-memory profile over it on the first answered question — while Settings said “saving” | Hydration reads eight collections, runs the migrations and parses every row; one unreadable record or a migration that throws lands in the failure path, which (correctly) gives the learner a working app on a fresh profile. But the repositories had already been pointed at the store, and every write goes through them with whatever the state holds. The first answered question credited the day and saved default settings — level, streak days, daily plan, placement result, content seed — over the learner's stored row; the first letter written saved a fresh progress row over a learned one. A transient failure to read became a permanent loss, and the storage probe reported the browser healthy throughout. | **RESOLVED** |
...
| **I-129** | Numbers | **P2** | Correct answers were answered by a sentence that repeated the question | A learner who tapped 4 under 사 and read 맞았어요 was then told 사는 4예요. Every numeral in the course did it, in all 32 languages. | **RESOLVED** |
| **I-130** | Numbers | **P2** | A misconception class was shown to learners as a raw key | A learner who chose the wrong number set in the *which numbers go here?* question saw the string `rationale.wrong_system_context`. | **RESOLVED** |
| **I-133** | Vocabulary levels | **P2** | The days of the week are level 9, and 85 other maximally useful words sit above level 6 | An editor marks every word 1 to 5 for how soon a learner needs it, and 363 words carry a 1 — the highest. Eighty-five of them are levelled above 6: 월요일, 화요일, 수요일, 목요일, 일요일 and 요일 itself at 9, 한국 at 10, 덥다 at 12, 비싸다 at 15, 시원하다 and 심심하다 at 13, 깨끗하다 and 조용하다 and 약국 at 10. A beginner course that does not teach the days of the week until level 9 is not teaching them when they are needed, and a learner placed at 3 will not meet 덥다 for months. | **RESOLVED** |
| **I-138** | Navigation | **P2** | Back meant something different on every screen | The header chevron and the phone’s Back button ran different code on the same screen. Switching between bottom tabs built a history chain, so Words → Letters → Review took four presses to reach the exit confirmation, and a lesson opened from a link went Home rather than to its lesson list. | **RESOLVED** |
| **I-140** | Numbers | **P2** | A wrong answer was told it was "a little different", and then told the answer twice more | The verdict on a wrong answer was 조금 달라요 — *it’s a little different* — which softens a judgement the screen has already delivered in red, and leaves a learner working out whether it counted. Under it sat 정답은 8, restating what the red cross and the blue tick already showed. | **RESOLVED** |
| **I-141** | Numbers | **P2** | The lesson on numbers past ten was a paragraph with hyphenated pseudo-spellings in it | The explanation opened with three worked examples, three arabic numerals and three hyphenated forms — 십일은 십-일, 11 — in one block of prose, in a course for people who cannot yet read the alphabet fluently. 십-일 is not how the word is written, spoken or spelled: a diagram drawn in punctuation that a beginner cannot tell from the word itself. | **RESOLVED** |
| **I-147** | i18n copy | **P2** | 224 translated strings that no screen in the product can draw | Seven keys times thirty-two languages, written, reviewed, shipped and unreachable — and one of them, *2시 15분*, repeated the gloss printed directly above it. A thirty-third string, `rationale.adjacent`, was in every bundle for a feedback path deliberately set to `null`. | **RESOLVED** |
| **I-148** | Korean | **P2** | 금요일 was glossed as 한 주의 다섯째 날, and it is the correct answer | Friday is the fifth day counting from Monday and the sixth counting from Sunday, which is how a Korean calendar prints the week. A learner who counted found the *right* answer wrong. 월요일 carried the same fault one place along, glossed 한 주의 첫날. | **RESOLVED** |
| **I-150** | Numbers | **P2** | The lesson list drew a completion tick beside Review due | *Review due* is the one status on that screen asking the learner to act, and it was the only one with a tick next to it saying the opposite. | **RESOLVED** |
| **I-151** | Numbers | **P2** | A lesson's status read the visit rather than the evidence | A learner who had read two explanation steps and been interrupted could come back to a row with no badge on it, reading *you have not started this*. | **RESOLVED** |
| **I-152** | Release engineering | **P2** | The delivery's own paperwork disagreed about which commit built it | `result/` is what a buyer is handed. Two files inside it named two different commits as the source of the same binary, and one of them recorded that the tree was dirty when the build ran — so neither answered the question the folder exists to answer. | **RESOLVED** |
| **I-154** | Release engineering | **P2** | The report's own metadata, and the cover of the PDF, described a build that does not exist | §2.1 — the first table a reviewer reads — said **1.0.2** with an Android version code of 3 and an iOS `CURRENT_PROJECT_VERSION` of 3, through two releases that shipped 1.0.3 at a code of 11. It also named commit `3833da71` and called the working tree *not clean*. The **cover of the PDF** said v1.0.2 as well, in three independent places: a literal in `build-pdf.mjs`, the subtitle argument in `package.json`, and the report's own front matter. A reader trusting any of them would have gone looking for an artefact nobody built. | **RESOLVED** |
| **I-159** | Release tooling | **P2** | `generate_audio.py --only` replaced the manifest the app loads | None shipped. A build made straight after a pronunciation check would have started with three recordings in its index and 6,861 on disk unreferenced — an app with almost no audio, and nothing failing to say so. | **RESOLVED** |
| **I-160** | Release tooling | **P2** | The app-icon gate could not tell the two platform sources apart | None shipped. Android and iOS could have swapped artwork with every gate green. | **RESOLVED** |
| **I-164** | Vocabulary Level Test | **P2** | "I don't know" was described everywhere as better evidence than a wrong guess, and was worth exactly the same | None visible, and the product was telling learners something false about what their honesty bought them. The screen offers the button on every question and the documentation said the model weighed it as cleaner evidence. | **RESOLVED** |
| **I-165** | Persistence | **P2** | An interrupted level test lost every answer, and the screen warned about it instead of fixing it | Eleven questions into an eight-minute assessment, a phone call cost eleven questions. The screen carried a leave guard whose whole job was to warn the learner that leaving would throw the sitting away. | **RESOLVED** |
| **I-166** | Release tooling | **P2** | `verify:release` could not pass at the commit the previous release was cut from | None shipped. It means the previous release's own claim to have passed its verification could not have been re-derived from the tree it names. | **RESOLVED** |
| **I-168** | Report | **P2** | The report carried three disagreeing sets of artefact figures, and none was the delivered one | None shipped. It is a defect in the document that exists to be trusted about what shipped, which is the one place a contradiction costs the most. | **RESOLVED** |
| **I-169** | Vocabulary Level Test | **P2** | Six grammatical forms were still asked as vocabulary above the floor, and 마리화나 was on no list | A learner at level 15 was asked what 다가 means and the correct answer was "multivalent". 다가 is the connective in 가다가 멈췄어요 — it is not a word with a meaning to choose. | **RESOLVED** |
| **I-173** | Vocabulary Level Test | **P2** | The warm-up ladder ran 2, 4, 6 even for a learner who had just said they did not know | A learner who declined the level-2 word was shown level 4 next, then level 6. The gentlest opening in the product was the one place a struggling learner could not escape. | **RESOLVED** |
| **I-174** | Vocabulary Level Test | **P2** | Above the beginner floor, difficulty was still frequency alone — and rank is shared between senses | 시기 sat at level 11 glossed "opportunity" though its everyday sense is *a period of time*; 화상 at 11 as "portrait" though its everyday senses are *a burn* and *video*; 전력 at 11 as "electricity"; 각하 at 12 as an honorific a learner will not meet. | **RESOLVED** |
| **I-175** | Vocabulary Level Test | **P2** | The cross-locale duplicate-answer check skipped every dictionary headword | `데모` shipped offering *demo · personality · feedback · a personality* — two options that are the same answer. | **RESOLVED** |
| **I-179** | Today's Vocabulary | **P2** | A second x/y counter on the session screen was read as the day's progress | The screen carried two `x / y` counters a few pixels apart measuring different things: the header counts words finished today, and the line under the word counted the screen's position within its word — 새 단어 · 1 / 2. It was reported as "the app says there are two new words today" by a learner who then met a third. | **RESOLVED** |
| **I-181** | Release engineering | **P2** | A negative test had silently stopped applying its own sabotage | None directly. It is a gate about gates: G4 in the negative suite asserts that `numbers:qa` catches 둘 개, the plain numeral in front of a counter. Its sabotage quoted a whole `n(...)` call, a `domain:` field was later added to that call, and the quoted text stopped matching — so the case had been asserting nothing. | **RESOLVED** |
| **I-183** | Persistence | **P2** | The upgrade fixture did not assert Numbers-course progress | A learner who had finished Numbers lessons had no test standing between them and a migration that dropped those rows. | **RESOLVED** |
| **I-184** | Release engineering | **P2** | An end-to-end assertion read the answer options and reported a vocabulary gloss as feedback | None. The screen was right; the test was wrong, and would have blocked a correct release. | **RESOLVED** |
| **I-187** | Educational content | **P2** | ____이 마음에 들어요 — a frame that excludes no noun | Two items, at levels 3 and 14. A road, a present, a bus stop: a learner can like any of them. | **RESOLVED** |
| **I-191** | Localization | **P2** | i18n:check passed a key that was present and empty | i18next renders an empty string rather than falling back, so the screen shows a blank button and no gate says so. | **RESOLVED** |
| **I-193** | Release engineering | **P2** | The technical disclosure's evidence table contradicted its own mechanism section | None to a learner. The document is prepared for counsel and two of its numbers disagreed by four points. | **RESOLVED** |
| **I-195** | Persistence | **P2** | A resumed sitting silently dropped an answered question whose item had left the bank | None from this release — it removed no item ids, the bank grew from 4,121 to 4,151 by addition. The defect is one content change away at any time, and when it fires the learner answers twelve questions and is scored on eleven with nothing on screen saying so. | **RESOLVED** |
| **I-199** | Release engineering | **P2** | The gate for a too-hard opening used a threshold six levels wide | None directly. It is why the reported defect shipped: the gate over it was green and correct by its own rule. | **RESOLVED** |
| **I-200** | Vocabulary Level Test | **P2** | The screen's pool fallback could cross a band the learner had not earned | Band 1 holds three contextual items, so the fallback fired on the first sentence of nearly every beginner sitting and served a band-2 one. | **RESOLVED** |
| **I-202** | Verification | **P2** | A release e2e case passed by beating hydration, and had been asserting nothing for a week | None directly. The cost is that a gate in `npm run test:e2e` reported green while asserting nothing, and the property it was written to protect — that the settings screen speaks a learner's language and not the storage engine's — was unguarded. | **RESOLVED** |
| **I-209** | Report | **P2** | Nine figures in this document disagreed with the tree, including the sentence I-168 was closed on | None to a learner. The document is handed to a reviewer as the authoritative description of the product, and a reviewer who checks one figure and finds it wrong is right to disbelieve the rest. | **RESOLVED** |
...
| **I-220** | Copy | **P2** | 이번 주에 32개를 첫 번에 떠올렸어요 — unidiomatic, and every language misdescribed the metric | 첫 번에 is not Korean anyone says, and "on the first try" was not what the number counted: `weeklyInsights` counts attempts in the last seven days that passed with no hint, and an attempt row carries no first-try flag. Twelve other Korean strings were fragments or odd collocations (복습할 때 as a status label, 오늘 몫은 끝났어요, 세는 말 앞은 띄어쓰기). | **RESOLVED** |
| **I-221** | Home | **P2** | Two requested quotations added, in 32 languages, with an honest exception to the attribution policy | 꿈을 크게 가져라. 깨져도 그 조각이 크다 (작자 미상) and the Carlyle stepping-stone line (Thomas Carlyle) now appear at the foot of Home. The first ships as authorship unknown with a localized "author unknown" byline — the fact stated, not a name invented; the second is marked attributed rather than established, because the English line is quoted under Carlyle's name everywhere and has not been located in his works. A Korean original leads and its translation follows; Korean shows the line once. | **RESOLVED** |
| **I-224** | Content safety | **P2** | The safety scan read 27 of the quotation library's 1,052 strings and reported zero findings | The quotations family pulled string literals out of `quotes.ts` with a regular expression; a backtick in a doc comment opened a template-literal match that swallowed most of the file, so the family reported "27 items, 0 findings" over a library it had not read, and tagged every non-Korean line as English. | **RESOLVED** |
| **I-226** | Report | **P2** | I-26's record said all Android launch bitmaps were wordless; ten of the twenty-two carry the Korean wordmark and twelve the English | A reader of this document was told the pre-Android-12 launch bitmap says nothing in any language. It has said *Han gyul* (default) and 한귤 (`drawable-ko`) since the commit "Stop the launcher icon being the first thing a cold start shows", by design, following the device locale; the iOS launch image is the wordless one. The capacitor config comment repeated the wordless claim. | **RESOLVED** |
| **I-227** | Report | **P2** | The seventeenth edition carried three word counts, two pack counts and a verdict that contradicted its own ledger | A buyer reading the report met 3,370 (§2.2), 3,393 (§1, §3, §8, §9, §11, §14) and 3,333 (§20) as the current corpus size, *20 complete + 12 at 600* beside *32 complete*, gap-fills at 1–5 as both 36 and 53, tests as both 1,512/811/2,689 and 1,523/833, and I-126 listed as needing data that is not open while the ledger marked it RESOLVED. | **RESOLVED** |
| **I-229** | Vocabulary levels | **P2** | The level model had one frequency source and an editorial usefulness mark per batch, and 1,031 words sat too high | 맛 at 23, 미리 at 24, 딱히 at 30: words a first-semester learner meets weekly were placed with the rare ones because the subtitle corpus had not seen their fused forms and a batch author had marked them 4 or 5. | **RESOLVED** |
| **I-233** | Web UI | **P2** | The chevron on the 소리가 만날 때 and Numbers cards sat inside the text column, not on the trailing edge | On the study-selection screen the lesson rows drew their arrow 16 px from the card's edge, the Numbers card 46 px in and 소리가 만날 때 78 px in — the arrow moved with the length of the title, and in a locale with a longer title it moved further. | **RESOLVED** |
...
| **I-243** | Korean content | **P2** | Editorial defects in the taught corpus: 고장나다 and 신경쓰다 taught joined spellings the standard writes as two words; 고마워 was an inflected form kept as a headword; 그렇게 was glossed "so, so much" under an example meaning "like that"; 빼다's Korean definition described taking out while the card taught subtracting; 어서 was defined as a greeting; 해마다's Korean definition contained the answer; 한잔 하실래요 was spaced against the headword 한잔하다; 등 (the back) sat at level 1 on the frequency of 등 "etc."; 당신 sat at level 1; 곳곳 was tagged an adverb; the ㅡ hint mixed speech levels | A learner met a non-standard spelling on a card, a gloss that did not match its example, a production question whose Korean prompt showed the answer, and 당신 in the first week. | **RESOLVED** |
| **I-244** | Content QA | **P2** | No machine-readable inventory listed every learner-reachable question with its prompt, answer, distractors, hint, feedback and evidence; runtime-generated families were sampled at three seeds | Defects in the letter pool (I-237), the reading distractors (I-238) and the matching grid (I-239) were invisible to every gate. | **RESOLVED** |
| **I-245** | Content QA | **P2** | No expert gold set held the taught Korean rules independently of the code that generates them | A generator could drift and its own tests would drift with it. | **RESOLVED** |
| **I-247** | Web UI | **P2** | The Home header squeezed the logo instead of scaling it: 29 px wide at 320 px with 200 % text on a first launch, and 98 px (distorted) with a one-day streak | The first-launch invitation chip ("Start today", 21 characters in French) is wider than the "0 days" it replaced, and the header's `auto 1fr` grid narrowed the logo's box while its height stayed 26 px — a smeared wordmark on the screen the app opens to. The same squeeze already applied to a streak learner at 200 % text on the narrowest phone. | **RESOLVED** |
...
| **I-251** | i18n copy | **P2** | Six interface-copy defects the gates could not see: Czech named a letter with the printer's word on 37 strings, Greek addressed the learner formally on 14 exercise instructions and informally on 110, and four single-string slips in Hungarian, Romanian, Russian and Ukrainian | A Czech beginner read *litera* — a piece of type — where every Czech textbook says *písmeno*, on the tab, the home card, the daily goal and every lesson prompt, while two strings already said *písmeno*. A Greek learner was addressed as *Πατήστε* in the matching and build exercises and as *Γράψε* on the next screen. A Hungarian learner met the one formal sentence in the level test's introduction; a Romanian learner read *Greșit de 1 dată*; a Russian or Ukrainian learner who is not a man read a masculine past tense on the mistake card's button. | **RESOLVED** |
| **I-252** | Release | **P2** | Dead code and stale files were shipping or tracked: an unused dependency, 36 unreferenced exports, a comment-only stylesheet, 22 orphaned report screenshots, five Windows download stubs and an empty file named `=` | None a learner could see; `@tanstack/react-query` was declared and never imported, and the lockfile recorded the mobile workspace at 1.0.5 while its package.json says 1.0.4, so an `npm ci` from a clean checkout was one drift away from refusing. | **RESOLVED** |
...
| **I-132** | My Learning | **P3** | Section headings started four pixels right of their own descriptions | 초기화, 백업, 앱 and 화면 each sat four pixels right of the text beneath them. Every box measurement said the column was straight. | **RESOLVED** |
| **I-139** | Copy | **P3** | Home repeated the streak in a row underneath it, and Backup made a privacy claim | A row reading 이번 주 · 7일 중 1일 · 0분 sat under Home’s cards, saying what the streak pill above it says and linking where it links. The Backup description ended 어디에도 올라가지 않아요 — an implementation detail phrased as a promise, on a screen a customer reaches when changing phones. | **RESOLVED** |
| **I-142** | Release engineering | **P3** | The report stated a test total its own breakdown contradicted | The document said 1,289 unit cases and, elsewhere, 976 + 216 + 96 — which is 1,288. A reader can check that in three seconds; a report that fails its own arithmetic is one nobody can use as a source. | **RESOLVED** |
| **I-149** | Korean | **P3** | One prompt still wrote its particle out as 을(를) | *순서대로 눌러서 {{value}}을(를) 만들어 보세요* — the register of an interface that could not decide, in a product that built `i18n/josa.ts` specifically so it would not have to. | **RESOLVED** |
| **I-15** | Audio | **P3** | 마디 was mispronounced in one voice | One word sounded wrong | **RESOLVED** |
| **I-153** | i18n copy | **P3** | Nine straight apostrophes, and a warning count that was stale by 64 | Filipino, French, Italian and Turkish wrote the ASCII apostrophe on pages whose other sentences use the typographic one — the last of the class the previous cycle started on. Separately, I-39 told a reader there were fifteen editorial warnings left to work through; there were seventy-nine. | **RESOLVED** |
...
| **I-167** | Patent package | **P3** | The disclosure package sent counsel to two files it had itself deleted | None — the package is internal and `patent/` is gitignored. It is here because the failure is the same one the whole package exists to prevent: a citation nobody can follow. | **RESOLVED** |
| **I-176** | Accessibility | **P3** | At 200% text the five-tab navigation bar falls below the 44px touch minimum | A learner who has turned text up to 200% gets navigation tabs 39px wide. The Level Test's own controls are unaffected — options and *I don't know* clear 44px at every viewport tested — so this is app chrome rather than the assessment. | **RESOLVED** |
...
| **I-192** | Vocabulary Level Test | **P3** | The adaptive policy's prose had drifted from its own constants | None. The code was right in both places; a reader of the file was told something false about the product. | **RESOLVED** |
...
| **I-204** | Numbers | **P3** | The Numbers question order is seeded from a blank record if the profile has not been read yet | A learner who opens a Numbers run on a slow launch may get the question order of a first attempt when it is not their first attempt — the same questions in the same order they saw last time. Nothing is lost or miscounted; the seeded shuffle simply seeds from the wrong number. | **RESOLVED** |
...
| **I-231** | Vocabulary content | **P3** | 108 English glosses were shared by two or more taught words; 0 are now | Two cards that both said *to boil* (삶다, 끓다) or *to smell* (맡다, 냄새나다) taught the learner a word without the distinction that makes it that word, and a level-test distractor drawn from the pair was a fair answer. | **RESOLVED** |
...
| --- | --- | --- |
| **I-04** | 3,393 of 10,000. `npm run vocabulary:qa:target` prints *3,393 headwords — 6,607 short of the 10,000 target* and has not been disabled, weakened or removed from `verify:release`. Sixty entries were added this cycle at 63 authored strings each — one Korean headword and example, an English gloss, meanings in seven inline locales, sentence translations in seven, and a meaning plus an example translation in each of the twenty-four copy locales — plus 240 pronunciation clips. That is the unit the remaining 6,607 would be written in.  **Re-measured 14 September 2026 (eighteenth pass, web only).** 3,867 of 10,000 — 6,133 short. 497 entries were added in seven reviewed batches (`content/vocabulary/entries/300.jsonl`–`306.jsonl`), every one with the full 63 authored strings and four recordings, every batch through the ten-step validation and the full content gate set; the candidates came from the National Institute of the Korean Language's learner list (grades A and B), which the level model now also reads (I-126). The rate is the honest fact: one batch of ~75 words is about an hour of authoring plus a twelve-minute rebuild, so the remaining 6,133 are roughly 81 batches. The target is unchanged and `vocabulary:qa:target` still prints the distance.  **Re-measured 14 September 2026 (eighteenth pass, web only).** 3,867 of 10,000 — 6,133 short. 497 entries were added in seven reviewed batches (`content/vocabulary/entries/300.jsonl`–`306.jsonl`), every one with the full 63 authored strings and four recordings, every batch through the ten-step validation and the full content gate set; the candidates came from the National Institute of the Korean Language's learner list (grades A and B), which the level model now also reads (I-126). The rate is the honest fact: one batch of ~75 words is about an hour of authoring plus a twelve-minute rebuild, so the remaining 6,133 are roughly 81 batches. The target is unchanged and `vocabulary:qa:target` still prints the distance.  **Re-measured 17 September 2026 (twentieth pass, web only).** 3,864 of 10,000 — 6,136 short; the nineteenth pass retired three non-standard headwords and no words were authored in this pass, which was an audit and delivery pass. `vocabulary:qa:target` still prints the distance. | Either author them, or restate the target. The delivery machinery is built and proven at 3,333 — bands, audio, level assignment, 32 locales — so the constraint is authoring, not engineering. |
...
| **I-230** | `scripts/content-translation-audit.mjs` (`translation:audit:check`, in `verify:quick`) reads every shipped row — 123,744 rows, 243,621 fields — for presence, script, Korean or English copied through, placeholders, terminal punctuation and length; 160 non-blocking length outliers, 0 blocking, results per locale and per row in `docs/content-translation-audit.json` and `docs/CONTENT_TRANSLATION_AUDIT_v1.0.5.md`. The model then read a seeded 90-row sample (3 per locale, `docs/content-translation-review-sample.json`): 87 PASS, 3 FLAGGED and corrected. The automated pass proves form, not meaning; the model reading is not a native speaker's (I-17, I-213). | A native reader per locale. Until then every row is AUTOMATED CHECKED and HUMAN REVIEW REQUIRED, and the documents say so. |
| **I-39** | `npm run locale:editorial` is new, and it reads for four things nothing else looked at:  * **Register.** Twenty-one of the shipping languages choose between a familiar and a polite second person, and the choice has to be the same on every screen. It counts the markers of each and fails the build on a language that uses both. It found **five languages mixing them** — de (12 strings), el (3), id (6), ro (2), and, once its own false positives were fixed, none in cs. All are now consistent with the register that language already used. * **One English sentence, two translations.** Where two keys hold the same English string their translations should match. Found the Level Test asking "What does this word mean?" in wording that differed from the reading exercise's in six languages; unified. * **Typography.** 71 straight apostrophes in languages whose English source writes the typographic one; all replaced. * **A label that became a paragraph.** A short English label translated several times longer, which is what breaks a layout at 200% text.  Writing it also found the writer out. Its first run reported seven mixed-register languages and three were its own fault: JavaScript's `\b` is defined against ASCII, so `\btes\b` matched inside *prêtes* and French "revisions ready" was reported as addressing the reader familiarly. Every pattern now goes through a Unicode-aware boundary, German and Italian are read with sentence-initial capitals lowered (so *Sie* meaning *she* is not counted), and the ambiguous markers — Spanish `su`, Czech `ty`, Dutch `u` as the abbreviation for hours — are named and excluded with the reason. **A linguistic check that cries wolf is worse than none**, because it is the kind people switch off.  **What is still not done, and this is the whole of the remaining item.** Nothing here reads a sentence for whether it is *good*. Register consistency is not naturalness, and an apostrophe is not a register. The 15 findings it still reports are deliberately left as warnings for a person: they are places where two screens word the same idea differently and only somebody who reads the language can say which is right, or whether both are.  Distinct from I-17, which is native-speaker review. This is the pass that should happen before one, and the mechanical half of it is now done and enforced in `verify:quick`.  **This cycle: Korean, read on the rendered screens.** Three classes of defect that no check looked for — one thing called two things (I-60), a register that slipped into 합쇼체 six times, and 262 example translations that invented a person the Korean does not have (I-59). All three are rules in `locale:editorial` or `examples:qa` now. What has still not happened is a native reading, in any of the thirty-two.  **Re-audited 24 August 2026.** The Korean interface was rewritten this pass to drop 낱자 and 낱말 (I-75), which is a mechanical editorial change and not a native reading. The row is unchanged: 31 of 32 languages have had a program read them and a person has not.  **Re-audited 25 August 2026.** Unchanged in substance. `qa:locales` renders 32 languages x 8 screens = 256 screens with no measurable problem, `locale:editorial` reports 0 errors and 38 warnings for a person to read, and neither of those is a native reading. Two category labels were renamed this cycle for a mechanical reason rather than an editorial one — Kyrgyz *Адамдар жана үй-бүлө* and Uzbek *O'qish va ish* each contained the answer to the question they were hinting at.  **Re-audited 26 August 2026.** Unchanged in substance: `locale:editorial` reports 0 errors and 38 warnings for a person to read; no native reading has happened in any of the 32.  **Re-audited 27 August 2026.** Unchanged in kind and larger in size: this pass wrote 78 new entries in ten languages, four More-about-it notes (one in 32 languages), and rewrote 17 shipped examples with their translations — all model-written, none read by a native speaker.  **Re-audited 17 September 2026 (twentieth pass).** `locale:editorial` reports 0 errors and 13 warnings, all split-translation judgements for a person. The gate now reads the register in the verb for Hungarian and Greek — which is how fourteen formal Greek exercise strings and one formal Hungarian sentence had passed it — and carries a per-locale glossary (Czech litera → písmeno); each rule was negative-tested. Still a mechanical pass, not a native reading. | A reading pass per locale, screen by screen, by somebody who speaks it. The 70 remaining warnings from `locale:editorial` are where to start — all of them split-translation judgements, since the straight-apostrophe class was closed by I-153. |
| **I-79** | Levels 28–30 held four hundred and seventy-seven of the old 3,333 words. Sixty entries were authored against the top of the scale; 48 landed at 28–30 and 11 more at 27, so the top band now holds 524 of 3,393. At ten new words a day that moves exhaustion from about nine weeks to about eleven. The shape of the problem is unchanged: level 30 alone still holds 334 words because it absorbs everything the difficulty model cannot place lower.  **Re-measured 13 September 2026 (web pass, v1.0.5), after the frequency reader was rebuilt (I-126).** Levels 28–30 now hold 520 of 3,370; level 30 alone holds 337. Measured from the plan's own zone rule rather than estimated: a learner at level 30 draws from a 520-word zone and, at the default ten new words a day, meets the last new word on day 52 (day 26 at twenty). The thin end of the scale is at the *bottom*, not the top: the zones around levels 1–2 and 7 are the smallest (level 6 holds exactly the 40 words `vocabulary:level:qa` requires), but a beginner's taught level rises with mastery long before a 40-word level is exhausted, so no measured learner runs dry there. `vocabulary:level:audit` still finds a fortnight of new words at every level. Nothing was bulk-generated for this measurement — the honest fix is still words that are advanced and ordinary, written and read one at a time.  **Re-measured 14 September 2026 (eighteenth pass).** The learner-list evidence (I-126) moved 1,031 words *down* and none up, so the top band thinned from 520 to 436 of 3,867 while every zone below it thickened; the model is now reading two independent frequency sources rather than one, and the previous edition's 520 included words that only sat at 28–30 because the subtitle corpus had never seen them. Level 30 alone holds 298. Measured from `docs/content-inventory.json` `zones` (the plan's own zone rule, all four daily goals). The summary's *five weeks* and the impact's *52 days* in the previous edition were two different measures of the same thing (all-new versus reserved-new); both are now stated with their definition.  **Re-measured 14 September 2026 (eighteenth pass).** The learner-list evidence (I-126) moved 1,031 words *down* and none up, so the top band thinned from 520 to 436 of 3,867 while every zone below it thickened; the model is now reading two independent frequency sources rather than one, and the previous edition's 520 included words that only sat at 28–30 because the subtitle corpus had never seen them. Level 30 alone holds 298. Measured from `docs/content-inventory.json` `zones` (the plan's own zone rule, all four daily goals). The summary's *five weeks* and the impact's *52 days* in the previous edition were two different measures of the same thing (all-new versus reserved-new); both are now stated with their definition. | More words that measure into 28–30. Level is computed from frequency, usefulness, concreteness, length and irregularity rather than declared, so the lever is the vocabulary chosen, not a number set in a file. |
| **I-20** | 71 words carry a third element in `vocabulary.en.json`, up from 35. They are the homographs and the usage traps: 배 is a stomach, a boat and a pear; 못 is the difference between will not and cannot; 아가씨 is a word to use about somebody and not to them. 38 of the 71 are inside the 600-word core band, and this pass wrote all 38 in each of the 22 partial languages — 638 paragraphs — because a note that exists in English and not in Tamil is an asymmetry a learner discovers by switching language.  **Re-audited 26 August 2026.** Unchanged: 71 words carry the block.  **Re-audited 27 August 2026.** Four notes were added where a learner genuinely needs one: 당신 (the polite-you trap — spousal or confrontational in real speech; written in all 32 written languages because the word is in the core band), 전세 (the lump-sum lease no other country has), 회식 (the semi-obligatory team dinner), and 좀처럼 (a negative-polarity adverb). 71 → 75.  **Re-audited 13 September 2026 (web pass, v1.0.5).** Twenty-four notes were written for the words a first-week learner most often stops on and that the one-line gloss could not carry: the ㄷ-, ㅂ-, 르- and ㅎ-irregulars a beginner meets first (듣다, 걷다, 춥다, 덥다, 다르다, 빠르다, 그렇다), 알다 and 좋아하다, the counters 개 · 살 · 명 · 마리, 잘, 고맙다 and 죄송하다 (which politeness each carries), 싶다, 아직 and 벌써, 너무, 때, 밥, 머리 and 마음. Every note was written in all 32 languages — the inline seven and the 24 copy packs — because the twenty-four are inside the core band. 75 → 99. `vocabulary:sense:qa` is clean; the notes add 77 kB to the precache (24 notes × 32 languages land in band 1) and every budget still holds. | Content, not code: write the block for the words a learner most often stops on. The machinery to show it has been there since the block existed. 99 of 3,370 carry one; the next candidates are the remaining irregular verbs of levels 4–8 and the homographs of the 600-word band. |
| **I-225** | `pdftoppm -r 40` over every page, ink coverage under 2%: pages 6, 8, 25, 42, 49, 61, 71, 82, 127, 233 and 250 of 281. Each is the tail of a chapter — `h1 { page-break-before: always }` starts every chapter on a fresh page, so a chapter whose last paragraph or small table spills by a line leaves a page holding only that. §20S and §20T (the eighth and ninth passes) were placed after §23 Verdict since the ninth pass, with §23.1 after them.  **Re-measured 17 September 2026 (twentieth pass).** The nineteenth edition rendered at 446 pages with 14 under 3 % ink, each a chapter tail or a single table row carried over. `build-report.mjs` now sets `orphans: 3; widows: 3` on paragraphs and list items, so a paragraph cannot leave one line behind; a table row is atomic and a one-row tail can still occur. The twentieth edition's count is in §20AD.6. | The two chapters are moved before §20U and §23.1 back under §23 (done). The chapter-per-page rule is kept as a design choice — a chapter that begins mid-page is harder to find — so tail pages remain: this edition renders at 238 pages with ten of them under 2% ink, each a chapter tail, listed in §20Z.10. Removing them would mean dropping the chapter break, which is a decision for the document owner rather than a defect to fix silently. (The earlier claim that `build-pdf.mjs` was the cause was wrong: that builder makes the patent documents; `build-report.mjs` already breaks tables between rows.) |
...
| **I-131** | `.toggleInput` is `position: absolute` with no offsets, and its label was not positioned, so its containing block walked up to `#root`. An absolutely positioned box is clipped by an ancestor only when that ancestor is in its containing-block chain, so the scroll pane's `overflow: hidden` did not clip it: two of them sat at document y≈1908 and y≈1960 and stretched the document to 1,961 px against an 844 px window.  A scrollable document carries the whole app when it moves. Its offset also survives a route change, so a non-zero one carried from My Learning into a shorter page rendered that page shifted with its bottom clipped — the intermittency depended on whether a gesture had reached the document.  Fixed in two layers: `html, body { overflow: hidden }` so the shell owns the viewport and the class of defect is structurally impossible, and `.toggleLabel { position: relative }` so the leak cannot start. **Negative-tested**: with the shell guard reverted, `tab-bar.spec.ts` fails with `Expected: <= 845, Received: 1961`.  `e2e/tab-bar.spec.ts` measures the bar's rectangle before and after a real wheel gesture at seven sizes, three text scales and both appearances — 31 cases. `e2e/privacy-stability.spec.ts` enters Privacy 108 times over six state-carrying paths at three sizes and measures on arrival and at the bottom. | Done. |
| **I-134** | The upright was authored two units below the top bar’s own start. Both are stroked centrelines with butt caps, so in those two units nothing covers the upright’s outer half. `strokeVectors` does close corners — the later stroke is extended half a pen — but along its own tangent, which lengthens the upright and can never move the bar sideways. A flush corner needs the two centrelines to start at the same point, which is how ㅁ and ㅂ were authored.  Measured at 512 px as the inked fraction of a one-pen square at the junction: ㄷ 90%, ㅌ 91%, ㄸ 92%, ㄹ 52%. All are 100% after. ㄹ carried a second offset — its waist began 3.3 units left of the stem it meets; moving the stem right by half a pen was tried and failed `letters:face`, because `shapeToFace` scales the letter and not the pen, so 4.5 authored units became 6.09 drawn. | Done. Coincident corner points in `digeut`, ㅌ and ㄹ; `glyph:structure` measures every junction and negative-tested against the restored geometry. See §20O.1. |
| **I-135** | The two ㄱ are identical — both 0.219 of the block wide, 0.197 deep. What stood between them was the ㅗ’s stem: 85 px of shared ink at 512². The face’s measured part boxes overlap (onset y 0–0.343, vowel 0.227–0.486), which is a correct measurement — Pretendard’s stem does rise into that gap — and the face affords it because its stem is thin and the gap wide. At one uniform pen the same arrangement puts the stem through the leg.  Separating the boxes was wrong: 고’s ㄱ interlocks with its ㅗ by design, and pushing them apart left 고 with a bar and no stem, which `glyphshape:qa` reported as the guide and demonstration teaching different shapes. | Done. `separateColliding` in `data/compose.ts` compares segment against segment and shortens the lower box from the top a quarter-pen at a time; of thirty-three measured blocks it moves one. The residual 12% component drift is declared per block and per component in `CANONICAL_COMPONENT_DEVIATIONS`. See §20O.2. |
| **I-136** | `recordIntroduced` writes a progress row when a word is *shown*; `recordReview` writes a memory row when one is *answered*. `weakestRecall` returned 0 for a word with no memory row — the strongest claim it can make, below `WEAK_RECALL` — so every word a learner had ever been shown and walked away from was classified `weak` and scheduled ahead of everything their level had chosen.  `dailyvocab:qa` could not see it: three levels, empty progress, and an aggregate assertion. The learner in the report had a state only the app creates, and the wrong words were three of ten. | Done. `weakestRecall` returns `null` for no evidence and such a word returns to the new pool, where the level governs it. The day also opens in the learner’s band, and a review is captioned as a review. `dailyplan:level` runs 5 profiles × 30 levels × 30 days and writes `docs/vocabulary-plan-audit.json`. See §20O.6. |
| **I-137** | `completeWord` returned the plan unchanged for a word the plan did not list. The session’s frozen queue and the stored plan are allowed to disagree: the plan is a derivation whose inputs include `levelFromProgress`, which moves as words are learned, so a learner who finishes the word that takes them past two thirds of their level flips the planning level mid-sitting and every remaining question is about a word the stored plan has never heard of.  Two tests had written the leak down as an invariant — `vocabularyDay.test.ts` asserting an unlisted word is ignored, and the property suite expecting that it credits nothing. | Done. Crediting is total: a word answered correctly in a sitting this app posed is credited, and the plan adopts it if it no longer lists it. The goal does not move and `completed` stays idempotent. A word’s own step position is shown so an introduction does not look ignored. See §20O.7. |
| **I-143** | Gradle packages `app/src/main/assets/public`, which Gradle does not build — `npx cap sync` copies it there, and the build has no opinion about how old it is. Measured on this pass: four release builds over four hours produced **byte-identical** APKs across a real source change, because `assets/public` was four hours old while `apps/web/dist` was current. The identical digests were the only clue and they read as reassuring.  Caught by comparing the file timestamps of `assets/public/index.html` (02:37) and `apps/web/dist/index.html` (06:50) while wondering why a `data-testid` had not changed a hash. After a sync the APK digest moved from `4211cdd1…` to `b13c7f46…`, which is the proof that the earlier four were stale. | Done. `native:bundle:check` opens the delivered APK and compares every file of `apps/web/dist` against `assets/public/` inside it — the artefact a customer installs, not a directory a build step claims to have written. It runs in `verify:release` immediately before `release:current`, which is the other half of the same question. Negative-tested by rebuilding the web app without syncing: 35 files missing, `index.html` and `offline-assets.json` different. |
...
| **I-178** | Two guards should have caught it. The category guard compares browse categories, which are topical: 약 is filed under body-health with 의사, 물 under food, so they differ and the pair passed. The noun-class guard would have caught it and is conditioned on *both* words being classified — 약 carried no class at all. Measuring the shipped bank, 76% of noun-slot context items have an unclassified answer, so that guard was inert for three quarters of them (I-182).  Fixed at the builder, not the answer key. `isConsumptionObjectFrame` reads the sentence for a consumption verb governing an object-marked blank, and `consumableWords` rejects any candidate that can be swallowed — judged from the candidate alone, so it holds when the answer is unclassified. Narrow by design: the comitative frame ____와 점심을 먹어요, which wants a person, is not caught.    before  다음 · 물 · 산 · 약   keyed 약, and 물 is also right   after   다음 · 산 · 약 · 하늘  keyed 약, and nothing else fits  The bank and `cloze.json` are regenerated from the same builder, so the identical question in Today's Vocabulary and Review is fixed by the same change. Negative case G6 restores 물 to the shipped bank and the gate fires. | Fixed. `scripts/lib/level-test-rules.mjs`, applied by the builder and by `leveltest:ambiguity`. |
| **I-180** | Instrumenting the selector over 157,799 adaptive questions — 200 simulated sittings at each of 30 levels — showed 23.7% of items asked *above* the running estimate and 32.3% below. Information theory explains it: the most informative item sits at the estimate, so an unconstrained selector sits there and drifts up whenever the bracket midpoint is above the posterior mean. The estimator was not wrong; there was no floor under the experience.  Two bounds fix it. One adaptive item in five is drawn a level below the estimate, on a cadence fixed by ordinal position so a resumed sitting reproduces the order. And the step is made asymmetric — three levels down, two up — because going down after a miss is the test agreeing with the learner while going up is it disagreeing on the strength of one answer that carries no information one time in four.    before  32.3% below · 44.0% at · 23.7% above   climb 2, 4, 6, 9, 12, 15, 18   after   40.1% below · 41.4% at · 18.4% above   climb 2, 4, 6, 8, 10, 12, 14  Accuracy is unchanged: ±3 at 90.2%, mean absolute error 1.64 levels, opening level 2, 20-30 items. The three shares are now measured inside `leveltest:qa` and bounded there. | Fixed. `EASY_EVERY`, `EASY_DROP`, `MAX_STEP_UP` and `SUSTAINED_CORRECT` in `apps/web/src/domain/levelTest.ts`. |
| **I-182** | Measured over the shipped bank: 205 of 268 noun-slot context items — 76% — have an answer with no noun class. The guard was doing nothing for three quarters of the items it exists to protect.  Two changes. `약` and `약물` gain a `consumable` class and a `food`/`consumable` conflict group, which closes the pair that shipped. More importantly the new frame rule is *one-sided*: it reads the class the sentence's predicate selects and judges the candidate alone, so it cannot be switched off by a gap in the answer's data. `questions:ledger` re-checks all 5,165 objective questions across the three sources that generate them. | Fixed for the consumption frame. The general fail-open shape of `classesCompete` remains for other frames — see the note. |
| **I-185** | `word_ireonada:context` shipped as `일곱 시에 ____.` keyed 일어나요, offering 도와요 · 연습해요 · 잘해요. *일곱 시에 연습해요* — *I practise at seven* — is as ordinary as *일곱 시에 일어나요*.  `leveltest:ambiguity:check` passed the bank containing it, with zero findings, and was right to by its own rules. Its weak-context rule asks whether the sentence carries an argument particle; 시에 is an argument particle. **A particle is not a constraint.** A time says when, and every predicate a learner knows can happen at seven o'clock.  Four more items of the same shape shipped beside it: 수업 시간에 ____, 아침에 일찍 ____, 발표 전에 ____, 다음 달에 ____. | Fixed at the source. `isUnconstrainedPredicateFrame()` in `scripts/lib/level-test-rules.mjs` classifies each non-blank eojeol as temporal, free adverb, subject pronoun or contentful, and the builder refuses a predicate blank with no contentful eojeol; the gate refuses one in the shipped bank. All five example sentences were rewritten in the editorial pack with their translations in 31 languages and new recordings. Negative case **G7** restores the photographed item and the gate exits 1. |
| **I-186** | The item shipped with 얼음 · 은행 · 학생 · 형. A student opening a door and an older brother opening a door are both ordinary Korean.  The noun-class guard exists for exactly this and never fired. It compares each distractor against the **answer's** class, and 은행 is not a person, so nothing rejected 학생 or 형. The defect is not that the guard is wrong but that it is pairwise: the constraint here belongs to the frame, not to the pair. | Fixed. `isAgentSubjectFrame()` — a blank that is the subject of an action carrying an object admits any human agent, so no person-class candidate may be offered, whatever the answer's class is. Evaluated on the candidate and the sentence alone, which is the case a pairwise guard cannot cover. Negative case **G8**. |
| **I-188** | 34 items over 16 stems. The Level Test bank has had a `shared-prompt` rule since the pass that found 더해요/곱해요 in it; `cloze.json` never had one, and both surfaces are built by the same generator from the same authored sentences. Each item is answerable alone — the rival word is not among its own four options — and the pair is the generator's own proof that the sentence does not pin its answer down. | Fixed. A `shared-stem` invariant in `scripts/question-ledger.mjs`, which is the only program that reads every surface — neither half of the defect is visible from inside one of them. Eighteen example sentences rewritten so that every stem is distinct, with translations in 31 languages and new recordings. |
| **I-189** | `question-ledger.mjs` found the answer with `options.find(o => o.isKey)`. In this codebase `isKey` means *this option's text is a translation key, render it through i18n* — not *this is the correct option*. Both buttons of every 한자어식/고유어식 question carry it, and 240 exercises carry it on no option at all, which the ledger recorded as `answer: null` and then skipped every answer invariant for.  Measured against `exercise.answer`, the index the running screen grades on: **336 of 368** questions were recorded with an answer the application does not accept, or with none. The ledger then checked that its own wrong answer appeared exactly once among the options and printed *every question has one answer*. | Fixed. The ledger reads `exercise.answer` and reports an `answer-index` problem when it is not an index. All 368 pass against the right field. |
...
| **I-190** | The seven translations carried inside an editorial entry move with the sentence, because they are in the same row. The other 24 languages live in `content/vocabulary/copy/<locale>.json` keyed by word id and do not.  Five gates read those files — `vocabulary:translation`, `translation:semantics`, `locale:content`, `locale:editorial`, `copy:generated` — and all five passed. None of them compares a translated example to the Korean example; they check key parity, gloss agreement and placeholder shape, which is one level away from the thing that was wrong. | Fixed. All 576 affected rows retranslated. `copy:fresh` records the Korean sentence each pack's translations were written against and fails when the corpus moves and the packs do not; it is in `verify:quick`. |
| **I-196** | Every contextual item took `anchor.level`, the frequency rank of the answer. Measured on the bank at 700c78db: 629 of 629. `지갑에 ____이 있어요` was a level-1 question containing 지갑, a level-9 word; 61 items (10%) contained a word ranked above their own level, the worst by 21.  The answer's rank is a fact about the answer. A gap-fill is a reading task before it is a vocabulary task, and nothing measured the frame. | Fixed at the source. `scripts/content/sentence_demand.py` computes a demand level from three floors — the hardest ordinary word in the frame, twelve grammar constructions each with the level a learner can be expected to have met it, and the eojeol count — and the item takes the higher of that and the answer's own level. 68 of 625 items moved, all upward. `docs/LEVEL_TEST_DIFFICULTY_AUDIT.md` records every floor and why. |
| **I-197** | Neither the estimator nor the step bound was wrong. The bracket's upper bound is 30 until something is missed, so its midpoint after three correct answers is 18; the posterior's prior is centred at 15 and deliberately weak, so for the first several questions it believes something near 15 about everybody. `MAX_STEP_UP` was the only thing holding the walk down.  A bound on step size limits how fast a sitting climbs. It says nothing about whether the learner has given any reason to climb. | Fixed. Six bands and an earned ceiling: `reach()` opens a band on three correct answers inside it across two distinct question kinds, and `reachCeiling` clamps the selector's target, the bounded step, the warm-up ladder and the screen's pool fallback. A miss costs one unit of evidence and opens a two-question confirmation window capped below the level missed. `docs/LEVEL_TEST_ADAPTIVE_POLICY.md` states the whole policy. |
| **I-198** | `planKinds` cycled `meaning, context, produce, context`, so index 1 was always contextual. | Fixed. The first `OPENING_ITEMS` = 5 are word questions; the first sentence is question six. The composition over the whole sitting is unchanged — twelve contextual, nine of each other kind — so nothing is lost, it is reordered. Measured across all sixteen simulated profiles: first sentence at Q6 in every one. |
| **I-203** | `LetterSessionPage` computed its resume index in a `useState` initialiser, from `progressFor('character', …)`. `ready` is false until the stored profile has been read, and the route mounts before that: `CorpusGate` lets the app through as soon as the corpus is available, and `LaunchSplash` covers the screen rather than holding the router. So the initialiser asked about a profile that was not there, found nothing learned and answered 0.  Found by an end-to-end case that had been dismissed as a flake four times — `an interrupted lesson resumes at the letter that is unfinished`, 9.5s green alone and 18-25s red in the suite. Under `--repeat-each=8 --workers=4` it failed **6 of 8**, and a 500 ms delay before the navigation changed nothing, which ruled out the write racing. Dumping the object store through the failure showed `character:ㅏ=learned` both before and after the navigation: the data was right and the read was early. | Fixed. The cursor is placed once, by an effect, on the first render where the profile can be read; a `placed` ref keeps the original invariant that finishing a letter never moves the cursor a second time. `recordIntroduced` is gated on the same flag — `applyIntroduced` only promotes an `unseen` row, and a row it cannot see is a row it invents, so against an un-hydrated profile it wrote a blank `introduced` row for whichever letter the un-placed cursor pointed at. 8 of 8 under the parallelism that produced 6 of 8 failures. |
| **I-205** | `build_level_test.mjs` refused a distractor sharing the answer's browse category — written to stop two right answers, which made 'unrelated to the sentence' the qualifying condition. `leveltest:ambiguity` asserted the same rule and `answerable.test.ts` asserted it a third time. Every gate was green and every gate was right to be: the item has exactly one defensible answer, which is all any of them asked. | Inverted for nouns: a distractor must share the answer's category or one of its tags. Predicates keep the old rule, where a same-area verb genuinely is a second right answer. Frames where a same-area noun would also be right are refused outright (I-207) rather than filled with a distant option. New gate `leveltest:distractors`, in `verify:quick`, negative-tested at 747 findings against the pre-fix builder. |
| **I-206** | `categories.py` writes its rules as `\b(head\|face\|…\|pain\|hurt\|…)` — a word boundary in front and nothing behind, so every alternative was also a prefix rule. `pain` matched painter, `sand` matched sandwich, `wind` matched window, `birth` matched birthday, `doll` matched dollar, `war` matched warehouse, `exam` matched example, `pass` matched password. | A `(?!\w)` lookahead on every rule, plus an English inflection and derivation table so that closing the accidental matches does not cost the 196 real ones it would otherwise. `-ter` is excluded (it gives painter → pain) and `-y`/`-ly` need a four-letter stem (many → man, busy → bus, early → ear). Then all eighteen category listings were read word by word: `OVERRIDES` 552 → 1,085, and 134 words moved. |
| **I-207** | `isUnconstrainedPredicateFrame(anchor.example)` and `isConsumptionObjectFrame(anchor.example)`. Both functions begin `if (!sentence.includes('____')) return false` and both were handed the sentence with the answer still in it. `isUnconstrainedPredicateFrame('일곱 시에 일어나요.')` is false; the blanked form is true. `leveltest:ambiguity` re-derives the frames from the shipped bank, where the prompt does carry the blank, so it computed them correctly and found nothing. | Pass `blanked`. Found only because inverting the distractor rule (I-205) removed the accidental cover and four `consumable-option` findings appeared at once. |
| **I-208** | Gap-fills at levels 1–5 fell 50 → 15 and at 6–10 fell 56 → 18. Two unit tests failed on it: `reviewLists.test.tsx > asks about a wrong word in more than one way` returned one mode where two were required, and `honours the size the learner picked` returned a plan of 2 where more than 5 was required. | Twenty-two hand-written items in `content/vocabulary/context-items.json`, validated by the builder against every rule it applies to its own output — particle agreement across all four options, category sharing, level spread, the safety lists and all the frame tests. A curated item that fails is a build failure. Levels 1–5 recover to 36. |
| **I-211** | The shipped commit scanned under the final policy: 4,333 findings — violence 2,236, sexual 823, political 592, mortality 194, profanity 149, gambling 146, drugs 127, self-harm 66 — in 9 of 13 families (docs/CHILD_SAFE_CONTENT_AUDIT.md §4). | 23 words retired with tombstones (docs/CHILD_SAFE_CONTENT_REMEDIATION_LEDGER.md), one injury-centred example rewritten with its 31 translations and recordings, one Korean gloss rewritten, the dictionary filtered at publication, everything downstream regenerated. Progress rows, saved words and review schedules for retired words are kept; the plan repair drops only what is still owed. |
| **I-212** | The gate's own header: "It does not touch the dictionary"; the excluded list compared whole headwords; meaning items' option ids were never resolved. §20Y.6 re-classifies each prior safety claim. | Claims re-classified as CONTRADICTED where they were, the gate replaced, and the report's safety section now quotes the audit file that the gate regenerates and checks. |
| **I-214** | `app.identity.json` said 1.0.6 build 24; `project.pbxproj` said MARKETING_VERSION 1.0.3 / CURRENT_PROJECT_VERSION 5; `store/release-notes.md` and the report front matter said 1.0.6; `registered` is false for both stores and every `build-info.json` on record says nothing was uploaded, so no store holds a higher number. | Every site now says 1.0.4 (identity, mobile package, `config/product.ts`, three legal documents, release notes, report front matter, both Xcode configurations); the build number is 25 on both platforms because 24 is the versionCode the delivered artefact used. `displayVersion()` renders `v1.0.4` in one place. `config/product.test.ts` pins the literal and the form; `version:check` pins the same literal and reads two more files. Negative-tested: setting `product.ts` to 1.0.6 fails the gate. |
| **I-215** | `styles.xml` → `windowSplashScreenAnimatedIcon = @mipmap/splash_icon`; the packaged icon's centre pixel in the delivered 1.0.6 APK reads rgb(255,119,0). `scripts/check-native-splash.py --apk` fails against that APK on exactly this. | The system splash icon is now a feathered cut of the approved artwork's own centre circle, generated from the same file as the launch bitmaps, drawn on the artwork's ground; the activity's first frame completes the picture. `scripts/check-native-splash.py` proves every launch frame is wordless where it must be and mark-free everywhere, on the source resources, the iOS catalogue and the delivered package (`splash:check` in `verify:quick`, `splash:bundle:check` in `verify:release`). Cold start recorded on the API 36 emulator — see §20Z.2. |
| **I-216** | `ChoiceExercise` before this pass: the answer rung was `setLevel(+1)` rendering `review.hint.reveal`; `choose()` then reported `correct: true, hintLevel: 3`; `WordSessionPage.advance` credited it. Reproduced in the unit harness (`answerReveal.test.tsx` fails against the old component). | UNANSWERED → HINT_VIEWED → ANSWER_REVEALED → retry. The reveal is an outcome: the correct option takes the blue `.right` box and the check mark, the others dim, every option is disabled, exactly one result is reported (`correct: false, revealed: true`), the card says the answer was seen and the item comes back, and a Next button moves on. The attempt row records `revealed`; the memory model reads it as not recalled; the daily session puts the word at the back of the retry pass; the review sitting rescues it two exercises on. `BuildExercise` reveals the same way. Tests: 6 component cases, 4 domain cases, a memory-model case, `e2e/answer-reveal.spec.ts` in the real session, and `e2e/hints.spec.ts` updated. |
| **I-217** | `.optionsChips { display: flex; flex-wrap: wrap }` — wrapping is not a layout. The syllable tray had the same defect (I-131) and was fixed with `optionRows`; the chips were not. | The chips are a grid; `useChoiceColumns` decides four across or two by two from a measurement of the chips themselves at the learner's type size before paint, re-measured on resize; a two-by-two cell may wrap its own text; the grid mirrors in RTL with the same row-major order; phrase options stay a single column and cannot form an orphan row. `e2e/choice-layout.spec.ts` drives a real gap-fill sitting at 320/360/375/390/412/430 px, 1× and 2× text, in Korean, English and Arabic, and asserts the rows are [4] or [2, 2], nothing overflows and every chip is at least 44 px tall. |
| **I-218** | `BuildExercise` before this pass: `useEffect` on `picked.length >= syllableCount` called `onAnswered`; no active slot; no Check. | The instruction says the action (아래 음절을 순서대로 눌러 단어를 완성하세요, 32 languages); the next slot is marked with an accent outline and its number; a tapped tile leaves the tray dashed and faded and appears in the slot; Undo and tapping a filled slot take a tile back, including the last; Check is disabled until every slot is filled and grades once; a polite live region counts slots; every slot and tile has an accessible name. `buildInteraction.test.tsx` walks correct, wrong, undo, repeated syllable, distractor, keyboard and screen-reader paths. |
| **I-219** | `MatchExercise` before this pass; the customer's screenshot. | The instruction is the action in order (왼쪽에서 한국어 단어를 먼저 누른 다음, 오른쪽에서 알맞은 뜻을 선택하세요); the columns are headed 한국어 단어 and 뜻; only the Korean column is enabled until a word is held; the meanings then become the marked target and the caption names the held word; a made pair carries the same numbered badge on both tiles and either side takes it apart; Check is disabled until every word is paired and grades all pairs together; the verdict is on the tiles in the same blue and red the choice question uses. `MatchExercise.test.tsx` walks the first-time sequence in taps. |
| **I-223** | `packages/design-tokens/tokens.css` — `--hg-text-body: 15px` and every neighbour in px; `grep -c rem` over `apps/web/src/**/*.css` = 4 files; the nine sites listed in the commit "the large-text measurements measure large text". | `scripts/lib/text-scale.mjs` and `e2e/helpers/textScale.ts` double the tokens, which is what a WebView does to every CSS pixel of text; the nine sites use them. Re-run at a real 150% and 200%: `numbers:layout` 45/45, `align:sections` 12/12, `legal:isolation` 16/16, `scroll:audit` 210/210, and 79 spec cases; the tab-bar targets measure 64×64–67 px at 320 px in Tamil, German, Korean and Arabic, so I-176 is closed. Text set as a bare `clamp()` (the Korean glyphs on the tiles) is pinned by design and is stated as not scaled. |
| **I-228** | `packages/content-safety/policy/runtime/<lang>.json` now carries one supplement per shipped language, lazily imported as its own `safety-<lang>` chunk; `runtimeEvaluatorFor(locale)` merges base + English + the learner's language; `guardLevelTestItems` reads meanings in every locale. `runtime.test.ts` proves every fixture that the full policy refuses is refused by the supplemented evaluator, and the negative case N6 in `scripts/content-gates-negative.sh` strips the English supplement and requires the suite to fail. The 24 kB route-chunk budget was not raised; supplements are exempt as lazy chunks (`content:safety:bundle:check`). Landed in 0bc459bb. | Done. |
| **I-232** | The WSL VM was torn down at 15:01 on 14 September 2026 while `git commit` was writing the batch 1–3 commit. Diagnosis, read-only: 1,092 zero-byte loose objects, all stamped 15:01; `git fsck --full` reported 1,070 missing blobs, each referenced only by the index the aborted commit was writing; no missing tree or commit reachable from a ref; `HEAD` and `refs/heads/main` both at d06c7faa, the remote's head the same; the lost commit 399d17ff present only as two reflog lines. Repair, in order: `git status --short`, branch, HEAD, `git log -5`, `git remote -v` recorded; the working tree fingerprinted (sha256 of every file) and archived (`/root/hangyul-backup/worktree-changes-20260914-2355.tgz`, the untracked-file inventory, a partial `tracked-changes.patch`); the empty objects *moved* to `/root/hangyul-backup/quarantine/`, not deleted; `git read-tree HEAD` to rebuild the index only; the two reflog lines removed after the logs were backed up; `git fsck --full --no-dangling` exit 0 with no output; the fingerprint taken again, identical. `git reset` was not needed: HEAD already named the last good commit, and only the index was damaged. | Done. The commits and the single push are recorded in `docs/WEB_CONTENT_QA_EVIDENCE_v1.0.5.md` §6. |
| **I-234** | Probed 2026-09-15 with 26 shapes beyond the existing fixtures; 10 passed. Four rules now refuse them, in `packages/content-safety/src/normalize.ts`/`evaluate.ts` and the Python port `scripts/content/child_safety.py` alike: a run of single syllables joins forward to the syllable word after it and never across punctuation (the false-positive cost was measured over the 84,038 Korean strings the app ships: 0 new findings); Cyrillic and Greek look-alikes are folded to Latin only inside a token that mixes scripts; a change of script is a token boundary; a stray word in Cyrillic, Han or kana is read against the lists of the languages written in it. Fixtures 368 → 406 (38 new: 29 negative, 9 positive guards such as 자, 살펴봐요 and 날씨 방); `child_safety.py --self-test` 406/406 agree with the TypeScript evaluator; the runtime suite holds every fixture through the learner's supplement; negative cases N9 and N10 put each rule back and require the fixtures to fail. | Done. Shapes still not refused, by decision: a term with Hangul inside a Latin word (섹s하다), repeated letters (sexxx), HTML entities — none can arise from the authoring pipeline, and each would cost real false positives. |
| **I-235** | Reproduced in the store on 2026-09-16 (`recordStudyTime(20_000)` → `summary.streak_days` 1). The rule is now `isQualifyingDay` in `domain/activity.ts`: a local calendar day with at least one answered question (letter, word, review or Numbers, right or wrong) or an item reaching learned; the first such day is Day 1; study time is recorded and never a streak day; the level test counts for nothing. `active_days` is the durable evidence and rows corroborate it — nothing is migrated. Numbers `exercise_attempted`/`review_completed` now credit the day. Home shows "Start today" for `status: never` and nothing until `ready`; My Learning's day count and the weekly summary read the same rule. 20 domain fixtures, 11 store fixtures (same-day repeats, next day, missed day, reload, duplicate write, DST, time-zone repeat, future and back-dated rows, an existing profile), 3 browser cases in `e2e/streak.spec.ts`. Negative cases N12 and N13 in `docs/content-gates-negative.json`. | Done. |
| **I-236** | Reproduced 2026-09-16: `recordAttempt` reaching `learned` left `characters_learned` at 0 because `trackActivity` had queued a state update first, so the second updater was deferred past the flag read. `creditActivity` now folds the activity row and `active_days` into the same updater from the same `prev`, idempotent under a repeated run; `updateProgress` and `recordNumbersEvent` credit inside their updaters. `store/streak.test.tsx` holds a Numbers lesson completing once. | Done. |
| **I-237** | 3,000-seed sweep on 2026-09-16: 139 distinct duplicate draws, all for ㅢ, whose pool joins its confusion group with the syllables built from its components. `answerability` had tried three seeds. The `read` builder now draws the pool wider and keeps the first three distinct romanisations; `answerable.test.ts` sweeps 400 seeds per character; negative case N14. | Done. |
| **I-238** | Read off the exercise inventory (`docs/exercise-inventory.json`) on 2026-09-16. `sameMeaning` in `features/learning/wordOptions.ts` used exact equality after folding; it now splits a gloss into senses and refuses a distractor when any sense folds to the same string as, or its content words are all among, a sense of the answer — in every locale, at build time of the question. `wordOptions.test.ts` holds 20 fixture pairs in both directions and five look-alike pairs that must stay available; negative case N16. The Level Test builder already had a shared-gloss-word rule and was not affected. | Done. |
| **I-239** | Measured by `scripts/exercise-inventory.mts` over every pair of same-level words in every pack. `buildDailyQuestions` now keeps the taught word's row and drops a later member whose meaning folds to one already on the grid; `repairCompletion` credits the dropped word through the question that did ask about it. `dailyQuestions.test.ts` holds the case; negative case N15. | Done. |
| **I-240** | All 902 pronunciation notes read against 표준 발음법 on 2026-09-16. `pronunciation.py`: the tensing recogniser skips ㅀ/ㄶ before ㄱㄷㅂㅈ and a sonorant cluster before a non-ending; a seventh pattern `insertion` (ㄴ 첨가, §29) with lesson copy in 32 languages holds 담요, 식용유, 큰일, 별일, 꽃잎, 나뭇잎, 일일이; §26 forms for 솔직하다, 솔직히, 일석이조. `qa_pronunciation.py` now carries pattern-label fixtures, makes every §29 candidate a named decision (91 read as liaison), and runs the pronunciation rows of `content/korean-gold-set.json`; negative cases N17 and N18. | Done. |
| **I-241** | `displayConjugations` in `packages/korean-morphology` substitutes the suppletive command on the card; the morphology itself is unchanged so a typed 먹으세요 still analyses to 먹다. Held by `conjugate.test.ts` and the `honorific_display` rows of the gold set; negative case N19. | Done. |
| **I-242** | Every gap-fill in `cloze.json` read four times on 2026-09-16 as the bank re-drew (608 items in the final bank; the builder is now seeded per item so a later re-read is a read of the items that changed). 42 pairs banned in `answer-conflicts.json`; 36 words whose frame admits any member of its class withdrawn from gap-fills with `ctx: 0` (they stay taught). A-24 to A-87 in `docs/AMBIGUITY_LEDGER.md`; negative case N20. | Done. |
| **I-248** | Seen first as two e2e cases (`choice-layout.spec.ts`, ko and ar) failing only inside a 29-minute run and passing alone. `apps/web/src/pages/reviewSessionHydration.test.tsx` mounts the page over a store that answers slowly with twelve due words: before the fix the empty state is shown and the question never arrives (2 of 3 fail); after it the first question appears and the empty state is never painted. Negative case N21 puts the mount-time resolution back and the two fixtures fail. A handed plan (`location.state.plan`) is unaffected; a profile with nothing due still reaches the empty state, after hydration. | Done: `ReviewSessionPage` resolves a fresh plan only once `ready`, draws nothing until then, and resolves once in an effect when the store answers. |
| **I-249** | Reproduced against the built bundle (`scripts/scratch/repro-chunk.mjs`, gitignored): a static server that 404s the Letters chunk; after tapping Letters `#root` had 0 children and the page error was `Unable to preload CSS for /assets/LettersPage-….css`. After the fix the same run renders the failed-screen panel — title, sentence, Reload, Go to home — with the bottom navigation still usable (screenshot in §20AD.1). `apps/web/src/ui/screenErrorBoundary.test.tsx` holds three cases: the panel appears instead of an empty tree, Reload reloads the document (a rejected `lazy()` never retries itself), and navigating away clears the failure. `screens:audit` gained a `screen failed` state, rendered on the five layout-deciding profiles including dark and 200 % text (171 renders green). | Done: `ui/ScreenErrorBoundary.tsx` around the routed screen in the tab, focus and not-found layouts, keyed to the path; `errors:screen.*` copy in all 32 locales. |
| **I-250** | `apps/web/src/store/storageWarning.test.tsx`: a driver holding a level-12 profile whose `getAll` rejects once; before the fix four writes reached the driver after one answered question and one preference change (negative run recorded in §20AD.2); after it none do and the stored row is byte-for-byte what it was. The verdict is `not-saving` with `reason: 'unreadable'`, and My Learning shows its own sentence — the stored progress could not be read this time, nothing from this session is kept, it is still on the device, reopen the app — rather than the sentence about a browser that cannot keep data. | Done: `LearnerProvider` detaches the repositories on a failed hydration (`wireRepositories(null)`), runs the session in memory, and re-wires them only for a restore from backup, which replaces the stored rows on purpose; `settings:storage.unreadable` in 32 locales. |
...
| **I-126** | `_concreteness` in `scripts/content/level.py` returns a template’s fixed weight for any tag in `ABSTRACT_TEMPLATES` and never consults the parts, so every `cmp:` antonym pair scores 0.80 and every `seq:` before-and-after pair 0.75 — a word is called unpicturable for having an opposite. 길다, 짧다, 멀다, 가깝다, 열다 and 닫다 are among the most picturable words in the language. The file’s own comment already records this class for verbs — it is what put 놀다 at 19 and 덩다 at 20 — and the fix then was the same one: move the words, not the weight.  **Fixed 3 September 2026.** `_concreteness` no longer returns a template's weight and stop. A `cmp:` or `seq:` tag with **two** parts, **both** picturable, on an adjective or a plain verb, now scores 0.15 instead of 0.80 — a contrast whose two ends can be drawn is drawable. 열다, 닫다, 길다, 짧다, 높다, 빠르다, 멀다, 가깝다, 서다 and 일어나다 all moved from 0.80 to 0.15 on that term.  The exception is narrow because two wider drafts were wrong and are recorded in the function's own docstring: letting *every* abstract template be argued with moved 414 words downward, taking 대출, 전세, 계약서 and 수수료 out of the high twenties; letting a picturable pole count outside `cmp:`/`seq:` moved 사망 (`solo:cross`), 좀비 (`solo:dark`) and 스트레스 (`solo:heavy`), whose tags use a pole as an icon hint. A one-part tag is an icon hint too — 모양 is `cmp:cloud`, 차이 is `cmp:picture` — and keeps the template's weight. A `-하다`/`-되다`/`-지다` **verb** keeps it as well: 비교하다 is `cmp:one\|two` and comparison is not a picture. Adjectives are exempt from that last rule, because Korean adjectives are overwhelmingly 하다 forms and excluding them would take the fix away from 깨끗하다 and 따뜻하다.  **45 of 3,333 words re-levelled**, every one a picturable quality or bodily action and every move between two and six levels: 깨끗하다 15→10, 무겁다 16→11, 넓다 16→11, 낮다 16→11, 느리다 17→12, 얇다 17→12, 어둡다 19→14, 좁다 19→13, 앉다 6→3. Every changed word was read; no override was added.  Re-run after the change: `vocabulary:level:qa` (every level valid, populated and harder than the one below), `vocabulary:level:audit` (a fortnight of new words at every level from 1 to 30), `vocabulary:recommendation:qa`, `dailyvocab:qa`, the level-test bank regenerated, and the 118 synthetic journeys — all pass.  **Re-measured 4 September 2026, and the residual has a different cause.** With the concreteness fix in place, the model was asked what it would give each of the 25 overridden words if the override were removed: 길다 9, 나쁘다 12, 놀다 11, 맛있다 10, 모르다 7, 싸다 14, 가깝다 8, 높다 8, 닫다 8, 멀다 8, 빠르다 8. Every override is still load-bearing, and the build reports none it agrees with. So the concreteness term is fixed and the words are still wrong, which means the concreteness term was not the whole cause.  The component breakdown says where the rest of it is. 길다 scores frequency 0.840, 짧다 0.852, 가깝다 0.813, 맛있다 0.855 — a frequency *cost*, where 1.0 is a word the corpora never saw. Those are ranks around 2,000 to 2,700 for first-semester adjectives, and the reason is I-73's: the frequency matcher counts surface forms, and an adjective lives its life inflected — 길어요, 긴, 짧은, 가까워요 — so the citation form ranks as if it were rare. The other half is 모르다, which *is* ranked correctly at 46 (frequency 0.473) and still lands at level 7, because linguistic 0.320 and semantic 0.361 outweigh it under `WEIGHTS`.  Neither is the antonym rule. I-126 is therefore recorded as what it is: its named cause is fixed in the model, the overrides that cause produced are kept because a *different* defect still needs them, and that defect is written down as I-133 with its measurement rather than left buried in this one.  **Re-measured 5 September 2026, after I-133 landed.** The usefulness ceiling is now a bound rather than a term, and it does most of what these overrides were doing one word at a time. The model's answer for each overridden word, with the ceiling applied and the override removed: 길다 5 (was 9, anchor 3), 나쁘다 5 (was 12, anchor 3), 놀다 5 (was 11, anchor 2), 맛있다 5 (was 10, anchor 3), 가깝다 5 (was 8, anchor 4), 닫다 5 (was 8, anchor 2), 싸다 5 (was 14, anchor 5 — the model now agrees and `build_vocabulary` reports the row as redundant).  So the gap the overrides assert has gone from four to nine levels down to one to four, and one of the twenty-five has stopped deciding anything. What is left is exactly what this issue's last measurement named: the frequency matcher counts surface forms, an adjective lives its life inflected, and 길다 ranks as if it were rare. That is a corpus problem, and it stays open here rather than being written off.  **Fixed at the source, 13 September 2026 (web pass, v1.0.5).** The residual named above — *the frequency matcher counts surface forms, and an adjective lives its life inflected* — is what `scripts/content/frequency.py` now reads correctly. `conjugate.frequency_forms()` gives every predicate a form model by class (V the verbal endings, C the connectives, N the adnominals, E the nominalisers, with the ㄹ-stem, ㄷ/ㅂ/ㅅ-stem and ㅏ/ㅓ-contraction alternants), and `measure()` runs in two passes: a form owned by one word is credited in full, a string two words share (긴 is 길다 and 기다; 나는 is 나 and 날다) is split in proportion to each owner's *unambiguous* evidence rather than given to both or to neither. A lemma guard from `content-cache/Category_Korean_lemmas.json` refuses an alternant that is a word in its own right. `content/vocabulary/frequency-fixtures.json` pins 96 forms of 20 words and `npm run frequency:check` (now the first step of `content:fresh:check`) runs them: 96/96 under the new reader, 52/84 under the old one. The re-ranking it produced: 있다 to rank 1 (was 3, and 118th under a naive first draft), 길다 893 → 568, 멀다 804 → 601, 힘들다 427 → 240, 만들다 171 → 94; 잇다 1 → 1,601, 긋다 4 → 2,718, 걸다 21 → 409, 갈다 28 → 380 — the four that had been living on other words' forms. 419 of 3,370 words changed level (179 harder, 240 easier); the 161 editorial anchors held, `vocabulary:level:qa` and `vocabulary:level:audit` pass, and three override rows were retired from `level-overrides.json` because the model now agrees with them (잇다 and 잘다 had been held down while the reader folded 있어요 and 잘게 onto them; 싸다's row was already reported redundant), with 싫다 pinned to 6 for an editorial reason recorded in the file (the other half of 좋다, level 1), which also keeps level 6 at the forty words a daily plan needs. Every override that survives was re-read against the new ranks and is recorded in `docs/VOCABULARY_LEVEL_CALIBRATION.md`. | Done. The concreteness term was fixed on 3 September, the usefulness ceiling on 5 September, and the frequency reader — the residual the two re-measurements pointed at — on 13 September. What remains is editorial: `level-overrides.json` is re-read after every batch and any row the model now agrees with is reported as redundant by `build_vocabulary.py --show-overrides`. |
...
| **I-147** | `ItemCard` draws `example_gloss` inside `{item.example && …}`, so on an item with no `example` it cannot appear. All six items of the *which system?* lesson and one time item were in that state; `example.ctxPeople` read *사람은 하나, 둘, 셋으로 세요*, which is the sentence `lesson.choosing.step1` already prints two screens earlier with the same three examples in it. `MISCONCEPTION_FEEDBACK.adjacent` is `null` on purpose — *정답은 오* is already on the screen — so nothing could ask for `rationale.adjacent`. | Done. The declarations and all thirty-three keys are gone from every bundle. `numbers:qa` §15 fails an `example_gloss` on an item with no example, and any `rationale.*` key that `exercises.ts` cannot ask for. |
| **I-148** | Found by reading the Korean bundle against the questions it answers. No automated rule could have found it: the string is present, translated, unique, and grammatical. | Done. Both glosses are relational and true under either convention: 한 주가 시작되는 날 and 주말 바로 앞의 날. Korean only — the other thirty-one bundles gloss the words as *Monday* and *Friday*, which needs no ordinal. Recorded as a rewrite with its reason in `docs/copy-audit-ko.json`. |
| **I-150** | The tick was drawn from `isDone`, which includes `review_due`. The rule at the top of `NumbersPage.tsx` has always said *completed and mastered and nothing else*; the code did not follow the comment above it. | Done. The tick follows the documented rule, and now lives inside the status pill rather than in a column of its own — see I-145. |
| **I-151** | `lessonStatus` asked `record.opened_at === null → 'available'` before it asked about any work. The page fires `lesson_opened` first in every ordinary sitting, so it is right almost always — and the stores are written optimistically, so `lesson_opened` can be the write that loses a race with a reload while the work that followed it was saved. | Done. `started_at` — set by the first step read, the first example seen and the first answer given — is asked first, so the status can only be wrong in the safe direction. `numbers:qa` §17 walks the state machine per lesson, including the transitions that must *not* complete a lesson. |
| **I-152** | `result/RELEASE_VALIDATION.md` named `2f282d9b`; `result/build-info.json` recorded `33291f0c` with `"dirty": true, "changed_files": 3`. `docs:consistency` reported the mismatch on the tree as delivered and had been reporting it since the artefacts were built. HEAD had moved two commits further on again, so the delivered APK was three commits behind the tree the report described. | Done. The artefacts are rebuilt from a clean checkout of HEAD, and `docs:consistency`'s existing rule — the one that found this — is what keeps the two files agreeing. See §20Q.5. |
| **I-154** | `docs:consistency` derives its figures and none of them was the version, the version code, the size of the issue ledger or the number of translated Numbers keys. Its commit rule compares two files inside `result/` and had no opinion about the report's summary of them. Measured on this tree before the fix: version 1.0.2 against a delivered 1.0.3, a stated code of 3 against a delivered 11, 142 issues against 153, and 272 Numbers keys against 269. | Done. `androidVersionCode`, `issuesTracked` and `numbersTranslatedKeys` are derived metrics now — so the default `docs:consistency` run rewrites them rather than reporting them — and a new rule compares the marketing version a document states, in its table **or in its front matter**, against the one `build-info.json` delivered; a version is not a number, so no numeric pattern could do it. The cover reads its version from `app.identity.json`, which is where the Gradle build and `version:check` read it from, and the subtitle no longer carries one. §2.1's prose rows were rewritten by hand. |
| **I-159** | `--only` is documented as the check you run *before* the full run. It narrows `entries`, and the manifest was written from `entries`, so it wrote a complete, valid, three-entry manifest over the real one. The run reported what it had done truthfully; nothing errored. Found by running it.  **Fixed 5 September 2026.** A partial run (`--only` or `--limit`) merges its entries into the existing manifest and rebuilds `version` from the merged set, rather than replacing the file. The prune step already refused to run for partial runs, for the same reason — half the guard was there. | Done. |
| **I-160** | `mobile:icons --check` compared every generated file against a fresh render and reported missing, stale and obsolete. Swapping the two source constants and re-running the builder moved both sides together, so the check stayed green on an Android launcher icon drawn from the iPhone artwork.  **Fixed 5 September 2026.** Each platform's delivered icon is re-rendered from the *other* platform's source and compared: byte-identical means it was built from the wrong file. Not a picture comparison — at 64 px the two sources differ by 0.48 of a level, so any perceptual threshold would be tuned to noise; provenance is compared, not artwork. The sources are also required to exist, be square and be their declared size. Negative-tested: a crossed pair gives 2 findings, a 256 px Android source gives 1, both exit 1. | Done. |
| **I-164** | It could not. Under a model in which both a wrong answer and a declared blank can only happen when the learner does not know, `P(wrong) = (1 − c)·(1 − known)` and `P(unknown) = 1 − known` differ by a constant factor, and a constant factor cancels when the posterior is normalised. Ten declared blanks and ten wrong answers at the same level produced identical estimates **to twelve decimal places**, and a sitting answered entirely *I don't know* asked exactly the same thirty questions as one answered entirely wrongly.  **Fixed 8 September 2026.** The two separate only if knowing can also produce a wrong answer, so the response model carries a slip term: `P(correct) = known·(1 − s) + (1 − known)·c`, `P(wrong) = known·s + (1 − known)·(1 − c)`, `P(unknown) = 1 − known`. A wrong answer is now compatible with knowing the word and mis-tapping; a declared blank is not.  `s` = 0.05 was swept against two simulated populations rather than chosen. Against learners who never mis-tap the whole sweep sits inside 0.04 levels of MAE — it costs nothing. Against learners who mis-tap 5% of what they know it moves MAE from 1.70 to 1.46. | Done. |
| **I-165** | The sitting was component state — an answer list, the current item, a used-id set and a deadline, none of which survived the screen unmounting. On a phone the leave is often not a decision: a call arrives, or the OS reclaims the tab.  **Fixed 8 September 2026.** Schema 14 adds `level_test_sitting`: the ordered ids already presented, the response to each, a per-sitting seed, the deadline and the interface language. Nothing about the *future* of the test is stored, because a stored list of remaining items would keep the questions stable and freeze the adaptation. Every later item is instead recomputed as a pure function of the seed and the question's index (`pickIndex`, FNV-1a), so a resumed sitting presents the same question, scores the same answers, and keeps adapting.  The deadline is stored with it, so force-quitting cannot buy more time, and the locale is, so a sitting is never resumed into a test re-rendered from a different language pack. A finished sitting is cleared in the same write that stores its result, so there is no instant at which the device holds both. The leave guard has gone with the thing it warned about.  `level-test.spec.ts` drives it through the built product across a full document reload; `upgradeCompatibility.test.ts` plants a complete learner at all twelve supported schema versions and proves the field arrives as `null` and nothing else moves. | Done. |
| **I-166** | `ce60516a chore(ios): bump version to 1.0.3 build 5` edited `App.xcodeproj/project.pbxproj` in Xcode and did not adopt `project-file.lock.json` with it, so `ios:project:check` — the fifth step of `verify:quick` — reported four findings on a clean checkout of `main`: the file's sha256 and blob id, and MARKETING_VERSION and CURRENT_PROJECT_VERSION reading 1.0.3 / 5 against an expected 1.0.2 / 4.  The gate was right and its instruction was the fix: the change was deliberate and made in Xcode, so `node scripts/check-ios-project.mjs --adopt` records the new state. The protected settings it also guards — development team, signing style, bundle identifier, deployment target and known regions — are byte-for-byte unchanged; nothing in the project file was edited from here.  **Fixed 8 September 2026.** | Done. The lock file is committed with this pass. |
| **I-168** | Three tables stated the APK and AAB sizes and hashes and no two agreed, and the delivered `result/build-info.json` agreed with none of them:  \| \| §2.2 and §18 \| §21's artefact table \| `build-info.json` \| \| --- \| --- \| --- \| --- \| \| APK \| 86.7 MB (88,242,570 B), `bcb94e2a…` \| 87,763,942 B (83.7 MiB), `7777b402…` \| 90,946,162 B (86.7 MiB), `96490914…` \| \| AAB \| 85.0 MB (86,451,898 B), `ca6fb668…` \| 85,973,157 B (82.0 MiB), `86b00c97…` \| 89,105,065 B (85.0 MiB), `5bf70c5c…` \|  One cell was internally inconsistent on its own: **86.7 MB (88,242,570 B)** pairs a size from the current build with a byte count from an older one, because the MB figure had been refreshed and the bytes in the same parenthesis had not. A fourth pair appeared in the prose beneath it — *the APK grew from 81.9 MB to 82.7 MB this cycle*.  The front matter had the same shape of fault: it said *artefacts built from commit bc3bec14 at versionCode 14* while `build-info.json` recorded a8e04b79 at versionCode 16.  **Fixed 8 September 2026.** Every artefact figure in the report is now taken from `result/build-info.json` for the build this document describes, and the shipped `docs/report.pdf` was itself stale — it predated the section it was supposed to contain. | Done. Regenerated after the rebuild, from one source. |
| **I-169** | Found by *reading* a rendered sample at eight levels rather than by any gate — the same method §7.2 exists to argue for. The floor at level 11 (I-162) cleared the beginner band; above it, six surfaces remained that are a grammatical form or a bound element carrying a rare Sino-Korean gloss, and therefore take their frequency rank from the ending rather than from the noun: 다가 (multivalent), 듯이 (as if), 먹기 (eating — 먹다 nominalised), 하이다 (a causative of 하다), 내야 (infield — 내다 plus -어야), 쓸데 (only exists inside 쓸데없다).  Reading also found two smaller things. `usable_dictionary_anchor` **documented** a rule refusing a plural whose singular is already an anchor and never applied it, so 새들 was in the bank alongside 새. And the reviewed drug exclusions named 대마초 and not 마리화나, which is the same substance under the loanword.  **Fixed 8 September 2026.** The six are a reviewed list in `learner-safety.json` under `notStandalone.grammaticalForm`, each with its reason written above it. They are listed rather than detected because no structural rule separates them from the words they resemble: 반면, 전면, 정의, 대가, 벌이 and 적의 decompose identically and are all ordinary vocabulary. A rule that removed 다가 would remove those, and a bank without 정의 is worse than one with 다가. The plural rule is now written as well as documented, and 마리화나 is on the list. | Done. |
| **I-173** | `phaseOf` returned `warmup` for the first three questions unconditionally, so the ladder was a schedule rather than an offer. **Fixed 8 September 2026**: the ladder holds only while the learner is answering correctly and ends on the first miss or decline, after which the bracket takes over and the next question is at or below the one just missed. Asserted directly, and negative-tested by restoring the unconditional branch. | Done. |
| **I-174** | The taught corpus is levelled by four weighted factors in `scripts/content/level.py`. Dictionary anchors had one: the frequency rank of their spelling. That is the same single-signal levelling that put 누가 at level 1 glossed "nougat", in its milder and commoner form — 67 anchors in the bank (7.7%) carry three or more senses and the rank is shared between all of them.  **Fixed 8 September 2026.** A dictionary anchor's rank is discounted by the square root of its sense count: 시기 moves to 18, 화상 to 19, 전력 to 17, 경계 to 15. The square root rather than the count itself because senses are not evenly used — one usually dominates — so multiplying would push ordinary polysemous words off the top of the scale. It is a heuristic and is labelled as one; what would settle it is per-sense frequency, which no corpus available here provides. It can only move a word up, and the failure it guards against is a rare sense asked too early. | Done. |
| **I-175** | `collideInAnyLocale` compares meanings through a map built from the taught corpus. Dictionary glosses are not in it, so any pair involving a dictionary headword missed the lookup and the check returned "no collision" — silently, for 875 of the anchors in the bank. Found by `leveltest:bank:check`, which reads the finished artefact rather than trusting the builder, immediately after the sense-count re-levelling moved new anchors into range.  **Fixed 8 September 2026.** English carries the dictionary glosses in the same map. One language rather than thirty-two, because that is where the glosses exist — and where the gap was. | Done. |
| **I-179** | Nothing was miscounted. `sessionProgress` counts distinct completed words against max(goal, plan.words.length) and the progress bar reads the same value, so there is one unit and one source; the plan really did hold three new words. The defect is that the smaller counter used a different unit in the same shape as the larger one.  The element is removed from the markup, from the accessibility tree and with its `wordStep` computation. No localisation key became unused — `learning:session.counter` still carries the day counter and the letter and review sessions. The 새 단어 / 복습 label stays: it is what tells a learner the app has noticed their level, and removing it would put back a defect already fixed once.  `wordSessionCounts.test.tsx` pins the unit across every shape a day takes — none, one, two, three or more, review-only, mixed, and a plan longer than the goal — that a word credited twice counts once, and that a reopened plan reads identically. | Fixed. `apps/web/src/pages/WordSessionPage.tsx`. |
| **I-181** | Running the suite prints `AssertionError: the sabotage no longer applies` and reports G4 as a problem, which is the harness being loud rather than the defect being caught. Re-anchored on the single line it actually needs, G4 catches again: `numbers:qa — 6 problem(s)`. The suite now reports 8 ok, 0 problems, including the new G6. | Fixed. `scripts/regression-gates-negative.sh`. |
| **I-183** | The fixture walks a learner from every stored schema version to the current one and asserts letters, words, sessions, attempts, the streak, the review queue, saved vocabulary, the daily plan's credited words, the level-test result and the content seed — but never read the `numbers` store. That matters because the v13 migration deliberately deletes Numbers rows: the first Numbers build wrote items into the *letter* stores under `number:` keys, and those are snapshotted to `meta` and removed. Nothing held the migration to leaving the real model alone.  A completed lesson is now planted for every version that has the store and has to arrive with its completion time and attempt counts intact. Removing the planted row fails two cases, so the assertion is doing work. | Fixed. `apps/web/src/storage/upgradeCompatibility.test.ts`. |
| **I-184** | `level-test.spec.ts` asserts that nothing on the question screen tells a learner how they are doing, and did it by scanning the whole page body for /correct\|incorrect\|wrong\|score/. The body includes the four answer options, which are English glosses of Korean words.  It passed for as long as it did by luck. When item selection changed in this pass, question one became 열다 and its options included *to be right, correct* — an ordinary gloss of 옳다 — so the gate reported that the app was telling the learner they were correct.  The scan is now taken over everything outside the answers group. Feedback would never be drawn inside that group, so the assertion still catches what it exists for while letting the vocabulary be vocabulary. | Fixed. `apps/web/e2e/level-test.spec.ts`. |
| **I-187** | `____이 마음에 들어요.` keyed 선물 against 길 · 전 · 정류장, and `이 ____이 마음에 들어요.` keyed 작품 against 부인 · 태양 · 행운. The second shows that a demonstrative in front of the blank does not constrain it. | Fixed. `isOpenEvaluativeFrame()` refuses the family, allowing one leading determiner. Both words were given constraining example sentences instead of losing their contextual item. |
| **I-191** | The gate blocked on four things: a key missing from the source locale, an incomplete plural set, a dropped placeholder, and a key the source does not have. Setting `common:actions.confirm` to "" in German and running `--check` exited 0. | Fixed. Two rules — *present but empty*, and *lost every word* for a value that is nothing but its placeholders where English said something. Both sweep the locale's own keys, because the source loop short-circuits on a plural base and a blank `_one` would have gone through the first shape of the fix as well. |
| **I-193** | §6.8 stated placement accuracy of 90.2% within ±3 and mean absolute error 1.64 against a simulated learner that fumbles; §12's table still carried 95.2% and 1.37 from the population that never fumbles. Eleven further measured figures were stale — question counts, option counts, bank size, the gate count in the architecture diagram, and the assertion count of the level-test suite. | Fixed. Every figure re-measured on this tree and both disclosures updated, English and Korean. `patent:evidence:check` continues to require that each cite the same paths. |
| **I-195** | `LevelTestPage` replays a stored sitting by looking each presented id up in the bank and **skipping one it cannot find**. An update regenerates the bank whenever the corpus moves. Two consequences: an answered item that has left the bank is dropped from the evidence the estimate is built from, and if the missing one is the question that was on screen when the app closed, `current` resolves to null and there is nothing to draw.  The three existing disqualifications — a changed language, an expired clock, a sitting whose own two records of itself disagree — are all about the *stored row*. None of them is about the bank this build ships. | Fixed. `sittingIsServable()` in `domain/levelTest.ts`; a sitting whose items this build cannot supply is not offered for resume and the learner starts a fresh one. That costs nothing — an unfinished sitting has never written a level — and it is the bar `isReadableSitting` already sets one layer out: discard rather than repair. |
| **I-199** | `leveltest:qa` measured *of a level-2 learner's first five questions, the share more than six levels above them* and reported 1.0% against a bound of 15%. A level-7 question put to a level-2 learner is five levels above them and was not counted. A level-8 sentence at question four was inside the threshold. | Fixed by replacing the measure rather than tightening it. `leveltest:policy` replays six beginner profiles across eight seeds through the real selection path and asserts that **nothing above the earned band is ever presented** — a property, not a share. The six-level metric is kept because it is comparable with earlier editions. |
| **I-200** | Given a level, `LevelTestPage` looks for the wanted kind at that level and falls back to level ±1 and ±2. Nothing bounded that search. | Fixed. The neighbour search is bounded by `reachCeiling`, and when nothing is left inside it the *kind* is dropped rather than the ceiling — a word question at the right level measures something, a sentence at the wrong one does not. |
| **I-202** | `settings no longer asks a learner to manage files, or explains the architecture` (journey.spec.ts) is a run of `toHaveCount(0)` assertions immediately after `page.goto('/me')`. A count of zero is satisfied by a document that has not rendered: the first poll succeeds and the matcher returns without retrying. On a quiet machine it beat hydration and passed in 2.4s; while another project on the same VM was compiling, the first poll landed after the screen had drawn and it failed in 11.8s and again in 12.2s.  What it then found was real: `f731bd43` (3 September) deliberately put *Save a copy* and *Restore a copy* back on that screen, and the case — written in the initial commit — still forbade `/Back ?up/i`. The product is right and the test was stale; the race is why nobody was told.  Two more cases in the same file had the same shape: the practice-style absence at journey.spec.ts:332 and the provenance absence at :780. | Fixed. All three now assert something that must be present before asserting what must not — the backup buttons, the voice control, the navigation — so the absences are measured against a screen that has drawn. The backup patterns are gone from the forbidden list and the reason is written in the file; **import**, **export** and the names of the machinery stay forbidden. |
| **I-209** | The cover said versionCode 21 against a delivered 22; §18.5 said built from bc3bec14 against ed44ed06, the APK grew from 81.9 MB to 82.7 MB against 86.9 MiB — the exact sentence I-168 names — and 13,738 audio slots against 13,996; §2.2 said 2,061 items per non-English language against 2,122, 619 note-carrying cards against 635, 786 recorded spoken forms in the paragraph under a table row saying 804, and 240 editorial bands against 237. | Nine figures corrected and four of them added to `docs:consistency` as derived metrics, along with two more phrasings of versionCode so the front matter's prose is read. The gate was green throughout and was right to be: it tracks 75 named metrics, none of these was one, and it did not read the front matter's describes line at all. |
...
| **I-220** | `domain/review.ts` `weeklyInsights`; the strings in `docs/UX_COPY_REMEDIATION_LEDGER.md`. | The sentence says what is measured in all 32 languages (이번 주에는 힌트 없이 32개를 맞혔어요 / You got 32 right without a hint this week); the heading is 얼마나 잘 기억하고 있는지; thirteen Korean strings rewritten; every Korean string read and recorded in `docs/copy-audit-ko.json`; `docs/UX_COPY_AUDIT.md`, `docs/UX_COPY_STYLE_GUIDE.md` and the generated `docs/UX_COPY_REMEDIATION_LEDGER.md` (752 rows) record the pass; `copy:remediation:check` fails if a ledger row's new text is not what the bundle carries. |
| **I-221** | `quotes.ts` rows `dream-big-pieces` and `carlyle-stepping-stone`; `docs/QUOTE_TRANSLATION_AUDIT.md` — one row per locale per quotation, generated from the runtime strings, the reviewer's judgments and the rendering measurements (768 measurements, 0 findings). | done — the human part is I-222. |
| **I-224** | `docs/child-safe-content-audit.json` before this pass: `quotations.items = 27`; the regex at the old `FAMILY_QUOTES` block; after reading the file properly the German article *die* was flagged through the English mortality list six times. | The family imports the library and scans every original, translation and byline in its own language: 22 items, 1,452 fields, 0 findings. The retired-word tombstones are also enforced at corpus ingestion now (`installBand`), so a band that reaches the app from a stale cache, an old backup or a mirror can never supply a retired word as an answer, a distractor or a search hit (`retiredIngest.test.ts`, negative-tested). |
| **I-226** | `apps/mobile/android/app/src/main/res/drawable*/splash.png` — 22 files, two artworks; `scripts/content/build_app_icons.py` `ANDROID_SPLASH` loop. | I-26's evidence is corrected below; the config comment says what the three frames are; `scripts/check-native-splash.py` tests the iOS image for wordlessness and the Android bitmaps for being the artwork, and states the locale gap (device locale for the native frame, app locale for the web one) rather than hiding it. |
| **I-227** | `docs:consistency:check` was failing on 24 claims when this pass opened, and the report said every figure was settled. Every current figure is now read from one generated file, `docs/content-inventory.json` (`npm run content:inventory`, in `verify:quick` as `content:inventory:check`); §1 states which sections are current and which are historical; the flagged lines were corrected from the inventory by `patch_report.py`, not by hand; the test counts were re-run. `docs:consistency:check` passes on the figures and, after the commits of the second reading, on the clean-tree rule. | Done. Keep it: a figure typed into the report without an inventory key is the defect. |
| **I-229** | `content/vocabulary/nikl-learner-vocabulary-2003.json` — the National Institute of the Korean Language's 5,965-word learner list (KOGL type 1, attributed in Legal & Licences) — is read by `scripts/content/learner_grade.py`; `level.py` takes the *lower* of the subtitle cost and the learner-list cost, and caps the editorial usefulness at the list's grade. 1,031 words moved down, 0 up, every anchor held, `vocabulary:level:qa`, `vocabulary:level:audit`, `leveltest:*` and `dailyplan:*` green; the negative case N7 removes the learner term and requires the level gate to fail. Landed in d06c7faa. | Done. The next evidence source to add is a spoken-Korean corpus, which is still not open data. |
| **I-233** | Root cause in `apps/web/src/pages/LettersPage.module.css`: `.sounds` was a flex row whose text column had no `flex: 1`, so the arrow followed the title. Every navigation card is now the same two-column grid (`minmax(0, 1fr)` text column, 24 px trailing slot, `justify-self: end`), the arrow is `aria-hidden` inside a `.rowTrailing` slot, the link keeps its 44 px height and its accessible name, and Arabic mirrors the slot to the left edge. `apps/web/e2e/letters-chevron.spec.ts` measures the rendered arrows in ko/en/ar at 320/360/390/412/430 px and 200 % text (every inset 18 px ±1, slot ≥ 24 px, no overlap, no sideways scroll) and fails on the old layout by 30–62 px. Screenshots from the production build in `docs/qa-screenshots/letters-chevron-*.png`. | Done. |
...
| **I-243** | All 3,867 entries (word, part of speech, level, English gloss, Korean definition, example, translation) read on 2026-09-16. Three tombstones (`retired-words.json`, category `editorial`), glosses and definitions corrected in the entry and in the 24 copy packs where the sense had drifted, the 한잔 example re-recorded in both voices, two level overrides with reasons, one part of speech. | Done. |
| **I-244** | `scripts/exercise-inventory.mts` writes `docs/exercise-inventory.json`: 20,302 rows over letter read/listen/distinguish/write, word read/produce/context/build, matching by level band, the Level Test bank, and the Numbers course, with 548,852 checks in 32 locales, a curriculum progression map, and per-row status labels (AUTOMATED CHECKED, MODEL-REVIEWED, HUMAN REVIEW REQUIRED). `exercise:inventory:check` is in `verify:release`. | Done. |
| **I-245** | `content/korean-gold-set.json`: 40 letters, syllables, final sounds, 27 sound-change rows with their §, particles including 로 after ㄹ, every supported irregular class, the suppletive honorifics, spacing, weak frames, gloss collisions, translation collisions and safety evasions, each family with must-not rows. Run by `apps/web/src/data/koreanGoldSet.test.ts` and `qa_pronunciation.check_gold_set`; `npm run gold:check`. | Done. |
| **I-247** | Measured in the production build at 320 px: fresh profile logo 115/71/29 px at 100/150/200 % text; streak profile 123/123/98 px. After the fix: 115/88/88 and 123/111/88, every one at the asset's own aspect ratio; `e2e/home-header.spec.ts` 25 cases green, `hdr-*.png` inspected. | Done: the brand header is a flex row whose heading shrinks first (weight 100, floor 88 px) and whose image scales with `max-width: 100%; height: auto`; the invitation chip folds onto two lines (`.streakStart`) and clamps at two as the last resort. `apps/web/src/ui/AppHeader.module.css`, `apps/web/src/pages/HomePage.module.css`. |
...
| **I-251** | Found by reading the exercise strings of all 32 locales (§20AD.3). Each fix is one row per string per locale in `docs/ux-copy-remediation.json` (old text, new text, reason) and `copy:remediation:check` holds the new text to the bundle. `locale:editorial-qa` now reads the register in the verb for Hungarian and Greek (koppintson/koppints, πατήστε/πάτησε) and carries a per-locale glossary (Czech: litera → písmeno); restoring any one of the three defects fails the gate (negative runs in §20AD.3). | Done. Model-reviewed, not native-reviewed; the rows are labelled so and I-17 stands. |
| **I-252** | §20AD.4 lists every removal with the proof it was unreferenced: an import graph over `apps/web`, `packages` and `scripts` from every entry point, a whole-repository search for each export by name, and the report's own image references against `docs/report-assets`. `typecheck`, `lint`, `test` and `build` are green after the removals and every `verify:quick` gate re-ran. | Done. |
...
| **I-132** | `.groupTitle` carried `padding-inline-start: var(--hg-space-1)`, which moves the first glyph without moving the element — an optical nudge for the uppercase tracking, applied to one of the two things sharing a rule.  `align:sections` measures a `Range` over each element's first text node rather than the element, on My Learning, Privacy and Legal, at five widths, 100/150/200% text, in Korean, German and Arabic — where the axis is mirrored rather than skipped. 60 groups, 240 lines. **Negative-tested**: restoring the padding produces 180 findings. | Done. |
| **I-139** | Both photographed by QA. The row, its stylesheet, its click target and the `weeklyReport` query that existed only for it are removed rather than hidden. The Backup sentence is rewritten in 32 locales to say what the button does; the upload claim stays on the Privacy screen, where it is the subject and can be supported. | Done. See §20O.9. |
| **I-142** | `docs:consistency` derives the web count and matches it in the suite table. The same number written as prose in two sentences matched no pattern, so the gate reported it as "not stated anywhere" and passed. A gate whose patterns are narrower than the document’s prose certifies the half it can read. | Done. Both prose shapes are patterns, so the total and its parts come from one derivation. See §20O.11. |
| **I-149** | The one occurrence in all 32 bundles across every namespace. The formatter could not have rescued it either: the placeholder renders a digit, and 35 is 삼십오 (를) while 20 is 이십 (을), so `endsInConsonant` returns null and falls back to the parenthesis it was written to remove. | Done. The sentence puts the number first — *{{value}} — 순서대로 눌러서 만들어 보세요* — and needs no particle. `numbers:qa` §16 fails any bundle that writes 은(는), 이(가), 을(를), 과(와) or 으로(로) longhand. |
| **I-15** | Regenerated, fixtured, checked on-device. | done |
| **I-153** | `locale:editorial` reported 79 warnings on this tree: 9 straight-apostrophe and 70 split-translation. The nine are mechanical and were fixed; the seventy are judgements about whether two contexts should share a sentence, which is a native reader's question and is why I-39 stays open. | Done for the nine. I-39's recommended fix now names the real number. |
...
| **I-167** | `patent/README.md` carried two file indexes. The first said the prior-art comparison documents had been replaced; the second, further down the same file, still listed them, and so did both attorney briefs — which is where a reader is told what to open first. The English disclosure also stated the shipped recording count as 6,870 in two voices; the correct figure is 6,938 utterances per voice, 13,876 recordings.  **Fixed 8 September 2026.** References corrected, the count re-derived from `apps/web/public/audio`, and `patent:evidence:check` added: it fails on any repository path, `npm run` command or figure the package cites that does not resolve, on an evidence-map row claiming verification with no reproduction command, and on a source path cited in one disclosure and not the other. It skips, and says so, when `patent/` is not in the checkout. | Done. |
| **I-176** | `leveltest:viewport` measures the question screen at 320×568, 360×640, 375×667, 390×844 and the same phone at 150% and 200% text, in English, German, Russian, Tamil and Telugu. No overflow, no clipped option, no overlap, and no control under 44px inside the screen itself. The navigation bar is under it on the five 200% combinations: a 390px phone at 200% lays out as 195 CSS pixels and five tabs cannot each be 44px wide in 195.  Reported by the gate rather than failed on, because it is arithmetic rather than a defect the gate can drive out, and because failing on it would make the gate unsatisfiable. Re-measured in the sixteenth pass with the type tokens actually doubled (the earlier measurement scaled the root font size, which no token reads — I-223): at 320 px the five targets are 64×64 px (ko, ar) and 64×67 px (ta, de), above the 44 px minimum, with labels on at most two lines. | Closed on re-measurement; see I-223 for the measurement method. |
...
| **I-192** | `SUSTAINED_CORRECT` is 8 and the paragraph beside it said "three consecutive correct answers". `EASY_EVERY` is 5 and the comment in `nextLevel` said "every third adaptive question". The simulated walk of a learner who answers everything correctly is 2, 4, 6, 8, 10, 12, 14, 16, 19 — eight two-level steps before the wide one is earned, which is the constant and not the prose. | Fixed, and pinned: a new case asserts that the climb is still capped one answer short of the threshold and released at it, reading the constant rather than a literal. |
...
| **I-204** | `NumberSessionPage` reads `record ?? blankLessonProgress(...)` and takes `attempt` from it in a `useState` initialiser. Before hydration the record is the blank, so `attempts.total` is 0. Same root cause as I-203 — a screen reading learner state before `ready` — and the same shape, `useState(() => …store…)`.  Unlike I-203 the consequence is cosmetic: the run is seeded differently, no progress row is written from the blank, and completion is derived from evidence in its own store rather than from this value.  **Re-investigated 13 September 2026 (web pass, v1.0.5) — not reproducible on this tree, and now pinned by a test.** `NumberSessionPage` has gated the exercise phase on `state.storage.checked` since 8b489ebe: the run is not built until the profile has been read, so the seed is taken from the stored attempt count, not the blank. `apps/web/src/pages/numberSessionSeed.test.tsx` proves it the way a slow launch would: a durable driver whose every method resolves late, a stored Numbers row with attempts already on it, the page rendered through the real router and storage — and the first practice question is the one `practiceExercises(lesson, storedAttempts)` builds, not attempt 0. Negative-tested by seeding the attempt from `useState(() => 0)`: the test fails. The same harness found that `wordSessionHydration.test.tsx` had been measuring a thrown error rather than a slow hydration (an unbound method on its proxy driver); it is bound now and still passes. | Done — nothing to change in the page; the guard predates this issue. The regression test is what was missing, and it exists now. |
...
| **I-231** | Counted by `docs/content-inventory.json` `duplicates.shared_english_glosses`: 108 when the pass opened, 0 now. 32 were rewritten in the first reading (나다 *to come out or to arise*, 꺼지다 *to go out, as a light or a fire does*, 지다 *to lose a match or a fight*, 삶다/끓다, 맡다/냄새나다, 검색/수색, 여러분/다들, 바꾸다/변하다 and the rest); the remaining 76 in the second reading, each pair read together so that the gloss names the distinction (늘 *always, as a habit* / 언제나 *always, at any time*; 매우 *very, in writing*; 묻다 *to ask, the plain verb*; 지지 *backing for a cause or a person* / 지원 *aid given to help someone*). 정말 gained its adverb part of speech; 찍다's Korean definition was trimmed to the one sense its card teaches. `leveltest:ambiguity:check` and `leveltest:distractors:check` stay green. | Done. The inventory count is the gate: a new batch that reuses a gloss shows up there. |
...
**A fifth, added this pass, and it is the one that matters most for §10.** The
Vocabulary Level Test's item difficulty is **curriculum-based**: a word's
frequency rank in Korean, or the level the teaching curriculum assigns it. It has
never been calibrated against learner responses, and it cannot be from here —
the application opens no network connection at runtime and collects nothing, so
there is no response data and no way to gather any without a change to the
privacy position that has not been made. Every accuracy figure in §10 is against
a *simulated* learner answering the way the model says someone of that ability
would. That is a check on the machinery and not on the assumption underneath it:
a simulation will report excellent accuracy for a badly calibrated bank for
exactly as long as the bank is wrong in the same way the simulation is. **The
claim §10 supports is that the test now asks a sensible sequence of answerable
questions and reports a stable number. It does not support the claim that level
20 means anything about a person.**

**An eighth, added in the fifteenth pass, about what a lexical policy can and
cannot promise.** The child-safe content policy (§20Y) is deterministic by
design: it finds what it names, in every inflection, spacing, spelling,
romanisation and translation that it names. It does not find a term absent
from every list, and the thirty non-Korean, non-English lists were written by
the engineer who wrote the policy and have been read by no native speaker
(I-213). Its frame and context rules catch the compositions of innocent words
that are listed — 여자를 타요, 아이를 괴롭혔어요 — and no others. The scan of
the shipped commit found 4,333 findings the previous gate had certified as
none; the fixed tree scans at 0 under the same policy, which is a claim about
the policy's lists and not about the language. What the gates *do* guarantee
from this edition on is structural: nothing reaches a learner — pack,
dictionary, bank, gap-fill, cache, bundle or package — without passing the one
policy, so a term added to the policy is removed everywhere at once.
...
structurally. Whether the items that *remain* at level 3 are level-3 words is the
same unanswered question it was before, and the same person answers it.
...
## **RELEASE CANDIDATE — EVERYTHING THIS REPOSITORY CAN SETTLE IS SETTLED**
...
person in a minute and to no gate. This edition adds a third of the same kind and
it is the reason for §20V: a reader photographed `일곱 시에 ____.`, a question
with two right answers, and the gate written for exactly that class passed it
because its test was for an argument particle and `시에` is one. **A rule that
tests a proxy passes everything the proxy does not distinguish.**

This edition adds a fourth, of a kind the previous three did not cover. A reader
photographed `창문으로 아침 ____이 들어와요` — 목적 · 비빔밥 · 빛 · 환경 — and
every gate passed it, correctly, because every gate asked *is there a second
right answer?* and there is not. **A question fails in two directions and only
one of them had a gate for six editions.** Nothing was measuring whether the
wrong answers were worth reading, and the rule written to stop the first failure
was, by construction, selecting for the second: all 625 contextual items in the
shipped bank had every distractor from a different subject area than the answer.
Pulling that thread also found two frame guards that had never executed — one of
them the fix §20V.1 recorded for I-185, believed on the strength of an item
leaving the bank for an unrelated reason (§20X.3). **A green gate does not prove
the code it was written about ran.** Three checkers
in this cycle were measuring something adjacent to their subject — the ambiguity
rule, the question ledger's answer field, and five locale gates that read the
translation files without ever comparing a translation to what it translates. That is why this edition does not use the
...
The suites run green in full: 3,206 unit cases across four packages — 1,570
web, 1,298 content safety, 242 Korean morphology, 96 handwriting — 640 end-to-end, 171 rendered
...
This edition adds a fifth kind, and it is the one that should worry a reader
most: **a measurement that measured nothing.** Nine QA scripts and specs said
"200% text" and scaled a root font size that no pixel token reads, so every
large-text figure this document had carried was the normal size counted
twice; and the safety scan's quotation family had read 27 strings of 1,052.
Both were green. Both are re-run for real in §20Z.8, and the rule they add to
the ones above is that **a gate has to be shown to change its answer when
the thing it measures changes** — the negative runs in §20Z.10 are that.

The nineteenth pass adds a sixth kind, and it is the quietest: **Korean that
was correct to every rule and wrong to a Korean.** 싫다 → 실타 was the right
sound under the wrong name; 먹으세요 was impeccable morphology and the form a
teacher corrects; 담요 → 다묘 satisfied the transcription rules and not the
standard's own example for §29; 의사가 환자를 보호했어요 passed every frame
rule and is a sentence. None of these was found by a gate. They were found by
reading every entry, every note, every gap-fill with every option in it and
every panel row as Korean, and then each was turned into a fixture that the
code had not written (`content/korean-gold-set.json`) and broken on purpose.
The reading was done by a model, and the labels say so: the Korean side of
the content is MODEL-REVIEWED, and no line of it is NATIVE-SPEAKER REVIEWED.
The conclusion this edition is entitled to is the narrow one: **no known
repository-actionable P0/P1 linguistic defects remain under the documented
full-corpus checks; the remaining human linguistic validation is named in
I-17, I-213, I-222, I-230 and I-246.**

The twentieth pass adds a seventh kind, and it is the one a reader of a
green suite should expect least: **a failure path that was correct about the
learner and wrong about the store.** The launch that could not read the
profile did everything the tests asked of it — a working app, no false alarm
about the browser — and then went on writing, because nothing had ever
followed that path to its first write (§20AD.2, I-250). Beside it, a class
that had no test because it had no code: a lazily loaded screen with no
error boundary above it (§20AD.1, I-249). Neither was found by a gate;
both were found by reading the code with the question *what happens next*,
and both are now held by tests that were shown to fail with the fix removed.
The edition also records, for the first time, what a reading of the
interface strings in thirty-two languages finds that a pronoun-counting
register rule cannot (§20AD.3), and it says in one place what it could not
do: build for iOS (§20AD.6).
...
report's own §2.1 said **1.0.2, versionCode 3**. Three documents, three
different answers to *what is in this folder*. `docs:consistency` had been
reporting the first of them since the artefacts were built and nothing had acted
on it; the version and the versionCode were outside every rule it had. They are
derived now (§20Q.6, I-152, I-154).

What remains are nine limitations. None is a defect, all nine are things a
buyer is entitled to know before release. The ninth is new this edition; the
eighth is the price of §20X; the second closed in an earlier one and is kept so that a
reader diffing against it can see where it went, and closing it made the third
larger:

1. **3,864 taught words against a stated target of 10,000.** The store copy
...
5. **What the question gates decide is narrower than what a reader wants
   them to.** Twenty-seven mechanical rules for ambiguity, nine more for
   distractor quality, five new frame classes, and a reviewed list of word pairs
   that no rule decides. None of it establishes that a Korean sentence has one
   defensible answer, or that a plausible wrong answer is actually wrong; each
   rule removes a class of question that cannot be either. The residue is a
   person reading, and this cycle's reading is recorded in
   `docs/AMBIGUITY_LEDGER.md`, `docs/CONTENT_QUALITY_AUDIT.md` and the `ctxWhy`
   beside each of the 54 refused items. A regenerated bank draws different
   distractors from the same stems, so the rules and the refusals carry forward
   and the reading does not.
6. **The level test places a simulated learner within ±3 levels 86.9% of the
   time, where it was 90.2%.** The 3.3 points were spent on the evidence gate in
   §20W and the trade is stated rather than hidden: a sitting now spends its
   first ten to twelve questions climbing through bands that a strong learner
   would previously have skipped in four, which leaves less evidence at the top
   of a thirty-question sitting — the 26–30 band is under-reported by 1.68 levels
   where it was 1.16. A test that measures an advanced learner half a level
   better by asking a beginner questions they cannot read is not the better test,
   but the number moved and a reader is entitled to it. `leveltest:qa` fails
   below 85% and prints the measured value on every run.
7. **The difficulty model still mis-scores a class of word** (`I-126`).
...
8. **The beginner band's gap-fills are hand-written and carry no audio.** The
   frame rules in §20X refused 174 generated items and most of the loss fell
   below level 6, because a level-2 card sentence is three words long and cannot
   constrain a blank. Twenty-two items are written by hand to replace them, and
   they are a different sentence from the card's — so the card's example
   recording is not a recording of them, and the renderer plays nothing rather
   than the wrong thing. Levels 1–5 hold 36 gap-fills where they held 50; levels
   6–10 hold 19 where they held 56. Recording the curated sentences needs the
   speech plan to learn about a second source of Korean, which this pass did not
   do.
9. **The two new quotations have been read by nobody but their author.**
   Sixty-four translations and sixty-four bylines ship; 21 rows are marked for
   a native speaker and the Carlyle attribution is conventional rather than
   located (I-222). The tables in `docs/QUOTE_TRANSLATION_AUDIT.md` say per row
   what a person has to confirm, and the rendering — every locale, six widths,
   two text sizes — is measured rather than assumed.

## 23.1 Issue ageing

Every issue that has appeared in more than one edition of this report, with what
was tried, why it was not enough, and what it is waiting on now. An issue is in
this table because it *survived* a pass, which is a different fact from its
severity.

| Issue | First seen | Editions | Fixes attempted | Why they were not enough | Root cause | Now | Evidence | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **I-03** | 1st | 8 | `NextStepCard` written, `routing:check` added, `.env.example` documented | Every one of them is the repository half. The card renders when the value exists and renders nothing when it does not, which is the correct behaviour for a value nobody here has | The destination is a business fact — a landing page, a store listing or a universal link — and it is not in either repository on this machine | Repository side complete. Set `VITE_HANGYUL_URL` at build time | `routing:check` reports which way a build went | **BLOCKED — external** |
| **I-04** | 1st | 8 | 2,581 → 2,948 → 3,333 words, each batch hand-authored through `content:build`, `examples:qa`, `vocabulary:sense:qa`, `conjugation:qa` | The gap is 6,667 words. Three passes have moved it by 752 | Authoring, not engineering: the delivery machinery is proven at 3,333 — bands, audio, level assignment, 32 locales | The target lives only in `docs/VOCABULARY_DATA.md`; no customer-facing string claims 10,000, and the store copy states 3,333 in all eight languages | `content:coverage`, `bundle:budget` | **OPEN — content** |
| **I-13** | 2nd | 7 | Wiktionary relation extraction, re-run after every expansion | 245 → 294 words carrying a relation across 752 new words, because the builder records only what two headwords state about the *taught* sense | There is no conservative open source for Korean lexical relations at this scale | Deliberately not fixed. Inventing similar words would be worse than a sparse section | `vocabulary:relations:qa`; the Level Test was checked separately for unrecorded opposites | **OPEN — accepted** |
| **I-17** | 1st | 8 | `locale:content:qa`, `translation:semantics`, `locale:editorial`, the copy ledger | Each one narrows what a machine can see. The Portuguese pack was written in European Portuguese in a pt-BR product for four batches and every gate was green | No automated check substitutes for a speaker | 32 languages await a reading, Korean among them | `docs/LOCALIZATION_NATIVE_REVIEW.md` | **OPEN — people** |
| **I-19** | 3rd | 6 | 10 → 20 complete locales; the 12 partial taken from 100 to the 600-word core band | 2,733 words × 12 languages remain, and completion brings four further debts (§20I.3) | Authoring | The picker names each language's depth before the learner chooses | `locale:content:qa` — 20 complete, 12 at 600 | **PARTIAL — content** |
| **I-20** | 3rd | 6 | 35 → 75 words carry the long note; all 38 inside the core band written in every partial language | 2% of the corpus | Authoring, one paragraph at a time | The machinery has been there since the block existed | `vocabulary.en.json` | **PARTIAL — content** |
| **I-39** | 4th | 5 | `locale:editorial` written; 78 defects found and corrected, including five German screens addressing the learner as *Sie*; the 70 split-translation warnings closed | It reads register, punctuation and duplication. Whether the prose reads naturally in Tamil is outside what it can ask | The same gap I-17 names | 0 errors, 0 warnings, and still unread by a speaker | `locale:editorial` | **PARTIAL — people** |
| **I-79** | 5th | 4 | 417 → 477 words at levels 28–30 | Re-measured this pass: 477 distinct words in the level-30 zone is 47 days at ten a day, which is the same seven weeks the issue was opened for | Authoring, at the top of the scale where words are hardest to choose | 0 short days inside the simulated window; the ceiling is real and further out than it was | `vocabulary:recommendation:qa`, 30,000 events | **OPEN — content** |
| **I-126** | 6th | 3 | `_concreteness` corrected for `cmp:`/`seq:` tags; 18 words moved by hand; the usefulness ceiling added (I-133) | The ceiling clamps a word that an editor marked *needed first*; it does not correct the frequency term that put it there | The frequency table counts written Korean, and the words this mis-scores are spoken ones | The overrides each name the cause; the ceiling narrowed the list rather than retiring it | `vocabulary:level:qa`, 162 anchors | **PARTIAL — needs a corpus that does not exist as open data** |

Three issues were opened *and* closed in this edition — I-158, I-159 and I-160 —
and none of them appears above, which is the point of the table: an issue that
survives a pass is a different kind of thing from one that a pass fixes.
...
task. `I-126` no longer belongs on this list: it moves a word by at most a band
and the ceiling holds it inside the one an editor chose, so it is a modelling
debt rather than something a learner meets.

The screenshots that opened this cycle are the argument again, and this time
they say something sharper than *look at the screen*. Both defects had a gate
watching them and both gates were passing, because both were measuring the
quantity the code computes rather than the one a reader sees — a badge's fill
rather than the ring it is painted with, a row's boxes rather than where its
letters start. An empty box is perfectly aligned; a disc clamped to its own
radius is exactly inside the box it is being cut by. Every check in this
repository is now worth asking the same question of: **is this measuring the
thing, or the number the thing was built from?**

<!-- issues:next -->

| ID | What | Why it matters | Effort |
| --- | --- | --- | --- |
| **I-04** | 3,864 of a stated 10,000 words | Buyers compare corpus size | HIGH (content) |
...
| **I-222** | 21 quotation translation rows and one attribution need a person | Nine Quote 1 rows and twelve Quote 2 rows are grammatically and semantically sound as far as the reviewer can establish and are marked NATIVE-SPEAKER REVIEW REQUIRED for register and idiom (vi, th, ar, bn, fil, hi, kk, ky, mn, ta, te, uz); the Carlyle attribution is conventional, not located in his works. | a reviewer per language |
| **I-246** | The exercise inventory's gloss-overlap heuristic flags 30,736 answer/distractor pairs across 32 locales for a human reader, and no locale is native-reviewed | Most hits are look-alike pairs by design ("here"/"there"); the pairs the runtime now refuses are the confirmed class. The remaining hits are a reading list, not a defect list. | LARGE |
...
| **I-230** | A 32-locale automated translation audit and a 90-row model reading: three wrong rows found and fixed, and no row is native-reviewed | A Tamil learner was shown துலங்கு (to shine) for 반응하다 (to react); a Mongolian learner an adjective that does not mean sweet under 불고기; a Brazilian learner the European Portuguese *a miúdo* under 비일비재. | HIGH (human) |
...
| **I-79** | A learner at the top of the scale runs out of new words in about six weeks — and the thinnest zone is no longer the top | Levels 28–30 hold 436 words between them. A learner placed at 30 draws from that zone and, at the default ten new words a day, exhausts it in 43 days if every word is new (87 days with the plan's reserved-new share). The thinnest zone is now 24-26 with 191 words: 19 days at ten a day. | HIGH (content) — the hard half: words that are advanced and ordinary |
| **I-20** | The hand-written More about it block is on 99 words of 3,370 | Word Detail is no longer a short page followed by nothing, but the paragraph written for the words where one line genuinely is not enough is on 99 of them — 3% of the corpus. | MEDIUM (content) — one paragraph per word, in ten languages |
