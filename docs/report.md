...
| **Version** | **1.0.3**, Android versionCode **11** — read from the delivered APK with `aapt2 dump badging`. iOS is deliberately behind at `MARKETING_VERSION` 1.0.2 / `CURRENT_PROJECT_VERSION` 4, because that project file is Xcode-managed and is not edited from this machine; `app.identity.json` records the pending action and `version:check` prints it |
...
| Numbers translated keys | 269 × 32 | `numbers:qa` |
...
**What is still open here, stated rather than fixed.** `MyPage` removed the
switch that turns `sound_free` on — the word questions it existed to avoid no
longer exist — so a learner arriving today cannot set it, and only a profile
that had it already carries it. The Numbers course honours the setting and it
honours missing audio; it cannot honour a preference the interface will not let
anybody express. The letter side has the same gap: `sound_recognition` and
`distinguish` are genuinely heard-only exercises, and the alphabet curriculum
was out of scope for the pass that removed the switch. Neither is a regression
introduced this cycle and both are a reason a native accessibility review is
still owed.
...
**What is not fixed.** `MyPage` removed the switch that turns `sound_free` on,
for the documented reason that the word questions it existed to avoid no longer
exist — so a learner arriving today still cannot set it, and only a migrated
profile carries it. The Numbers course honours the setting and the missing
audio; it cannot honour a preference the interface will not let anybody express.
That is stated in §15 rather than fixed quietly here.
...
| **I-154** | Release engineering | **P2** | The report's own metadata table described a build that does not exist | §2.1 — the first table a reviewer reads — said **1.0.2, Android versionCode 3, iOS CURRENT_PROJECT_VERSION 3**, through two releases that shipped 1.0.3 at versionCode 11. It also named commit `3833da71` and called the working tree *not clean*. A reader trusting the table would have gone looking for an artefact nobody built. | **RESOLVED** |
...
| **I-146** | All nineteen lessons list `listen_choose` in `exercise_kinds`. `settings.sound_free` has existed since §36 and `domain/review.ts` has always honoured it; `features/numbers/exercises.ts` never asked. Neither did the course ask the player, so a build with no clips in it or a manifest that failed to load produced the same dead end from the other direction. | Done. `practiceExercises`, `masteryExercises` and `exerciseCoverage` take a `soundFree` option and drop the heard-only kind; `NumberSessionPage` reads `settings.sound_free` and the player's own `ready && !available`, once, when a run mounts, so a manifest that finishes loading mid-check cannot change the questions or the count printed on the way in. Dropping the kind is enough: `numbers:qa` §11 measures that every lesson still asks every item in both phases. Two lessons — minutes and weekdays — fall to one question shape, which the gate reports rather than hides. The residual is stated: `MyPage` removed the switch that turns `sound_free` on, so a learner arriving today cannot set it; that is I-39's neighbour and is written up in §15 rather than fixed quietly here. |
...
| **I-154** | `docs:consistency` derives 57 figures and none of them was the version, the versionCode, the size of the issue ledger or the number of translated Numbers keys. Its commit rule compares two files inside `result/` and had no opinion about the report's summary of them. Measured on this tree before the fix: version 1.0.2 against a delivered 1.0.3, versionCode 3 against 11, 142 issues against 152, 272 Numbers keys against 269. | Done. `androidVersionCode`, `issuesTracked` and `numbersTranslatedKeys` are derived metrics now — so the default `docs:consistency` run rewrites them rather than reporting them — and a new rule compares the marketing version a document states against the one `build-info.json` delivered, which no numeric pattern could do. §2.1's prose rows were rewritten by hand. |
