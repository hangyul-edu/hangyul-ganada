...
| **I-33** | Content | **P4** | A secondary category can come from a sense the card does not teach | 김치 is tagged *communication* as well as *food*. Secondary tags feed search and recommendations rather than the browsing structure, so nothing shows a learner the word under a wrong heading — but a recommendation could. | **OPEN** |
...
| **I-33** | Wiktionary topics are page-level, not per sense. 김치's page carries `Kimchi` and `Photography`, because 김치 is what Koreans say instead of "cheese" for a photograph — an interjection sense the card does not teach. `categories.WIKTIONARY_TOPICS` maps Photography to communication, so the food card picks it up.  Found by `content:vocabulary:check` after the dictionary fetch grew the wikitext cache from 13,495 pages to 50,128: 김치, 교실 and 만두 changed category as new topic data arrived. 교실 gaining *home* from the `Rooms` topic and 만두 gaining *food* from `Foods` are both right.  This is the same root cause as I-18 — a page describes several senses and the card teaches one — at the one place a `senseId` cannot reach, because the topics are not attached to a sense in the source. | Take topics from the sense rather than the page, which the source does not support; or curate the mapping so that a topic only contributes when it agrees with the gloss-matched primary. The second is cheap and would have caught this one. |
...
| **I-33** | A secondary category can come from a sense the card does not teach | 김치 is tagged *communication* as well as *food*. Secondary tags feed search and recommendations rather than the browsing structure, so nothing shows a learner the word under a wrong heading — but a recommendation could. | LOW — a rule in categories.classify, and a re-read of what moves |
