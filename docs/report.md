...
**RELEASE CANDIDATE — EVERYTHING THIS REPOSITORY CAN SETTLE IS SETTLED.** Every
engineering gate that can run here runs green, the artefacts are built from a
committed tree with `source_state.dirty` false, and `release:current` reports
both delivery manifests at HEAD. Of the eight issues not marked resolved, none
is a defect in this code or content: three are content authoring at a scale this
pass could not reach (I-04, I-20, I-79), two need a speaker of each language
(I-17, I-39), one needs a value that only the product's owner has (I-03), one
needs a spoken-Korean frequency corpus that is not available as open data
(I-126), and one is a deliberate refusal to invent data (I-13). I-19 — word meanings
complete in twenty languages and six hundred words deep in twelve — closed this
cycle, and it closed by writing the rows rather than by restating the claim.

**Two things stand between this document and shipping, and neither can be done
from this machine:**

1. **A speaker of each language reads the pack.** 32 languages, and Korean is
   one of them. §22 and I-17 say what has and has not happened.
2. **The signed binary runs on a physical handset.** The matrix is written out
   in `result/RELEASE_VALIDATION.md`; everything here was measured in headless
   Chromium at phone viewports.

The previous edition of this line named a third reason — *the tree is
uncommitted, so `release:current` is pending by design*. That one was inside
this repository, and it is gone: the tree is committed, the delivery is at HEAD,
and a reason that a build could fix should never have been sitting beside two
that no build can.
...
A 30-item adaptive placement test over 30 levels and a **4,199-item** bank, 596
...
against the current corpus — rebuilt again this pass at 3,393 words — and it had to be, because the bank was once
...
The bank is rebuilt from the corpus on every content change, and the corpus
changed twice this cycle. The current figures are printed by
`npm run leveltest:locale:check`; the shape of the answer is that a language's
bank is built from the words it has meanings for, and **every language now has
meanings for every word**, so the three-row table this section used to carry —
English, nine complete locales, and *the other 22* at a fraction of them — has
collapsed to one row. That collapse is the finding, not the count.
...
all 4,199 items and passes — **after** the bank was rebuilt. On the shipped bank
...
**Blocked outside this repository: 1 · Partial: 4 · Resolved: 148**
