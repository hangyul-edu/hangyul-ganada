...
version: 1.0.4
web_version: 1.0.5
native_status: not modified or built in the seventeenth pass — delivered Android versionCode 25 / iOS build 25
date: 13 September 2026
describes: The seventeenth pass, web only — the Android and iOS projects were not modified or built and the delivered artefacts stay at 1.0.4, versionCode 25; the web product is v1.0.5. Four large-text gates the previous pass had not reached were measuring normal text and now scale the tokens; a correct second attempt at a letter was graded over the rejected first one; the end of the alphabet named nothing to do next; the frequency reader that decides a word's level credited 있어요 to 잇다 and read 길다 as rare, and was rebuilt at the source with 419 words re-levelled and every anchor held; thirty dictionary-shaped glosses became glosses, 24 notes were written in 32 languages, 17 curated beginner gap-fills recorded in both voices; the child-safety scanner refuses spelled-out jamo and Latin terms hidden in Korean fields. Every gate added was broken on purpose and shown to fail. The sixteenth pass, which this builds on, began with four customer screenshots and a version number. The tree carried 1.0.6 on Android and 1.0.3 on iOS with nothing ever uploaded under either; the intended release is 1.0.4 and every file now says so, at build number 25 on both platforms. A cold start on Android 12+ showed an orange brand mark the approved splash does not carry; the system frame is now cut from the artwork itself and a gate proves no other mark is referenced or packaged. Two requested quotations were added in 32 languages with an honest attribution — one as author unknown, one as attributed — and a locale-by-locale audit of both, generated from the runtime strings and 768 rendering measurements. Show answer in Today’s Words printed a sentence restating the option and counted the tap after it as a pass; a reveal is an outcome now, highlighted in blue, counted as nothing, retried later. Four gap-fill chips could wrap three and one; they are four across or two by two, by measurement. The word-ordering tray and the matching grid say the first action and grade only on Check. Every Korean interface string was read and thirteen rewritten, the recall sentence in all 32 languages, because it misdescribed its metric. Nine large-text measurements this document had reported were found to have measured normal text, and were re-run for real. Artefacts rebuilt at 1.0.4, versionCode 25.
...
**Current figures and historical figures.** A number in §2.2, §8, §9, §10.8, §11.1, §14, §16 and §20AB is a *current* figure: it was read from `docs/content-inventory.json` (`npm run content:inventory`) or `docs/report-metrics.json` for this edition, and `docs:consistency:check` fails if it drifts. A number inside a pass narrative (§20 through §20AA) is the figure *at that pass* and is left as written — 3,333 in the fourteenth pass, 3,370 in the seventeenth — so that the passes can be read as history. Where a narrative said *now*, the current value is in §2.2, not in the narrative.
...
| --- | --- | --- |
| Words shipping | 3,867 | `vocabulary.json` `words.length`; the same figure is `taught_entries` in `docs/content-inventory.json` |
| Categories | 18 | `vocabulary.json` `categories` |
| Study sets | 781 (five words each) | per category, ⌈words ÷ 5⌉ summed |
...
| Pronunciation notes | 902 | words carrying a `say` field in `vocabulary.json` |
| Of those, shown as a note on a card | 699 | `pronunciation.note_for`; liaison is taught once in the lesson instead |
...
| Unwritten vocabulary rows | 0 | every one of the 32 content packs holds all 3,867; `translation:audit:check` reads every row |
| Long *More about it* definitions | 99 | third element in `vocabulary.en.json` |
| Level-test items, English | 4,191 | `public/level-test/manifest.json` |
| Level-test contextual items | 613 | the bank, `kind === "context"`; 39 of them hand-written and recorded in both voices |
| Level-test reach, every non-English language | 2,253 items each | `manifest.json` `reach` |
| Level-test distinct words asked about | 1,790 | `leveltest:bank:check`; `docs/content-inventory.json` |
| Level-test items at levels 1–10, from the curated corpus | 100% | `leveltest:bank:check`; see I-162 |
| Audio clips | 15,840 | distinct files in `public/audio/manifest.json` |
| Audio voice slots | 15,960 | the same manifest, two voices per entry |
...
| Levels held to an editorial band | 233 | `lvm` in `vocabulary.json`; see I-133 |
| Issues tracked | 230 | `docs/issues.json` |
| Signed APK | 90,552,277 bytes (86.4 MiB) | `result/build-info.json` |
| Signed AAB | 88,728,421 bytes (84.6 MiB) | same |
| Tests | 3,156 across 97 files | `npm test` |
...
recorded spoken form because some rule changes them; 699 of those get a *note
...
the stroke data and 15,840 pronunciation clips in two voices.
...
An adaptive placement test over 30 levels and a **4,191-item** bank, 613 of them
...
what the adaptive walk spends: every non-English language now has **2,253**
...
**15,840 distinct files over 15,960 voice slots, 81.9 MB**, two Korean neural
...
| Suite | Cases |
| --- | --- |
| Web unit (`vitest`) | **1,525** (84 files) |
...
| | |
| --- | --- |
| Unit and integration tests | **3,156 across 97 files** — handwriting-core 96, korean-morphology 237, content-safety 1,298, web 1,525 (71 of them the Numbers journeys, negative tests, migration fixtures, exercise-engine tests of §20K and the question-type cases of §20P) |
...
rewritten since and renumbered; the citations on the 5 open, 5 partial and 1 blocked issues were brought forward to this numbering, and the resolved ones
...
**Blocked outside this repository: 1 · Partial: 5 · Resolved: 219**
...
The suites run green in full: 3,156 unit cases across four packages — 1,525
web, 1,298 content safety, 237 Korean morphology, 96 handwriting — 634 end-to-end, 166 rendered
