...
## 7. The claims of report(20260916-012042) — the eighteenth edition, read for the nineteenth pass

Written 2026-09-16. The named attachment is not on this machine (repository, home, the Windows profile's Desktop, Documents, Downloads and Telegram folders were searched); its timestamp names a download made after the eighteenth edition was committed, so `docs/report.pdf` in the tree — sha256 `514fd1f3d4f67a04e9d3272d57ea6a51d1ebb23bca520a18b7a8fe68e53afbed`, 6,630,602 bytes, 426 pages, `CreationDate` 2026-09-14T17:06:28Z, built from `docs/report.md` at `d511ff1e` — was read as the current source. Whether the two files are byte-identical could not be established: EXTERNAL. Every page of the tree's PDF was rendered and inspected (`docs/report-pdf-inspection.json` from `report:pdf:inspect`); the Markdown was read in full.

| # | Claim as written | Where | Finding | Label | Now |
|---:|:---|:---|:---|:---|:---|
| 1 | A streak day is any day with recorded study activity — an attempt, a completed item, or measured time on a session screen | §20B | True of the code; twenty seconds on a session screen with nothing answered made a streak day, a Numbers answer did not, and a never-active learner read *0 days*. Reproduced against the live store. | **CONTRADICTED** (by the product rule) | `isQualifyingDay`: an answered question or completed item; §20B rewritten; I-235 |
| 2 | Reaching `learned` is recorded as a day's outcome by `trackActivity` | §13 / `LearnerProvider` | The completion flag was noted inside an updater and read before React ran it; `characters_learned` stayed 0 for every letter learned by writing. | **CONTRADICTED** | crediting inside the updater; I-236 |
| 3 | Words shipping 3,867; pronunciation notes 902; patterns 6 / noted 5; Level Test 4,191 items, 613 contextual, reach 2,253; tests 3,156 across 97 files; 230 issues | §2.2 | All true at the eighteenth pass; the corpus is 3,864 after three editorial retirements, ㄴ 첨가 is a seventh pattern, the bank is 4,156 / 576 / 2,212, the suites 3,202 across 101 files, 244 issues. | **PARTIALLY VERIFIED** (true then, stale now) | regenerated by `docs:consistency` and read by hand for the rows it does not own |
| 4 | The current/historical-figures paragraph | §1 | Present twice, word for word. | **CONTRADICTED** | one copy, pointing at §20AC |
| 5 | Every generated question is built and checked (`answerability`) | §19 / §20X | Three seeds per question. Over 3,000 seeds the letter-sound question for ㅢ drew "i" beside "i" 139 times. | **PARTIALLY VERIFIED** | the builder filters by label; a 400-seed sweep is a test; I-237 |
| 6 | A distractor sharing the answer's meaning in any locale is rejected | §10.8, §20X | True of the Level Test builder (shared gloss word). Not true of the runtime reading/production questions, which rejected only identical glosses: 220 near-synonym pairs in English. | **PARTIALLY VERIFIED** | `sameMeaning` over senses; I-238 |
| 7 | Every contextual stem was composed with all four of its options and read | §20X, ambiguity ledger | Verified for the readings that made the claim; a re-drawn bank invalidates a reading, and the fourth reading found 64 more. | **PARTIALLY VERIFIED** | the builder is seeded per item so a re-read is incremental; A-24 to A-87; I-242 |
| 8 | The matching grid grades on Check and has one right pairing | §20Z | In 31 languages two same-level words can share a gloss (896 pairs) and both can land in one grid. | **CONTRADICTED** | grid rows deduplicated by meaning; I-239 |
| 9 | 902 pronunciation notes, each naming the pattern that changes the word | §2.2, §12 | Seven ㅀ-stems named tensing for aspiration; 옮기다·굶주리다 named tensing with none; 담요·식용유 transcribed against §29; 큰일·별일 under liaison. | **CONTRADICTED** | `pronunciation.py` corrected; pattern fixtures and the gold set; I-240 |
| 10 | The conjugation panel shows the rows worth showing under the labels they get | §20D | 먹으세요, 마시세요, 자세요, 있으세요, 말하세요 under *Please do*. | **CONTRADICTED** | suppletive commands on the card; I-241 |
| 11 | The 884 Korean interface strings were read | §20AB.10 | Read again with three new strings; one letter hint mixed speech levels (합쇼체 in a 해요체 set). | **VERIFIED** (one P3 found) | hint rewritten; copy ledger 887 |
| 12 | No locale has been reviewed by a native speaker | §11.3, §22 | Still true. | **VERIFIED** | unchanged; I-17, I-246 |
| 13 | Android / iOS deliveries at 1.0.4, versionCode 25, not rebuilt | §2.1 | `git diff --name-only` shows no native path; `version:check` green. | **VERIFIED** | unchanged by design |
| 14 | Every gate added was broken on purpose and shown to fail | §20AB.11 | True of N1–N11; this pass adds N12–N21 for its own gates. | **VERIFIED** | `docs/content-gates-negative.json` |
| 15 | The verdict's first limitation: 3,393 taught words | §23 | Two passes old inside a current section. | **PARTIALLY VERIFIED** | 3,864 |
| 16 | Eleven near-blank pages and two chapters after the verdict (I-225) | §23 | The inspection record for this edition is `docs/report-pdf-inspection.json`: 446 pages, 0 blank, 10 image pages under 300 characters, verdict on page 438; see §9.8 of the QA evidence. | **VERIFIED / re-measured** | see the evidence document |

Every current figure a reader might diff is in `docs/report.md` §2.2, `docs/content-inventory.json` and `docs/exercise-inventory.json`.

## 8. The Korean reading of 2026-09-16

What was read, in full: `content/vocabulary/entries/*.jsonl` (3,867 kept rows: headword, part of speech, level, English gloss, Korean definition, example, English translation); `apps/web/src/data/characters.ts` (40 letters, 33 syllables, 15 lessons, 12 units, hints and mnemonics in Korean and English); the 902 pronunciation notes (`say`/`sayWhy`) against 표준 발음법 §5, §9–§12, §17–§19, §23–§26, §29; every gap-fill in `cloze.json` with all four options substituted, four times; the 368 Numbers questions from `content-cache/question-ledger.json`; the conjugation panel's rows for the suppletive verbs; the 64 streak strings. Method: model reading against the references named in `content/korean-gold-set.json` `sources`; label MODEL-REVIEWED; no row is NATIVE-SPEAKER REVIEWED.

| Dimension | P0 | P1 | P2 | P3 | Fixed |
|:---|---:|---:|---:|---:|:---|
| Orthography | 0 | 0 | 4 (고장나다, 신경쓰다, 한잔 하실래요, 고마워) | 1 (큰 일이 벌어졌어요) | 4 of 4 P2; the P3 kept (both readings are standard) |
| Phonology | 0 | 13 notes (7 ㅀ-stems, 옮기다, 굶주리다, 담요, 식용유, 큰일, 별일) | 3 (솔직히, 솔직하다, 일일이, 일석이조 §26/§29 forms) | 0 | all |
| Morphology | 0 | 5 panel rows (먹다, 마시다, 자다, 있다, 말하다) | 1 (곳곳 tagged adverb) | 1 (마저 tagged adverb: a particle) | P1 and P2 fixed; the P3 kept (the corpus has no particle class) |
| Syntax / answer validity | 0 | 64 gap-fills; 1 letter-sound draw (ㅢ); 220 reading-question pairs (en) | 0 | 0 | all |
| Semantics | 0 | 0 | 5 (그렇게, 빼다, 어서, 해마다, 빗) | 1 (찍다 taught as *to stamp* where *to take a photo* is the beginner sense; 사진 carries that sense) | P2 fixed; P3 kept and recorded |
| Pragmatics | 0 | 0 | 1 (당신 at level 1) | 1 (저 여자는 의사예요 — blunt but textbook) | P2 fixed |
| Pedagogy / level | 0 | 0 | 2 (등 at level 1; lesson-syllables-more title differs in 30 languages) | 1 (zh-CN 天连续 after a number) | 등 fixed; the title kept (both describe the lesson); zh-CN rewritten |
| Translation collision (matching grid) | 0 | 896 pairs / 31 locales | 0 | 0 | runtime dedupe |

Items removed from learner reach: three headwords retired (still in progress rows, never asked); 36 words withdrawn from gap-fills (still taught, still asked as meaning, production and build); one gap-fill (밤에 혼자 가면 ____) whose frame had too few distractors left after its pair was banned was not rebuilt.
