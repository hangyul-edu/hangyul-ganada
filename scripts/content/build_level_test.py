...
def build() -> dict:
    ...
    # The hand-written gap-fills, levelled by *their own* sentence.
    #
    # A curated item (content/vocabulary/context-items.json) is a different
    # sentence from the card's, and until v1.0.5 it inherited the card
    # sentence's demand — or, for a word whose card sentence had been read and
    # refused (`ctx: 0`), no demand at all, which the policy gate then failed.
    # The demand of a frame is a fact about the frame, so it is computed here
    # for the curated frame exactly as for a generated one.
    curated_path = ROOT / "content" / "vocabulary" / "context-items.json"
    if curated_path.exists():
        by_id = {row["id"]: row for row in rows}
        for item in json.loads(curated_path.read_text(encoding="utf-8"))["items"]:
            row = by_id.get(item["answer"])
            if row is None or not row.get("surface"):
                continue
            blanked = f"{item['before']}____{item['after']}"
            sentence = f"{item['before']}{row['surface']}{item['after']}"
            level, demand = context_level(blanked, row["level"], levels, sentence=sentence)
            row["curated_context_level"] = level
            row["curated_context_demand"] = {
                "eojeol": demand.eojeol,
                "noun": demand.noun_demand,
                "hardest": demand.hardest_word,
                "grammar": list(demand.names),
