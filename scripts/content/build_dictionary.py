...
def build() -> dict[str, object]:
    ...
    for word, entries in sorted(by_word.items()):
        ...
        records.append(
            {
                "id": base if base != "dict_x" else f"dict_{len(records)}",
                "headword": word,
    ...
    files["manifest.json"] = {
        ...
        "chunks": {name: len(rows) for name, rows in sorted(chunks.items())},
        "source": SOURCE,
    }
    return files
...
def main() -> int:
    ...
    index_bytes = (OUT / "index.json").stat().st_size if (OUT / "index.json").exists() else 0

    print(f"Dictionary — {manifest['headwords']:,} headwords, {manifest['senses']:,} senses")
    print(f"  {manifest['examples']:,} examples")
    print(f"  {len(manifest['chunks'])} chunks, largest {max(manifest['chunks'].values()):,} entries")
