...
def _takes_a_rate(provider_id: str) -> bool:
    """Whether this engine can be asked to speak more slowly.

    Microsoft, Azure and Google all take a rate; ElevenLabs does not. The
    distinction matters twice: the generator does not claim a rate it did not
    ask for, and this file does not fail a corpus for not having one.
    """
    return provider_id not in {"elevenlabs"}
...
def main() -> int:
    ...
    elif not _takes_a_rate(provider_id):
        # An engine with no rate parameter cannot be asked to slow down, so
        # comparing its corpus against `SPEECH_RATE` would fail every time. What
        # matters is that the *checks* use the rate the clips were actually
        # spoken at, which they do — see `_TEMPO` above.
        print(f"  rate:   {float(spoken_rate):g}× — this engine takes no rate parameter")
