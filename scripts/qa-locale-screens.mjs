...
/** `--summary <file>` writes one row per locale (screens rendered, findings) for the per-locale audit table. */
...
if (summaryPath) {
  ...
  for (const locale of LOCALES) {
    ...
    summary[locale] = { screens: SCREENS.length, found: own.length, corrected: 0, kinds: own.map((f) => `${f.screen}: ${f.kind}`) };
  }
  writeFileSync(summaryPath, `${JSON.stringify({ _comment: 'GENERATED by scripts/qa-locale-screens.mjs --summary: per locale, the screens rendered at 390 px and what was measured wrong on them (overflow, sideways, untranslated, empty, overlapping, ellipsed). "corrected" is what a later run no longer finds; it is 0 when nothing was found.', renderedAt: new Date().toISOString(), viewport: PHONE, locales: summary }, null, 1)}\n`);
