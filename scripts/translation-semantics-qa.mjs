...
const NEGATION = {
  ...
  ro: /\bnu\b|\bnicio/i, el: /(?:^|\P{L})(?:δεν|μην?|όχι|ποτέ|σπάνια|σταμάτ)/iu,
  /*
   * Hungarian marks negation twice, and the second mark is often the only
   * one in the sentence: 아무도 눈치채지 못했어요 is «Senki sem vette észre»,
   * where both `senki` and `sem` carry it and neither is `nem`. aligha and
   * ritkán are the rarity reading of 좀처럼, like редко and σπάνια.
   */
  ...
  /*
   * Kyrgyz harmonises the negative suffix across four vowels — -ба/-бе/-бо/-бө
   * and -па/-пе/-по/-пө — and the list here originally enumerated only two of
   * them for п, so 겁내지 마세요 → «Коркпоңуз» and 생각나지 않아요 → «эсиме
   * түшпөй жатат» were reported as losing the negation they plainly carry.
   * The classes below cover the whole harmony set, and `п[аеоө]д` catches the
   * negative past (-падык, «жолукпадык»), whose д the [йң] classes miss.
   */
  ky: /эмес|жок|элек|ба[йң]|бе[йң]|п[аеоө][йң]|п[аеоө]д[ыиуү]|бо[йң]|бө[йң]|м[оө]й|мой|бол?бо|байт|бейт|лба|лбе|рба|рбе|збе|зба|түк/iu,
  mn: /гүй|биш|үгүй|болохгүй|бүү|битгий/iu,
